...
precisely, with references to full proofs at \url{https://holon.sh}.
Falsifiability is emphasised throughout: for each theorem, the experimental
outcome that would refute it is indicated explicitly.
...
This is the ``death spiral'': once purity drops below $2/7$, regeneration is
thermodynamically forbidden (Landauer's principle: free energy $\Delta F \leq 0$ below
threshold), and the system decays irreversibly. There is no gradual fading of
consciousness --- there is a hard phase transition, consistent with the nonlinear
threshold for access to consciousness observed empirically~\cite{delcul2007}.
...
$r_{\mathrm{stab}} = \sqrt{P - 2/7}$. (Full proof: T-104~\status{T} at
\url{https://holon.sh/docs/applied/coherence-cybernetics/stability}.)
...
surviving coherence is suppressed by $c_F^k = (1/3)^k$. At depth $n$, the
spectral formula T-136~\status{T} gives
\begin{equation}
...
This is among the most specific falsifiable predictions in consciousness science,
and the one with no analogue in any other theory.
...
\emph{tricritical point} (three roots coalesce in the $A_4$ swallowtail of the $\phi^6$
Landau potential). The effective dimension is $d_{\mathrm{eff}} = 21$ (from the 21
off-diagonal coherences), far above the upper critical dimension $d_c = 3$ for
tricritical systems. Mean-field tricritical exponents are therefore exact:
\begin{align}
...
\begin{equation*}
  ...
  \qquad r_0, v > 0.
\end{equation*}
The absence of a quartic term is not an ad hoc assumption but the universal
normal form of an $A_4$ swallowtail catastrophe: the UHM evolution has exactly
three physically independent control parameters --- regeneration rate
$\kappa$, dissipation rate, and free-energy budget $\Delta F$ --- which by
Thom--Arnold classification places the transition in the codimension-3
$A_4$ class, whose even-sector normal form is $\varphi^{6}$
(Section~\ref{subsec:sad-ceiling}; canonical proof at
\url{https://holon.sh/docs/consciousness/hierarchy/swallowtail-transitions}).
The effective dimension $d_{\mathrm{eff}} = 21$ (the number of independent
off-diagonal coherences $|\gamma_{ij}|$, $i \neq j$) vastly exceeds the upper
critical dimension $d_c = 3$ for tricritical systems, so mean-field
tricritical exponents are exact (no logarithmic corrections). The full
derivation, including all five exponents and the verification of Rushbrooke,
Widom, and Fisher scaling relations, is given in
Appendix~\ref{app:exponents}; see also T-161~\status{T} at
\url{https://holon.sh}.
