...
\textbf{Why Bures specifically?} T-187~\status{T} provides a \emph{quadruple} characterization: (Char-I)~Petz extremality (minimal in the Petz poset), (Char-II)~Uhlmann purification universality, (Char-III)~SLD-Cram\'{e}r--Rao saturation, and (Char-IV)~MaxEnt covariance identification $G^{ij} = C^{ij}$ (T-189~\status{T}): the maximum entropy principle uniquely identifies the inverse metric tensor with the quantum covariance matrix, deriving $g_B$ from the statistical structure of the state itself.
