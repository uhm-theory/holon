...
nothing is postulated ad hoc. The flagship results — $\Pcrit = 2/7$, the
critical exponents, the spectral-action derivation — are proved fully in the
appendices. The complete formalization (192 theorems, T-1 through T-192)
is self-contained within this paper and its appendices; extended proofs
and computational details are available in the documentation archive.
...
transformation between the dissipation and regeneration functors (T-88~\status{T}).
The derivation uses Yoneda's embedding to identify $|\mathrm{Hom}(i,j)| \leftrightarrow |\gamma_{ij}|$,
the Bures metric on $\mathcal{C}_7$ to make this identification quantitative, and
Stinespring's dilation theorem to ensure CPTP-compatibility. The factor structure
$|\gamma_{OE}|\cdot|\gamma_{OU}|/\gamma_{OO}$ comes directly from the adjunction
$\mathcal{D}_\Omega \dashv \mathcal{R}$ between dissipation and regeneration: $|\gamma_{OE}|$
quantifies the $O\to E$ morphism strength (sensing of interiority), $|\gamma_{OU}|$
the $O\to U$ morphism strength (integration coupling), and $\gamma_{OO}$ the $O$-self-loop
(ground anchoring).
...
is exactly representable via T-128~\status{T}: $D_\mathrm{diff}^\mathrm{7D} = 1 + \mathrm{Coh}_E(\Gam)/\mathrm{Coh}_E^{\max}\cdot(N-1)$,
which agrees with the 42D partial-trace expression by Morita equivalence (T-58~\status{T}):
$\ShInf(\mathcal{C}_7) \simeq \ShInf(\mathcal{C}_{42}^\mathrm{PW})$. All UHM observables
$P, R, \Phi, D_\mathrm{diff}$ are computable in the minimal 7D formalism.}
