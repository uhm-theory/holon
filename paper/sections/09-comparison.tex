...
To the author's knowledge, the following eight properties are unique to UHM
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Hard problem localized:} T-188~\status{T} reduces ``why experience?'' to
        ``why quantum mechanics?'' via the cohesive closure chain. No consciousness-specific
...
arises from two recent no-go results on the compatibility of
consciousness and quantum mechanics with classical scientific
\emph{objectivism}.

\textbf{The two no-go results.}
\begin{itemize}[leftmargin=2em,nosep]
  \item \emph{List (2025) quadrilemma for
        consciousness}~\cite{list2025quadrilemma}: the five theses
        --- first-personal realism (FPR), non-solipsism (NS), one
        world (OW), non-fragmentation (NF), non-relationalism (NR)
        --- are jointly inconsistent. Objectivism is defined as
        OW $\wedge$ NF $\wedge$ NR.
  \item \emph{DeBrota--List (2026) heptalemma for quantum
        mechanics}~\cite{debrota2026heptalemma,debrotalist2026preprint}:
        the seven theses $\{\mathrm{Loc}, \mathrm{MI}, \mathrm{MR},
        \mathrm{NS}, \mathrm{OW}, \mathrm{NF}, \mathrm{NR}\}$ are
        jointly inconsistent with the predictions of QM.
\end{itemize}

The authors identify three non-objectivist routes (relationalist,
fragmentalist, many-subjective-worlds) in each case and leave the
selection to inference to the best explanation, providing no
measurable discriminator.

\textbf{UHM's response: the categorical-monistic route.} Theorem~T-221
(\S\ref{subsec:extended-closures}) establishes that UHM realises a
\emph{fourth} non-objectivist route, outside the DeBrota--List
taxonomy. In the UHM cohesive $\infty$-topos $\mathfrak T =
\mathbf{Sh}_\infty(\mathcal C_7, J_{\mathrm{Bures}}, \omega_0)$:
FPR is \emph{forced} by T-186 ($F \cong \&|_{\mathcal D}$); NS is
\emph{conventional} by T-215; OW is \emph{derived} by T-120 via
Gelfand--Naimark--Connes reconstruction; NF is \emph{definitional} by
T-211 Giraud; and NR is structurally relaxed to
\emph{NR}$_{\mathrm{site}}$ --- facts are absolute up to isomorphism
in $\mathfrak T$ but indexed by the internal site object
$\mathcal C_7$ (the relativisation parameter $\Gam$ is
$\mathfrak T$-internal, not external in Fine's~\cite{fine2005tense}
sense). The single structural replacement
$\mathrm{NR} \rightsquigarrow \mathrm{NR}_{\mathrm{site}}$ resolves
both the quadrilemma and the heptalemma simultaneously.

\textbf{Relation to the three other routes.} The three alternatives
of DeBrota--List are reductive truncations of the UHM structure, not
competing positions:
\begin{itemize}[leftmargin=2em,nosep]
  \item \emph{Relational QM} (Rovelli~\cite{rovelli1996rqm}) is
        recovered as the 1-truncation $\tau_{\leq 1}(\mathfrak T)$.
        Its documented first-personal deficit (Glick 2021: ``still
        aim to provide a description of external
        reality''~\cite{glick2021qbism}) corresponds exactly to the
        $n \geq 2$ content of the $\&$-modality discarded by
        1-truncation.
  \item \emph{Fragmentalism}
        (Fine~\cite{fine2005tense}, Lipman~\cite{lipman2023subjective})
        amounts to dropping descent in a sector, violating T-211
        Giraud (no longer an $\infty$-topos).
  \item \emph{Many-subjective-worlds}
        (Mermin~\cite{mermin2019better}, List 2023) amounts to
        pointwise Yoneda without gluing, losing covering
        $\{U_i \to W\}$ coherence.
\end{itemize}

\textbf{Empirical discriminator.} Where DeBrota--List leave
route-selection to ``inference to the best explanation'', UHM
provides a measurable criterion via the $\pi_{\mathrm{bio}}$ protocol
...
subjects (fragmentalism); per-subject $\Phi$ with no cross-subject
