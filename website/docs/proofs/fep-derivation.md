...
:::warning Предположение
Примитивность $\mathcal{L}_\Omega$ (единственность стационарного состояния) — предположение, не доказанное для конкретной конструкции $L_k = \sqrt{\chi_{S_k}}$. Верификация для $N = 7$ — открытая задача.
:::
