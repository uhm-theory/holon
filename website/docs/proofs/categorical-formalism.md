...
**Доказательство:** Следует из ассоциативности композиции CPTP-каналов и тождества $\mathrm{id} \circ \Phi = \Phi = \Phi \circ \mathrm{id}$. ∎
...
где $F(\rho, \sigma) = \left(\mathrm{Tr}\sqrt{\sqrt{\rho}\sigma\sqrt{\rho}}\right)^2$ — fidelity (верность).
...
| Связь с fidelity | $d_B^2 = 2(1 - \sqrt{F})$ | Квантовая интерпретация |
...
- $\omega_0$ — фундаментальная частота (единственный свободный параметр)
...
(c) На границе жизнеспособности $\kappa_0 > 0$ (иначе система не может поддерживать P > P_crit).
...
| $\omega_0$ | Фундаментальная частота (свободный параметр) | Примитив 𝔗 |
