...
**Теорема** ниже (Трек A) показывает, что 7 — **минимальное** значение, при котором условия (AP)+(PH)+(QG) *могут быть выполнены*. Независимо, [структурный вывод через октонионы](/docs/proofs/theorem-octonionic-derivation) (Трек B) даёт $N = 7$ из постулатов P1+P2 через теорему Гурвица.

**Честная формулировка:** "Если мы изучаем системы с (AP)+(PH)+(QG), то N ≥ 7. Мы выбираем N = 7 как минимальный нетривиальный случай. Это значение независимо подтверждается октонионной структурой."
:::
...
Данная часть резюмирует [полный вывод](/docs/proofs/theorem-octonionic-derivation), дающий $N = 7$ из постулатов P1+P2, **независимо** от (AP)+(PH)+(QG).
:::

### 9.1 Постулаты и вывод

**[П] P1:** Пространство внутренних степеней свободы ≅ Im($\mathcal{A}$), где $\mathcal{A}$ — нормированная алгебра с делением.
**[П] P2:** $\mathcal{A}$ неассоциативна.

**[С] Вывод:** P1 → [Т] Гурвиц → $\mathcal{A} \in \{\mathbb{R}, \mathbb{C}, \mathbb{H}, \mathbb{O}\}$ → P2 исключает $\mathbb{R}, \mathbb{C}, \mathbb{H}$ → $\mathcal{A} = \mathbb{O}$ → $N = 7$.
...
| **Статус** | ✅ Доказано | ✅ Математически строго при [П] P1+P2 |
...
Замыкание моста (AP)+(PH)+(QG) ↔ P1+P2 — [открытая проблема](#проблема-5-мост).

### Проблема 5: Замыкание моста {#проблема-5-мост}

**Проблема 5 (Замыкание моста).**
Доказать или опровергнуть: $(AP) + (PH) + (QG) \Rightarrow P1 + P2$.

Три сходящихся аргумента (обратимость, исключительность, граница Кэли-Диксона) мотивируют связь, но формальное доказательство отсутствует. См. [подробный анализ](/docs/proofs/theorem-octonionic-derivation#мост).
...
**Связанные документы:**
- [Структурный вывод через октонионы](/docs/proofs/theorem-octonionic-derivation) — Трек B: P1+P2 → 𝕆 → N=7
