...
3. **Свобода выбора:** Существование альтернативных представителей в том же классе $[\varphi]$ отражает фундаментальную [свободу воли](/docs/core/ethics/freedom) — систем может реализовывать разные пути к одному аттрактору.
...
3. По теореме о сходимости примитивных CPTP-каналов (аналог Перрона-Фробениуса для квантовых каналов), предел $\lim_{\tau \to \infty} e^{\tau \cdot \mathcal{L}_\Omega}[\Gamma]$ существует и является проекцией на инвариантное подпространство.
...
2. **Редуцированная матрица** $\rho_E = \mathrm{Tr}_{-E}(\Gamma)$ выделяет [измерение Опыта](/docs/core/structure/dimension-e) как привилегированное.
...
:::info Ключевая связь
Оператор самомоделирования $\varphi$ играет центральную роль в [регенеративном члене](/docs/core/dynamics/evolution#3-регенеративный-член) уравнения эволюции: **$\Gamma_{\text{target}} := \varphi(\Gamma)$**.
:::

### 9.1 Регенерация как стремление к самомодели

[Регенеративный член](/docs/core/dynamics/evolution#3-регенеративный-член) уравнения эволюции $\Gamma$:
...
где:
- $\kappa(\Gamma)$ — [коэффициент регенерации](/docs/core/foundations/axiom-septicity#категориальный-вывод-kappa0) (категориальный вывод)
- $\Gamma_{\text{target}} := \varphi(\Gamma)$ — **целевое состояние, определяемое оператором самомоделирования**
- $\Theta(\Delta F)$ — функция Хевисайда, активирующая регенерацию при $\Delta F > 0$
...
$$
\Gamma_{\text{target}} - \Gamma = \varphi(\Gamma) - \Gamma \neq 0
$$
...
[Мера рефлексии](/docs/core/consciousness/self-observation#мера-рефлексии-r) $R$ и регенеративный член $\mathcal{R}$ связаны:
...
$$
\|\mathcal{R}[\Gamma, E]\| \propto \|\Gamma_{\text{target}} - \Gamma\| = \|\varphi(\Gamma) - \Gamma\| = \sqrt{P \cdot (1 - R)}
$$
...
Это согласуется со свойством неподвижной точки $\varphi(\Gamma^*) = \Gamma^*$: самосогласованность достигается на полном 7-мерном пространстве, где неассоциативность интегрирована в структуру. Мост формально открыт [И]. См. [структурный вывод](./theorem-octonionic-derivation).
:::
...
Тензорная факторизация $\varphi$ — ключевое свойство, обеспечивающее [совместимость $\mathcal{R}$ с запретом сигнализации](/docs/proofs/physics-correspondence#запрет-сигнализации). Оно гарантирует, что самомоделирование автономных подсистем не создаёт каналов сверхсветовой коммуникации.
:::
...
Полное доказательство: [Физическое соответствие — Запрет сигнализации](/docs/proofs/physics-correspondence#запрет-сигнализации).
...
- [Категорный формализм](/docs/proofs/categorical-formalism) — категорная структура УГМ, n-усечения и [запрет сигнализации как естественная трансформация](/docs/proofs/categorical-formalism#запрет-сигнализации-естественная-трансформация)
- [Голоном](/docs/core/structure/holon) — определение $\mathbb{H}$
- [Физическое соответствие — Запрет сигнализации](/docs/proofs/physics-correspondence#запрет-сигнализации) — полные доказательства
