...
$$
K_m := \sqrt{w_m} \cdot P_m, \quad m = 1, \ldots, M
$$

где:
- $\{P_m\}$ — проекторы на подпространства семимерного [базиса измерений](/docs/core/structure/dimensions) $\{|A\rangle, |S\rangle, |D\rangle, |L\rangle, |E\rangle, |O\rangle, |U\rangle\}$
- $w_m \geq 0$, $\sum_m w_m = 1$ — веса, отражающие «глубину самонаблюдения» по каждому измерению

**Условие CPTP:**

$$
\sum_{m=1}^{M} K_m^\dagger K_m = \sum_{m=1}^{M} w_m P_m = I \quad \Leftrightarrow \quad \sum_{m=1}^{M} w_m = 1 \text{ и } \sum_m P_m = I
$$
...
| Вес | Интерпретация |
|-----|---------------|
| $w_A$ | Внимание к различиям (Актуальность) |
...
$$
\mathcal{P}_{\text{uniform}}(\Gamma) = \frac{1}{7} \sum_{m=1}^{7} P_m \Gamma P_m = \frac{1}{7} \cdot \text{diag}(\Gamma)
$$

где $\text{diag}(\Gamma)$ — диагональ матрицы в базисе измерений.

**Определение 2.9 (E-акцентированное самонаблюдение):**
...
При $\alpha \to 1$: самонаблюдение фокусируется на опыте, $\varphi(\Gamma) \approx P_E \Gamma P_E$ + якорь.

**Теорема 2.1 (Неподвижная точка канонической φ):**

Для $\varphi_{\text{UHM}}$ с $k < 1$ существует единственная неподвижная точка:

$$
\Gamma^* = \frac{I}{7}
$$

независимо от выбора весов $\{w_m\}$.

**Доказательство:**

$$
\varphi_{\text{UHM}}(\Gamma^*) = k \cdot \mathcal{P}_{\text{pred}}\left(\frac{I}{7}\right) + (1 - k) \cdot \frac{I}{7}
$$

$$
= k \cdot \sum_m w_m P_m \frac{I}{7} P_m + (1 - k) \cdot \frac{I}{7} = k \cdot \frac{1}{7} \sum_m w_m P_m + (1 - k) \cdot \frac{I}{7}
$$

$$
= \frac{k}{7} \cdot I + \frac{1 - k}{7} \cdot I = \frac{I}{7} = \Gamma^*
$$

(использовали $\sum_m w_m P_m = I$ при полном наборе ортогональных проекторов). ∎

**Следствие 2.1:** Неподвижная точка $\Gamma^* = I/7$ — максимально смешанное состояние, соответствующее **равновесию самопознания**: система равномерно осознаёт все семь измерений.

:::warning Физическая интерпретация
$\Gamma^* = I/7$ — это идеализированный предел. Реальные системы находятся вблизи неподвижной точки: $\Gamma \approx \Gamma^* + \delta\Gamma$, где $\delta\Gamma$ отражает специфику конкретной системы.
:::
