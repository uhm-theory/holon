...
**Замещающий канал** $\varphi_k(\Gamma) = (1-k)\Gamma + k\rho_*$ является канонической физической реализацией оператора самомоделирования ([доказательство](/docs/consciousness/foundations/self-observation#теорема-физическая-реализация-phi)). Здесь $\rho_*$ — единственное стационарное состояние $\mathcal{L}_\Omega$ [Т], $k \in (0,1)$ — степень самомоделирования. Канал **точен** при $k \to 1$ (полная сходимость к $\rho_*$), но для промежуточных значений $k$ реализует приближённое самомоделирование — система находится в динамическом балансе между текущим состоянием и внутренней моделью. Остальные три определения эквивалентны замещающему каналу через [теорему эквивалентности](#эквивалентность-определений-phi) [Т].
:::
...
**Направление (1)⟹(2)** следует из примитивности $\mathcal{L}_\Omega$ [Т]: левый сопряжённый $\varphi$ проецирует на инвариантное подпространство, а примитивность гарантирует сходимость $e^{\tau \mathcal{L}_\Omega}$ к этому проектору. **Направление (2)⟹(1):** любой минимизатор вариационного функционала при условии CPTP является стационарной точкой $\mathcal{L}_\Omega$, а по примитивности она единственна $= \varphi$. Таким образом, (2)⟹(1) также [Т] через единственность стационарного состояния. Все три направления имеют статус **[Т]**.
:::
...
:::tip Упрощение при примитивности [Т]
[Примитивность $\mathcal{L}_\Omega$](/docs/core/operators/lindblad-operators#примитивность-ℒω) гарантирует **ровно один** нулевой режим ($\lambda_0 = 0$, кратность 1). Формула упрощается:

$$
\varphi(\Gamma) = \langle L_0 | \Gamma \rangle \, R_0 = \mathrm{Tr}(L_0^\dagger\,\Gamma) \cdot \rho_*
$$

где $R_0 = \rho_*$ — единственное стационарное состояние, $L_0$ — соответствующий левый собственный вектор.
:::
...
:::info Ключевая связь
Оператор самомоделирования $\varphi$ определяет целевое состояние регенерации: $\rho_* = \varphi(\Gamma) = \lim_{\tau\to\infty} e^{\tau\mathcal{L}_\Omega}[\Gamma]$ — единственное стационарное состояние [Т] ([примитивность](/docs/core/operators/lindblad-operators#примитивность-ℒω)).
:::
...
- $\rho_* = \varphi(\Gamma)$ — единственное стационарное состояние $\mathcal{L}_\Omega$ [Т] (примитивность)
