...
:::warning Уточнение строгости аргумента необходимости
Доказательство выше показывает, что **данные 7 конкретных функций** (F1-F7) попарно независимы и все необходимы для (AP)+(PH)+(QG). Однако строгая необходимость $N \geq 7$ требует дополнительно показать, что **никакое альтернативное 6-мерное разбиение функций** не может удовлетворить всем трём аксиомам. Текущее доказательство не исключает возможности иного набора из 6 функций, покрывающего (AP)+(PH)+(QG). Строгое доказательство несуществования такой 6D-альтернативы — открытый вопрос.

**Статус:** Доказательство достаточности 7 измерений — **[Т]**. Строгая необходимость (невозможность 6D-альтернативы) — **[С]** при допущении, что функциональное разбиение F1-F7 единственно.
:::
...
Все три пробела закрыты через формулу κ₀ (Th. 15.3.1, [Т]), вывод формы ℛ [Т] и функциональную независимость (Лемма 2.2, [Т]). Условных результатов в теореме минимальности **не осталось**.
