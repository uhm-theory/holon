...
| von Foerster | КК |
|--------------|-----|
| $\mathrm{Observer} \in \mathrm{System}$ | $\varphi(\Gamma) \approx \Gamma$ |
...
**Фокус:** Минимизация свободной энергии.

**Отображение в КК:**

| FEP | КК |
|-----|-----|
| Свободная энергия $F$ | [Жизнеспособность](/docs/core/dynamics/viability) (обратная связь) |
...
L2 требует $R \geq R_{\text{th}} = 0.3$ и $\Phi \geq \Phi_{\text{th}} = 1.0$.
...
- $D: X \times G \to G$ — функция решения (decision)
- $A: G \times W \to W$ — функция действия (action)
- $W$ — мир (world)
...
**Теорема Хоффмана (Fitness-Beats-Truth):**

Эволюция не оптимизирует восприятие истины, а оптимизирует приспособленность (fitness).
...
| $G$ (действия) | Пространство CPTP-каналов $\{\Phi\}$ |
...
1. $F_{\mathrm{Hoffman}}$ полон (full) и верен (faithful)
...
graph TB
    ...
    subgraph "Теории сознания"
        AP[Автопоэзис<br/>Maturana, Varela]
        IIT[IIT<br/>Tononi]
        FEP[FEP<br/>Friston]
    ...
    subgraph "Панпсихизм"
        ...
        PC[Конституивный<br/>панпсихизм]
...
|--------|------------|-------------|
| Кибернетика-I | Инженерные системы | Нет самореференции |
| Кибернетика-II | Эпистемология | Нет феноменологии |
| IIT | Оценка сознания | Нет динамики |
| FEP | Нейронауки, ИИ | Нет E-измерения |
...
|-------------|---------|-------------|
| F1 Компактность | $\|\Gamma\|_F \leq R_{\max}$ | $\mathrm{Tr}(\Gamma) = 1$ |
| F2 Диссипативность | $\dot{W} \leq -\alpha W + \gamma(\|e\|)$ | [$\mathcal{D}[\Gamma]$](/docs/core/dynamics/evolution#2-диссипативный-член) |
| F3 Невырожденность | $\lambda_{\min}(\Gamma) \geq \varepsilon$ | [Жизнеспособность](/docs/core/dynamics/viability) |
| F4 Рефлексивное сжатие | $\|\varphi(\Gamma_1) - \varphi(\Gamma_2)\|_F \leq k \|\Gamma_1 - \Gamma_2\|_F$, $k < 1$ | [Теорема 7.2](./theorems#теорема-72-неподвижная-точка-рефлексии) |
...
$$
\mathrm{Interior}(\Gamma) := \rho_E = \mathrm{Tr}_{-E}(\Gamma), \quad \text{со спектром } \{\lambda_k, |q_k\rangle\}
$$
