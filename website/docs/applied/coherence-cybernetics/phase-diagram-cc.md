...
5. **Self-consistent gate: the exact fold [Т in the model].** Restoring $\kappa_{\text{eff}} = \kappa \cdot g_V(P)\,(1 - 1/(7P))$ makes the living stationary branch appear through a saddle-node, and the fold is **exact**: with $s = 7P - 2$ and $c = 7P^* - 1$, self-consistency forces $\Lambda^2 = (s+1)/c$, and the tangency condition reduces to the quintic
$$
(2\Lambda - 1)\,(c^2\Lambda^4 - 1) \;=\; 4c\,\Lambda^2\,(1 - \Lambda),
$$
whose unique root $\Lambda^* \in (1/\sqrt{c},\, 1)$ gives the fold in closed form: $u^* = \dfrac{c\Lambda^{*2} + 1}{c\Lambda^*(1-\Lambda^*)(c\Lambda^{*2} - 1)}$, $P_{\text{fold}} = \dfrac{c\Lambda^{*2} + 1}{7}$. The classification over targets is complete and exact: for $c < c^\dagger = 392/121 = 2^3\cdot 7^2/11^2 \approx 3.240$ the $g_V$-clamp is inactive at the fold and the quintic root governs; for $c \geq c^\dagger$ the clamp is reached first — on the clamped branch the tangency condition degenerates to the quadratic $c\Lambda^2 + 2\Lambda - 1 = 0$, whose critical point falls outside that branch, so the fold sits exactly at the kink $\Lambda^* = \sqrt{2/c}$: **the living branch is born exactly at the reflexivity ceiling** $P_{\text{fold}} = 3/7$ ($R = 1/3$), with the elementary form $u^* = 3/(\sqrt{2c} - 2)$. At the switch target $P^{*\dagger} = 513/847 \approx 0.606$ both formulas give $u^* = 11/2$ exactly. At the Goldilocks ceiling $c = 2$: $8\lambda^5 - 4\lambda^4 + 8\lambda^3 - 8\lambda^2 - 2\lambda + 1 = 0$, $\Lambda^* = 0.858013$, $u^* = 21.4811$, $P_{\text{fold}} = 0.3532$ — the quintic and the direct numerical fold agree to $10^{-11}$ (spot-checks: $c = 1.5$: $u^* = 75.371$ to $10^{-10}$; $c = 3$: $6.61011$ to $10^{-13}$; edge regime $c = 4$: $3.62132$, $c = 6$: $2.04904$, with the stationary roots merging at $P = 3/7$). **Galois status [T]:** the $c = 2$ quintic is irreducible over $\mathbb{Q}$ with Galois group $S_5$ (order $120$), hence **not solvable in radicals**: the fold constant is an algebraic number of degree five admitting no radical expression — in contrast to the gate-free floor $x_{\min} = 1 + \sqrt{2}$, which is radical. "Closed form" for the self-consistent dead-zone boundary terminates, provably, at the quintic itself. This fold is the microscopic Phase-III boundary, realizing exactly the saddle-node normal form of the [bifurcation analysis](/docs/applied/coherence-cybernetics/bifurcation); the heuristic $r_c = P_{\text{crit}}/(7P)$ misses it by two orders of magnitude ($0.116$ vs $21.5$) and should be read as a topology marker, not a numerical boundary.
:::
