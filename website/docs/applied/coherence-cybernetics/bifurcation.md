...
Катастрофы более высокой коразмерности ($A_5$ — «бабочка», $D_4$ — «кошелёк»), требующие $\geq 4$ управляющих параметров, могут описывать переходы с участием L4 и композитных систем. Переход L3→L4 как $A_5$-бабочка — **[С]** (L4 гипотетичен). Это — открытая проблема.
:::
