...
**Что измеряет.** Насколько внутренняя модель $\varphi(\Gamma)$ близка к реальному состоянию $\Gamma$. Если $R = 1$, система *абсолютно точно* видит себя. Если $R = 0$, самомодель полностью неадекватна.
...
| L2 | $R \geq R_{\text{th}} = 1/3$, $\Phi \geq \Phi_{\text{th}} = 1$, $D_{\text{diff}} \geq 2$ | $R_{\text{th}}$ **[Т]** ($K=3$ из [триадной декомпозиции](/docs/core/operators/lindblad-operators#триадная-декомпозиция)), $\Phi_{\text{th}}$ **[Т]** (T-129), $D_{\min}$ **[Т]** (T-151) | Сознание: система видит себя, интегрирована, различает состояния |
