...
| **Quality** | $\{[|q_k\rangle]\} \subset \mathbb{P}(\mathcal{H}_E)$ | Характер переживания |
...
$$
D_{\text{diff}}(\Gamma) := \mathrm{rank}(\rho_E) \in \{1, 2, \ldots, 7\}
$$
...
:::warning Эмпирические пороги
Значения $R_{\text{th}} = 0.3$, $\Phi_{\text{th}} = 1.0$ имеют **эмпирическое обоснование** через связь с GWT и IIT. Они не выведены теоретически из аксиом УГМ.

**См.:** [Обоснование порогов](/docs/proofs/interiority-hierarchy#обоснование-порогов)
:::
...
где:
- $P_{\text{critical}} \approx 0.3$ — **эмпирическое значение**, не выведенное из аксиом
- $\varepsilon_{\text{death}} = 0.01 \cdot P_{\text{critical}} / \tau_{\text{char}}$ — порог скорости распада
...
где $\theta_i$ — пороги для каждого измерения.
