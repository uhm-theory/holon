...
:::note О τ_char
Характерное время $\tau_{\text{char}}$ определяется системой и может быть оценено как:
- Для биологических систем: период метаболического цикла ($\sim 1$ секунда для клетки)
- Для нейронных систем: постоянная времени мембраны ($\sim 10$ мс)
- Для социальных систем: период обновления ($\sim 1$ год)
:::
