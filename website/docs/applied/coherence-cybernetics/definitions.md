...
:::warning Неопределённые величины
Числители компонент тензора ($I_{\mathrm{env}}$, $I_{\mathrm{struct}}$, $C_{\mathrm{used}}$, $C_{\mathrm{max}}$, $I_{\mathrm{verify}}$, $I_{\mathrm{self}}$, $I_{\mathrm{exp}}$, $I_{\mathrm{mem}}$, $I_{\mathrm{ground}}$, $I_{\mathrm{unity}}$, $I_{\mathrm{social}}$) **не имеют формальных определений** в текущей версии КК. Их операционализация зависит от конкретной системы (биологическая, ИИ, организационная) и является частью [исследовательской программы](./applications). Без формальных определений тензор $\sigma_{\mathrm{sys}}$ остаётся **схемой**, а не строгим определением.
:::
