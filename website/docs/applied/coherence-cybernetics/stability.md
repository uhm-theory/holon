...
$$
\frac{dP}{d\tau} = \underbrace{-2\mathrm{Tr}(\Gamma \cdot \mathcal{D}_\Omega[\Gamma])}_{\text{диссипация } \leq 0} + \underbrace{2\kappa(\Gamma) \cdot \Theta(\Delta F) \cdot \mathrm{Tr}(\Gamma \cdot (\rho_* - \Gamma))}_{\text{регенерация}}
$$

**Условие гомеостаза** — регенерация компенсирует диссипацию:

$$
\kappa(\Gamma) \cdot \Theta(\Delta F) \cdot \mathrm{Tr}(\Gamma \cdot (\rho_* - \Gamma)) \geq \mathrm{Tr}(\Gamma \cdot \mathcal{D}_\Omega[\Gamma])
$$
...
[Теорема T-59 (κ_bootstrap)](/docs/core/foundations/axiom-omega#теорема-kappa-bootstrap-bound) [Т]: $\kappa_{\text{bootstrap}} \geq 2/9$ обеспечивает **минимальную регенерацию** даже при $\mathrm{Coh}_E = 0$:

$$
\kappa(\Gamma) \geq \kappa_{\text{bootstrap}} \geq \frac{2}{9} > 0
$$
...
$$
\kappa_{\text{bootstrap}} \cdot \Theta(\Delta F) \cdot \mathrm{Tr}(\Gamma \cdot (\rho_* - \Gamma)) \geq \mathrm{Tr}(\Gamma \cdot \mathcal{D}_\Omega[\Gamma])
$$
...
2. **$\Delta F$ (доступная свободная энергия)** — затвор Ландауэра ($\Theta(\Delta F)$)
...
### 6.1 Затвор Ландауэра

Затвор Ландауэра в регенерации $\mathcal{R} = \kappa(\rho_* - \Gamma) \cdot \Theta(\Delta F)$:

$$
\Theta(\Delta F) = \begin{cases} 1 & \text{если } \Delta F > 0 \\ 0 & \text{если } \Delta F \leq 0 \end{cases}
$$

**Следствие:** при $\Delta F \leq 0$ регенерация **полностью отключена** — система деградирует до тривиального $I/7$ независимо от $\kappa$.
...
**Энергетическая смерть:** $\Delta F \to 0$ → $\Theta = 0$ → регенерация выключена → необратимая деградация.
...
| 2 | $\Delta F$ gate | Бинарный $\Theta({\Delta F})$ вызывает осцилляции | Плавный Ландауэр: $\Theta_\varepsilon(\Delta F) = \sigma(\Delta F / \varepsilon)$ |
| 3 | Chicken-egg $R \leftrightarrow \varphi$ | $R$ зависит от $\varphi$, $\varphi$ зависит от $R$ | Bootstrap: [T-59](/docs/core/foundations/axiom-omega#теорема-kappa-bootstrap-bound) [Т] — $\kappa_{\mathrm{bootstrap}} \geq 2/9$ обеспечивает начальную регенерацию |
| 4 | $\kappa_{\mathrm{bootstrap}}$ занижен | Из-за неполного инициализирования | Канонический: $\kappa_{\mathrm{bootstrap}} = \omega_0/7 \geq 2/9$ |
...
| Гомеостаз | $\kappa \cdot \Theta(\Delta F) \cdot \mathrm{Tr}(\Gamma(\rho_* - \Gamma)) \geq \mathrm{Tr}(\Gamma \cdot \mathcal{D}_\Omega[\Gamma])$ | $\kappa / \Gamma_2$ | — |
| Спираль смерти | $\kappa_{\text{bootstrap}} \geq 2/9$ | $\kappa_{\text{bootstrap}}$ | T-59 [Т] |
