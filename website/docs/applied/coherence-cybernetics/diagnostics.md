...
**Механизм:** $\Delta F \to 0$ → затвор Ландауэра закрывается ($\Theta(\Delta F) = 0$) → регенерация полностью отключена → необратимая деградация.
...
**Вмешательство:** **немедленное** восстановление $\Delta F > 0$ — единственный способ. Без энергии никакие $h^{(R)}$-интервенции не работают.
...
- [ ] **Регенерация:** $\kappa_{\text{bootstrap}} \geq 2/9$ (T-59 [Т])
