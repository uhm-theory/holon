...
graph TD
    ...
    subgraph "Структурные свойства"
        P3["Св. 3: ∞-Терминальный объект"]
        P4["Св. 4: Самомоделирование φ"]
        P5["Св. 5: Стратификация"]
    end
    subgraph "Следствия"
        S["Теорема S: dim=7"]
        APQ["AP+PH+QG+V"]
...
## Таблица следствий из аксиом

| Аксиома / Свойство | Ключевое следствие | Формула |
|---------------------|-------------------|---------|
| Акс. 1 (Структура) | Единственность примитива | $\mathcal{E} := \text{Sh}_\infty(\mathcal{C})$ |
| Акс. 2 (Метрика) | Принцип информационной различимости | $J_{Bures}$ — единственная монотонная метрика |
| Акс. 3 (Размерность) | Семимерность | $N = 7$ |
| Акс. 4 (Масштаб) | Связь внутреннего и физического времени | $\omega_0 > 0$ |
| Акс. 5 (Page-Wootters) | Эмерджентное время τ | $[\hat{C}, \Gamma_{total}] = 0$ |
| Св. 3 (∞-Терминальный объект) | Направленность эволюции | $\forall \Gamma, \exists! f: \Gamma \to T$ |
| Св. 4 (Самомоделирование) | Форма φ | $\varphi \dashv i$ (сопряжение)* |
| Св. 5 (Стратификация) | Иерархия сложности | $X = \bigsqcup_\alpha S_\alpha$ |

*Вариационная форма $\varphi = \arg\min \mathbb{E}[S_{spec} + D_{KL}]$ — **теорема** о свойствах категориально определённого φ.

## Аксиоматическая база

:::info Связь с аксиоматикой УГМ
КК строится на **Аксиоме Ω⁷** (пять аксиом с ∞-топосом Sh_∞(𝒞) как примитивом). Все остальные утверждения — **следствия** из этих аксиом.
:::

### Аксиома Ω⁷ (Пять аксиом)

| № | Аксиома | Формулировка |
|---|---------|--------------|
| 1 | Структура (∞-Топос) | $\mathcal{E} := \text{Sh}_\infty(\mathcal{C})$ — примитив; см. [топология Гротендика](/docs/core/foundations/axiom-omega#топология-гротендика) |
| 2 | Метрика (Бюрес) | Топология Гротендика $J$ индуцирована метрикой Бюреса $d_B$ ([Теорема Ченцова-Петца](/docs/core/foundations/axiom-omega#топология-гротендика)) |
| 3 | Размерность | $N = 7$ — размерность базового пространства |
| 4 | Масштаб | $\omega_0 > 0$ — характерная частота системы |
| 5 | Page-Wootters | $[\hat{C}, \Gamma_{total}] = 0$ — ограничение тензорной декомпозиции $\mathcal{H} = \mathcal{H}_O \otimes \mathcal{H}_{rest}$ |

:::note Структурные свойства (не аксиомы)
Следующие свойства **выводятся** из структуры ∞-топоса, а не постулируются:

| № | Свойство | Формулировка |
|---|----------|--------------|
| 3 | ∞-Терминальный объект | $\text{Map}_{\mathcal{C}_\infty}(\Gamma, T) \simeq *$ |
...
$\dim(\mathcal{H}) = 7$ — **минимальная** размерность для системы, удовлетворяющей (AP), (PH), (QG) ([двухтрековое обоснование](/docs/core/foundations/axiom-omega#октонионная-структура)).
...
```mermaid
graph TD
    O["Аксиома Ω⁷: ∞-топос"] --> APQ["Аксиома Септичности"]
    APQ --> S["Теорема S: dim = 7"]
    S --> D["Уравнение эволюции"]
    D --> K["κ = κ bootstrap + κ₀ Coh E"]
