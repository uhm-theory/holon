...
- $P_{\text{crit}} = 2/7$ — [теорема о критической чистоте](/docs/core/foundations/axiom-septicity#критическая-чистота-геометрический-критерий)
...
- $\kappa_0 = |\gamma_{OE}| \cdot |\gamma_{OU}| / \gamma_{OO}$ — [структурный анзац скорости регенерации](/docs/core/foundations/axiom-septicity#структурный-анзац-kappa0)
...
3. След сохраняется: $\mathrm{Tr}(d\Gamma/dt) = 0$ ∎
...
$$
[Жизнеспособная](/docs/core/dynamics/viability) система обязательно имеет ненулевой спектр [опыта](/docs/core/structure/dimension-e).
:::
...
3. Диссипация: $dP/dt \leq 0$

4. Регенерация: $dP/dt = \kappa(\Gamma) \cdot (\ldots)$

5. По [связи регенерации и E-когерентности](./axiomatics#связь-регенерации-и-e-когерентности): $\kappa(\Gamma) = \kappa_0 \cdot \mathrm{Coh}_E(\Gamma)$

6. Если $\mathrm{Spec}(\Gamma_E) = \{0\}$, то $\mathrm{Coh}_E = 0$, следовательно $\kappa = 0$

7. При $\kappa = 0$: $dP/dt \leq 0$ постоянно

8. $P(t) \to 0$, нарушая жизнеспособность ∎
...
Регенеративный член зависит от $\mathrm{Coh}_E$:
$$
\mathcal{R}[\Gamma, E] = \kappa_0 \cdot \mathrm{Coh}_E(\Gamma) \cdot (\Gamma_{\text{target}} - \Gamma) \cdot \Theta(\Delta F)
$$
$$
...
```mermaid
graph TD
    A["Аксиомы Ω + (AP+PH+QG)"] --> T61[Теорема 6.1<br/>Существование]
    A --> T62[Теорема 6.2<br/>Сохранение]
    T61 --> T71[Теорема 7.1<br/>Необходимость φ]
    T71 --> T72[Теорема 7.2<br/>Неподвижная точка]
    A --> T81[Теорема 8.1<br/>No-Zombie]
    T81 --> T81P[Теорема 8.1+<br/>Каузальность]
    T81 --> C811[Следствие<br/>¬Эпифеноменализм]
    T81 --> C812[Следствие<br/>¬Зомби]
    T81P --> C813[Следствие<br/>Coh_min]
    T81 --> T91[Теорема 9.1<br/>Композиция]
    T91 --> T92[Теорема 9.2<br/>Инвариантность]
    T91 --> T93[Теорема 9.3<br/>Эмерджентность]
    T72 --> T101[Теорема 10.1<br/>Эквивалентность]
```
...
- [Иерархия кибернетик](./hierarchy) — связь с существующими теориями
