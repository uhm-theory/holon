...
[Эмерджентность](./theorems#гипотеза-93-эмерджентность) — следствие нелинейности.
...
L2 требует $R \geq R_{\text{th}} = 1/3$, $\Phi \geq \Phi_{\text{th}} = 1$ и $D_{\text{diff}} \geq 2$ ([теоремы из ПИР](/docs/core/foundations/axiom-septicity#пороги-l2-строгий-вывод)).
...
- [Теоремы](./theorems) — эмерджентность, композиция
