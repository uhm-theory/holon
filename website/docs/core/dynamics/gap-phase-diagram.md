...
:::tip Теорема 5.2 (Swallowtail-каскад и L-уровни) [С]
При 3 управляющих параметрах возникает **swallowtail** (ласточкин хвост) — катастрофа с 4 листами. [Т] — стандартный результат теории катастроф (Арнольд, 1975).

Соответствие листов swallowtail [уровням интериорности](/docs/consciousness/hierarchy/interiority-hierarchy) носит **модельный** характер [С] (при допущении, что $V_{\text{eff}}$ адекватно аппроксимируется нормальной формой $A_4$):
...
:::tip Теорема 5.3 (Три минимума Gap-потенциала и L-уровни) [С]
...
**Переходы между L-уровнями** — фазовые переходы первого рода (fold-бифуркации): [С]
...
— подавление октонионной неассоциативности ниже порога swallowtail. [С]

:::warning Статус swallowtail-модели [С]
Теоремы 5.2 и 5.3 предполагают, что эффективный потенциал $V_{\text{eff}}(G)$ адекватно аппроксимируется нормальной формой $A_4$ вблизи трикритической точки. Строгое доказательство того, что полный потенциал $V_{\text{Gap}}$ (из [термодинамики Gap](/docs/core/dynamics/gap-thermodynamics#потенциал-v-gap)) допускает $A_4$-редукцию, не завершено. Отождествление минимумов с L-уровнями — **модельная гипотеза**, а не следствие аксиом.
:::
