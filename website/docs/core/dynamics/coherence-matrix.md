...
| $\gamma_{AD}$ | **Сигнал** | Динамически поддерживаемое различение |
| $\gamma_{AL}$ | **Категоризация** | Логически упорядоченное различение |
| $\gamma_{AE}$ | **Внимание** | Осознанное различение |
| $\gamma_{AO}$ | **Эмерджентность** | Возникновение различий из основания |
| $\gamma_{AU}$ | **Дифференциация** | Различение, сохраняющее целостность |
| $\gamma_{SD}$ | **Метаболизм** | Динамическое поддержание устойчивой формы |
| $\gamma_{SL}$ | **Закон** | Логически устойчивая структура |
| $\gamma_{SE}$ | **Гештальт** | Структурная организация переживания |
| $\gamma_{SO}$ | **Архетип** | Устойчивые формы, укоренённые в основании |
| $\gamma_{SU}$ | **Симметрия** | Структурное выражение единства |
| $\gamma_{DL}$ | **Вычисление** | Логически управляемый процесс |
| $\gamma_{DE}$ | **Аффект** | Переживаемое качество изменения |
| $\gamma_{DO}$ | **Генезис** | Порождающий процесс из основания |
| $\gamma_{DU}$ | **Целенаправленность** | Интегрированное изменение |
| $\gamma_{LE}$ | **Понимание** | Переживаемая логическая связность |
| $\gamma_{LO}$ | **Необходимость** | Логика, укоренённая в природе бытия |
| $\gamma_{LU}$ | **Самосогласованность** | Логическая интеграция целого |
| $\gamma_{EO}$ | **Глубина** | Связь переживания с источником |
...
| Когерентность | Физика | Биология | Когниция |
|---------------|--------|----------|----------|
| $\gamma_{AS}$ | Нарушение симметрии → частицы | Дифференциация → морфология | Распознавание → концепт |
| $\gamma_{AD}$ | Распространение сигнала | Стимул → отклик | Восприятие → реакция |
| $\gamma_{AL}$ | Алгебра наблюдаемых | Таксономия | Классификация |
| $\gamma_{AE}$ | Измерение → результат | Сенсорная дифференциация | Фокус внимания |
| $\gamma_{AO}$ | Вакуумные флуктуации → частица | Спонтанное формообразование | Инсайт |
| $\gamma_{AU}$ | Голографический принцип | Орган внутри организма | Анализ внутри понимания |
| $\gamma_{SD}$ | Диссипативные структуры | Гомеостаз | Рабочая память |
| $\gamma_{SL}$ | Уравнения состояния | Генетический код | Правила грамматики |
| $\gamma_{SE}$ | Наблюдаемая ↔ спектр | Нейронный коррелят | Гештальт-восприятие |
| $\gamma_{SO}$ | Фундаментальные константы | Генетический план | Врождённые категории |
| $\gamma_{SU}$ | Теорема Нётер | Организменная когерентность | Системность мировоззрения |
| $\gamma_{DL}$ | Уравнения движения | Метаболические пути | Алгоритмическое мышление |
| $\gamma_{DE}$ | Энергия → температура | Нейродинамика → эмоция | Чувство изменения |
| $\gamma_{DO}$ | Космогенез, рождение пар | Биогенез, онтогенез | Творчество |
| $\gamma_{DU}$ | Принцип наименьшего действия | Целенаправленное поведение | Волевой акт |
| $\gamma_{LE}$ | Математическая красота | Адаптивная когниция | Момент «ага!» |
| $\gamma_{LO}$ | «Почему эти законы?» | Эволюционные ограничения | Априорное знание |
| $\gamma_{LU}$ | Калибровочная инвариантность | Системная связность | Связность мировоззрения |
| $\gamma_{EO}$ | Наблюдатель ↔ вакуум | Глубинное переживание | Медитативные состояния |
| $\gamma_{EU}$ | Нелокальные корреляции | Единство перцептивного поля | Интеграция сознания |
| $\gamma_{OU}$ | Вселенная как замкнутая система | Экосистема как целое | Недвойственное осознавание |
...
[Плоскость Фано](../../proofs/theorem-octonionic-derivation#плоскость-фано) PG(2,2) выделяет **7 триплетов** — линий, на которых октонионное умножение замкнуто. Каждый триплет $(e_i, e_j, e_k)$ определяет ассоциативную подалгебру, изоморфную Im($\mathbb{H}$).

**Предсказание [И]:** Когерентности внутри Фано-триплетов могут проявлять более сильные корреляции, чем между триплетами. Мост формально открыт.
