...
— полный энергетический оператор. Физические состояния удовлетворяют $[\hat{C}, \Gamma_{\mathrm{total}}] = 0$ ([T-87 [Т]](/docs/core/operators/emergent-time)). Из этого ограничения следует эмерджентное время $\tau$ через механизм Page-Wootters.
