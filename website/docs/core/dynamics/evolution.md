...
Все 4 операционализации ΔF согласованы с **единой канонической формулой** через [метрику Бюреса](/docs/proofs/emergent-time#41-метрика-бурес):
...
- $\varphi(\Gamma)$ — [самомодель](/docs/proofs/formalization-phi)
:::
...
$$
\mathrm{Tr}_A[\tilde{\mathcal{R}}_A[\Gamma_{AB}]] = \kappa_A \cdot \Theta(\Delta F_A) \cdot \left(\underbrace{\mathrm{Tr}_A[(\varphi_A \otimes \mathrm{id}_B)(\Gamma_{AB})]}_{\Gamma_B} - \underbrace{\mathrm{Tr}_A[\Gamma_{AB}]}_{\Gamma_B}\right) = 0
$$

Ключевое свойство: для любого CPTP-канала $\Phi_A$ на подсистеме $A$: $\mathrm{Tr}_A[(\Phi_A \otimes \mathrm{id}_B)(\rho_{AB})] = \rho_B$. Это следует из условия полноты операторов Крауса: $\sum_m K_m^\dagger K_m = I$. ∎

**Следствия:**

1. Нелинейность $\kappa(\Gamma)$ **не нарушает** запрет сигнализации — скалярный множитель выносится за след
2. Защита **структурная**: не зависит от конкретного вида $\kappa$, $\varphi$ или $\Delta F$
3. Теория полностью совместима с результатами Гизина (1990) и Полчинского (1991) — нелинейность УГМ принципиально отличается от «нелинейной квантовой механики» Вайнберга
...
| **NS3** (CPTP-свойство φ) | $\varphi$ — CPTP-канал | Определение [оператора самомоделирования](../consciousness/self-observation#оператор-самомоделирования-φ) |

Полное доказательство с категориальной формализацией: [Соответствие с физикой: Запрет сигнализации](../../proofs/physics-correspondence#запрет-сигнализации).
...
$$
S[\Gamma(\tau)] = \int_0^T \mathcal{L}(\Gamma, \dot{\Gamma}) \, d\tau
$$
...
$$
\frac{\delta S}{\delta \Gamma} = 0 \quad \Rightarrow \quad \frac{d\Gamma}{d\tau} = f(\Gamma, \nabla_\Gamma F, \ldots)
$$
...
- Мост формально открыт

[Структурный вывод →](../../proofs/theorem-octonionic-derivation)
:::
...
- [Формализация φ](../../proofs/formalization-phi) — спектральная формула φ и $R^{(n)}$
- [Иерархия интериорности](../../proofs/interiority-hierarchy) — уровни L0→L4 и метастабильность L3
