...
*Proof.* ($\subseteq$): $\|\sigma_{\mathrm{sys}}\|_\infty < 1$ implies, in particular, $\sigma_U = 2\Phi_{\mathrm{th}}/(\Phi_{\mathrm{th}} + \Phi) < 1$, i.e. $\Phi > \Phi_{\mathrm{th}} = 1$. By Cauchy–Schwarz, $\sum_i \gamma_{ii}^2 \geq (\sum_i \gamma_{ii})^2/7 = 1/7$, and $P = \operatorname{Tr}\Gamma^2 = (1 + \Phi)\sum_i \gamma_{ii}^2 > 2 \cdot \tfrac{1}{7}$ — hence $P > 2/7$. (Errata 2026-07-22: the earlier argument via $\sigma_A$ did not suffice; with the pre-errata $\sigma_U = 1 - \Phi$ the embedding failed on an explicit counterexample — see the errata note at [T-92](/docs/applied/coherence-cybernetics/theorems#теорема-101-эквивалентность-условий).) ($\subsetneq$): Counterexample — the pure state $|1\rangle\langle 1| \in \mathcal{V}_P$ ($P = 1$), but $\sigma_U = 2\Phi_{\mathrm{th}}/(\Phi_{\mathrm{th}} + \Phi) = 2 \geq 1$ (zero integration $\Phi = 0$), therefore $\|\sigma_{\mathrm{sys}}\|_\infty \geq 1$ and $|1\rangle\langle 1| \notin \mathcal{V}_{\mathrm{full}}$. $\blacksquare$
