...
Значение $P_{\text{crit}} = 2/N$ выводится из **пяти независимых аргументов**, сходящихся к одному значению:

| Путь | Критерий | Результат | Статус |
|------|----------|-----------|--------|
| 1. Геометрический | $\|\Gamma - I_N/N\|_F^2 > \|I_N/N\|_F^2$ | $P > 2/N$ | ✅ Строго |
| 2. Информационный | $D_{KL} \geq \frac{1}{2}$ нат | $P \gtrsim 2/N$ | ✅ Строго (в линейном приближении) |
| 3. Структурный | SNR $\geq 1$ | $P \geq 2/N$ | ✅ Строго |
| 4. Спектральный | $\lambda_{\max} \approx 1/2$ | $P \approx 2/N$ | ⚠️ Приближённо |
| 5. Автопоэтический | нарушение $U(N)$ | $P > 2/N$ | ⚠️ Структурный аргумент |

**Примечание:** Пути 1-3 дают **строгие** неравенства, сходящиеся к $P_{\text{crit}} = 2/N$. Пути 4-5 дают **согласованные** результаты через другие рассуждения. Строгое доказательство **эквивалентности** всех пяти путей — открытая задача.
...
Критическая чистота связана с минимальной скоростью течения [внутреннего времени](../../proofs/emergent-time):
...
[Подробнее →](../../proofs/emergent-time#8-связь-с-критической-чистотой)
:::
...
$$
\Gamma(0) \in \mathcal{V} \land \mathcal{R}[\Gamma] \geq \mathcal{D}[\Gamma] \implies \Gamma(t) \in \mathcal{V} \quad \forall t > 0
$$
:::
...
- [Теорема об эмерджентном времени](../../proofs/emergent-time) — временна́я интерпретация P_crit
