...
Число 7 — **аксиома** ([Аксиома 3](/docs/core/foundations/axiom-omega#аксиоматика)), характеризующая класс изучаемых систем. [Теорема S](../../proofs/theorem-minimality-7) объясняет, *почему этот класс интересен*: 7 — минимальная размерность для (AP)+(PH)+(QG).
...
| VI | [Основание](./dimension-o) | $O$ | Питать + Измерять время | Вакуум $\vert 0\rangle$, Часы | Квантовое поле + Часы | $e_6$ |
| VII | [Единство](./dimension-u) | $U$ | Интегрировать | След $\mathrm{Tr}$ | Нормировка | $e_7$ |
...
[Подробнее: Теорема об эмерджентном времени →](../../proofs/emergent-time)
:::

## Октонионная интерпретация {#октонионная-интерпретация}

:::info Структурный вывод N = 7 через октонионы
Число 7 измерений получает **второе, независимое обоснование** через [структурный вывод](../../proofs/theorem-octonionic-derivation): если пространство внутренних степеней свободы изоморфно Im(𝕆) (мнимой части октонионов), то $N = \dim(\text{Im}(\mathbb{O})) = 7$.
:::
...
Конкретное отождествление $e_i$ ↔ измерение — **интерпретация** [И], мост формально открыт. $G_2$ действует на Im(𝕆); вопрос о том, является ли эта симметрия физически реализованной в пространстве {A,S,D,L,E,O,U}, остаётся [открытой проблемой](../../proofs/theorem-octonionic-derivation#открытые-проблемы).
:::
...
- [Теорема S](../../proofs/theorem-minimality-7) — доказательство минимальности 7D (Трек A)
...
- [Эмерджентное время](../../proofs/emergent-time) — время из структуры Γ
