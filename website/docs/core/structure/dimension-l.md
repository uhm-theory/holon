...
| II | Жизнь | $\Omega_{viable}$ — P > [P_crit](/docs/proofs/theorem-purity-critical) | $\sum_j R_j P_j$ | QECC |
...
Определения $I_{\text{verify}}$, $\theta_L$ и $\sigma_L$ **строго определены** через классификатор подобъектов Ω и энтропию фон Неймана.
:::
...
#### Строгое определение σ_L {#строгое-определение-sigma-l}
...
$$
\sigma_L(\Gamma) = \frac{7(1 - \gamma_{LL})}{6}
$$

**Доказательство формулы для 7D:**
...
|----------|-------------|--------|
| $I_{\text{verify}}$ | $I(\Gamma : L) = S_{vN}(\rho) - S_{vN}(\rho \| L)$ — взаимная информация | ✅ Формализовано |
| $\theta_L$ | $\gamma_{LL} \cdot \log(N)$ — пропускная способность | ✅ Формализовано |
| $\gamma_{LL}$ | Населённость измерения L | ✅ Определено |
| $\sigma_L$ | $S_{vN}(\rho_L) / (\gamma_{LL} \cdot \log N)$ — логическая нагрузка | ✅ Формализовано |
...
Измерению соответствует $e_4 \in \mathrm{Im}(\mathbb{O})$. Детали, $G_2$-оговорка и Фано-триплеты: [Октонионная интерпретация](./dimensions#октонионная-интерпретация), [структурный вывод](../../proofs/theorem-octonionic-derivation).
:::
