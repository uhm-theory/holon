...
— множество предикатов, истинных на $\Gamma$. Логическая система: **интуиционистская** (Гейтинговая алгебра — внутренняя логика топоса) [Т]. Это следует из стандартного результата теории топосов: подобъектный классификатор $\Omega$ в любом топосе образует объект Гейтинговой алгебры, а не булевой — отсюда интуиционистская (а не классическая) логика.
:::
...
:::warning Условие полной положительности [С]
Равенство $\sum_k L_k^\dagger L_k = \mathbb{1}$ гарантирует **сохранение следа** (TP), но не **полную положительность** (CP). Полная положительность требует, чтобы операторы $L_k = \sqrt{\chi_{S_k}}$ были корректно определены как линейные операторы на $\mathcal{H}$, что обеспечивается атомарностью подобъектов $S_k$ (каждый $\chi_{S_k}$ — проектор). Это верно по построению для атомов классификатора, но доказательство CPTP-свойства полного канала зависит от структуры стратификации [С].
:::
