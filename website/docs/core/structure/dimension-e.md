...
- $\mathrm{Coh}_E \approx \min\!\left(\gamma_{EE} + 2\sqrt{\sum_{i \neq E} |\gamma_{Ei}|^2},\; 1\right)$ — [приближение E-когерентности](/docs/applied/coherence-cybernetics/definitions#e-когерентность), с нормализацией $\mathrm{Coh}_E \in [0, 1]$ ✅
...
Измерению соответствует $e_5 \in \mathrm{Im}(\mathbb{O})$. Детали, $G_2$-оговорка и Фано-триплеты: [Октонионная интерпретация](./dimensions#октонионная-интерпретация), [структурный вывод](../../proofs/theorem-octonionic-derivation).
:::
...
- [Иерархия интериорности](../../proofs/interiority-hierarchy) — формальные определения L0→L1→L2→L3→L4
