...
- $\kappa(\Gamma) = \kappa_{\text{bootstrap}} + \kappa_0 \cdot \mathrm{Coh}_E(\Gamma)$ — скорость регенерации, где $\kappa_{\text{bootstrap}}$ — минимальная регенерация даже при отсутствии E-когерентности, $\kappa_0$ — [категориальный вывод](../foundations/axiom-septicity#структурный-анзац-kappa0)
...
Измерению соответствует $e_3 \in \mathrm{Im}(\mathbb{O})$. Детали, $G_2$-оговорка и Фано-триплеты: [Октонионная интерпретация](./dimensions#октонионная-интерпретация), [структурный вывод](../../proofs/theorem-octonionic-derivation).
:::
