...
где:
- $\kappa(\Gamma) > 0$ — скорость регенерации
...
Измерению соответствует $e_6 \in \mathrm{Im}(\mathbb{O})$. Детали, $G_2$-оговорка и Фано-триплеты: [Октонионная интерпретация](./dimensions#октонионная-интерпретация), [структурный вывод](../../proofs/theorem-octonionic-derivation).
:::
