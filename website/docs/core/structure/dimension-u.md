...
:::warning Уточнение K₁
K₁(Mₙ(ℂ)) = 0 для конечномерных матричных алгебр. Нетривиальная K-теоретическая информация содержится в K₀ (проекторы) или в K-теории C*-алгебр бесконечномерных систем. Формализация связи Φ с K-теоретическими инвариантами — открытая задача.
:::
...
Мера сознательности $C = \Phi \times D_{\text{diff}} \times R$ (определение см. [самонаблюдение](../../core/consciousness/self-observation#мера-сознательности-c)).
...
Измерению соответствует $e_7 \in \mathrm{Im}(\mathbb{O})$. Детали, $G_2$-оговорка и Фано-триплеты: [Октонионная интерпретация](./dimensions#октонионная-интерпретация), [структурный вывод](../../proofs/theorem-octonionic-derivation).
:::
