...
$$
\text{Meaning}_{\text{peak}}(\mathbb{H}) = \max_t \left[ P(\Gamma_t) \cdot D_{\text{diff}}(\Gamma_t) \cdot \Phi(\Gamma_t) \cdot R(\Gamma_t) \right]
$$
...
Если существует то, что традиции называют «Богом» или «Высшим», оно — не «внешний законодатель», а **сама целостность $\Gamma$**, проявляющаяся через каждый Голоном. См. [Принцип имманентности](../foundations/consequences#4-принцип-имманентности).
:::
...
$$
P(\Gamma) < P_{\text{crit}} = \frac{2}{7}, \quad \frac{dP}{dt} < 0 \quad \Rightarrow \quad P \to \frac{1}{7}
$$
...
$$
\text{Meaning}_{\text{total}}(\mathbb{H}) = \int_0^{T} P(\tau) \cdot D_{\text{diff}}(\tau) \cdot \Phi(\tau) \cdot R(\tau) \, d\tau
$$
