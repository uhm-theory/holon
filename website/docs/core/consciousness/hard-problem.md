...
4. **Условия** сознательности (L2: $R \geq R_{th}$, $\Phi \geq \Phi_{th}$ — [эмпирические пороги](../../proofs/interiority-hierarchy#обоснование-порогов))
...
$$
C \geq C_{th} \approx 0.6
$$

при $R \geq R_{th} \approx 0.3$ и $\Phi \geq \Phi_{th} \approx 1.0$.
...
**Позиция Hoffman:** Пространство-время не фундаментально; реальность — сеть сознательных агентов.

**Связь с УГМ:**

| Аспект | Hoffman | УГМ | Совместимость |
|--------|---------|-----|---------------|
| Примитив | Conscious Agent | $\Gamma$ | Агент $\approx$ L2-Голоном? |
...
Сознательный агент Hoffman = Голоном с $R \geq R_{th}$, $\Phi \geq \Phi_{th}$ (L2-Голоном). Марковское ядро = CPTP-канал. Это требует формального доказательства.
:::

#### Global Workspace Theory (Baars)

**GWT:** Сознание = глобальная доступность информации.
...
| HOT | Рефлексия $R$ = higher-order | Концептуально |
| Панпсихизм | L0 = универсальная интериорность | Формализовано |
| Hoffman | Агент $\approx$ L2-Голоном | Гипотеза |
...
1. Формальное доказательство эквивалентности с Hoffman
...
3. Предсказывает условия сознательности ($R \geq R_{th}$, $\Phi \geq \Phi_{th}$ — эмпирические пороги)
