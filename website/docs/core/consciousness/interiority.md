...
|---------|----------|---------------|
| $d_{FS} = 0$ | $|\psi\rangle = e^{i\theta}|\phi\rangle$ | Одинаковые качества |
| $d_{FS} = \pi/2$ | $\langle\psi|\phi\rangle = 0$ | Максимально различные качества |
...
При $R \geq R_{th} \approx 0.3$ и $\Phi \geq \Phi_{th} \approx 1.0$ компоненты опыта становятся рефлексивно доступными.
...
| Качество | $[|q_{\text{red}}\rangle] \in \mathbb{P}(\mathcal{H}_E)$ | Оттенок красного |
...
| Качество | $[|q_{\text{pain}}\rangle] \in \mathbb{P}(\mathcal{H}_E)$ | Острая/тупая/пульсирующая |
