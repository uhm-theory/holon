...
- **Гравитация:** [С] Спектральное действие Чамседдина-Конна из конечной тройки
:::
...
**Гипотеза [Г]:** Отношение собственных времён может зависеть от различия конфигураций:
...
**Прогресс:** [Конечная спектральная тройка](#теорема-спектральная-тройка) $(A_{\text{int}}, H_{\text{int}}, D_{\text{int}})$ построена [Т]. Спектральное действие $S = \text{Tr}(f(D/\Lambda))$ даёт $\int(a_0\Lambda^4 + a_2\Lambda^2 R + \ldots)\sqrt{g}\,d^4x$ с $a_2 = \text{Tr}(D_{\text{int}}^{-2})$ [С]. Открыто: полная проверка аксиом NCG для непрерывной×конечной тройки.
:::
...
| **Уравнения Эйнштейна** | [С] | Спектральное действие из $(A_{\text{int}}, H_{\text{int}}, D_{\text{int}})$ |
...
2. **Уравнения Эйнштейна:** Как получить $R_{\mu\nu} - \frac{1}{2}g_{\mu\nu}R = 8\pi G T_{\mu\nu}$ на компактифицированном $M^4$? — [С] через [спектральное действие Чамседдина-Конна](#теорема-спектральная-тройка)
