...
| **Аксиома 4 (Масштаб)** | $\omega_0 > 0$ — фундаментальная частота | Связывает внутреннее время $\tau$ с физическим временем $t$ |
...
### Свойство 4: Вариационный принцип {#свойство-4}

:::note Свойство 4 (Принцип максимального сжатия)
Оператор самомоделирования $\varphi$ определяется вариационно:
...
- $\mu$ — инвариантная мера на пространстве состояний
:::

**Категориальное определение (разрешение цикличности):**

Оператор φ определяется как **левое сопряжение** к вложению подобъектов:
...
**Теорема (Согласованность φ и L_k):**

Для φ, определённого сопряжением, и L_k, определённых из Ω:

$$
\varphi(\Gamma) = \lim_{t \to \infty} e^{t\mathcal{L}_\Omega}[\Gamma]
$$

где $\mathcal{L}_\Omega$ — [логический Лиувиллиан](../dynamics/evolution#логический-лиувиллиан).

**Следствие:** φ — стационарное распределение динамики L_k. Цикличность разрешена: L_k и φ **независимо** выводятся из Ω.
...
где:
- $\kappa_{\text{bootstrap}} = \|\eta\| \approx 0.003$ — минимальная регенерация из единицы сопряжения
