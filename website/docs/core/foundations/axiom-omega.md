...
:::info Статус N = 7
Размерность $N = 7$ — **фундаментальная аксиома**, а не выводимая теорема. Интерпретация через условия (AP)+(PH)+(QG) в [Теореме S](./axiom-septicity#теорема-s-семимерность--следствие-из-аксиомы) объясняет, *почему этот класс интересен*, но не выводит N = 7 из ничего.
:::
...
| 4 | φ | ℒ_Ω | $\lim_{t \to \infty} e^{t \cdot \mathcal{L}_\Omega}[\Gamma]$ |
...
def calibrate_lambda(system, n_samples=1000):
    ...
    for sample in range(n_samples):
        ...
        Gamma_t1 = system.evolve(dt=0.01)
