...
|------|-------------|--------|
| **A** | [Теорема S](./axiom-septicity#теорема-s-семимерность--следствие-из-аксиомы): (AP)+(PH)+(QG) → N ≥ 7 | ✅ Доказано |
| **B** | [Структурный вывод](../../proofs/theorem-octonionic-derivation): P1+P2 → 𝕆 → dim Im(𝕆) = 7 | ✅ Математически строго |

Мост (AP)+(PH)+(QG) → P1+P2 — три сходящихся аргумента, [формально открыт](../../proofs/theorem-octonionic-derivation#мост).
:::
...
- $L_k := \sqrt{\gamma_{kk}} \cdot |k\rangle\langle k| / \sqrt{7}$ — нормированные операторы Линдблада

**УРОВЕНЬ 2: ТЕОРЕМЫ** (доказываются строго)
- $P_{crit} = 2/7$ ([критическая чистота](/docs/core/dynamics/viability#критическая-чистота))
- $R_{th} = 1/3$ ([теорема о пороге рефлексии](/docs/core/foundations/axiom-septicity#теорема-порог-рефлексии))
- $\Phi_{th} = 1$ ([теорема о пороге интеграции](/docs/core/foundations/axiom-septicity#теорема-порог-интеграции))
- $\kappa_{\text{bootstrap}} > 0$ (теорема о минимальной регенерации)
- **ПИР как следствие** $J_{Bures}$ ([вывод ниже](#пир-как-теорема))
...
∞-группоидная структура $\mathbf{Exp}_\infty$ (экспериенциальное пространство) связана с [иерархией интериорности](/docs/proofs/interiority-hierarchy) через механизм n-усечения.
:::

**Теорема (Гомотопическая классификация):**
...
Подробности: [Категорный формализм §10.6](/docs/proofs/categorical-formalism#связь-с-иерархией-интериорности).
...
$$
L_k := \sqrt{\chi_{S_k}} = \sqrt{\gamma_{kk}} \cdot |k\rangle\langle k|
$$

где $S_k$ — k-й атом классификатора Ω.

**Нормализованные операторы (CPTP-условие):**

Для выполнения CPTP-условия $\sum_k L_k^\dagger L_k = \mathbb{1}$ используем нормализованную форму:

$$
\tilde{L}_k := \frac{|k\rangle\langle k|}{\sqrt{N}} = \frac{|k\rangle\langle k|}{\sqrt{7}}
$$

Тогда:
$$
\sum_k \tilde{L}_k^\dagger \tilde{L}_k = \frac{1}{7} \sum_k |k\rangle\langle k| = \frac{\mathbb{1}}{7} \cdot 7 = \mathbb{1} \quad \checkmark
$$
...
[Подробнее о согласованности →](../../proofs/emergent-time#pw-как-теорема)

### Принцип Информационной Различимости как Теорема {#пир-как-теорема}

:::info ПИР — не аксиома, а следствие $J_{Bures}$
Принцип Информационной Различимости (ПИР), ранее формулировавшийся как постулат, **выводится** из топологии Гротендика $J_{Bures}$.
:::
...
**Принцип:** Два состояния $\Gamma_1, \Gamma_2$ *онтологически различимы* ⟺ $d_B(\Gamma_1, \Gamma_2) > 0$.
...
3. "Онтологическая различимость" в топосе = разделимость покрытиями
4. Следовательно: $\text{Distinguishable}(\Gamma_1, \Gamma_2) \Leftrightarrow d_B(\Gamma_1, \Gamma_2) > 0$ ∎
...
:::info Второе обоснование N = 7 — [Структурный вывод](../../proofs/theorem-octonionic-derivation)
Независимо от Теоремы S, число 7 выводится из двух постулатов через теорему Гурвица:

**[П] P1:** Пространство состояний ≅ Im($\mathcal{A}$), где $\mathcal{A}$ — нормированная алгебра с делением.
**[П] P2:** $\mathcal{A}$ неассоциативна.

**[С] Вывод:** [Т] Гурвиц → $\mathcal{A} \in \{\mathbb{R}, \mathbb{C}, \mathbb{H}, \mathbb{O}\}$ → P2 исключает $\mathbb{R}, \mathbb{C}, \mathbb{H}$ → $\mathcal{A} = \mathbb{O}$ → $N = \dim(\text{Im}(\mathbb{O})) = 7$.
...
Мост (AP)+(PH)+(QG) → P1+P2 — [три сходящихся аргумента, формально открыт](../../proofs/theorem-octonionic-derivation#мост) [И].
:::
...
Полная формализация оператора φ: [Формализация оператора φ](/docs/proofs/formalization-phi) — единственный канонический источник.
:::

**Каноническое определение (категориальное):**

Оператор φ определяется как **левое сопряжение** к вложению подобъектов (см. [полное определение](/docs/proofs/formalization-phi#φ-как-левый-сопряжённый-к-включению-подобъектов)):
...
Следующие три определения φ эквивалентны (см. [доказательство](/docs/proofs/formalization-phi#теорема-φ-как-стационарное-распределение)):
...
:::note Теорема 3.1 (Вариационная характеризация φ) — [полное доказательство](/docs/proofs/fep-derivation)
...
**Важно:** Это **характеризация** (теорема), а не определение φ. FEP Фристона является **классическим пределом** этого принципа ([Теорема 4.2](/docs/proofs/fep-derivation#4-классический-предел-вывод-fep)).
:::
...
См. [Конструктивные алгоритмы](/docs/formal/computational#конструктивные-алгоритмы-из-l-унификации) для реализации.
...
См. [Формализация φ](../../proofs/formalization-phi) для полной спецификации.
...
**Обоснование:** E (Опыт) имеет первичную связь с часами; U (Единство) — вторичную.
...
|-----------|----------------|---------------|
| E (Опыт) | 1.0 | Референсное значение |
...
**Для минимального 7D-формализма** (частичный след не определён):
...
| **ПИР (теорема)** | **Различимость ⟺ d_B > 0** | **Топология $J_{Bures}$** |
...
**Производные теоремы:**
- **ПИР из $J_{Bures}$:** Принцип информационной различимости — следствие топологии
...
**Связанные документы:**
- [Структурный вывод N=7 через октонионы](../../proofs/theorem-octonionic-derivation) — P1+P2 → 𝕆 → N=7 (Трек B)
...
- [Вывод FEP из УГМ](../../proofs/fep-derivation) — доказательство вариационной характеризации φ (Теорема 3.1) и вывод FEP как классического предела (Теорема 4.2)
- [Теорема об эмерджентном времени](../../proofs/emergent-time) — вывод времени из ∞-структуры
- [Категорный формализм: Топология](../../proofs/categorical-formalism#63-топология-гротендика-на-densitymat-и-exp) — Bures-покрытия и сайт
- [Математический аппарат: Топология](../../formal/specification#топология-гротендика) — формальная спецификация
- [Вычислительная реализация: Алгоритмы](../../formal/computational#алгоритмы-топологии) — конструктивные алгоритмы
- [Свобода воли](../ethics/freedom) — полная формализация
