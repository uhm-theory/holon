...
#### Теорема (Нижняя граница κ_bootstrap) [Т] {#теорема-kappa-bootstrap-bound}

Для примитивного Лиувиллиана $\mathcal{L}_\Omega$ с Фано-структурированным диссипатором:

$$\kappa_{\text{bootstrap}} \geq \frac{\lambda_{\text{gap}}(\mathcal{L}_\Omega)}{7} \geq \frac{2}{9} \approx 0.22$$

где $\lambda_{\text{gap}}$ — спектральный зазор. Оценка: скорость декогеренции $\Gamma_{\text{dec}} = 2$ [Т-39i], контракция $c = 1/3$ [Т] → $\lambda_{\text{gap}} \geq 2/3$.

**Следствие:** Время генезиса конечно: $\tau_{\text{genesis}} \leq (9/2)\ln(7) \approx 8.8$ в единицах $1/\omega_0$.
