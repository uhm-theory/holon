...
- **(V)** — следствие ПИР ([теорема T16](/docs/core/foundations/axiom-septicity#формулировка-пир), выводится из A1+A2 через семантику Крипке—Жуаля)
...
|------|----------|-----------|
| **Геометрический** | $\lVert\Gamma - I_N/N\rVert_F^2 > \lVert I_N/N\rVert_F^2$ | $P > 2/N$ |
...
**Принцип Информационной Различимости (ПИР)** — **теорема** УГМ, выводимая из аксиом A1 (∞-топос) и A2 (метрика Бюре) через семантику Крипке—Жуаля. При серьёзном принятии A1 (реальность = ∞-топос), «онтологическая значимость» = «истинность во внутренней логике $\mathbf{Sh}_\infty(\mathcal{C})$» = «наличие нетривиального $J_{Bures}$-покрытия» — это тавтология, а не допущение. Ранее ПИР имел статус постулата [П] из-за «неустранимого семантического моста» (шаг 3), который оказался тавтологией из A1.
:::

### Формулировка ПИР [Т] {#формулировка-пир}

**Теорема T16 (ПИР):** ПИР следует из A1+A2 через семантику Крипке—Жуаля.
...
3. Отождествление «онтологической значимости» с «разделимостью покрытиями» — содержание теоремы ПИР (T16)

:::tip Почему ПИР — теорема [Т], а не постулат
При серьёзном принятии A1 (реальность = ∞-топос), шаг (3) — **не** семантическое допущение, а **тавтология**: «существовать» = «быть истинным во внутренней логике $\mathbf{Sh}_\infty(\mathcal{C})$» (Крипке—Жуаль) = «иметь нетривиальное $J_{Bures}$-покрытие» (A2). Шаги (1)–(2) — следствия A1+A2 [Т], шаг (3) — определение онтологии в рамках A1 [Т].

**Ключевое содержание теоремы T16:** Нетривиальный шаг — применение **семантики Крипке—Жуаля** в ∞-топосе $\mathbf{Sh}_\infty(\mathcal{C})$: «$\varphi$ истинно в точке $U$» ⟺ «$\exists$ покрывающее семейство $\{U_i \to U\} \in J_{Bures}$, на котором $\varphi$ верифицируемо» ([Lurie, HTT, §6.2.2]). Именно этот факт превращает «онтологическую значимость = различимость в $J_{Bures}$» из философского утверждения в формальное следствие A1+A2. Без семантики Крипке—Жуаля шаг (3) потребовал бы дополнительного постулата.
:::
...
**Теорема (Единство порогов) [Т]:** Все пороги выводятся из единственной метрики — метрики Бюреса, которая является [единственной монотонной римановой метрикой](/docs/proofs/dynamics/emergent-time#41-метрика-бурес) на пространстве квантовых состояний (теорема Ченцова-Петца). ПИР — теорема [Т] (T16), условность снята.

### Онтологический статус ПИР [Т]

:::tip Интернализация ПИР через семантику Крипке—Жуаля (Теорема T16)
ПИР — **теорема [Т]**, следствие аксиом A1+A2 через внутреннюю логику ∞-топоса. Бывший «семантический мост» (шаг 3) оказался **тавтологией** при серьёзном принятии A1: если реальность = ∞-топос, то «существовать» = «быть истинным во внутренней логике».
:::

ПИР **выводится** из структуры примитива $\mathfrak{T} = (\mathbf{Sh}_\infty(\mathcal{C}), J_{Bures}, \omega_0)$:

1. Топология $J_{Bures}$ (Аксиома 2) определяет понятие "близости" состояний — **[Т]**
2. Состояния, неразличимые в этой топологии, онтологически эквивалентны (по определению топоса) — **[Т]**
3. **[Т] Тавтология из A1:** Отождествление «разделимости покрытиями» с «онтологической значимостью» — **определение онтологии** в рамках A1 (Крипке—Жуаль: $\varphi$ истинно ⟺ $\exists$ покрывающее семейство $\in J_{Bures}$, на котором $\varphi$ верифицируемо)
...
:::tip Статус: два уровня определения κ₀
**Концептуальное определение [Т]:** $\kappa_0 = \|\mathrm{Nat}(\mathcal{D}_\Omega, \mathcal{R})\|$ — норма естественного преобразования между сопряжёнными функторами. Это следствие [L-унификации](/docs/core/foundations/axiom-omega#внутренняя-логика).

**Операциональная формула [О]:** $\kappa_0 = \omega_0 \cdot |\gamma_{OE}| \cdot |\gamma_{OU}| / \gamma_{OO}$ — **структурный анзац**, согласованный с категориальным определением через отождествление $|\mathrm{Hom}(i,j)| \leftrightarrow |\gamma_{ij}|$. Это отождествление — **не теорема**, а модельное соглашение, переводящее абстрактную категориальную норму в вычислимую формулу.
:::
...
E-когерентность измеряет степень согласованности измерения Опыта в матрице когерентности Γ.
...
**Статус:** Концептуальное определение $\kappa_0 = \|\mathrm{Nat}(\mathcal{D}_\Omega, \mathcal{R})\|$ — **категориальная теорема [Т]**, выведенная из сопряжения $\mathcal{D}_\Omega \dashv \mathcal{R}$. Операциональная формула $\kappa_0 = \omega_0 \cdot |\gamma_{OE}| \cdot |\gamma_{OU}| / \gamma_{OO}$ — **структурный анзац [О]**, переводящий абстрактную норму в вычислимую форму через модельное соглашение $|\mathrm{Hom}(i,j)| \leftrightarrow |\gamma_{ij}|$. Экспериментальная проверка — тест на согласованность этого отождествления.
...
Мост (AP)+(PH)+(QG)+(V) → P1+P2 — [полная цепочка T1–T16, **все 12 шагов [Т]**](../../proofs/minimality/theorem-octonionic-derivation#мост).
...
Связь $(AP)+(PH)+(QG)+(V) \Longrightarrow P1+P2$ установлена через **полную формальную цепочку** из 12 шагов (Теоремы T1–T16), **все шаги — теоремы [Т]**. Условие (МП) из предыдущей версии **снято** — оно стало теоремой (следствие T11–T13: ранг Хои + L-унификация + вынужденная BIBD).
:::
...
8. **Пороги из Принципа Информационной Различимости (ПИР [Т])**:
