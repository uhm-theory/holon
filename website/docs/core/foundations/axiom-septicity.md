...
- **Трек B**: [Структурный вывод через октонионы](../../proofs/theorem-octonionic-derivation) — P1+P2 → 𝕆 → dim Im(𝕆) = 7
:::
...
:::info Статус
ПИР — фундаментальный постулат УГМ, не выводимый из более простых принципов. «Доказательство» выше демонстрирует совместимость ПИР с остальными аксиомами, а не его вывод.
:::
...
Полная формализация оператора φ (три эквивалентных определения, доказательство эквивалентности, алгоритмы): [Формализация оператора φ](/docs/proofs/formalization-phi).
:::
...
### Теорема о κ_bootstrap {#теорема-kappa-bootstrap}

:::warning Теорема (Значение κ_bootstrap)
Минимальная скорость регенерации **строго выводится** из структуры категории:
...
:::info Аппроксимация Coh_E в 7D
В минимальном формализме частичный след не определён (7 — простое число, нет тензорной факторизации). Используется аппроксимация через E-строку матрицы Γ:

$$
\mathrm{Coh}_E^{7D}(\Gamma) := \gamma_{EE} + 2\sqrt{\sum_{i \neq E} |\gamma_{Ei}|^2}
$$

**Обоснование:** Эта формула аппроксимирует чистоту E-подсистемы:
- $\gamma_{EE}$ — «населённость» E-измерения (диагональ)
- $\sqrt{\sum_{i \neq E} |\gamma_{Ei}|^2}$ — модуль когерентностей E с другими измерениями

**Диапазон:** $\mathrm{Coh}_E^{7D} \in [\gamma_{EE}, \gamma_{EE} + 2\sqrt{6}|\gamma_{E,\max}|]$

**Ограничение:** Это эвристическая аппроксимация, не строгий вывод из $\mathrm{Tr}(\rho_E^2)$. Для точных вычислений используйте [расширенный тензорный формализм](/docs/core/dynamics/coherence-matrix#расширенный-тензорный-формализм-операциональный).
:::
...
Здесь показаны базовые уровни L0-L2. Полная иерархия интериорности L0→L4 (включая L3 — сетевое сознание и L4 — унитарное сознание) определена в [Иерархия интериорности](../../proofs/interiority-hierarchy).
:::

### Пороги L2: Строгий вывод из принципа информационной различимости {#пороги-l2-строгий-вывод}

:::tip Статус порогов L2
- $R_{\text{th}} = 1/3$ и $D_{\min} = 2$ — **строго выводятся** из [Принципа Информационной Различимости](/docs/core/foundations/axiom-omega#примитив) (ПИР).
- $\Phi_{\text{th}} = 1$ — **определение по соглашению**, содержательно обоснованное через K-теорию C*-алгебры (см. [ниже](#теорема-порог-интеграции)).
:::
...
:::warning Определение (Порог интеграции)
...
Значение $\Phi_{\text{th}} = 1$ — это **определение по соглашению**, а не результат вывода. K-теоретический аргумент ниже показывает, что при $\Phi > 1$ возникает нетривиальная $K_1$-группа, но сама граница $\Phi = 1$ выбрана как естественная точка нормировки (когерентности превышают диагональ). Интерпретация через K-теорию обосновывает содержательность этого порога, но не выводит конкретное числовое значение из первых принципов.
:::

**Определение и полное доказательство:** см. [Мера интеграции Φ](/docs/core/structure/dimension-u#мера-интеграции-φ) и [Теорема о пороге интеграции](/docs/core/structure/dimension-u#теорема-порог-интеграции).
...
(См. [каноническое определение R](/docs/core/consciousness/self-observation#мера-рефлексии-r))
:::
...
**(e)** Для $K = 3$ альтернатив (модель/хаос/среда):
$$P(H_1) > \frac{1}{3}$$
...
- $\Phi_{\text{th}} = 1$ — [доказательство](#теорема-порог-интеграции)
...
- $\mathcal{R}[\Gamma, E] = \kappa \cdot (\Gamma_{\text{target}} - \Gamma) \cdot \Theta(\Delta F)$ — регенерация, где $\Delta F$ — [градиент свободной энергии](../dynamics/evolution#каноническое-delta-f)
...
:::warning Теорема S: обоснование выбора Аксиомы 3 ([полное доказательство](../../proofs/theorem-minimality-7))
...
:::info Структурный вывод через октонионы (Трек B)
Независимо от Теоремы S, значение $N = 7$ получает **второе обоснование** через алгебры с делением:
- **[П] P1:** пространство состояний ≅ Im($\mathcal{A}$), $\mathcal{A}$ — алгебра с делением
...
[Полный вывод →](../../proofs/theorem-octonionic-derivation)
:::

### Мост к P1+P2 [И] {#мост-p1p2}

:::warning Мост формально открыт
Связь между условиями (AP)+(PH)+(QG) и постулатами P1+P2 установлена содержательно, но **не доказана формально**.
:::

Три сходящихся аргумента связывают Теорему S с октонионным выводом:

| Аргумент | Условие УГМ | Свойство алгебры |
|----------|-------------|------------------|
| **Обратимость** | (AP): $\varphi$ обратима | P1: алгебра с делением |
| **Исключительность** | Теорема S: $N \geq 7$ | P2: dim Im ≤ 3 при ассоциативности |
| **Альтернативность** | (QG): минимальная нелинейность | Граница К-Д: $\mathbb{O}$ — последняя альтернативная |

Замыкание моста — [открытая проблема](../../proofs/theorem-octonionic-derivation#открытые-проблемы).
...
8. **Выведенные теоремы из Принципа Информационной Различимости (ПИР)**:
   ...
   - $\Phi_{\text{th}} = 1$ — порог топологической целостности (K₁-нетривиальность C*-алгебры) — **определение по соглашению**
...
- [Теорема о минимальности 7D](../../proofs/theorem-minimality-7) — полное формальное доказательство (Трек A)
- [Структурный вывод через октонионы](../../proofs/theorem-octonionic-derivation) — P1+P2 → 𝕆 → N=7 (Трек B)
- [Эмерджентное время](../../proofs/emergent-time) — τ из структуры Γ
- [Иерархия интериорности](../../proofs/interiority-hierarchy) — уровни L0→L1→L2→L3→L4
