...
Оператор $\varphi$ — **следствие** динамики, а не её предпосылка. Стационарное состояние $\rho^*_{\mathrm{diss}} = I/7$ определяется **до** $\varphi$ через примитивность $\mathcal{L}_\Omega$ [Т-39a]. Цикличности нет: каждый уровень зависит только от предыдущих.
:::
