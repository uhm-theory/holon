...
- **(V)** — математическое условие ($P > P_{\text{crit}}$ [Т]); онтологическая интерпретация через ПИР ([определение T16](/docs/core/foundations/axiom-septicity#формулировка-пир), встроенное в A1+A2)
...
### Пороги L2: математические теоремы и интерпретация через ПИР [О] {#пороги-l2-строгий-вывод}
...
Порог дифференциации $D_{\min} = 2$ **выводится** из условия $\Phi \geq 1$ [Т] (T-129). Статус повышен с [С] до **[Т]** через [T-151](/docs/proofs/consciousness/substrate-closure#t-151): $\Phi_{\text{th}} = 1$ теперь [Т] (не [О]), и условность снята.
:::
...
8. **Пороги (теоремы [Т]; интерпретация через ПИР [О])**:
