...
2. **Идемпотентность (идеального φ):** $\varphi \circ \varphi = \varphi$ — для идемпотентного определения (Определение 3). Каноническая форма $\varphi_{\text{coh}}$ с параметром сжатия $k < 1$ является **сжимающим** отображением (не идемпотентным); идемпотентная проекция — предел $\lim_{n\to\infty} \varphi_{\text{coh}}^n$
...
Каноническое когерентно-сохраняющее самомоделирование для УГМ определяется однозначно (с точностью до параметра сжатия $k$) через:
