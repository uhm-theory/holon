...
В ранних формулировках УГМ использовались записи $L_k = \sqrt{\chi_{S_k}}$ (характеристический морфизм) и $L_k = \sqrt{\gamma_k}|k\rangle\langle k+1| \otimes P_{\text{strat}}^{(k)}$ (переходные операторы). Обе записи **устарели**: первая математически некорректна ($\sqrt{\chi} = \chi$ для $\chi \in \{0,1\}$), вторая смешивает роль гамильтониана (переходы) и диссипатора (проекции). Каноническое определение — проекторы $L_k^{\text{atom}} = |k\rangle\langle k|$ и $L_p^{\text{Fano}} = \frac{1}{\sqrt{3}}\Pi_p$ — см. [§Деривация из классификатора](#деривация-из-классификатора). Единственность Фано-формы: [Т] (Sol.61, [теорема](#теорема-единственность-фано)).
:::
