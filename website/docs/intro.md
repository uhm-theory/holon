...
:::info Теорема S (обоснование Аксиомы 3)
N = 7 (Аксиома 3) — **минимальная** размерность для выполнения (AP)+(PH)+(QG). [Доказательство →](./proofs/theorem-minimality-7)

**Второе, независимое обоснование:** постулаты P1+P2 (алгебра с делением + неассоциативность) через теорему Гурвица дают $N = \dim(\mathrm{Im}(\mathbb{O})) = 7$. [Структурный вывод →](./proofs/theorem-octonionic-derivation)
:::
...
| **Октонионная структура** | P1+P2 → $\mathbb{O}$ → N=7, $G_2$, Фано | ✅ [Т] при P1+P2 |
...
**Пороговые значения ([теоремы из ПИР](./core/foundations/axiom-septicity#пороги-l2-строгий-вывод)):**
...
- Пороги $X^{(n)}_{\text{th}} = 1/(n+1)$ **выводятся** из принципа байесовского доминирования

:::note Статус уровней
- **L0–L2**: стабильные состояния для биологических систем
- **L3**: метастабильное состояние (конечное время жизни $\tau_3$)
- **L4**: теоретический предел, недостижим для биологических систем ($P > 6/7$ требует почти полной когерентности)
:::
...
| **Необходимость интериорности** | $\text{Viable}(\mathbb{H}) \Rightarrow \text{Spectrum}(\Gamma_E) \neq \{0\}$ | ⚠️ | [Теорема 8.1](./applied/coherence-cybernetics/theorems#теорема-81-условная-необходимость-интериорности-no-zombie) |
...
8. **Октонионную структуру** — P1+P2 → 𝕆 → N=7, $G_2$-симметрия, плоскость Фано, код Хэмминга ([Трек B](./proofs/theorem-octonionic-derivation))
...
### Принимает как примитив ([категориальный разрыв](./core/consciousness/hard-problem#теорема-нетривиальность)):
...
Примитив УГМ — **минимальный** среди всех возможных аксиоматических выборов: одна аксиома вместо двух-трёх ([обоснование](/docs/core/consciousness/hard-problem#минимальность-аксиомы)). Из него **выводится**: форма содержания опыта ([единственный функтор](/docs/core/consciousness/hard-problem#теорема-единственность-фв)), идентичность квалиа ([лемма Ёнеды](/docs/core/consciousness/hard-problem#реляционная-идентичность)), имманентность описания ([замкнутость через φ](/docs/core/consciousness/hard-problem#самореферентная-замкнутость)).
:::
...
| **[Сознание](./core/consciousness/self-observation)** | Иерархия L0→L1→L2→L3→L4 |
