...
2. **Классификация уровней** L0/L1/L2 — см. [Иерархия интериорности](/docs/proofs/interiority-hierarchy)
...
class Holon:
    ...
    где скорость регенерации:
        ...
        κ_bootstrap ≈ 0.003 (минимальная регенерация из единицы сопряжения)
        κ₀ = ω₀ · |γ_OE| · |γ_OU| / γ_OO
    ...
    # Минимальная регенерация (из единицы сопряжения D_Ω ⊣ R)
    KAPPA_BOOTSTRAP = 0.003  # гарантирует κ > γ при любом Coh_E
    ...
    def compute_e_coherence(self):
        """
        E-когерентность: Coh_E(Γ) = γ_EE + 2√(Σ|γ_Ei|²).

        Для минимального 7D-формализма — мера активации измерения Опыта.
        При высокой E-когерентности регенерация эффективнее.
...
:::info Статус: Расширенные возможности
Следующий класс добавляет меры сознательности (Φ, R, D_diff) и классификацию уровней L0/L1/L2.
Базовая функциональность (эволюция, регенерация, bootstrap) уже включена в класс `Holon`.
:::
...
class HolonExtended(Holon):
    ...
    - Меры интеграции Φ, рефлексии R
    - Классификацию уровней интериорности L0/L1/L2
    ...
    PHI_TH = 1.0  # Порог интеграции для L2
    ...
    def compute_reflection(self):
        ...
        Упрощённая версия: без оператора самомоделирования φ.
        Полная реализация требует CPTP-канал φ.

        Returns:
            R ∈ [0, 1] — мера самореферентности
        """
        # Заглушка: используем чистоту как приближение
        # В полной версии R = 1 - ||Γ - φ(Γ)||²_F / ||Γ||²_F
        return min(1.0, self.compute_purity())

    def classify_level(self):
        """
        Классификация уровня интериорности: L0, L1, L2.

        Уровни:
            - non-viable: P ≤ P_crit (система не жизнеспособна)
            - L0: P > P_crit (базовая интериорность)
            - L1: L0 + Φ > 0 (феноменальная геометрия)
            - L2: L1 + R ≥ R_th, Φ ≥ Φ_th (когнитивные квалиа)

        Returns:
            Строка: "L0", "L1", "L2", или "non-viable"
        """
        P = self.compute_purity()
        ...
        if R >= self.R_TH and Phi >= self.PHI_TH:
            return "L2"  # Когнитивные квалиа
        elif Phi > 0:
            return "L1"  # Феноменальная геометрия
        else:
            return "L0"  # Базовая интериорность
```
...
for t in range(500):
    ...
    if t % 100 == 0:
        ...
        print(f"t={t}: P={P:.4f}, Φ={Phi:.4f}, уровень={level}")
```

:::warning Ограничения расширенной реализации
1. **Мера R упрощена:** Полная версия требует оператора самомоделирования φ
2. **D_diff не реализована:** Требует частичного следа (тензорный формализм)
3. **Γ_target = const:** В полной теории $\Gamma_{target} = \varphi(\Gamma)$

Для исследовательской работы см. [Формализация φ](/docs/proofs/formalization-phi).
:::
