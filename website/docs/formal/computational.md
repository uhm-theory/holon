...
class Holon:
    ...
    def _compute_regeneration(self, delta_F, dt):
        """
        ...
        # Условие корректности: α = κ·dt < 1 для сохранения положительности
        # См. теорему о CPTP-структуре регенерации в evolution.md
        alpha = kappa * dt
        if alpha >= 1.0:
            # Адаптивный шаг для гарантии положительности
            dt = 0.9 / kappa  # α = 0.9 < 1
            alpha = kappa * dt

        # R[Γ, E] = κ · (Γ_target - Γ)
        return kappa * (self.Gamma_target - self.Gamma) * dt

    def evolve(self, dt, delta_F=0.0):
        """
        ...
        self.Gamma += self._compute_regeneration(delta_F, dt)
    ...
    def bootstrap(self, max_steps=1000, target_P=None):
        ...
        for step in range(max_steps):
            ...
            self.evolve(dt=0.01, delta_F=delta_F)
...
for t in range(1000):
    ...
    holon.evolve(dt=0.01, delta_F=0.5)
...
for t in range(500):
    holon.evolve(dt=0.01, delta_F=0.3)
...
1. **Мера R — ЗАГЛУШКА:** `compute_reflection()` возвращает `purity`, не истинную рефлексию
...
def compute_lindblad_from_omega(Gamma: np.ndarray) -> list:
    ...
    Свойство (CPTP-условие):
        Σ_k L_k† L_k = 𝟙 — выполняется автоматически
    """
    L_operators = []
...
def phi_from_liouvillian(Gamma: np.ndarray, H: np.ndarray,
                          tau_period: int = 7, dt: float = 0.1) -> np.ndarray:
    """
    Вычисление φ(Γ) как стационарного распределения ℒ_Ω.

    Теорема:
        φ(Γ) = lim_{t→∞} e^{t·ℒ_Ω}[Γ]
    ...
    n_steps = int(tau_period / dt)

    for _ in range(n_steps):
        # Эволюция на один шаг: Γ_{t+dt} = Γ_t + dt · ℒ_Ω[Γ_t]
        L_Omega = logical_liouvillian(current, H)
        current = current + dt * L_Omega
...
def bures_distance(rho: np.ndarray, sigma: np.ndarray) -> float:
    """
    Вычисление метрики Бюреса между матрицами плотности.
...
class OmegaClassifier:
    ...
    def verify_cptp(self) -> bool:
        ...
        Это условие автоматически выполняется из структуры
        классификатора (разложение единицы на атомы).
        """
