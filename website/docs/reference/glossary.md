...
| **Энтропия свободы** | $S_{\text{freedom}} := \sum \log \lvert\pi_n(\mathrm{Map}(\Gamma, T))\rvert$ — количественная мера пространства выбора |
...
| **ПИР (Принцип Интериорной Рефлексии)** | Теорема **[Т]** (T16): каждый Голоном содержит подструктуру, моделирующую его собственное целое. Формально: $\exists\varphi: \Gamma \to \Gamma$, $\varphi$ — CPTP-канал с $F(\Gamma, \varphi(\Gamma)) > 0$. Выводится из A1+A2 через семантику Крипке—Жуаля |
...
Связь (AP)+(PH)+(QG)+(V) → P1+P2 — полная цепочка из 12 шагов (T1–T16), **все [Т]**. Бывшее условие (МП) доказано T11–T13. См. [мост](/docs/proofs/minimality/theorem-octonionic-derivation#мост).
:::
