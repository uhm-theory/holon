...
| **Метрика Бурес** | $d_B(\Gamma_1, \Gamma_2) = \arccos(\sqrt{F})$, где $F = \left(\text{Tr}\sqrt{\sqrt{\Gamma_1}\Gamma_2\sqrt{\Gamma_1}}\right)^2$ — [верность Уломана](/docs/proofs/emergent-time#41-метрика-бурес); угловое расстояние в пространстве состояний |
...
| **Универсальный порог рефлексии** | — | Формула $R^{(n)}_{\mathrm{th}} = 1/(n+2)$: L2: $R_{\mathrm{th}} = 1/3$, L3: $R^{(2)}_{\mathrm{th}} = 1/4$, L4: $R^{(3)}_{\mathrm{th}} = 1/5$. Порог интеграции: $\Phi_{\mathrm{th}} = 1$ |
