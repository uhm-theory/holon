...
class Holon:
    ...
    def set_target_state(self, Gamma_target=None):
        ...
        В полной теории ρ* = φ(Γ) — единственное стационарное состояние
        L_Ω (примитивность [Т]). Форма ℛ = κ·(ρ*−Γ)·Θ(ΔF) полностью
...
4. **Γ_target = const:** В полной теории $\rho_* = \varphi(\Gamma)$ — единственное стационарное состояние $\mathcal{L}_\Omega$ [Т], зависит от Γ
