...
:::danger[Корректность заголовка — [И]]
Заголовок «Стандартная модель из $G_2$» — **преувеличение**. $\mathrm{rank}(G_2) = 2 < \mathrm{rank}(\mathrm{SM}) = 4$, поэтому калибровочная группа SM **не является подгруппой** $G_2$. Корректная формулировка:
- $\mathrm{SU}(3)_C$ из $G_2$ как стабилизатор O-направления — **[Т]** (стандартный математический факт)
- Электрослабый сектор $\mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$ из Фано-электрослабой конструкции (ФЭ) — **[С при (ФЭ)]** (одна минимальная гипотеза вместо цепочки SU(6) → SU(5) → SM)
- Полное соответствие «SM из $G_2$ + (ФЭ)» — **[С при A5 + (ФЭ)]** (условно на аксиому Page-Wootters и Фано-электрослабую конструкцию)
:::
...
:::warning[Статус: \[С при (ФЭ)\] для электрослабого сектора]
Электрослабый сектор $\mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$ выводится из Фано-электрослабой конструкции (ФЭ): Хиггсова линия $\{A,E,U\}$ канонически разлагает $\bar{3} \to \{E,U\} \oplus \{L\}$, что определяет $\mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$. Одна минимальная гипотеза (ФЭ) заменяет цепочку $\mathrm{SU}(6) \to \mathrm{SU}(5) \to \mathrm{SM}$.
:::
...
- (B) $\mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$ из Фано-электрослабой конструкции (ФЭ): Хиггсова линия $\{A,E,U\}$ канонически разлагает $\bar{3} \to \{E,U\} \oplus \{L\}$ — **[С при (ФЭ)]** (добавляет rank 2 в 42D PW-расширении)
...
| Фано-электрослабая конструкция (ФЭ) | Хиггсова линия $\{A,E,U\}$ | rank 2 — электрослабое взаимодействие | **[С при (ФЭ)]** |
...
$\mathrm{SU}(3)_C$ (от $G_2$ на часовом факторе) и $\mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$ (от (ФЭ) на системном факторе) действуют на **разные** тензорные множители, поэтому коммутируют и rank складывается: $2 + 2 = 4 = \mathrm{rank}(\mathrm{SM})$. Статус: **[С при (ФЭ) + A5]**.
...
:::warning[Статус: Теорема \[С при (ФЭ)\]]
Хиггсова линия $\{A,E,U\}$ канонически определяет электрослабую калибровочную симметрию $\mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$.
:::

**Фано-электрослабая гипотеза (ФЭ).** *Каноническая декомпозиция $\bar{3} \to 2_{EU} \oplus 1_L$, индуцированная Хиггсовой линией $\{A,E,U\}$, определяет эффективную калибровочную симметрию электрослабого сектора.*
...
| Критерий | Старый подход [Г] (SU(6)) | Новый подход [С при (ФЭ)] |
|----------|--------------------------|--------------------------|
| Число гипотез | $\geq 3$ (SU(6), SU(5)-вложение, GJ-разложение) | 1 (ФЭ) |
...
:::warning[Статус: Теорема \[Г\]]
Аргумент через орбиты $S_4$ на 6 точках не определён строго: число орбит зависит от действия $S_4$ на парах vs на триплетах; утверждение «три класса $\to$ три поколения» неформализовано. Число минимумов $V_{\mathrm{eff}} \leq 3$ из Swallowtail — верхняя оценка, не нижняя.
:::
...
:::tip[Статус: Теорема \[Т\] для SM-части; \[С при (ФЭ)\] для электрослабого вывода; \[Г\] для $G_2$-экстра]
$G_2$-генераторы порождают $\mathrm{SU}(3)_C$ (8 глюонов) и 6 $G_2$-экстра бозонов. Фано-электрослабая конструкция (ФЭ) определяет $\mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$ (4 бозона).
:::

| Поле | Группа | Число | Масса | Статус |
|---|---|---|---|---|
| Глюоны $g$ | $\mathrm{SU}(3)_C$ | 8 | 0 (конфайнмент) | SM [Т] |
| $W^\pm, Z$ | $\mathrm{SU}(2)_L$ | 3 | $M_W, M_Z$ (Хиггс) | SM [С при (ФЭ)] |
| Фотон $\gamma$ | $\mathrm{U}(1)_{\mathrm{EM}}$ | 1 | 0 | SM [С при (ФЭ)] |
...
Статус \[Т\*\]: вывод верен, доказательство через октонионные структурные константы $f_{ijk}$ (не через $V_3$, который суммирует по не-Фано тройкам).
:::
...
Фановское правило отбора (разд. 9.3) порождает иерархию масс $m_t \gg m_c, m_u$, разрешая уязвимость К-1 (парадокс IR fixed point).
:::
...
| $\mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$ из Фано-электрослабой конструкции (ФЭ) | [С при (ФЭ)] |
| Согласование двух $\mathrm{SU}(3)$ ($G_2$ и 42D PW) | [Т] |
| Полное SM из $G_2$ + (ФЭ) | [С при A5 + (ФЭ)] |
| Кварки и лептоны как Gap-конфигурации | [Г] |
| Три поколения из Фано-структуры | [Г] |
...
| Хиггс как Gap-конденсат когерентности E-U | [Г] |
...
| Фановское правило отбора Юкавских | [Т*] (через $f_{ijk}$, не $V_3$) |
| Массовая иерархия $m_t \gg m_c \gg m_u$ из Фано-отбора | [Т*] |
...
| $m_{3/2} \sim 10^{13}$ ГэВ | [Г*] (условно на $\mu_{\text{phys}} = M_P$) |
...
| Нормальная иерархия нейтрино | [Г] |
