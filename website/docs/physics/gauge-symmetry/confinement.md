...
- Струнное натяжение $\sqrt{\sigma} \approx 457$ МэВ — **[Т]** (Sol.60): секторная коррекция $|\gamma_{3\to\bar{3}}| \approx 2.7\bar{\varepsilon}$; единственность минимума $V_{\text{Gap}}$ [Т] (T-64)
...
с натяжением струны $\sqrt{\sigma} \approx 457$ МэВ (при секторной коррекции $|\gamma_{3\to\bar{3}}| \approx 2.7\bar{\varepsilon}$).
...
**Шаг 5 (Секторная коррекция).** Из T-64 [Т] (единственный вакуум), секторная когерентность $|\gamma_{3 \to \bar{3}}| \approx 2.7\bar{\varepsilon}$ (а не средняя $\bar{\gamma} \approx 0.047$). Поскольку $\sigma \propto |\gamma|^4$:

$$\sqrt{\sigma_{\text{corrected}}} = \sqrt{\sigma_{\text{naive}}} \cdot \left(\frac{|\gamma_{3\to\bar{3}}|}{|\bar{\gamma}|}\right)^2 \approx 60 \cdot (2.7)^2 \approx 60 \cdot 7.6 \approx 457 \text{ МэВ}$$
...
:::tip[Статус: Теорема \[Т\] (Sol.60)]
Количественная оценка. Секторная коррекция из единственного вакуума (T-64 [Т]) даёт $\sqrt{\sigma} \approx 457$ МэВ, расхождение с экспериментом $< 4\%$.
:::
...
7. Деконфайнмент при $T_c \sim \Lambda_{\mathrm{QCD}} \sim 170$ МэВ [Г] (согласие с решёткой); параметр порядка — петля Полякова [Г] (качественная модель)
...
| Струнное натяжение $\sqrt{\sigma} \approx 457$ МэВ из Gap-трубки (Sol.60) | [Т] |
...
1. ~~**Количественная проверка конфайнмента.**~~ Решено полностью (Sol.60 [Т]): топологическое доказательство закона площади через T-73 + T-69 + секторная коррекция даёт $\sqrt{\sigma} \approx 457$ МэВ (расхождение $< 4\%$ от наблюдаемых 440 МэВ). Единственность минимума $V_{\text{Gap}}$ (T-64 [Т]) фиксирует $|\gamma|_{3\bar{3}}$ однозначно.
