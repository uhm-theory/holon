...
description: "Вычисление Λ из Gap: спектральная формула [Т], O-секторное доминирование [Т] (Sol.63), тройное подавление, тождества Уорда, дзета-регуляризация, когомологическое обнуление и SUSY-компенсация [Т] — оценка ~10⁻¹²⁰±¹⁰"
---
...
| Фермионный | $H_{\text{ferm}}^{\text{int}} = \mathbb{C}^7$ (внутренние фермионные моды, $G_2$-сингл.) | При точной внутренней SUSY: $\mathrm{Tr}(1)_{\text{total}} = 7 - 7 = 0$ [С] |
...
:::note Компенсация $\mathrm{Tr}(1)_{\text{total}} = 0$ [С]
...
**Статус [С].** Точность компенсации $7 - 7 = 0$ обусловлена предположением о точном внутреннем спаривании $\mathbf{14} \to \mathbf{7}_{\text{лёгких}} + \mathbf{7}_{\text{тяжёлых}}$ при $G_2$-голономии. Это стандартный результат для $G_2$-многообразий в $\mathcal{N}=1$ SUSY, однако подтверждение полного спектра в конечной спектральной тройке T-53 требует явного построения оператора Дирака на $H_{\text{int}}^{\text{ferm}} = \mathbb{C}^7$. До этого компенсация остаётся **[С]** (обусловлена точным внутренним SUSY-спариванием).
:::

:::info Прогресс
SUSY-компонента повышена с **[С]** до **[Т]** благодаря спектральному действию (Sol.40). Уточнение: это повышение относится к **структурной формуле** $\Lambda_{\text{CC}} \sim \varepsilon^{12}$ при условии компенсации; сама компенсация $\mathrm{Tr}(1)_{\text{total}} = 0$ остаётся [С]. Секторная компонента уточнена через Sol.39 [Т]. Подробнее: [полный бюджет с доказательствами](/docs/proofs/gap/lambda-budget#спектральная-формула).
:::
...
## 4c. Доминирование O-сектора в $\Lambda$ (Sol.63) {#доминирование-o-сектора}
...
**Шаг 2 (Оценка секторов).** Из [секторной Gap-границы](/docs/physics/cosmology-phys/berry-phase#теорема-секторная-gap-граница) [Т] (Sol.59):
...
Результат Sol.63 устанавливает глубокую связь: $\Lambda$ определяется O-сектором — тем же сектором, который порождает время через [Пейдж–Вуттерс механизм](/docs/core/foundations/spacetime#теорема-время-из-o). Наличие наблюдателя (O-сектор с $\mathrm{Gap}(O,i) \approx 1$) неизбежно генерирует положительную вакуумную энергию. Перекрёстная ссылка: [структурная необходимость $\Lambda > 0$](#теорема-лямбда-положительна) [Т] (Sol.50).
:::
...
**Статус**: [П] → [Г] → **[С] со структурным замыканием** ($\sim 10^{-120 \pm 10}$). Все коэффициенты определены через $\theta^*$ (T-79 [Т]) — свободных параметров нет (Sol.73). Оставшийся зазор — вычислительная задача (числовая минимизация на $(S^1)^{21}/G_2$).
...
| Секторная Gap-граница | [Berry-фаза](/docs/physics/cosmology-phys/berry-phase#теорема-секторная-gap-граница) | $\mathrm{Gap}(O,i) \approx 1$, $\mathcal{G}_{\text{total}} = \mathcal{G}_O + O(\bar{\varepsilon}^2)$ [Т] (Sol.59) |
| O-секторное доминирование | [§4c](#теорема-лямбда-o-доминирование) | $\Lambda_{\text{CC}} \propto \mathcal{G}_O$ = «стоимость наблюдения» [Т] (Sol.63) |
