...
Результат Sol.63 устанавливает глубокую связь: $\Lambda$ определяется O-сектором — тем же сектором, который порождает время через [Page-Wootters механизм](/docs/core/foundations/spacetime#теорема-время-из-o). Наличие наблюдателя (O-сектор с $\mathrm{Gap}(O,i) \approx 1$) неизбежно генерирует положительную вакуумную энергию. Перекрёстная ссылка: [структурная необходимость $\Lambda > 0$](#теорема-лямбда-положительна) [Т] (Sol.50).
:::
