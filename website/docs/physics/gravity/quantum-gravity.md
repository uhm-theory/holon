...
**Шаг 1 (Аксиомы NCG для произведения тройки).** KO-размерность $4 + 6 = 10 \equiv 2 \pmod{8}$. Условие первого порядка, реальная структура, ориентация, двойственность Пуанкаре — все проверены (стандартный результат Chamseddine-Connes-Marcolli).
