...
:::warning Теорема 2.2 (Низкоэнергетический предел → действие Эйнштейна-Гильберта) [С]
**Статус [С]:** Спектральное действие Чамседдина-Конна $S = \mathrm{Tr}(f(D/\Lambda))$ из конечной спектральной тройки $(A_{\text{int}}, H_{\text{int}}, D_{\text{int}})$ [Т] даёт действие Эйнштейна-Гильберта с $G_N \sim 1/(a_2\Lambda^2)$, где $a_2 = \mathrm{Tr}(D_{\text{int}}^{-2})$ определяется Gap-параметрами [С]. Аргумент через теорему Лавлока для идентификации проецированного Gap-действия с действием Эйнштейна-Гильберта дополняет спектральный подход, но применимость Лавлока к дискретной структуре когерентностей не верифицирована строго (см. [ограничения аргумента Лавлока](/docs/physics/gravity/einstein-equations#34-ограничения-аргумента-лавлока)).
...
**(a) Спектральное действие (Chamseddine-Connes).** Конечная спектральная тройка $(A_{\text{int}}, H_{\text{int}}, D_{\text{int}})$, где $A_{\text{int}} = \mathbb{C} \oplus \mathbb{H} \oplus M_3(\mathbb{C})$ — стандартный алгебрат частиц и $D_{\text{int}}$ определяется через Gap-параметры, при разложении спектрального действия $\mathrm{Tr}(f(D/\Lambda))$ порождает действие Эйнштейна-Гильберта:

$$
S_{\text{EH}} = \frac{a_2}{2} \int d^4x \sqrt{g} \, R + O(\Lambda^0)
$$

Ньютонова постоянная: $G_N = \frac{3\pi}{a_2 \Lambda^2}$, где момент $a_2 = \mathrm{Tr}(D_{\text{int}}^{-2})$ вычисляется из спектра внутреннего оператора Дирака [Т как стандартный результат некоммутативной геометрии].
...
Статус повышен с [Г] до [С]: спектральный аргумент строг для гладкого предела, условен на существование полной спектральной тройки для Gap-формализма.
:::
