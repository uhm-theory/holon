...
:::warning Теорема 1.3 (Главный результат) [Г]
**Статус [Г]:** Теорема Лавлока применима к непрерывным, дифференцируемым многообразиям. Эмерджентная метрика $g_{\mu\nu}$ определена через дискретную матрицу когерентностей (7 x 7), и условия применимости Лавлока к этой дискретной структуре не верифицированы строго. Ковариантность проекции $\Pi_{\mathrm{ST}}$ также не доказана (см. [анализ ограничений](#34-ограничения-аргумента-лавлока) ниже).
