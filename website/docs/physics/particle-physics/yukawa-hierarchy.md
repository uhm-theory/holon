...
:::tip [Т] Теорема
Строго доказано. Следует непосредственно из алгебры октонионов $\mathbb{O}$ через структурные константы $f_{ijk}$.
:::
...
:::warning [Г] Гипотеза
Значение $\epsilon_\text{eff} \approx 0.06$ определяется из наблюдаемых масс, а не вычислено из первых принципов. Различие между $\epsilon \approx 0.01$ и $\epsilon_\text{eff} \approx 0.06$ объясняется множителем $\lambda_3/(4\pi)$, но его точное значение требует непертурбативного вычисления.
:::
...
:::warning [Г] Гипотеза
Числовые предсказания масс кварков зависят от параметра $\epsilon_\text{eff}$, введённого постфактум. Структура иерархии — [Т], числа — [Г].
:::
...
Все значения в таблице — **оценки порядка величины**, а не точные предсказания. Параметр $\epsilon_\text{eff} \approx 0.06$ определён **из наблюдаемых масс** (post-hoc), а не вычислен из первых принципов. Согласие для $u$-кварка ($\sim 2$ МэВ vs наблюдаемое $2.2$ МэВ) является следствием подгонки $\epsilon_\text{eff}$, а не независимого предсказания. Точные значения требуют решёточного вычисления $V_3$-петлевых вкладов.
:::
...
- **Три поколения:** Единственность $(1,2,4)$ → [Три поколения фермионов](./fermion-generations.md)
