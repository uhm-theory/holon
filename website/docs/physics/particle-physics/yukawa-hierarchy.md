...
:::tip [Т] Теорема
Верхние и нижние кварки получают массы через один Хиггсовый дублет с различными ориентациями в Фано-пространстве. Массовый механизм $b$-кварка — **петлевой** (не древесный), с QCD-IR-усилением. Полная теорема: [Секторная RG для $m_b/m_t$](#теорема-mb-mt).
:::
...
#### Предупреждение о непертурбативности $\lambda_3$ {#предупреждение-λ3}

:::warning Применимость петлевых вычислений
$\lambda_3 \approx 74 \gg 4\pi$ — параметр кубического члена $V_3$ лежит в непертурбативном режиме. Петлевые разложения по $\lambda_3/(16\pi^2) \approx 0.47$ в данном разделе являются формальными оценками порядка величины и **не** являются контролируемыми пертурбативными вычислениями. Строгие результаты требуют непертурбативных методов (решётка, бутстрап, или функциональная ренормгруппа).
:::
...
| $b$ | 3 | 1 (A) | 1-loop + QCD-IR [С] | $\approx 4.2$ ГэВ | 4.18 ГэВ |
...
#### Теорема (Секторная RG для $m_b/m_t$) [С] {#теорема-mb-mt}

:::tip [С] Условная
Расхождение с наблюдениями сокращено с $\sim \times 2$ (Sol.36) до $\lesssim 5\%$. Ключевая коррекция: правильный учёт QCD-IR-**усиления** (а не подавления) Юкавской связи $b$-кварка. Условно на значении $\varepsilon_{33} \approx 0.06$ из [единственного самосогласованного вакуума](/docs/core/dynamics/gap-thermodynamics#теорема-единственный-вакуум) (T-61).
:::
...
:::warning Остаточное расхождение $\sim \times 4$
Значение $y_t(m_t) \approx 1.0$ согласовано с [quasi-IR fixed point](#3-quasi-ir-fixed-point-и-масса-t-кварка) и $m_t = y_t \cdot v/\sqrt{2} \approx 173$ ГэВ. Расхождение $0.097/0.024 \approx 4$ может быть связано с: (1) непертурбативностью $\lambda_3 \approx 74$ (петлевая оценка $y_b$ ненадёжна); (2) неучтёнными двухпетлевыми QCD-коррекциями; (3) точным значением $\varepsilon_{33}$ (используется $0.06$, но допустим диапазон $0.01\text{–}0.1$). Полное решение требует непертурбативного (решёточного) вычисления.
:::
...
| **Sol.44** | **1-loop + QCD-IR-усиление** | **$\approx 0.097$** | **$\sim \times 4$** |
| Наблюдение | — | $0.024$ | — |
:::
