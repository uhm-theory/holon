...
7. [Массовый спектр и сравнение с наблюдениями](#7-массовый-спектр-и-сравнение-с-наблюдениями) (диагонализация с seesaw-поправками)
...
### Теорема 5.3 (Различие $Y^u$ и $Y^d$) [С] {#thm-5-3}

:::tip [С] Условная
Верхние и нижние кварки получают массы через один Хиггсовый дублет с различными ориентациями в Фано-пространстве. Условно на SU(5)-унификации ($y_t^{(\text{tree})} = y_b^{(\text{tree})}$ на масштабе GUT).
:::
...
**(c)** На древесном уровне: $y_t^{(\text{tree})} = y_b^{(\text{tree})}$ (SU(5)-соотношение). QCD-поправки при RG от $M_\text{GUT}$ до $m_b$:

$$\frac{y_b(\mu_\text{EW})}{y_t(\mu_\text{EW})} = \frac{y_b(\mu_\text{GUT})}{y_t(\mu_\text{GUT})} \times \left(\frac{\alpha_s(m_b)}{\alpha_s(M_\text{GUT})}\right)^{12/(33-2N_f)}$$

С $\alpha_s(m_b) \approx 0.22$, $\alpha_s(M_\text{GUT}) \approx 0.04$, $N_f = 5$:

$$y_b/y_t \approx 1 \times (0.22/0.04)^{12/23} \approx 5.5^{0.52} \approx 2.3$$

Это значение $y_b/y_t \approx 2.3$ — при масштабе GUT. При EW: $m_b/m_t \approx 0.024$ — из-за IR fixed point для top ($y_t \sim 1$) и отсутствия IR FP для bottom ($y_b \ll 1$). Результат: $m_b \approx 4.2$ ГэВ, $m_t \approx 173$ ГэВ.
