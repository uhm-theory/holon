...
*Доказательство.* Стабилизатор не-коллинеарной тройки $\{p_1, p_2, p_3\}$ в PSL(2,7):

- PSL(2,7) действует 2-транзитивно на 7 точках (стандартный факт)
- Размер орбиты: $|\text{PSL}(2,7)|/|\text{Stab}(\{p_1, p_2, p_3\})| = 168/|\text{Stab}|$
- Если транзитивно на 28 тройках: $|\text{Stab}| = 168/28 = 6$

Проверка через лемму Бёрнсайда: количество орбит $= \frac{1}{168}\sum_{g \in \text{PSL}(2,7)} |X^g|$, где $X$ — множество 28 не-коллинеарных троек. Поскольку PSL(2,7) действует 2-транзитивно, а каждая не-коллинеарная тройка определяет **уникальный** четвёртый дополнительный элемент (точку пересечения двух линий через пары тройки), действие на тройках подчиняется действию на «метках» этих четвёртых точек. Из 2-транзитивности: всего **одна** орбита на не-коллинеарных тройках. $\blacksquare$
...
#### 4.6.2 Расхождение $m_2/m_3$ — частично решено [С] (Sol.38)

~~Расхождение (фактор ~50) было открытой проблемой.~~ **Решено [С]** (Sol.38): O-секторная спектральная тройка даёт дираковские Юкавские через $m_D^{(k)} = \omega_0 \cdot \mathrm{Gap}(O,k) \cdot |\gamma_{O,\mathrm{partner}(k)}| \cdot \sin(2\pi k/7)$. Расхождение $m_2/m_3$ снижено с $\times 50$ до $\times 1.8$ (до $\times 1.2$ с RG). См. [нейтринные массы](/docs/physics/particle-physics/neutrino-masses#теорема-отношение-нейтринных-масс).
