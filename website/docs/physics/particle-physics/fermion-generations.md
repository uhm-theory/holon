...
#### 4.6.2 Расхождение $m_2/m_3$ (фактор ~50)

Расхождение становится следствием двух различных механизмов генерации масс:
- $\nu_\tau$ ($k=1$): древесная Юкавская $\sim O(1)$ → $m_{\nu_\tau} \sim m_\tau^2/M_R$
- $\nu_\mu$ ($k=4$): непертурбативная → подавление определяется конфайнмент-масштабом, а не $m_\mu^2/m_\tau^2$
