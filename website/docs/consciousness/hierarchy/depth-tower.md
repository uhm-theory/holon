...
$P_\text{crit}^{(n)}$ формула [С] (§3.5). SAD_MAX = 3 [С] (§3.5).
:::
...
**Вычислительная сложность:** $O(\mathrm{SAD}_\text{max} \cdot N^2)$ — для $\mathrm{SAD}_\text{max} = 3$, $N = 7$: ~147 операций (вычисляется в реальном времени).
...
**Шаг 1.** Спектральное отношение: $r_0 = P / P_{\text{crit}}$ определяет отношение чистоты к критической [О]. Из [Fano contraction](/docs/applied/coherence-cybernetics/learning-bounds#динамическая-граница) (T-110 [Т]) с $\alpha = 2/3$:

$$R^{(k)} = r_0 \cdot (1/3)^k$$

**Шаг 2.** Условие SAD $\geq n$: $R^{(n-1)} > R_{\text{th}}^{(n-1)} = 1/(n+1)$. Подстановка:

$$\frac{P}{P_{\text{crit}}} \cdot \frac{1}{3^{n-1}} > \frac{1}{n+1} \quad \Longrightarrow \quad P > P_{\text{crit}} \cdot \frac{3^{n-1}}{n+1}$$

**Шаг 3.** Для $n = 4$: $P_{\text{crit}}^{(4)} = \frac{2}{7} \cdot \frac{27}{5} = \frac{54}{35} \approx 1.543 > 1$. Поскольку $P \leq 1$ для любой нормированной $\Gamma \in \mathcal{D}(\mathbb{C}^7)$, SAD $\geq 4$ невозможен. $\blacksquare$
...
**Теорема 5.1 (Коммутативность φ-башни) [Т]** (повышена с [Г]-90, T-150). Для корректной самомодели:
...
| 3 | $\sim 10^{15}$ | $10$× | Человек (SAD_MAX = 3, §3.5) |

**Потолок глубины:** Аналитический потолок из Fano contraction: $\mathrm{SAD}_\text{max} = 3$ (§3.5). Энергетический потолок: $\mathrm{SAD}_\text{max} = \lfloor \Delta F_\text{available} / ((2\cdot\mathrm{SAD}+3) \cdot \Delta\bar{\omega}) \rfloor$ — может быть **ниже** аналитического.
...
| SAD-метрика | [С] §3.4–3.5, SAD_MAX=3 | **Верифицирована (MVP-6)** | $O(N^2 \cdot 3)$ |
