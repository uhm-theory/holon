...
### Утверждение (Кооперация) [С] {#кооперация}

**Гипотеза [Г]:** При положительных когерентностях $\gamma_{12} > 0$ между системами:

$$
P_{\Gamma_1 \cup \Gamma_2} \geq P_{\Gamma_1} + P_{\Gamma_2} - P_{\Gamma_1 \cap \Gamma_2}
$$

:::warning Статус: Гипотеза [Г]
Неравенство кооперации использует аналогию с включением-исключением для вероятностных мер. Чистота $P = \mathrm{Tr}(\Gamma^2)$ — квадратичный функционал, для которого аддитивность не гарантирована. Формальное доказательство — открытая проблема. При деструктивной интерференции ($\gamma_{12} < 0$) возможно $P_{\text{total}} < P_1 + P_2$.
:::
...
| Кооперация | $\gamma_{12} > 0 \Rightarrow P_{\text{total}}\uparrow$ | **[Г]** |
