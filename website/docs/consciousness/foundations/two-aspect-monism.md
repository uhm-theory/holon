...
Тегмарк показал, что квантовая когерентность в тёплом мозге декогерирует за $\sim 10^{-13}$ с, что на 10 порядков быстрее нейронных процессов ($\sim 10^{-3}$ с). Если УГМ требует «настоящих» квантовых когерентностей в биологических системах, этот аргумент — серьёзный вызов.
...
| Аспект | IIT | УГМ |
|--------|-----|-----|
| Мера | $\Phi$ (единственная) | $C = \Phi \times R$ (интеграция × рефлексия) |
