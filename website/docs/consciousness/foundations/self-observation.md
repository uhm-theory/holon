...
Оператор $\varphi$ имеет явную физическую реализацию как **замещающий канал** (см. [теорему ниже](#теорема-физическая-реализация-phi)): $\varphi_k(\Gamma) = (1-k)\Gamma + k\rho^*$, где $\rho^*$ — единственное стационарное состояние $\mathcal{L}_\Omega$ [Т-39a]. Это снимает «операциональный разрыв»: $\rho^*$ выводится из примитивности Линдбладиана, $k$ — наблюдаемый параметр (отношение предиктивной к реактивной активности).
:::
...
где $\rho^*$ — единственное стационарное состояние $\mathcal{L}_\Omega$ [Т-39a](/docs/core/operators/lindblad-operators#примитивность-ℒω), $k = 1 - R$ — степень самомоделирования, определяемая мерой рефлексии $R$ (см. [ниже](#теорема-k-из-r)).

**Доказательство.** По [Т-39a]: $\mathcal{L}_\Omega$ примитивен, следовательно $\rho^*$ — единственный аттрактор. Из [Т-39d](/docs/proofs/categorical/formalization-phi) (эквивалентность трёх определений φ): динамическое определение даёт $\varphi(\Gamma) = \lim_{\tau \to \infty} e^{\tau \mathcal{L}_\Omega} \Gamma = \rho^*$. Замещающий канал $T_k(\Gamma) := (1-k)\Gamma + k\rho^*$ — выпуклая комбинация $\mathrm{Id}$ и $\mathcal{C}_{\rho^*}$ (замещающего канала [Т-39f](/docs/core/dynamics/evolution#вывод-формы-регенерации)), следовательно CPTP при $k \in [0,1]$. Сжимаемость: $\|T_k(\Gamma_1) - T_k(\Gamma_2)\|_F = (1-k)\|\Gamma_1 - \Gamma_2\|_F$ с константой сжатия $(1-k) < 1$. $\blacksquare$
...
**Измерение:** $R = 1 - \|\Gamma - \varphi(\Gamma)\|_F^2 / \|\Gamma\|_F^2 = 1 - k^2\|\Gamma - \rho^*\|_F^2/\|\Gamma\|_F^2$.

#### Неподвижная точка (КК-4) [Т] {#неподвижная-точка-кк4}

$\Gamma^* = \rho^*$ — единственная неподвижная точка ($\varphi_k(\Gamma^*) = \Gamma^*$ при $k > 0$).

**Доказательство.** $(1-k)\Gamma^* + k\rho^* = \Gamma^*$ $\Rightarrow$ $k(\Gamma^* - \rho^*) = 0$ $\Rightarrow$ $\Gamma^* = \rho^*$ (при $k > 0$). Единственность из примитивности $\mathcal{L}_\Omega$ [Т-39a]. $\blacksquare$

:::info Снятие циркулярности (P4.3)
Определение φ **не содержит порочного круга**: стационарное состояние $\rho^*$ выводится из примитивности Линдбладиана $\mathcal{L}_\Omega$ [Т-39a] — это свойство **динамики**, не зависящее от $\varphi$. Параметр $k = 1 - R$ определяется мерой рефлексии (см. [теорему ниже](#теорема-k-из-r)). Таким образом, $\varphi_k$ определён через независимые объекты ($\rho^*$ из динамики, $R$ из состояния системы), а не через себя.
:::
...
$$
k = 1 - R, \quad R = \mathrm{Tr}(\Gamma\varphi(\Gamma))
$$

**Доказательство (Sol.77).** Из T-62 [Т] (Sol.30): $\varphi_k(\Gamma) = (1-k)\Gamma + k\rho^*$. Мера рефлексии $R = \mathrm{Tr}(\Gamma\varphi(\Gamma))$ измеряет перекрытие состояния с его самомоделью. Подставляя:

$$
R = \mathrm{Tr}(\Gamma[(1-k)\Gamma + k\rho^*]) = (1-k)\mathrm{Tr}(\Gamma^2) + k\mathrm{Tr}(\Gamma\rho^*)
$$

При $\Gamma = \rho^*$ (неподвижная точка): $R = 1$, $k = 0$ — тождественное отображение. При $\mathrm{Tr}(\Gamma\rho^*) = 0$ (ортогональность): $R = 0$, $k = 1$ — полная замена. Соотношение $k = 1 - R$ самосогласованно и единственно. $\blacksquare$
...
где $k \in (0, 1)$ — степень самомоделирования, $\rho^*$ — единственное стационарное состояние $\mathcal{L}_\Omega$ [Т-39a](/docs/core/operators/lindblad-operators#примитивность-ℒω).
...
Деполяризующий канал и термализация — частные случаи [замещающего канала](#теорема-физическая-реализация-phi) $\varphi_k(\Gamma) = (1-k)\Gamma + k\rho^*$ с $\rho^* = I/N$ и $\rho^* = \rho_{\text{th}}$ соответственно. В УГМ $\rho^*$ — единственное стационарное состояние $\mathcal{L}_\Omega$ [Т-39a], что фиксирует выбор однозначно.
:::
