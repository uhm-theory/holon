...
где $\rho^*$ — единственное стационарное состояние $\mathcal{L}_\Omega$ [Т-39a](/docs/core/operators/lindblad-operators#примитивность-ℒω), $k \in (0,1)$ — степень самомоделирования.
...
**Физическая интерпретация:** $\rho^*$ — внутренняя генеративная модель (предсказание); $k$ — степень доверия к модели (precision weighting в предиктивном кодировании).
...
Определение φ **не содержит порочного круга**: стационарное состояние $\rho^*$ выводится из примитивности Линдбладиана $\mathcal{L}_\Omega$ [Т-39a] — это свойство **динамики**, не зависящее от $\varphi$. Параметр $k$ — наблюдаемая величина (отношение предиктивной к реактивной активности в нейронных системах). Таким образом, $\varphi_k$ определён через независимые объекты ($\rho^*$ из динамики, $k$ из наблюдения), а не через себя.
:::
