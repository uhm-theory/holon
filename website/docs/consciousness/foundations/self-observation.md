...
Определение φ **не содержит порочного круга**: стационарное состояние $\rho^*$ выводится из примитивности Линдбладиана $\mathcal{L}_\Omega$ [Т-39a] — это свойство **динамики**, не зависящее от $\varphi$. Мера рефлексии $R(\Gamma) = 1 - \|\Gamma - \rho^*\|_F^2/\|\Gamma\|_F^2$ определяется **только** состоянием $\Gamma$ и аттрактором $\rho^*$, а параметр $k = 1 - R$ выводится из $R$ (см. [теорему ниже](#теорема-k-из-r)). Таким образом, $\varphi_k$ определён через независимые объекты ($\rho^*$ из динамики, $R$ из состояния системы), а не через себя.
:::
...
$$
\|\Gamma - \rho^*\|_F^2 = \mathrm{Tr}(\Gamma^2) - \frac{1}{7} = P - \frac{1}{7}, \quad R = \frac{1}{7P}
$$

Определяя $k := 1 - R$: при $\Gamma = \rho^*$ имеем $P = 1/7$, $R = 1$, $k = 0$ — тождественное отображение. При $P \to 1$ (чистое состояние): $R = 1/7$, $k = 6/7$ — сильная коррекция. Соотношение $k = 1 - R$ не содержит циркулярности: $R$ определён через $\Gamma$ и $\rho^*$, а не через $k$. $\blacksquare$
