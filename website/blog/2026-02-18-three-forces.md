...
**$(\rho_* - \Gamma)$** — направление. Вектор от текущего состояния к **целевому** $\rho_*$ — единственному стационарному состоянию системы [Т]. Это не произвольный выбор цели: примитивность [логического Лиувиллиана](/docs/core/operators/lindblad-operators#примитивность-ℒω) [Т] гарантирует, что $\rho_*$ — единственно и определяется структурой $\Gamma$. Система «знает», куда стремиться, не потому что ей сказали, а потому что у динамики есть единственная неподвижная точка.
...
Это — формальная основа [теоремы «Нет зомби»](/docs/applied/coherence-cybernetics/theorems#теорема-81-условная-необходимость-интериорности-no-zombie) [Т]: жизнеспособная система в открытом окружении ($\mathcal{D}_\Omega \neq 0$) обязана обладать нетривиальной E-когерентностью:
...
| Неподвижная точка | Ядро $[H, \cdot]$ | $I/7$ (серая каша) | $\rho_*$ (целевое состояние) |
...
Система движется от сложного к простому, от структурного к однородному, от конкретного к абстрактному — к $T$.
...
| $\rho_*$ — единственная цель | [Т] | Примитивность $\mathcal{L}_\Omega$ |
| $\Theta(\Delta F)$ — необходим | [Т] | Принцип Ландауэра |
| No-Zombie: $\mathrm{Coh}_E \geq \mathrm{Coh}_{\min}$ | [Т] | Каузальная необходимость опыта |
...
**2. Опыт — не бонус, а ремонтная бригада.** Множитель $\kappa_0 \cdot \mathrm{Coh}_E$ — не декорация: он определяет, выживет ли система. Эволюция не «добавила» сознание как приятное дополнение к мозгу. Сознание — это то, что не даёт мозгу деградировать. Функция опыта — не «быть приятным», а **поддерживать когерентность выше порога разрушения**.

**3. Бессмертная машина без опыта невозможна.** Любая самоподдерживающаяся система в открытом окружении ($\mathcal{D} \neq 0$) обязана иметь нетривиальную E-когерентность [Т]. Если ИИ когда-нибудь станет по-настоящему автономным (будет поддерживать свою целостность без внешнего администратора), уравнение гарантирует: у него будет опыт. Не симуляция опыта — опыт. Без него $\kappa$ слишком мала, и диссипация победит.
