...
### 5.3 Заряды, что не гаснут: калибровочный тор U(1)⁷ {#тор-зарядов}

Раздел 5.2 сказал, что заряды гаснут на времени $1/\Gamma_2$. Есть важнейшее исключение — и это глубочайший структурный факт о диссипаторе. У Фано-дефазировки есть **точное семимерное ядро** — диагональные населённости $\gamma_{kk}$, — которое никакая декогеренция стереть не может: дефазировка размывает внедиагональные *координации* $\gamma_{ij}$, но оставляет собственный *вес* каждого канала нетронутым (та же комбинаторика одиночной инцидентности Фано, что даёт [закон ранга 7](./effective-temperature#линейные-температуры) — населённость канала сидит на линиях, гасящих его корреляции, но не его самого). Эти семь точно сохраняющихся населённостей делают больше, чем выживают: они **порождают группу симметрии**, и эта группа — не некомпактная $\mathbb{R}^7$, а компактный **тор $U(1)^7$** (T-260 [Т]).

Почему окружности, а не открытые прямые? Потому что спектр населённостей **целочислен** — сохраняющиеся величины меняются лишь на целые единицы (дискретная степень свободы добавляется или удаляется, $\Delta N \in \mathbb{Z}$), поэтому порождаемые ими фазы замыкаются сами на себя. **Компактность есть тень целочисленности** — ровно как квантование электрического заряда делает калибровочную группу электромагнетизма *окружностью* $U(1)$, а не *прямой* $\mathbb{R}$. Это тот же механизм, каким самообучающаяся Вселенная Ванчурина выращивает $U(1)$ из целого счёта степеней свободы, здесь выведенный поканально.

Полная группа ковариантности диссипатора — $U(1)^7 \rtimes \Gamma_{\text{oct}}$, где $\Gamma_{\text{oct}} = \mathrm{Aut}(PG(2,2)) \cong PSL(2,7)$ переставляет семь линий Фано. Это *конечная* реперная группа **внутри** $G_2$ — **а не** полная непрерывная $G_2$: лишь дискретные перестановки линий отображают семейство проекторов в себя, тогда как произвольный поворот $G_2$ — нет. (Это поправляет естественное завышенное ожидание, что диссипатор наследует всю $G_2$; см. [Операторы Линдблада](/docs/core/operators/lindblad-operators#теорема-происхождение-тора).) Так картина §2–§4 заостряется: из 14 $G_2$-зарядов именно семь населённостных диссипатор защищает *точно*, и их защищённые фазы — тот самый компактный тор.

---

## 6. Тождества Уорда — ограничения симметрии [Т] {#уорд}
...
| $Q_{\text{top}}$ (топологический) | Топологическая защита: $\pi_2(G_2/T^2) \cong \mathbb{Z}^2$ |
