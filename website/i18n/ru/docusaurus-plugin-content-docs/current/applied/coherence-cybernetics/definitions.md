...
$\mathrm{Coh}_E \in [0, 1]$ — доля E-вклада в чистоту через ортогональную проекцию Гильберта–Шмидта $\pi_E$; минимум $0$ достигается состояниями с пустым E-сектором, а каждое жизнеспособное состояние, *поддерживаемое Ω-динамикой*, остаётся строго выше $1/7$ (No-Zombie, T-38a — её гипотеза $\mathcal{D}_\Omega \neq 0$ несущая: одного $P > 2/7$ для пола недостаточно; максимально смешанное состояние даёт ровно $1/7$). Первичный формализм — 7D (не требует тензорной факторизации).
:::
...
Не все системы «сознательны одинаково». Термостат реагирует на среду, но не рефлексирует. Собака рефлексирует, но не рефлексирует *о своей рефлексии*. Человек способен к мета-рефлексии, но не до бесконечной глубины (SAD$_{\max} = 3$, T-142). Иерархия интериорности L0–L4 формализует эти различия: каждый уровень — новое качественное свойство внутреннего мира, доступное при выполнении определённых математических условий.
...
Тензор напряжений — вектор из семи компонент, по одной на измерение:

$$
\sigma_{\mathrm{sys}}(\Gamma) := [\sigma_A, \sigma_S, \sigma_D, \sigma_L, \sigma_E, \sigma_O, \sigma_U]^T \in \mathbb{R}^7
$$

**Каноническое определение — через инварианты $\Gamma$**: см. [Формальные определения через Γ-инварианты](#sigma-sys-formal) ниже ([T-92](./theorems#теорема-101-эквивалентность-условий) [Т]). Все семь компонент — однозначные функции $\Gamma$ **без свободных параметров**, и равносильность жизнеспособности $\Gamma \in \mathcal{V}_{\mathrm{full}} \Leftrightarrow \lVert\sigma_{\mathrm{sys}}\rVert_\infty < 1$ доказана именно для этой формы.

#### Панель нагрузки: операционный прокси, а не определение {#панель-нагрузки}

:::caution Эта таблица как определение отменена
До T-92 компоненты записывались как отношения «запрос к ёмкости». Они сохранены, потому что это именно то, что инженер *снимает с работающей системы*, — но это **операционный прокси** с семью свободными параметрами $\theta_i$, и он **не** является функцией $\Gamma$. Для проверки жизнеспособности по T-92 его использовать нельзя.

| Компонент | Прокси нагрузки | Что нагружено |
|-----------|-----------------|---------------|
| $\sigma_A^{\mathrm{load}}$ | $I_{\mathrm{env}} / \theta_A$ | Артикуляция |
| $\sigma_S^{\mathrm{load}}$ | $I_{\mathrm{struct}} / \theta_S$ | Структурная сложность |
| $\sigma_D^{\mathrm{load}}$ | $C_{\mathrm{used}} / C_{\mathrm{max}}$ | Вычисление |
| $\sigma_L^{\mathrm{load}}$ | $I_{\mathrm{verify}} / \theta_L$ | Логическая проверка |
| $\sigma_E^{\mathrm{load}}$ | $(I_{\mathrm{self}} + I_{\mathrm{exp}}) / \theta_E$ | Опыт |
| $\sigma_O^{\mathrm{load}}$ | $(I_{\mathrm{mem}} + I_{\mathrm{ground}}) / \theta_O$ | Основание |
| $\sigma_U^{\mathrm{load}}$ | $(I_{\mathrm{unity}} + I_{\mathrm{social}}) / \theta_U$ | Единство |

где $\theta_i > 0$ — ёмкости по измерениям.
:::

**Две панели отвечают на разные вопросы и не взаимозаменяемы.** $\sigma^{\mathrm{load}}$ — то, что *требует* среда: причина, измеряемая снаружи системы. $\sigma_{\mathrm{sys}}(\Gamma)$ — *возникший дефицит*: следствие, считываемое с состояния. Их связывает не тождество, а [уравнение эволюции](/docs/core/dynamics/evolution): длительная нагрузка ведёт состояние, а теорема о жизнеспособности говорит именно о состоянии. Из-за смешения этих двух панелей нагрузка может выглядеть тревожно при здоровом $\Gamma$ (обеспеченная система под большим запросом) — и наоборот, дефицит может быть тяжёлым при низкой нагрузке (истощённая система в покое).
...
> **Эррата 2026-07-22**: строки $\sigma_E$, $\sigma_U$ — перенормированные формы; см. [эррата-ноту у T-92](/docs/applied/coherence-cybernetics/theorems#теорема-101-эквивалентность-условий).
...
## Геометрическая интуиция {#геометрическая-интуиция}
...
**Интуиция 3: Плоскость Фано.** Семь измерений не произвольны — они связаны структурой конечной проективной плоскости $PG(2,2)$ (плоскость Фано). Это означает, что каждое измерение связано ровно с тремя другими «линиями» (тройками), и эта комбинаторная структура определяет операторы Линдблада. Плоскость Фано — это «скелет» 7D-пространства, и его симметрия — не полная группа перестановок $S_7$, а куда более жёсткая **группа автоморфизмов $PG(2,2)$**, исключительная простая группа $\mathrm{PSL}(2,7) \cong \mathrm{GL}(3,2)$ порядка **168** (подгруппа $S_7$). Лишь эти 168 перестановок сохраняют структуру линий; та же 168-кратная симметрия управляет тем, какие символические системы могут повторяться меж культур ([T-256](/docs/applied/research/one-grammar)) и фиксирует компактный калибровочный тор диссипатора ([T-260](./g2-noether-cc#тор-зарядов)).
