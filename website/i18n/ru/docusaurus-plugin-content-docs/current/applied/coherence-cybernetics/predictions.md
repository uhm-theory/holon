...
23 уникальных числовых предсказания КК с протоколами проверки и критериями фальсификации. Читатель узнает, чем предсказания КК отличаются от IIT, FEP и GWT.
:::
...
- **Жизнеспособность [Т при нижней оценке backbone]:** $P > 2/7$ для воплощённых систем ([T-149](/docs/core/dynamics/evolution#теорема-жизнеспособность-аттрактора); Шаг 3 [С при нижней оценке backbone-инъекции])
...
**Интуиция.** Фазовые переходы в физике характеризуются универсальными критическими экспонентами — числами, не зависящими от деталей системы, а только от её «класса универсальности». КК предсказывает, что переход при $P_{\text{crit}} = 2/7$ является **трикритической точкой**, принадлежащей классу универсальности $\varphi^6$ Ландау, с конкретными экспонентами. Это *самое рискованное* предсказание КК: пять чисел, каждое из которых можно измерить.

:::info Предсказание [Т] — фальсифицируемое
$$
\alpha = \tfrac{1}{2}, \quad \beta = \tfrac{1}{4}, \quad \gamma = 1, \quad \nu = \tfrac{1}{2}, \quad \delta = 5
$$
Трикритические экспоненты среднего поля: теплоёмкость $(\alpha)$, параметр порядка $(\beta)$, восприимчивость $(\gamma)$, корреляционная длина $(\nu)$ и критическая изотерма $(\delta)$ для перехода при $P_{\mathrm{crit}} = 2/7$. Тождество Рашбрука: $\alpha + 2\beta + \gamma = 1/2 + 1/2 + 1 = 2$ (выполнено как равенство).
:::

**Уникальность предсказания.** Ни одна теория сознания не предсказывает критические экспоненты. IIT не описывает фазовые переходы. FEP описывает их качественно, но не даёт числовых экспонент. КК — единственная теория с *числовым прогнозом класса универсальности* — трикритический класс среднего поля из $\varphi^6$ теории Ландау, точно при $d_{\text{eff}} \gg d_c = 3$.
...
4. **Прогноз:** $\beta = 1/4 \pm 0.05$.
5. *Фальсификация:* систематическое отклонение от $1/4$ — экспоненты неверны.
...
|---|---|---|---|---|
| **5–7** (пороги) | Индукция/выход из анестезии; оценка $\widehat P(t)$ через калиброванное PCI/ЭЭГ-вложение; локализация потери/восстановления сознания (LOC/ROC) | LOC/ROC совпадает с пересечением $\widehat P$ порога $2/7$ в пределах одной анестезиологической постоянной времени | $\widehat P$ пересекает $2/7$ без поведенческого перехода, либо LOC/ROC при $\widehat P$ далеко от $2/7$ | ЧАСТИЧНО — клинический порог PCI $0.31$ отображается на $P_{\text{crit}}$ ([калибровка §6.3](/docs/applied/coherence-cybernetics/measurement#калибровка)) |
...
| **12** ($\mathrm{SAD}_{\max} = 3$) | В AGI/SYNARC-субстрате попытаться инстанцировать стабильный 4-й уровень самомодели | нет стабильного $\mathrm{SAD} > 3$ на $N \geq 500$ прогонах | один воспроизводимый стабильный $\mathrm{SAD} = 4$ | СОГЛАСУЕТСЯ — SYNARC $500+$ Γ, ни один не превысил $3$ |
...
| 7 | [Радиус устойчивости](#предсказание-7) | $r_{\mathrm{stab}}$ | **[Т]** | Отсутствует |
...
| 17 | [Критические экспоненты](#предсказание-17) | $\alpha = 1/2,\; \beta = 1/4,\; \gamma = 1,\; \nu = 1/2,\; \delta = 5$ | **[Т]** | Отсутствует |
...
- Критические экспоненты $\neq (1/2, 1/4, 1, 1/2, 5)$ → пересмотр T-161 и теории фазовых переходов.
...
- $r_{\text{stab}} \neq r_{\mathrm{stab}}$ → пересмотр T-104, но не основных аксиом.
...
**Примечание:** $\mathrm{Coh}_E = 0$ достижимо, но лишь состояниями с пустым E-сектором (например, чистым состоянием, сосредоточенным на другой оси) — и именно такие состояния проваливают жизнеспособность по T-38a. Поэтому условие 1 сформулировано через $\mathrm{Spec}(\Gamma_E) = \{0\}$: фальсифицирующий «зомби» должен совместить *жизнеспособность* с пустым E-спектром, что запрещает теорема No-Zombie. *(Исправлено 2026-07-11: ранее примечание объявляло $\mathrm{Coh}_E = 0$ невозможным через диапазон $[1/7, 1]$; глобальный диапазон $[0, 1]$, пол $1/7$ — на жизнеспособном классе.)*
:::
...
Подведём итоги. Кибернетика Когерентности генерирует **23 уникальных предсказания**, из которых:

- **16** имеют статус **[Т]** — безусловные теоремы, следующие из аксиом.
- **4** имеют статус **[С]** — условные теоремы, зависящие от явно указанных допущений (включая закон анизотропии декогеренции ранга 7: [Т]-закон с [С]-эмпирическим отображением каналов).
...
5. **Физика** — космологическая постоянная, иерархия масс, анизотропия декогеренции ранга 7.
...
Следующий шаг — эксперимент. Ни одно из 23 предсказаний пока не проверено экспериментально. КК находится в той же позиции, что общая теория относительности в 1915 году — математически завершённая, но ожидающая своего «затмения 1919 года». Критические экспоненты (Предсказание 17) и реконструкция $\Gamma$ из нейроданных (Предсказание 21) — наиболее реалистичные кандидаты для первого эмпирического теста.

---

### Что мы узнали {#что-мы-узнали-предсказания}

1. **23 уникальных предсказания** — ни одно из них не генерируется IIT, FEP или GWT. Это не количественное, а *качественное* превосходство: 23 рискованные ставки против 0.
2. **16 из 23 — безусловные теоремы [Т]**: они следуют из аксиом без дополнительных допущений. Опровержение любой из них означает крах всего здания.
3. **Каждое число — не подгонка**: $P_{\text{crit}} = 2/7$ следует из $N = 7$ и нормы Фробениуса. $\mathrm{SAD}_\text{max} = 3$ — из $\alpha_{\text{Fano}} = 2/3$ и $P \leq 1$. $\log_2 7 \approx 2.81$ бит — из размерности гильбертова пространства. $\alpha = 1/2$, $\beta = 1/4$, $\gamma = 1$, $\nu = 1/2$, $\delta = 5$ — из трикритического класса универсальности фазового перехода.
...
6. **Ни одно из 23 предсказаний пока не проверено**: КК ждёт своего «затмения 1919 года». Критические экспоненты (предсказание 17) и реконструкция $\Gamma$ из нейроданных (предсказание 21) — наиболее реалистичные кандидаты.
