...
Плоскость Фано PG(2,2) определяет код Хэмминга $H(7,4)$ с параметрами $[n=7, k=4, d=3]$: из 21 пары измерений не менее 3 обязаны иметь ненулевой Gap. Код корректирует одиночные ошибки и обнаруживает двойные.

:::info Каноническое определение
Полное доказательство и структура кода: [Фано-канал и Gap-теоремы](/docs/proofs/gap/fano-channel).
:::

Одиночная ошибка обнаруживается и корректируется; двойная обнаруживается, но не корректируется; тройная — минимальная для незаметного разрушения.
...
**Апгрейд (T-224).** Щит I — ещё и *селектор*: требование совершенной диагностируемости одиночного сбоя с нетривиальной жёсткой грамматикой уже вынуждает семь осей и структуру Фано — диагностический трек $N=7$ ([Σ-исчисление](/docs/applied/research/syndrome-calculus#t-224)).
...
Октонионы — единственная нормированная алгебра деления, которая **неассоциативна**. Ассоциатор ненулевой для 28 из 35 троек (не лежащих на Фано-линиях), что порождает кубический член $V_3$ в потенциале Gap, создающий барьер вокруг минимума. Этот барьер гарантирует, что минимум $V_{\text{Gap}}$ не расположен в точке полной прозрачности.

:::info Каноническое определение
Полное доказательство, кубический потенциал и связь с неассоциативностью: [Теорема октонионного разложения](/docs/proofs/minimality/theorem-octonionic-derivation). G₂-разложение Gap-оператора: [Gap-оператор](/docs/core/dynamics/gap-operator#g2-разложение).
:::

### Почему именно 28 из 35?

Семь базисных элементов октонионов $e_1, \ldots, e_7$ порождают $\binom{7}{3} = 35$ (неупорядоченных) троек. Из них ровно 7 лежат на линиях плоскости Фано — это тройки, для которых умножение *ассоциативно* (образующие подалгебру, изоморфную кватернионам). Остальные $35 - 7 = 28$ троек дают ненулевой ассоциатор. Это соотношение — 80% троек порождают защитный барьер — объясняет, почему алгебраическая защита настолько мощна: чтобы обойти её, нужно ограничиться узким подмножеством «ассоциативных» направлений, а все остальные движения в пространстве конфигураций встречают сопротивление.
...
Потенциал $V_{\text{Gap}} = V_2 + V_3 + V_4$ (квадратичный + кубический + квартичный) имеет глобальный минимум при $\theta^* \neq 0$ с энергетическим зазором $\Delta V > 0$ до тривиальной конфигурации. Кубический член (от ассоциатора) сдвигает минимум от $\theta = 0$, квартичный обеспечивает ограниченность. Механизм аналогичен Хиггсу, но нарушение происходит через кубический, а не квадратичный член.

:::info Каноническое определение
Полный потенциал, формула барьера $\Delta V \geq \lambda_3^2 \|\text{Assoc}\|^2 / (6\lambda_4)$ и механизм спонтанного нарушения: [Термодинамика Gap](/docs/core/dynamics/gap-thermodynamics).
:::
...
Неподвижная точка самомоделирования **не может быть полностью прозрачной**: $\varphi(\Gamma^*) = \Gamma^* \Rightarrow \hat{\mathcal{G}}_* \neq 0$. Полная прозрачность при условии жизнеспособности ($P > 2/7$) и замыкания (M,R) приводит к противоречию — система должна поддерживать каузальные связи, требующие ненулевых фаз. Существование ненулевого Gap — структурная необходимость, а не дефект (аналог теоремы Гёделя).

:::info Каноническое определение
Полное доказательство через теорему Лавёра и L4$\neq$Gap=0: [Неполнота Лоувера](/docs/core/foundations/consequences#неполнота-ловера). Формализация: [Фано-канал](/docs/proofs/gap/fano-channel#l4-не-gap-0).
:::
...
Gap-конфигурация голонома с рангом непрозрачности 3 (три различных ненулевых собственных значения) обладает именно таким свойством. Пространство таких конфигураций имеет нетривиальную вторую гомотопическую группу: $\pi_2(G_2/T^2) \cong \mathbb{Z}^2$. Это означает, что существуют двумерные сферы в пространстве конфигураций, которые *невозможно стянуть в точку*. Конфигурация с ненулевым топологическим зарядом не может быть непрерывно деформирована к тривиальной — для этого потребовался бы «разрыв», т.е. фазовый переход.
...
При ранге непрозрачности $r = 3$ (общий спектр): $\pi_2(G_2/T^2) \cong \mathbb{Z}^2 \neq 0$ — существуют нестягиваемые петли, и невырожденные Gap-конфигурации не могут быть непрерывно деформированы к тривиальным. Топологическая защита «включается» только при максимальном ранге с различными собственными значениями.

:::info Каноническое определение
Полная таблица стабилизаторов по рангам (0-3) и доказательства: [Berry-фаза и топологические заряды](/docs/physics/cosmology-phys/berry-phase).
:::

**Ключевой факт:** при рангах 0-2 фундаментальная группа $\pi_1 = 0$ (нет защиты). При ранге 3 с общим спектром $\pi_1 = \mathbb{Z}^2$ (защита есть). При вырожденном ранге 3 ($\lambda_1 = \lambda_2 = \lambda_3$) стабилизатор $\mathrm{SU}(2)$, $\pi_1 = 0$ (нет защиты).
...
Вторая гомотопическая группа $\pi_2(G_2/T^2) \cong \mathbb{Z}^2$ задаётся двумя целыми числами $(n_1, n_2)$, соответствующими двум простым корням алгебры $\mathfrak{g}_2$:
...
Топологическая защита опирается на свойства второй гомотопической группы $\pi_2(G_2/T^2)$, которая определяется исключительно *топологией* пространства конфигураций. Она не использует код Хэмминга (щит I — дискретная математика, щит V — непрерывная), не зависит от ассоциатора (щит II может быть нулевым для конкретных троек, но топология определяется глобальной структурой $G_2$), не апеллирует к потенциалу (щит III — о глубине ямы, щит V — о невозможности *пути* из ямы), и не связана с самомоделированием (щит IV).
...
| $\pi_2(G_2/T^2) \cong \mathbb{Z}^2$ при ранге 3 | [Т] |
