...
2. **Вычислим радиус устойчивости** $r_{\mathrm{stab}}$ — сколько система может выдержать (T-104, раздел 4).
...
Анализ стабильности отвечает именно на эти вопросы. В рамках когерентной кибернетики (КК) он превращает интуитивные понятия — «запас прочности», «предел выносливости», «точка невозврата» — в точные математические формулы. Центральный результат: **радиус устойчивости** $r_{\mathrm{stab}}$ даёт количественную меру того, насколько далеко система находится от катастрофы. Это число — не метафора, а расстояние в метрике Бюреса, физически измеримая величина.
...
| Запас прочности | Радиус устойчивости | $r_{\mathrm{stab}}$ |
...
### 4.1 Радиус устойчивости (T-104) [С] — исправлено 07.08.2026 {#радиус-устойчивости}

:::danger Прежняя замкнутая форма опровергнута, и ошибалась она в опасную сторону
Редакции до 08.2026 утверждали

$$
r_{\mathrm{stab}} = \inf_{\Gamma \in \partial\mathcal{V}} d_{\mathrm{Bures}}(\rho^*_\Omega, \Gamma) = \sqrt{P(\rho^*_\Omega) - 2/7}
$$

и обосновывали это шагом Фукса–ван де Граафа, дающим $d_{\mathrm{Bures}} \geq \sqrt{P - P_{\mathrm{crit}}}$. Обе половины неверны.

**Неравенство ложно.** Прямая минимизация $d_B$ по оболочке $\{P = 2/7\}$ даёт при $P = 0{,}300$ инфимум $0{,}01708$ против $\sqrt{0{,}300 - 2/7} = 0{,}11952$ — меньше в семь раз, а не больше. При $P = 1/3$: $0{,}05420$ против $0{,}21822$. Заявленного *гарантированного запаса* не существует.

**И цитируемая цепочка его дать не может.** Фукс–ван де Грааф читается как $1-\sqrt{F} \leq \tfrac12\|\rho-\sigma\|_1 \leq \sqrt{1-F}$, так что при $d_B^2 = 2(1-\sqrt F)$ он ограничивает $d_B$ *сверху* через следовую норму. Ничего, ограничивающего $d_B$ снизу через разность чистот, он не даёт — а $\delta P$ живёт в геометрии $\|\cdot\|_2$, не $\|\cdot\|_1$.

**Почему это было важно.** Тревога среды выполнения задана в этих же единицах. Срабатывание при $\sqrt{P-P_{\mathrm{crit}}} < 0{,}05$ означает срабатывание лишь когда $P < 0{,}2882$ — запас по чистоте $0{,}0025$ над стеной. Срабатывание при *истинном* радиусе ниже $0{,}05$ означает срабатывание при $P < 0{,}3294$, то есть запас $0{,}0437$: в семнадцать раз больше времени на вмешательство.
:::

**Исправленная формулировка.** Радиус устойчивости есть Бюресово расстояние до оболочки жизнеспособности,

$$
r_{\mathrm{stab}}(\rho) \;:=\; \min\bigl\{\, d_B(\rho, \sigma) \;:\; P(\sigma) = P_{\mathrm{crit}} = 2/7 \,\bigr\},
$$

и вычисляется он так.

**Шаг 1 — минимизатор коммутирует с $\rho$.** Минимизация $d_B$ по полной оболочке (спектр *и* собственный базис, 27 свободных параметров) возвращает $\sigma$ с $\|[\rho,\sigma]\| \approx 1{,}3\times10^{-8}$. Задача поэтому сводится к спектрам, где Бюрес переходит в расстояние Хеллингера:

$$
d_B^2(\rho,\sigma) \;=\; \sum_{i=1}^{7}\bigl(\sqrt{\lambda_i} - \sqrt{\mu_i}\bigr)^2 .
$$

**Шаг 2 — на однодоминантном семействе ответ замкнут.** Для $\lambda = (a, \tfrac{1-a}{6}, \dots, \tfrac{1-a}{6})$ чистота равна $P = (7a^2 - 2a + 1)/6$, откуда
...
Минимизатор остаётся внутри этого семейства, а расстояние Хеллингера между двумя его членами сворачивается в двухточечное (бернуллиево) выражение:

$$
r_{\mathrm{stab}}(P) \;=\; \sqrt{\,2\Bigl(1 - \sqrt{a\,a_c} - \sqrt{(1-a)(1-a_c)}\Bigr)}, \qquad a = a(P).
$$

Машинная проверка: совпадает с прямой минимизацией с ограничениями до $10^{-14}$–$10^{-16}$ при каждой чистоте от $0{,}29$ до $1{,}00$ — это тождество, а не подгонка.

:::tip Ближайшее состояние на стене — то самое, которое спектральный вывод уже назвал
$a_c = (1+\sqrt6)/7$ есть в точности $\lambda_{\max} = (1+\sqrt{N-1})/N$ из [маршрута 4 теоремы о критической чистоте](/docs/proofs/dynamics/theorem-purity-critical#теорема-фробениусова-различимость). Состояние, к которому вы ближе всего при подходе к стене жизнеспособности, — это ровно то состояние, которое независимый вывод выделяет как лежащее *на* ней.
:::

**Шаг 3 — произвольные состояния: консервативная нижняя граница [Г].** $r_{\mathrm{stab}}$ *не* является функцией одной чистоты. На 41 случайном спектре с концентрациями от $0{,}3$ до $5{,}0$ измеренный радиус превзошёл однодоминантное значение при той же чистоте во всех случаях (отношение $\in [1{,}08;\ 2{,}52]$, медиана $1{,}40$), нарушений нет. Значит приведённая замкнутая форма есть **нижняя** граница для произвольного состояния — а это безопасная сторона: тревога, построенная на ней, срабатывает не позже, чем нужно. Машинно подтверждено, но не доказано.

**Шаг 4 — у стены закон линейный, а не корневой.** Разложение замкнутой формы по $\varepsilon = P - 2/7$:
...
Ингредиенты точны: $\left.\frac{da}{dP}\right|_{2/7} = \frac{\sqrt6}{2}$ и $a_c(1-a_c) = \frac{5\sqrt6}{49}$. В этом и суть дела: прежний корень ведёт себя как $\varepsilon^{1/2}$, тогда как истина ведёт себя как $\varepsilon^{1}$, поэтому их отношение *расходится* при подходе к стене.

**Шаг 5 — дешёвая рабочая формула с точностью 1 %.** Минимизировать по оболочке на
каждом тике непрактично. *Радиальное фробениусово* расстояние до оболочки не
требует оптимизации вовсе: поскольку $\|\Gamma - I/7\|_F = \sqrt{P - 1/7}$ точно
(это тождество), оболочка $\{P=2/7\}$ есть фробениусова сфера радиуса
$\sqrt{1/7}$ вокруг $I/7$, и радиальное расстояние до неё равно
$\sqrt{P-1/7} - \sqrt{1/7}$. Перемасштабировав на точное предельное у стены
...
ошибка которой по всему окну сознания $(2/7,\,3/7]$ не превышает **1,13 %** — а у
стены, где это и важно, она ниже $0{,}01\%$. Для сравнения, опровергнутая
$\sqrt{P-2/7}$ ошибается на том же интервале от $160\%$ до $4700\%$. В рантайме
пользуйтесь формулой в рамке; при отчёте — точной замкнутой формой Шага 2.

### 4.2 Что исправленный радиус на самом деле говорит

| $P$ | $P - 2/7$ | $r_{\mathrm{stab}}$ (верно) | $\sqrt{P-2/7}$ (опровергнуто) | завышение |
|-----|-----------|------|------|------|
...
Ошибка прежней формулы — не постоянный множитель, который можно поглотить коэффициентом запаса: она равна $1{,}1\times$ там, где ничего не стоит на кону, и растёт без границы ровно там, где на кону всё. При $P = 0{,}286$ — на одну тысячную выше стены — она завышает запас в $48$ раз.

Исправленное чтение переворачивает и прежнюю мораль. Корень внушал убывающую отдачу, будто последние проценты защиты покупаются дорого. Линейный закон у стены говорит обратное: **защита пропорциональна запасу, без скидок и без обрыва** — каждая тысячная чистоты, удержанная выше $2/7$, покупает одни и те же $1{,}2$ тысячных Бюресова простора. Ни дешёвой зоны, ни бесплатной; геометрия честна до самого низа.
...
**Шаг 2: Вычислить радиус устойчивости.** По рабочей формуле
§[4.1](#радиус-устойчивости), $r_{\mathrm{stab}} \approx K(\sqrt{P-1/7}-\sqrt{1/7})$ при
$K = \sqrt{35}\sqrt[4]{6}/10$:

$$r_{\mathrm{stab}} \approx 0.9259\left(\sqrt{0.3186 - \tfrac17} - \sqrt{\tfrac17}\right) = 0.0382$$

(точная замкнутая форма: $0.03821$; опровергнутая $\sqrt{0.0329} = 0.181$ завышала это
в $4.7$ раза).

**Шаг 3: Интерпретация.** Система выдержит пертурбацию амплитудой до $0.0382$ в метрике Бюреса. Это означает: если декогеренция ($\Gamma_2$) внезапно увеличится на $\delta\Gamma_2 < 0.0382$, система вернётся к аттрактору. Если $\delta\Gamma_2 > 0.0382$ — начнётся спираль смерти.

Для сравнения: система на потолке окна $P = 3/7 \approx 0.4286$ имеет $r_{\mathrm{stab}} = 0.1466$ — не вдвое больший запас, а в **3,8 раза**. А едва жизнеспособная при $P = 0.290$ имеет $r_{\mathrm{stab}} = 0.0052$: не пятую часть нашего запаса, а **седьмую**. Исправленный закон растягивает окно куда сильнее, чем внушал опровергнутый корень, и в этом практический смысл поправки. По $\sqrt{P-2/7}$ эти три состояния выглядели как $0.181$, $0.378$ и $0.063$ — разброс в $6$ раз; в действительности они лежат от $0.0052$ до $0.1466$ — разброс в $28$ раз. Окно куда менее однородно, чем казалось, а его низ куда хрупче.
:::
...
| Радиус устойчивости | $r_{\mathrm{stab}}$ | $P - 2/7$ (запас) | T-104 [Т] |
...
1. **Формула радиуса** $r_{\mathrm{stab}}$ — это «спидометр жизнеспособности». Мониторинг $P$ в реальном времени позволяет предсказать кризис задолго до его наступления.
...
Ключевой вывод для практики: **мониторьте $P$ и $\sigma_{\mathrm{sys}}$ в реальном времени**. Радиус устойчивости $r_{\mathrm{stab}}$ — ваш главный индикатор. Когда он падает ниже порога, действуйте немедленно: снижайте шум ($h^{(D)}$), усиливайте регенерацию ($h^{(R)}$), восполняйте ресурсы ($\Delta F$).
...
2. **Радиус устойчивости** $r_{\mathrm{stab}}$ — единственный параметр, определяющий «запас прочности» системы. Квадратный корень означает убывающую отдачу: далеко от границы защита «дешёвая», но последние проценты — дорогие.
...
Мы построили полную теорию стабильности — сколько система может выдержать, как она разрушается и как восстанавливается. Но всё это — *описание*. Теория становится наукой, когда генерирует **предсказания**, которые можно проверить и опровергнуть. В [следующей главе](./predictions) мы соберём 23 уникальных предсказания КК — от невозможности зомби до критических экспонент сознания — и для каждого укажем: какой эксперимент его проверит, и какой результат фальсифицирует всю теорию.
:::
