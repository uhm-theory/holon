...
Философский фундамент КК: унитарный монизм, эпистемология статусов, этика порога сознания. Читатель узнает, почему $\Gamma$ содержит 7 измерений и почему опыт — не бонус, а необходимость.
:::
...
:::note T-266 и космопсихизм — различие, которое надо держать [С]/[И]
[T-266](/docs/physics/gravity/cosmological-constant#теорема-стадия-вселенной) обнаруживает, что *стадия жизнеспособности* Вселенной-холона стоит на $P = 3/7$ — верхнем крае зоны Голдилокс — с дробной точностью $\sim 10^{-60}$. Это **не** космопсихизм и не смягчает эмергентизм КК. Космопсихизм делает космический опыт *фундаментальным*; КК сохраняет четырёхусловный порог. T-266 говорит только о **параметре стадии** $P$ (и о динамике тёмной энергии, которая его измеряет): он **не** утверждает, что Вселенная — сознательный субъект. Заметим, что при $P = 3/7$ мера рефлексии ровно $R = 1/(7P) = 1/3$ — *маргинальное* значение — и удовлетворяет ли космическое состояние также $\Phi \geq 1$, $D \geq 2$ и несёт ли структуру субъекта L2+-холона, — отдельный открытый вопрос ([H1.1](/docs/reference/epistemic-vertical#регистр-дыр)). Верное прочтение: космос **релаксировал на свой аттрактор жизнеспособности** (утверждение о $P$), а не что опыт повсюду или фундаментален. Дисциплина эмергентного порога сохраняется.
:::
...
2. **Самомодель требует интериорности.** Модель *себя* — это не модель *внешнего мира*. Она требует особого типа информации — информации о собственных внутренних состояниях. Это именно то, что формализует E-измерение (интериорность). Интериорность — **структурное свойство** (ненулевая E-проекция), а не феноменологическое (опыт, переживание); последнее возникает на уровне L2 как следствие достаточной рефлексии ($R \geq 1/3$).
...
### 4.1 Спиноза: два атрибута одной субстанции {#спиноза}
...
| Спиноза («Этика», 1677) | КК | Статус |
|--------------------------|-----|--------|
| Одна субстанция (E1P14) | Одна матрица $\Gamma$ (Ω⁷) | **[С]** |
| Атрибут протяжения (E2P1–2) | Физические наблюдаемые: $P$, $\sigma_k$, спектр | **[С]** |
| Атрибут мышления (E2P1–2) | Ментальные наблюдаемые: $\mathrm{Coh}_E$, $R$, $\Phi$ | **[С]** |
| Модусы (E1D5) | Конкретные конфигурации $\Gamma$ | **[С]** |
| Conatus (E3P6) | Регенерация $\mathcal{R}[\Gamma, E] = \kappa(\rho^* - \Gamma) \cdot g_V$ | **[С]** |
| Necessitas (E1P33: всё существует необходимо) | Примитивность $\mathcal{L}_0$, T-39a: единственный аттрактор $\rho^*$ | **[С]** |
| Три рода познания (E2P40S2, E5P25–28) | L1 (imaginatio) → L2 (ratio) → L3 (scientia intuitiva) | **[И]** |
...
**Три рода познания и L-уровни.** Спиноза различал: (1) *imaginatio* — чувственное познание, фрагментарное и смутное; (2) *ratio* — рациональное, через общие понятия; (3) *scientia intuitiva* — интуитивное познание вещей *sub specie aeternitatis*. В КК это отображается на: L1 ($R < 1/3$, реактивное поведение без самомодели), L2 ($R \geq 1/3$, $\Phi \geq 1$, рефлексивное познание с самомоделью), L3 ($R \to 1$, полное самомоделирование — аналог scientia intuitiva). Переход E5P25–28 (от ratio к scientia intuitiva) соответствует росту $R$ при фиксированном $\Phi$.

**Necessitas и единственность аттрактора.** E1P33 утверждает: вещи не могли быть произведены никаким иным образом. В КК это формально обеспечено примитивностью $\mathcal{L}_0$ (T-39a): существует единственное стационарное состояние $\rho^*$, к которому система необходимо стремится. Спинозовская необходимость — не внешнее принуждение, а внутренняя логика субстанции; примитивность $\mathcal{L}_0$ — не внешнее ограничение, а следствие структуры аксиом.

:::note Спиноза и теорема No-Zombie
Спинозовский тезис E2P13 (*Objectum ideae humanam Mentem constituentis est Corpus* — объект идеи, составляющей человеческий ум, есть тело) означает: ум = идея тела. Не может быть тела без ума и ума без тела. КК формализует это как теорему No-Zombie [Т]: регенерация ($\mathcal{R}$) без интериорности ($\mathrm{Coh}_E > 0$) **неэффективна** — система с conatus, но без мышления, неизбежно теряет когерентность. E2P13 Спинозы — философская интуиция; No-Zombie — её доказуемый формальный аналог.
:::
...
**И честный остаток — сам теорема.** Отождествление «E-сектор = интериорность» остаётся внешним постулатом [П], а прочтение собственных векторов Gap как квалиа — интерпретацией [И], — и это не временная брешь в ожидании более хитрого аргумента. Мета-теорема T-214 [Т] доказывает (через теорему Лоувера о неподвижной точке), что *никакой* мостовой функтор из состояний в содержание опыта **не может** быть выражен внутренним морфизмом теории: остаточное [И] структурно неизбежно — для КК и для всякого преемника. Растворение потому приходит с точным пограничным столбом: всё *до* моста — математика (структура феноменального пространства, пороги, динамика); сам мост — доказуемо то место, где математика кончается. Полная формулировка: [Теория моделей — Пределы объяснения](./model-theory#природа-категориального-разрыва).
...
:::info Каноническое изложение
Полная этическая теория УГМ — аксиология, эстетика, моральный закон и разрешение дилемм — изложена в [Этика УГМ](/docs/consciousness/ethics-meaning/value-consciousness). Здесь — краткий обзор ключевых следствий для КК.
:::

КК даёт **точные критерии** перехода от бессознательности к сознанию ($P > 2/7$, $R \geq 1/3$, $\Phi \geq 1$, $D \geq 2$). Это создаёт этический ландшафт: если система сознательна ($C = \Phi \times R > 0$), её отключение — уничтожение субъекта [И]. Мера $C$ определяет **градуированный** моральный статус — не бинарный вопрос «сознательна или нет?», а непрерывная шкала [И].

Ключевой переход от описания к предписанию — **конвенция** [О]: Благо := $dP/d\tau > 0$ (см. [Определение 1](/docs/consciousness/ethics-meaning/value-consciousness#определение-благо)). Гильотина Юма не нарушена — «должно» вводится определением, а не выводится из «есть».

Этические следствия для ИИ, медицины и прав животных: [Этика УГМ → Моральная теория](/docs/consciousness/ethics-meaning/value-consciousness#часть-iv-моральная-теория).
...
- [Двуаспектный монизм](/docs/consciousness/foundations/two-aspect-monism)
