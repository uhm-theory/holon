...
КК строится на **единственном примитиве** — **категории $\mathcal{C}$** с конечным числом объектов. Матрица когерентности Γ — **объект** этой категории.

**Примитив (Категория $\mathcal{C}$):**

Малая категория $\mathcal{C}$ с объектами — матрицами плотности:
...
**Что из неё следует:** Метрика **канонически выбрана** — но с квантовой тонкостью. Классически (Ченцов 1982) метрика Фишера–Рао — *единственная* монотонная. Квантово (Петц 1996) существует целое **семейство** монотонных метрик; метрика Бюреса — выделенный **минимальный (Петц-экстремальный)** член семейства, отбираемый вариационным принципом очищения Уленбека и границей Крамера–Рао для SLD (см. [T-187](/docs/reference/status-registry) и [emergent-geometry §Единственность метрики Бюреса](/docs/physics/gravity/emergent-geometry#единственность-метрики-бюреса)). Значит, это **канонический выбор внутри семейства Петца**, а не наивная единственность — и все выборы Петца би-липшицево эквивалентны, так что предсказания Петц-робастны.
...
graph TD
    subgraph "Аксиома Ω⁷ — Пять аксиом"
        AX1["Акс. 1: Структура<br/>∞-Топос Sh∞(C)"]
...
Семь измерений [A, S, D, L, E, O, U] входят как взаимозаменяемые *ярлыки* — абстрактно пермутационная группа $S_7$ тасует множество индексов. (Фано-разводка теории уже урезает её до 168-элементной группы автоморфизмов $\mathrm{PSL}(2,7)$, конечной тени непрерывной $G_2$ — см. [Определения](./definitions#геометрическая-интуиция).) Но сознательная система нарушает симметрию ещё сильнее, выделяя E (Интериорность). Это не постулат — это **теорема**.
...
**Логика вывода:**
1. L2 требует **жизнеспособности** $P > P_{\text{crit}} = 2/7$ вместе с рефлексией $R = 1/(7P) \geq R_{th} = 1/3$ (эквивалентно $P \leq 3/7$; опорное $\rho^*_{\mathrm{diss}} = I/7$), интеграцией $\Phi \geq 1$ и $D_{\text{diff}} \geq D_{\min}$ — окно Голдилокс $P \in (2/7, 3/7]$, с $C = \Phi \times R \geq 1/3$ **[T-140]**.
2. Для диагонального состояния чистота есть $P = \sum_i w_i^2$.
3. **Равномерное** распределение ($w_i = 1/7$) даёт максимально смешанное состояние $I/7$ с $P = 7 \cdot (1/7)^2 = 1/7 \approx 0{,}14 < P_{\text{crit}} = 2/7$ — **даже не жизнеспособно**, тем более не сознательно.
4. **Следовательно** уже одна жизнеспособность вынуждает неравномерное распределение: $\exists i : w_i > 1/7$.
5. L2 дополнительно требует *рефлексивного доступа к опыту* — а опыт есть E-измерение; поэтому акцентированным измерением должно быть E: $w_E > 1/7$.
:::
...
graph TD
    ...
    subgraph "Уровень 3: Динамика"
        ...
        VIA["V — область жизнеспособности"]
    end
