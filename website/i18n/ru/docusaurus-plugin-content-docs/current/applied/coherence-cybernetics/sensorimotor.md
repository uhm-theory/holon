...
### 1.3 Термодинамическая трихотомия каналов {#термодинамическая-трихотомия}

Теорема T-102 говорит, что у среды ровно три двери в холон. Естественный следующий вопрос: различны ли эти три двери *по роду*, или это лишь три ярлыка на одном механизме? Ответ: они несут три термодинамически различных — и совместно исчерпывающих — режима обмена, и трихотомия есть открыто-квантовый образ старейшей классификации в физике: разбиения обмена энергией в первом начале на **работу**, **тепло** и **химическую работу** (обмен веществом). Чтобы это увидеть, прочтём каждый канал через то, что он делает с двумя базовыми функционалами состояния — энтропией фон Неймана $S(\Gamma) = -\mathrm{Tr}(\Gamma \ln \Gamma)$ и чистотой $P(\Gamma) = \mathrm{Tr}\,\Gamma^2$.

#### Теорема T-258 (Термодинамическая трихотомия каналов) [Т] {#теорема-термодинамическая-трихотомия}

:::tip Утверждение
Пусть $\dot{\Gamma} = h$ — возмущение из трёхканального базиса T-102. Тогда энтропийно-чистотные сигнатуры каналов попарно различны и исчерпывают базис:

1. **Гамильтонов канал** $h^{(H)} = -i[\delta H, \Gamma]$: $\dot{S} = 0$ **и** $\dot{P} = 0$ — единственный изэнтропический, сохраняющий чистоту канал (**работа**: он перенацеливает состояние, не тратя и не ввозя порядок);
2. **Диссипативный канал** $h^{(D)} = \delta\Gamma_2 \cdot \mathcal{D}_{\text{Fano}}[\Gamma]$: $\dot{S} \geq 0$ и $\dot{P} = -\tfrac{4}{3}\,\delta\Gamma_2 \cdot C(\Gamma) \leq 0$, строго при когерентной массе $C(\Gamma) = \sum_{i \neq j}|\gamma_{ij}|^2 > 0$ (**тепло**: единственный канал, способный лишь производить энтропию);
3. **Регенеративный канал** $h^{(R)} = \delta\kappa_{\text{eff}}\,(\rho^* - \Gamma)$:
$$
...
$$
— оба знака возможны; **единственный** канал, способный понижать энтропию и поднимать чистоту (**химическая работа / вещество**: питание ввозит негэнтропию).

Следовательно, три канала реализуют три различных **типа сигнатуры** — *консервативный* ($\dot{S} \equiv 0 \equiv \dot{P}$ на всяком состоянии), *знако-определённый диссипативный* ($\dot{S} \geq 0$ **и** $\dot{P} \leq 0$ на всяком состоянии) и *знако-неопределённый* (достижимы оба знака) — и эта типизация исчерпывающа (размечает весь базис T-102) и различает каналы. Трихотомия тем самым *наблюдаема как классификация*. Одна мгновенная пара знаков определяет действующий канал **в общем случае, но не универсально**: на диагональном состоянии тепловой канал вырождается в $(0,0)$ и на миг неотличим от работы, а регенеративный шаг к более смешанной цели с малым перекрытием может транзиентно войти в квадрант $(+,-)$ тепла. Точна и универсальна именно *типовая* трихотомия; одноразовая идентификация — в общем случае, а достоверная следует из отклика канала, прощупанного по состояниям.
:::

**Доказательство.**
*(i)* Для $\dot{\Gamma} = -i[\delta H, \Gamma]$: $\dot{S} = -\mathrm{Tr}(\dot{\Gamma}\ln\Gamma) = i\,\mathrm{Tr}([\delta H, \Gamma]\ln\Gamma) = i\,\mathrm{Tr}(\delta H\,[\Gamma, \ln\Gamma]) = 0$, ибо $[\Gamma, \ln\Gamma] = 0$; весь спектр инвариантен при унитарном сопряжении, так что всякий спектральный функционал — в частности $P$ — сохраняется.

*(ii)* Фано-канал **унитален**: $\sum_p \Pi_p = 3\cdot\mathbb{1}$ (каждый канал лежит ровно на трёх линиях), так что $\mathcal{F}(\mathbb{1}/7) = \mathbb{1}/7$. Унитальная CPTP-полугруппа мажорирует вниз, откуда $S$ не убывает, с равенством ровно на диагональных состояниях. Поэлементно инцидентность BIBD (всякая пара каналов делит ровно одну линию) даёт $\mathcal{D}_{\text{Fano}}[\Gamma]_{ij} = -\tfrac{2}{3}\gamma_{ij}$ при $i \neq j$ и $0$ на диагонали, откуда $\dot{P} = 2\,\mathrm{Tr}(\Gamma\,\mathcal{D}_{\text{Fano}}[\Gamma]) = -\tfrac{4}{3}\sum_{i\neq j}|\gamma_{ij}|^2$.

*(iii)* Прямое вычисление при $\mathrm{Tr}\,\dot{\Gamma} = 0$: $\dot{S} = -\delta\kappa_{\text{eff}}\,\mathrm{Tr}((\rho^*-\Gamma)\ln\Gamma) = \delta\kappa_{\text{eff}}[S(\rho^*) + D(\rho^*\|\Gamma) - S(\Gamma)]$, используя $\mathrm{Tr}\,\rho^*\ln\Gamma = -S(\rho^*) - D(\rho^*\|\Gamma)$. Оба знака реализуются: $\Gamma = 0{,}98\,|\psi\rangle\langle\psi| + 0{,}02\,\mathbb{1}/7$ с ещё более чистой целью даёт $\dot{S} = -0{,}095$ (питание очищает); $\Gamma = 0{,}9\,|\psi\rangle\langle\psi| + 0{,}1\,\mathbb{1}/7$ с целью $\mathbb{1}/7$ даёт $\dot{S} = +3{,}208$. Формула для чистоты немедленна из $\dot{P} = 2\,\mathrm{Tr}(\Gamma\dot{\Gamma})$ и положительна, когда перекрытие с целью превышает текущую чистоту. $\blacksquare$

**Машинная верификация.** Триста случайных полноранговых состояний на канал: $|\dot{S}|, |\dot{P}| \leq 3\cdot10^{-15}$ для $h^{(H)}$; $\min \dot{S} = +0{,}337 \geq 0$ и формула чистоты точна до $10^{-16}$ для $h^{(D)}$; энтропийная формула $h^{(R)}$ точна до $2\cdot10^{-15}$, свидетели обоих знаков — как указано. Типовая определённость на $3000$ состояниях: работа $|\dot{S}|,|\dot{P}| \leq 6\cdot10^{-10}$ (консервативный), нарушений знако-определённости тепла $0/3000$; граничные вырождения подтверждены — тепло на диагональном состоянии даёт ровно $(0,0)$, а регенеративный шаг к смешанной цели с малым перекрытием попадает в $(+,-)$, так что пара знаков — общий, а не универсальный идентификатор (*типовая* трихотомия точна).

#### Гранд-канонический словарь [И] {#гранд-канонический-словарь}

Сигнатуры отождествляют три канала с тремя сопряжёнными парами гранд-канонического ансамбля — и, член за членом, с тремя преобразованиями Лежандра, которыми *Самообучающаяся Вселенная* Ванчурина (SLU, 2026) строит физику из обучающейся системы с ограниченными ресурсами. Там агент не может измерить три экстенсивные переменные — смещение $\Delta q^\mu$, быструю энтропию $S_x$, число $N$ быстрых степеней свободы — и моделирует каждую её интенсивной сопряжённой: калибровочным полем $A_\mu$, температурой $T$, химическим потенциалом $\mu$.

| Режим обмена | Гранд-каноническая пара | Ванчурин (SLU) | Канал УГМ | Сигнатура $(\dot{S}, \dot{P})$ |
|---|---|---|---|---|
| Работа | (смещение, сила) | $\Delta q^\mu \leftrightarrow A_\mu$ | $h^{(H)}$ ($\delta H_{\text{eff}}$) | $(0,\ 0)$ |
| Тепло | $(S,\ T)$ | $S_x \leftrightarrow T$ | $h^{(D)}$ ($\delta\Gamma_2$) | $(\geq 0,\ \leq 0)$ |
| Вещество | $(N,\ \mu)$ | $N \leftrightarrow \mu$ | $h^{(R)}$ ($\delta\kappa$) | $(\mp,\ \pm)$ |

Пять структурных проверок словаря, каждая заякорена в корпусе:

1. **Нет четвёртого канала ↔ нет четвёртого аргумента.** Полнота T-102 зеркалит полноту фундаментального термодинамического соотношения $U(S, V, N)$: открытую систему можно вести ровно тремя способами — совершать над ней работу, греть её или кормить. Ванчурин выводит тот же счёт из трёх недоступных экстенсивных аргументов своей пошаговой потери $L(q, \Delta q, S_x, N)$; два доказательства исчерпываемости (LGKS и каскад Лежандра) приходят к одной трихотомии с противоположных концов — перекрёстная валидация обоих.
2. **Оси фазовой диаграммы гранд-каноничны.** [Фазовая диаграмма](/docs/applied/coherence-cybernetics/phase-diagram-cc) сознания уже живёт в координатах $(t, r) = (T_{\text{eff}}/T_c,\ \kappa/\Gamma_2)$ — температура и отношение питания. Под словарём это в точности $(T, \mu)$: QCD-аналогия «барионный химический потенциал $\mu_B \leftrightarrow r$» тем самым поднята от визуальной параллели до структурного соответствия, а порог питания [T-259](/docs/applied/coherence-cybernetics/phase-diagram-cc#теорема-окно-питания) читается как порог конденсации по химическому потенциалу.
3. **Питание живёт на канале Основы.** Ядро регенерации $\kappa_0 = \omega_0|\gamma_{OE}||\gamma_{OU}|/\gamma_{OO}$ несёт O-канал — канонически канал «поддерживать существование, *питать*, параметризовать внутреннее время». В SLU химический потенциал так же замкнут на часы: $h = |\mu|\varepsilon$ связывает квант действия с химическим масштабом на шаг времени. Обе теории прикрепляют канал вещества и внутренние часы к одному носителю.
4. **Дискретность и $U(1)$.** В SLU $U(1)$-фаза волновой функции есть термодинамическая эквивалентность $S \to S + h\Delta N$ при *целочисленных* скачках $N$. На стороне УГМ это теперь **теорема**: [T-260](/docs/core/operators/lindblad-operators#теорема-происхождение-тора) показывает, что сохранённые заряды Фано-диссипатора — ровно семь паспортных населённостей, их экспонента — диагональный тор $U(1)^7$ (полная группа ковариантности $U(1)^7 \rtimes \Gamma_{\!\text{oct}}$), а компактность каждого фактора вынуждена *целочисленностью* спектра заряда — проекторные заполнения $\{0,1\}$ и целые счётчики суб-холонов нейрогенеза (⊕). Механизм SLU, поканально и выведенно [Т]; соответствие с собственной $U(1)$ SLU остаётся интерпретивным [И].
5. **Линейно-разрешённые температуры.** Тепловой канал УГМ несёт семь скоростей $\{\gamma_p\}$ — по одной на линию Фано, — тогда как SLU несёт один скаляр $T$. Изотропная точка $\gamma_p = \gamma$, единственная $G_2$-симметричная конфигурация, есть в точности сеттинг SLU: корпус уточняет единую температуру до *спектра семи линейных температур*, где анизотропия меряет, насколько неравномерно среда греет когерентную структуру холона, — и уточнение фальсифицируемо: [закон анизотропии ранга 7](/docs/applied/coherence-cybernetics/effective-temperature#линейные-температуры) [Т] вынуждает 14 точных линейных соотношений среди 21 парной скорости декогеренции и делает семь линейных температур восстановимыми томографией.

Честность статусов: сигнатурная теорема — [Т]; сам словарь — отождествление с (работа, тепло, вещество) и с $(A, T, \mu)$ SLU — интерпретация [И], точная на сигнатурах и счёте. По *динамическим законам* реестр теперь **замкнут** ([T-262](/docs/core/dynamics/evolution#теорема-динамическая-трихотомия)): все три ноги выведены как точные геометрические потоки — работа = изометрический (киллингов) драйв каждой монотонной метрики; тепло = градиентный поток негэнтропии $D(\Gamma\|\mathbb{1}/7)$ по Карлену–Маасу с линейными температурами как весами транспорта; вещество = BKM-градиентный поток $D(\rho_*\|\Gamma)$ ([T-261](/docs/core/dynamics/evolution#теорема-регенерация-градиентный-спуск)); компактность калибровочного тора также выведена ([T-260](/docs/core/operators/lindblad-operators#теорема-происхождение-тора)). Реестр надстроен двумя нормативно-физическими замками: поток канала вещества — **единственный оптимальный алгоритм обучения** ([T-263](/docs/core/dynamics/evolution#теорема-наилучший-обучающий-поток) — наискорейший BKM-спуск по смесевой геодезической, геометрия единственна по Грасселли–Стритеру), а слоган SLU *«гравитация = эффективность обучения»* получил знак и разрешение по секторам ([T-264](/docs/physics/gravity/einstein-equations#теорема-информационно-гравитационная-взаимность): $G^{-1}\propto$ QFI пространственных фаз, $\Lambda\propto\mathcal{G}_O^2$ = квадрат необучаемости часов). Остаётся [И] лишь *межтеорийное* отождествление с самой SLU.
