...
| 5 | $\sigma_E$ (интериорность) | $(N - D_{\mathrm{diff}})/(N-2)$ | $< 0.9$ | $0.9 - 1.0$ | $\geq 1.0$ |
| 6 | $\sigma_O$ (основание) | $1 - \kappa_0/\kappa_{\mathrm{bootstrap}}$ | $< 0.3$ | $0.3 - 0.6$ | $\geq 0.6$ |
| 7 | $\sigma_U$ (единство) | $2\Phi_{\mathrm{th}}/(\Phi_{\mathrm{th}} + \Phi)$ | $< 0.9$ | $0.9 - 1.0$ | $\geq 1.0$ |
...
2. **Радиус устойчивости (T-104 [Т]).** $r_{\text{stab}}$. При $P = 0.4$ (типичное «здоровое» значение), $r_{\text{stab}} \approx 0.34$. Это означает, что пертурбации с $\|h^{\text{ext}}\| < 0.34$ безопасны. При $\sigma \approx 0.5$ система находится примерно на расстоянии $r_{\text{stab}}$ от границы — ровно одна типичная пертурбация до проблемы.
