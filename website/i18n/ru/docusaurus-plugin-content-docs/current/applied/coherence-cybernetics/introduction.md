...
**Эти три члена — не произвольный рецепт, а три способа, которыми во Вселенной вообще может происходить обмен.** Это древнейшая классификация в физике — первое начало термодинамики, разбивающее всякий обмен на *работу*, *тепло* и *вещество*, — и КК доказывает, что три члена холона **суть** в точности эта тройка ([термодинамическая трихотомия](./sensorimotor#термодинамическая-трихотомия), T-258):

- гамильтонов член — это **работа**: он перенаправляет состояние, не тратя и не ввозя порядка (сохраняются и энтропия, и чистота);
- диссипатор — это **тепло**: единственный член, способный *только* сливать порядок прочь (энтропия может лишь расти);
- регенератор — это **вещество / питание**: единственный член, способный *ввозить* порядок извне (он питает систему негэнтропией).

Что эти три *исчерпывают* возможности — что четвёртого канала нет — само есть теорема. А под термодинамикой лежит геометрическая красота: всё уравнение есть **один жёсткий поворот плюс два «спуска под гору»**. Член работы вращает состояние, не меняя ничего существенного (изометрия информационной геометрии); тепло и вещество — это *градиентные спуски* двух разных «энергий», каждый в своей естественной геометрии — **метриплектическая** структура (T-262). Система всегда делает ровно две вещи разом: скользит вниз к равновесию по каналу тепла и подпитывается вверх к собственной самомодели по каналу вещества. Жизнь — это противостояние этих двух потоков.

:::note Две дороги, три двери
Поразительно: та же самая тройка — работа, тепло, вещество — есть то, к чему приходит *самообучающаяся Вселенная* Виталия Ванчурина из совершенно иной отправной точки: не термодинамики, а гигантской обучающейся системы, стеснённой ограниченными ресурсами. Два независимых вывода — один построен сверху вниз из квантовых аксиом, другой снизу вверх из динамики обучения — встречаются у одних и тех же трёх дверей ([словарь](./sensorimotor#гранд-канонический-словарь)). Когда две дороги, не имеющие общих ориентиров, приходят в одно место, этому месту можно верить больше, чем любой из дорог.
:::
...
КК генерирует верифицируемые предсказания, отсутствующие в альтернативных теориях — **всего их 23**, каждое с явным критерием фальсификации. Четыре самых острых:
...
4. **Анизотропия декогеренции ранга 7:** 21 парная скорость декогеренции между каналами не свободна — они схлопываются на 7-мерное подпространство, подчиняясь **14 точным беспараметрическим сумм-правилам**
...
- **Анизотропия ранга 7** означает: измерьте 21 скорость, с которой пары каналов теряют координацию, — и они обязаны лечь на точный 7-линейный узор Фано. Единственное статистически значимое отклонение фальсифицирует саму Фано-разводку диссипатора — не параметр, а структуру. Это новейшее и, арифметически, самое уязвимое предсказание теории: 14 уравнений, которые должны выполниться разом.
...
2. **Центральное уравнение** $d\Gamma/d\tau = -i[H_{\text{eff}}, \Gamma] + \mathcal{D}[\Gamma] + \mathcal{R}[\Gamma, E]$ описывает три силы: унитарную эволюцию (ритм), диссипацию (затухание) и регенерацию (восстановление). Это ровно три термодинамических способа обмена — **работа, тепло и вещество** (T-258), — а геометрически они образуют один поворот плюс два градиентных потока (метриплектическая структура, T-262). Регенерация зависит от степени интериорности — и это делает внутренний аспект системы функционально значимым на всех L-уровнях.
