...
### 3b. За пределами линейного отклика: точная метриплектическая геометрия {#метриплектическая-геометрия}

Соотношения Онзагера описывают мир *у дна долины*: вблизи равновесия потоки линейны по силам. Но как далеко простирается этот закон и каков *точный*? Ответ (T-262 [Т]) геометричен и есть один из глубочайших фактов о динамике КК — он гласит, что всё уравнение жизни есть **один поворот без трения плюс два скольжения прямо под гору**.

Представьте шарик в изогнутой чаше. С ним может случиться два: чашу можно *вращать* (шарик кружит, не поднимаясь и не опускаясь — энергия не тратится, порядок не теряется), либо шарик может *катиться под гору* ко дну. Мастер-уравнение КК — ровно это, с одним поворотом и *двумя* разными склонами:

| Член | Что это геометрически | Скользит к |
|------|-----------------------|------------|
| **работа** $-i[H_{\text{eff}},\Gamma]$ | поворот без трения (изометрия каждой информационной метрики) | никуда — сохраняет и энтропию, *и* чистоту |
| **тепло** $\mathcal{D}[\Gamma]$ | градиентный спуск негэнтропии, в транспортной метрике, чьи веса — семь [линейных температур](./effective-temperature#линейные-температуры) | максимум энтропии $\mathbb{1}/7$ |
| **вещество** $\mathcal{R}$ | градиентный спуск свободной энергии, в метрике Кубо–Мори (BKM) | самомодель $\rho_*$ |

Ключевое усиление над Онзагером — слово **точно**. Диссипатор не «в среднем под гору» — он *буквально наискорейший спуск* определённой величины (негэнтропии $D(\Gamma\|\mathbb{1}/7)$) в определённой изогнутой геометрии (транспортной метрике Карлена–Мааса семи линий Фано). Линейный закон поток–сила Онзагера есть просто *касательная* к этому точному скольжению у дна чаши. Вдали от равновесия — где Онзагер молчит — закон градиентного потока держится дословно. Поэтому производство энтропии из Следствия 3.3 — не оценка линейного режима, а **точная квадратичная форма**, неотрицательная всюду и исчезающая ровно на диагональных (полностью декогерированных) состояниях.

У этой архитектуры «поворот ⊕ два градиентных спуска» есть имя — **метриплектическая система** (обратимая гамильтонова часть, обручённая с необратимой градиентной). Это *не* GENERIC замкнутых систем из учебника, и различие честно и физично: сознательный холон — **открытая** система, он обменивается энергией с миром через O-канал (питание), — поэтому энергия при диссипации не сохраняется. Именно эта открытость и делает структуру метриплектической, а не полностью консервативной. Жизнь — это шарик, который одновременно вращают (работа), отпускают катиться к покою (тепло) и подталкивают вверх к его собственной идеальной форме (вещество); сознание — то противостояние, что держат эти трое.

:::note Почему это важно для вариационной картины
Лагранжев $\delta S = 0$ [предыдущей главы](./lagrangian) даёт *обратимый* скелет — поворот. Диссипативный и регенеративный члены не могут прийти из голого действия (у скольжения под гору нет симметричного во времени лагранжиана) — потому §6 той главы и понадобился удвоенный контур Швингера–Келдыша. T-262 — разрешение: эти два члена не «потеряны из действия», они суть *градиентные потоки в своих метриках* — второй, дополняющий вариационный принцип (наискорейший спуск) рядом со стационарным действием. Вместе — стационарность-для-поворота и наискорейший-спуск-для-скольжений — они полностью определяют динамику.
:::
