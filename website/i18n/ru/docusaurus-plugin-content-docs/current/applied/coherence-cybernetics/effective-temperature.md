...
(размерная эвристика [И], метящая топологию границы; точная микроскопическая граница — седло-узловая складка живой ветви — и замкнутое окно сознания по $r$ даны в [T-259](/docs/applied/coherence-cybernetics/phase-diagram-cc#теорема-окно-питания))

### Линейно-разрешённые температуры: закон анизотропии ранга 7 {#линейные-температуры}

Тепловой канал [термодинамической трихотомии (T-258)](/docs/applied/coherence-cybernetics/sensorimotor#теорема-термодинамическая-трихотомия) несёт семь скоростей $\{\gamma_p\}$ — по одной на линию Фано, — так что эффективная температура выше уточняется до *спектра семи линейных температур* $T_p \propto \gamma_p$. Эта анизотропия не свободна: при произвольных положительных скоростях линий парные скорости декогеренции подчиняются точному закону [Т]

$$
r_{ij} \;=\; \frac{1}{6}\sum_{p\,:\,|\ell_p \cap \{i,j\}| = 1} \gamma_p ,
$$

— **совместная линия пары сокращается**, и каждая из $21$ парной скорости есть среднее четырёх линий, встречающих пару ровно в одной точке (изотропная точка: $r = \tfrac{2}{3}\gamma$, воспроизводя T-110). Следствия [Т]: карта инцидентности ($21$ скорость $\leftarrow$ $7$ температур) имеет ранг ровно $7$, так что (i) измеренные скорости удовлетворяют **14 точным линейным соотношениям** — резкая сигнатура Фано-разводки, ибо любой не-Фано дизайн из семи линий даёт другой конус соотношений; и (ii) семь линейных температур **восстановимы** из парной томографии декогеренции (наименьшие квадраты на карте инцидентности; машинная проверка: закон точен до $4\cdot10^{-16}$, реконструкция до $3\cdot10^{-15}$). $G_2$-симметричная точка $\gamma_p = \gamma$ — единственная изотропная конфигурация — есть однотемпературный предел, принимаемый скалярно-$T$ трактовками (например, «Самообучающейся Вселенной» Ванчурина). Измеренное нарушение 14 соотношений в межканальных данных декогеренции опровергло бы саму Фано-структуру диссипатора [С].
...
Среднеполевые критические показатели **точны** для Gap-системы по трём взаимно-согласованным механизмам (см. полный аргумент в [Механизм точности](/docs/consciousness/hierarchy/swallowtail-transitions#механизм-точности)):

**(I) Топологическая жёсткость.** По классификации Тома–Арнольда, катастрофы коразмерности 3 в одной координате параметра порядка единственно $A_4$ (swallowtail) с нормальной формой $V_0(x) = x^5/5$. Пять критических показателей $\{\alpha,\beta,\gamma,\nu,\delta\} = \{1/2, 1/4, 1, 1/2, 5\}$ — **топологические инварианты** этого класса катастроф — нечувствительны к гладким деформациям потенциала.

**(II) Детерминированная УГМ-динамика.** Поток $d\Gamma/d\tau = \mathcal L_\Omega(\Gamma) + \mathcal R(\Gamma)$ детерминирован; нет термодинамического ансамблевого усреднения. Gap-Ландау-потенциал — эффективный потенциал потока, его седловые точки — фактические детерминированные аттракторы $\Gamma$-динамики. Никакая стохастическая Гинзбург-поправка неприменима.

**(III) Перекрёстная проверка large-$N$.** Размерность параметра порядка УГМ вблизи порога $d_{\mathrm{eff}} = \binom{7}{2} = 21$ (независимые off-diagonal когерентности в $\mathfrak{su}(7)$, с $\mathrm{dim}\,\mathfrak{su}(7) = 48 = 6_{\mathrm{diag}} + 42_{\mathrm{off}}$, давая 21 комплексный off-diagonal mode, спаренный Фано-линиями; см. [Gap-семантика](/docs/physics/dual-aspect/gap-semantics#разделение-когерентностей)). Если бы УГМ переинтерпретировалась как стохастическая $O(N)$-тензорная модель, ведущие $1/N$ поправки были бы $\sim 1/21 \approx 5\%$, в пределах экспериментального разрешения.

**Замечание о традиционном Гинзбурге.** Обычный критерий Гинзбурга требует пространственной размерности; УГМ не имеет пространственного интегрирования (голон $(0{+}1)$-D), так что пространственная форма критерия напрямую неприменима. Механизмы (I)–(III) выше дают подлинное обоснование.
:::

Следствие: **среднеполевая теория Gap точна** — нет необходимости в ренормализационных поправках. Критические показатели можно вычислить аналитически. Gap-теория — одна из немногих физических систем, допускающих подлинно точное описание фазового перехода, потому что детерминированно-динамическая формулировка не вводит стохастических флуктуаций изначально.
