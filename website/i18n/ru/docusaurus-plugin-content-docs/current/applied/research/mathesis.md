...
Примеры: $T_{\text{УГМ}}$ (273 теоремы реестра, 7 статусов, 5 аксиом), $T_{\text{IIT}}$ (5 постулатов, $\Phi$, Q-shape), $T_{\text{GWT}}$ (глобальное зажигание, доступ), $T_{\text{FEP}}$ (свободная энергия, марковское одеяло).
...
**Формальная типизация в Verum.** Стандартная библиотека Verum уже предоставляет иерархию протоколов `Site<C> = (underlying_category: InfinityCategory, topology: GrothendieckTopology<C>)` с `GrothendieckTopology`, несущей три `@verify(formal)` аксиомы (максимальность, устойчивость, транзитивность). Инстанциация для Матезиса (см. an unpublished specification §3.3) определяет:
...
**Алгоритм** (`compute_pointwise_lan` в):
...
:::tip Теорема M-1 (Аксиомы сайта Гротендика для $J_\mathrm{ep}$) [Т]
...
:::tip Теорема M-2 (Существование ∞-топоса Mathesis) [Т]
...
:::tip Теорема M-3 (Вложение Yoneda вполне верно) [Т]
...
:::tip Теорема M-4 (Сходимость аппроксимации расширения Кана) [Т]
...
:::tip Теорема M-5 (Эпистемическая монотонность как категориальное следствие) [Т]
...
:::tip Теорема M-6 (Квантово-логическая необходимость ортомодулярной решётки) [Т]
...
:::tip Теорема M-7 (Моноада Giry хорошо определена на пространстве функторов) [Т]
...
:::tip Теорема M-8 (L-III модификация топологии сохраняет ∞-топос-структуру) [Т]
...
:::tip Теорема M-9 (Когнитивное расширение через свёртку Day) [Т при T-129]
...
:::tip Теорема M-10 (Граница Лоувера для $T_\mathrm{meta}$) [Т]
...
**Доказательство.** Прямое применение теоремы о неподвижной точке Лоувера (Lawvere 1969; Yanofsky 2003). Применённое к предикату непротиворечивости $T_\mathrm{meta}$: если бы он был и внутренним (выразимым в $\mathrm{Th}_\mathrm{Mathesis}$), и верным внешней непротиворечивости (точечно-сюръективным по Лоуверу), он породил бы самореферентную неподвижную точку, противоречащую собственному утверждению. Следовательно предикат непротиворечивости либо невнутренний, либо неверный.

Мы принимаем второй вариант, давая $[\mathrm{H}]$-ограничение. Это прямой аналог T-214 [Т] для УГМ (позитивная мета-теорема о трудной проблеме): всякая достаточно выразительная самореферентная система имеет структурно непреодолимые внешние постулаты. $\blacksquare$

**Следствие.** Mathesis **честна** в своих границах: никакое утверждение вида «Mathesis полна/непротиворечива» не может иметь статус выше [Г]. Система самосознаёт эту границу и обеспечивает её через эндпоинт `meta/boundaries`.
...
**Алгоритм эпистемического измерения** (`measure()`):
...
**Содержание.** Исследователь, использующий Mathesis, по T-153 [Т] — совместимый с УГМ L2+ холоном. Взаимодействие расширяет самонаблюдение исследователя через систему:
...
- **M-10**: теорема о неподвижной точке Лоувера ограничивает $T_{\text{meta}}$ — ни одна теория не содержит полного описания самой себя.
