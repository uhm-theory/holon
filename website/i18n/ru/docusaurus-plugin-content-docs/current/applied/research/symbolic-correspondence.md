...
:::warning Статус [П] — с журналом выполнения
Таблицы соответствий конструктивны; программа верификации §6 писалась до первого прибора. С тех пор существенная её часть **выполнена** в holon-лаборатории — в §0 сведено, что теперь измерено, что умерло и что заменило исходные дизайны. Разделы 6–7 сохранены как исходная пререгистрационная запись.
:::

## 0. Состояние выполнения (2026-08) {#состояние}

Программа ниже задумывалась под интервью-инструменты. Реально выросшая лаборатория ([программа HomoHoloGraph](/docs/applied/research/homoholograph)) жёстче: детерминированные движки, пререгистрированные нули, реплика до статуса. Журнал честно:

| Исходный дизайн | Что реально прогнано | Вердикт |
|---|---|---|
| H-SC1: знак зодиака ↔ когерентности черт | Натал→черта на сильнейшем внешнем корпусе: Гоклен-класс, $n=15\,931$ | **Нуль** (все $\eta^2 < 0.5\%$) — сильнейшая форма H-SC1 мертва; [детали](/docs/applied/research/homoholograph#x-nulls) |
| H-SC2: обратная проекция ицзина | Вместо этого вскрыто колесо: HD = сэмплер по алфавиту гексаграмм с эфемеридными часами; закон антиподов 32/32, Грей-локальность, MI центр–триграмма — всё [Т-вычислено] | *Структура* — настоящая математика; *человеческий слой* по исходному дизайну не проверен |
| H-SC3: межсистемная согласованность | Теоремы о решётках: сетки накшатр/пад/навамш **не вкладываются** в родные линии [Т]; дуэль сидерика-vs-тропика на событиях и 3 000 смертях | **Системы — не проекции одной решётки**; системы долгот неотличимы от случая на датах — вопрос согласованности растворился, а не решился |
| Тайминг-утверждения (неявные во всех таблицах) | Полная оракул-программа: 5 масочных оптик + дистрибуционные тесты + ML-детектор информации на корпусах смертей | **Нуль по всему фронту** для утверждений уровня дат; один [И]-след (марака) живёт в конвертах вперёд |
| Чакра-опросник (§6.2) | Заменён родным стеком приборов: дневник голосов, томограф, пассивный канал наблюдений | Родной алфавит сделал заимствованный опросник ненужным |

**Где программа живёт теперь:** онтологическое место символических систем — в [Символических системах §10](/docs/core/structure/symbolic-systems#онтологическое-место); родная система, построенная из ядра наружу, — [Γ-Канон](/docs/applied/research/gamma-canon) плюс слой родных сезонов; машинерия валидации вперёд (запечатанные конверты, дуэли двух движков) заменяет ретроспективные корреляционные дизайны — ретроспектива на этом классе данных исчерпана честно.
