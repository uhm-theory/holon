...
description: "Эталонный протокол эмпирической валидации УГМ: 4 фазы, 23 предсказания, TMS-EEG, критические экспоненты"
---
...
- [23 уникальных предсказания КК](/docs/applied/coherence-cybernetics/predictions) — полный список предсказаний с формулами
...
УГМ — одна из наиболее формально проработанных теорий сознания: ~210 теорем, 23 числовых предсказания, категорный фундамент. Но **ни одно предсказание не проверено экспериментально**. Теория без эмпирики — философия, какой бы строгой ни была математика.
...
12 из 23 предсказаний проверяемы in silico на любой реализации **Γ-нативного агента** — системы, эволюция которой задаётся линдбладовской динамикой ℒ_Ω = ℒ₀ + ℛ на матрице когерентности Γ ∈ D(ℂ⁷). Не требуют нейроданных, субъектов, этического одобрения. Если хотя бы одно фальсифицировано — стоп, ревизия теории до перехода к дорогостоящим нейроэкспериментам.
...
### 3.1b. Формальное определение π_bio [Т при калибровке] {#pi-bio-definition}

:::warning Определение (Нейронное якорное отображение π_bio)
**Нейронное якорное отображение** — CPTP-канал:

$$
\pi_{\text{bio}}: \mathcal{O}_{\text{neural}} \to \mathcal{D}(\mathbb{C}^7)
$$

где $\mathcal{O}_{\text{neural}} = \mathbb{R}^d$ — пространство нейронных наблюдаемых (спектральные плотности мощности, вызванные ТМС потенциалы, функциональная связность, вегетативные индексы). Отображение определяется следующей конструкцией.
:::

**Конструкция (4 шага).**

**Шаг 1 (Семь канонических наблюдаемых).** Извлекаем из нейронных данных 7 диагональных проекций $\hat{\gamma}_{kk}$, соответствующих 7 измерениям УГМ, через следующую операционализацию:

| Измерение | Нейронная наблюдаемая | Извлечение |
|-----------|------------------|------------|
| A (Артикуляция) | Спектральная краевая частота (95-й перцентиль) | Спектр мощности ЭЭГ |
| S (Структура) | Дальнодействующие временные корреляции (показатель DFA) | Временной ряд ЭЭГ |
| D (Динамика) | Перестановочная энтропия | Сложность ЭЭГ |
| L (Логика) | Кросс-частотная связь (theta-gamma PAC) | Фазово-амплитудная связь |
| E (Интериорность) | PCI (индекс пертурбационной сложности) | ТМС-ЭЭГ |
| O (Основа) | Вариабельность сердечного ритма (RMSSD) | ЭКГ/ВСР |
| U (Единство) | Глобальная функциональная связность (средний PLI) | Связность ЭЭГ |

**Шаг 2 (Нормализация к диагонали матрицы плотности).** Применяем softmax-нормализацию для обеспечения $\sum_k \hat{\gamma}_{kk} = 1$ и $\hat{\gamma}_{kk} > 0$:

$$
\gamma_{kk} = \frac{\exp(\beta \cdot z_k)}{\sum_{j=1}^{7} \exp(\beta \cdot z_j)}, \quad z_k = \frac{x_k - \mu_k}{\sigma_k}
$$

где $x_k$ — сырая наблюдаемая, $\mu_k, \sigma_k$ — среднее и стандартное отклонение по популяции (из калибровочной когорты), а $\beta > 0$ — температурный параметр, калиброванный под соответствие эмпирического PCI* = 0.31 ↔ P_crit = 2/7.

**Шаг 3 (Внедиагональные когерентности через Холецкого).** Восстанавливаем внедиагональные элементы из попарных нейронных корреляций:

$$
|\gamma_{ij}| = \sqrt{\mathrm{Coh}_{ij}^{\text{neural}}} \cdot \sqrt{\gamma_{ii} \gamma_{jj}}, \quad \arg(\gamma_{ij}) = \phi_{ij}^{\text{PLV}}
$$

где $\mathrm{Coh}_{ij}^{\text{neural}}$ — квадрат модуля когерентности между наблюдаемыми $i$ и $j$ (из кросс-спектральной плотности), а $\phi_{ij}^{\text{PLV}}$ — величина фазовой синхронизации (phase-locking value) между соответствующими нейронными сигналами. Регуляризуем через разложение Холецкого: $\Gamma = LL^\dagger / \mathrm{Tr}(LL^\dagger)$ для обеспечения положительности и единичного следа.

**Шаг 4 ($G_2$-ковариантное прокрустово выравнивание).** По T-123 [Т], представление единственно с точностью до $G_2$. Фиксируем $G_2$-калибровочную свободу выравниванием восстановленной $\Gamma$ к каноническому эталону через:

$$
\Gamma_{\text{aligned}} = \arg\min_{g \in G_2} \| g\Gamma g^\dagger - \Gamma_{\text{ref}} \|_F
$$

где $\Gamma_{\text{ref}}$ — усреднённая по популяции $\Gamma$ из бодрствующего базового состояния калибровочной когорты. Это прокрустова задача на многообразии $G_2$, решаемая градиентным спуском на 14-мерной группе Ли (стандартная риманова оптимизация).

**Свойства π_bio.**

| Свойство | Статус | Обоснование |
|----------|--------|---------------|
| CPTP | [Т] | Softmax + Холецкий → положительность, сохранение следа; композиция CPTP-каналов — CPTP |
| $G_2$-ковариантность | [Т] | Прокрустово выравнивание шага 4 |
| Единственность с точностью до $G_2$ | [Т] | T-123 |
| Калибруемость | [Г] | Требует эмпирической подгонки $\beta$ под соответствие PCI* ↔ P_crit |

**Статус:** Конструкция **формально корректно определена** [Т] (шаги 1–4 — явные CPTP-операции). **Калибровка** (выбор $\beta$ и популяционных норм $\mu_k, \sigma_k$) эмпирична [Г] — требует завершения эксперимента Фазы II (§3.3). После калибровки π_bio становится валидированным измерительным инструментом [Т].

**Критерий фальсификации:** Если не существует значения $\beta$, при котором $P_{\text{boundary}} = 2/7 \pm 0.05$ у всех 50 испытуемых (p < 0.01), то π_bio фальсифицирован в текущей форме. Это не фальсифицирует УГМ — только данную конкретную операционализацию. Альтернативные операционализации (другие нейронные наблюдаемые, другая нормализация) подлежат исследованию.
