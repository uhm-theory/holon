...
**Предсказание 17** (критические экспоненты β=1/3, ν=1/4, γ=3/4) — самое ценное, потому что самое рискованное:
- Три конкретных числа, каждое фальсифицируемо
...
|------|------|-----|----------------|
| **I. Цифровая** | 0–6 мес. | 10 предсказаний in silico (Γ-нативный агент) | Бесплатно, без этики, проверяет фундамент |
...
10 из 22 предсказаний проверяемы in silico на любой реализации **Γ-нативного агента** — системы, эволюция которой задаётся линдбладовской динамикой ℒ_Ω = ℒ₀ + ℛ на матрице когерентности Γ ∈ D(ℂ⁷). Не требуют нейроданных, субъектов, этического одобрения. Если хотя бы одно фальсифицировано — стоп, ревизия теории до перехода к дорогостоящим нейроэкспериментам.
...
**Предсказание:** β = 1/3 ± 0.05 (T-161).

**Фальсификация:** β ∉ [0.28, 0.38] при N=10⁴.

**Значение:** Если in silico подтвердит β=1/3, переходим к нейроэксперименту (Фаза II) с высокой уверенностью.
...
**Все 10 экспериментов Фазы I подтверждены** → переход к нейроданным.
...
:::tip Уникальность
Это **первый в истории** тест критических экспонент фазового перехода для сознания. Ни IIT, ни GWT, ни FEP не предсказывают конкретных экспонент. Подтверждение β=1/3 означает: сознание принадлежит конкретному классу универсальности — как ферромагнетик или сверхтекучая жидкость.
:::
...
- ν = 1/4: корреляционная длина (из пространственной протяжённости EEG-ответа на TMS)
- γ = 3/4: восприимчивость (из амплитуды вариабельности PCI вблизи порога)

**Фальсификация:**
- β ∉ [0.28, 0.38] при N=50 (p < 0.01)
- ν ∉ [0.20, 0.30]
- γ ∉ [0.70, 0.80]

**Статистический план:** Нелинейная регрессия (power law fit), bootstrap для 95% CI, сравнение с альтернативными экспонентами (mean field: β=1/2, Ising 3D: β≈0.326).
...
**Предсказание:** T_ign ~ (P − P_crit)^{−1}. Дивергенция вблизи порога (критическое замедление).
...
| 5 | Коллективное сознание | IV.2 | Φ_⊗(коорд) ≤ Φ_⊗(случ) | [Т]/[С] |
...
| 11 | N=7 для соц. обучения | — | N=5 соц. обучается | [С] |
...
| 17 | Экспоненты β=1/3 | I.8 + II.2 | β ∉ [0.28, 0.38] | [Т] |
...
| **L2 — Структурный** | Конкретное числовое предсказание | P_crit ≠ 2/7; β ≠ 1/3; R_th ≠ 1/3 | Фундаментальная ревизия конкретной теоремы |
...
| Критические экспоненты | Нет | Нет | Нет | β=1/3, ν=1/4, γ=3/4 |
...
| Самый рискованный тест | Нет единого числа | «Зажигание» (качественно) | «Мета-когниция» (качественно) | **β = 1/3** (одно число, фальсифицируемо) |

УГМ отвечает на критику ConTraSt (Yaron et al. 2022): методологический выбор не предрешает результат, потому что предсказания — **числовые**, а не качественные. β=1/3 либо подтвердится, либо нет — независимо от парадигмы.
...
Самый рискованный тест — **критические экспоненты β=1/3** (Pred 17). Ни одна другая теория сознания не делает столь конкретного числового предсказания о фазовом переходе. Подтверждение означает: сознание — такой же фазовый переход, как намагничивание или сверхтекучесть, со своим классом универсальности. Опровержение означает: УГМ фундаментально ошибается в структуре перехода.
