...
*Доказательство.* Предположим, что операторы Линдблада $L_m = \chi_{S_m}$ диагональны в базисе указателя $\{|a_k\rangle\}$ (это обеспечивается тем, что $\{|a_k\rangle\}$ является одновременным базисом собственных векторов всех характеристических морфизмов $\chi_{S_m}$). Обозначим $(L_m)_{kk} \equiv \langle a_k | L_m | a_k \rangle \in \mathbb{R}$ (вещественно, поскольку $L_m$ эрмитов).

Вычислим матричный элемент уравнения Линдблада для $k \neq l$:
$$
...
где последнее равенство — тождество $ab - \tfrac{1}{2}(a^2+b^2) = -\tfrac{1}{2}(a-b)^2$ для вещественных $a,b$. Следовательно:
$$
\Gamma_{kl}(t) = \Gamma_{kl}(0)\,e^{-\gamma_{kl} t}, \quad \gamma_{kl} = \frac{1}{2}\sum_m \gamma_m \bigl|({\chi_{S_m}})_{kk} - ({\chi_{S_m}})_{ll}\bigr|^2
$$

**Позитивность $\gamma_{kl} > 0$:** так как $\gamma_m > 0$ (скорости Линдблада), достаточно показать, что хотя бы для одного $m$ выполнено $(\chi_{S_m})_{kk} \neq (\chi_{S_m})_{ll}$. Это гарантируется тем, что $|a_k\rangle$ и $|a_l\rangle$ — **различные** собственные векторы наблюдаемой $\hat{A}$, которую операторы $\chi_{S_m}$ нетривиально различают (т.е. $\hat{A}$ — ненейтральная измеримая величина). При $t \to \infty$ все $\Gamma_{kl} \to 0$ ($k \neq l$), оставляя классическую смесь $\sum_k p_k |a_k\rangle\langle a_k|$. $\blacksquare$
