...
**(e)** Наблюдаемое значение: $\delta_\text{CP} \approx 65.7° \pm 1.5°$ (PDG 2024). Расхождение сырого Фано-значения ~17%. Источники:
...
Наблюдаемое (**каноническое значение, SSOT**): $|\delta_\text{CP}| = 65.7° \pm 1.5°$ (глобальный фит PDG 2024). Чистейшее древесное (tree-level) определение — комбинация LHCb, представленная на ICHEP 2024, — даёт $\gamma \equiv \delta_\text{CP} = 64.6° \pm 2.8°$, ложась практически на предсказание ниже. Прежняя цифра «$69°\pm4°$» устарела. Расхождение сырого значения $\sim 11°$ (устраняется двухпетлевой поправкой ниже).
...
Расхождение с глобальным фитом $65.7°$: $\sim 1.2°$ ($< 1\sigma$); относительно прямой древесной комбинации LHCb $64.6° \pm 2.8°$ предсказанное $64.5°$ ложится в пределах $\sim 0.1°$ ($\approx 0.04\sigma$) — почти точное совпадение. **Улучшенное согласие** — и намного лучше с текущими значениями, чем с прежней цифрой $69°$.
...
Наблюдаемое значение $\delta_\text{CP} = 65.7° \pm 1.5°$ (PDG 2024). Предсказанное значение $\approx 64.5°$ отклоняется от центрального экспериментального значения на $\sim 1.2°$ ($< 1\sigma$). Знак двухпетлевой поправки зафиксирован SM RG [С]; точное значение зависит от пороговых поправок ГУТ [Г].
:::
...
**(d)** Уточнение: предсказание $\delta = 64.5°$ vs наблюдаемое $\delta = 65.7° \pm 1.5°$. Расхождение $< 1\sigma$. При $\delta = 65.7°$: $J_\text{pred} \approx 3.1 \times 10^{-5}$ — в согласии с наблюдаемым $J \approx 3.08\times10^{-5}$.

:::info Честная оценка точности J
Из 4 параметров формулы ($s_{12}$, $s_{23}$, $s_{13}$, $\delta$) только **один** ($\delta$) предсказан теорией. Остальные три — наблюдаемые. Остаточное фазовое расхождение мало́: $\sin(64.5°)/\sin(65.7°) = 0.903/0.911 = 0.991$ ($\sim 1\%$).
...
Подстановка экспериментальных масс (PDG): $m_d = 4.7$ МэВ, $m_s = 93.5$ МэВ, $m_u = 2.2$ МэВ, $m_c = 1.3$ ГэВ. Результат $|V_{us}| \approx 0.222$ — в согласии с наблюдаемым $0.2243 \pm 0.0005$.
...
| $\lVert V_{us}\rVert \approx \sqrt{m_d/m_s} \approx 0.222$ | Следствие [Г] | Текстура + $m_d = 4.7$ МэВ, $m_s = 93.5$ МэВ (PDG) | Стандартный Фрич |
...
## 10. УГМ и Cabibbo Angle Anomaly (T-265) {#10-cabibbo-angle-anomaly}

**Cabibbo Angle Anomaly** (CAA) — дефицит $\sim 3.2\sigma$ в тесте унитарности первого ряда CKM:

$$
|V_{ud}|^2 + |V_{us}|^2 + |V_{ub}|^2 = 0.9985(5) < 1,
$$

где $V_{ud} = 0.97373(31)$ из сверхразрешённого ядерного $\beta$-распада, а $V_{us}$ из $K_{\ell 3}$ (с тех пор появилось и второе внутреннее напряжение между $V_{us}$ из каонных и пионных полулептонных распадов). Стандартная таксономия разрешений делится на два семейства:

- **Извлечение в Стандартной Модели** — ядерные/адронные радиопоправки (прежде всего $\gamma W$-бокс, $\Box_{\gamma W}^A = 3.90(9)\times 10^{-3}$, и его зависимость от ядерной структуры), решёточный формфактор $f_+^K(0) = 0.9698(17)$ и напряжение $K$–$\pi$ в $V_{us}$.
- **За пределами Стандартной Модели** — 4-е поколение, **vector-like кварки** (феноменологически «самый перспективный» кандидат глобального фита), **MeV-стерильные нейтрино** (повышают извлекаемое $|V_{ud}|$), лептокварки, триплеты векторных бозонов, vector-like лептоны или иначе модифицированные $W$–кварковые связи.

Спектр УГМ фиксирован и сталкивается лоб в лоб с BSM-семейством.

### Теорема 10.1 (Канал разрешения CAA) [Т-структурно]+[С] {#thm-10-1-t-265}

**Утверждение (T-265).** В УГМ физическая матрица смешивания кварков — **точно $3\times3$ CKM-матрица и точно унитарна**; следовательно, если CAA сохранится, она обязана разрешиться **целиком в секторе извлечения Стандартной Модели** и **не** через новое кварковое, лептонное или бозонное состояние. Каждый ведущий BSM-канал исключён фиксированным спектром УГМ:

| BSM-канал | Вердикт УГМ | Основание |
|---|---|---|
| **4-е поколение** | **исключено [Т]** | $N_{\text{gen}} = 3$ точно — поколения суть квадратичные вычеты $\{1,2,4\} = \mathrm{QR}(7)$, **единственная** подгруппа порядка 3 в $\mathbb{Z}_7^\ast$, замкнутая относительно ассоциативного Фано-произведения ([§1](/docs/physics/particle-physics/fermion-generations#1-число-поколений-из-топологии-gap-вакуума)); не 2, не 4, не 6 |
| **Vector-like кварки** | **исключено [Т-структ]** | фермионы УГМ киральны по построению: $\gamma_5 = i\Gamma_O\Gamma_A\Gamma_S\Gamma_D$ действует с определённым собственным значением на внутренний спинор ([SM-сектор, киральность](/docs/physics/gauge-symmetry/standard-model#кираль)) — во фрейме нет некиральной (vector-like) кварковой конфигурации |
| **MeV-стерильное $\nu$** | **исключено [С]** | нейтринный сектор — type-I seesaw с тремя правыми $\nu_R = (1,1)_0$ при $M_R \sim 3\times10^{14}$ ГэВ и нормальной иерархией ([нейтринные массы §2](/docs/physics/particle-physics/neutrino-masses#seesaw)) — в спектре нет стерильного состояния масштаба эВ–МэВ |
| **Лептокварки / лишние калибр. бозоны** | **исключено [Т-структ]** | калибровочный сектор — $G_2 \to \mathrm{SU}(3)_C\times\mathrm{SU}(2)_L\times\mathrm{U}(1)_Y$ с ровно SM-контентом [Т], и **единственный** скаляр — уникальная хиггс-линия $\{A,E,U\}$ [Т] — нет ни лептокваркового скаляра, ни триплета векторных бозонов |

**Следствие.** УГМ делает резкое, фальсифицируемое предсказание о *канале* аномалии: дефицит живёт в **$\gamma W$-боксе / ядерно-структурных радиопоправках, решёточных формфакторах $K/\pi$ или напряжении $K$–$\pi$ в $V_{us}$** — адронных/ядерных входах SM — а не в самой матрице смешивания.

**Самосогласованность.** Именно это даёт право собственному выводу угла Кабиббо в корпусе ([§3](#3-угол-кабиббо)) фиксировать нормировку $C_{\text{norm}}$ из условия унитарности CKM: фундаментальная CKM УГМ точно унитарна [Т из $N_{\text{gen}}=3$], поэтому измеренный дефицит $\sim 0.15\%$ в УГМ — **артефакт извлечения**, а не свойство смешивания.

**Что УГМ *не* предсказывает [О].** **Величина** и **знак** дефицита — адронная/ядерная физика Стандартной Модели (размер $\Box_{\gamma W}$, ядерно-структурные поправки, значения формфакторов); УГМ не выводит число $\sim 0.15\%$. Этот остаток — по-настоящему открытая часть.

:::warning Фальсификация T-265
Если будет установлено, что CAA **требует** 4-го поколения, vector-like кварка, MeV-стерильного нейтрино или лептокварка — например, коллайдерное открытие такого состояния или прецизионный глобальный фит, исключающий SM-радиационное разрешение с высокой значимостью — то ядро УГМ $N_{\text{gen}}=3$ [Т] фальсифицируется. И наоборот, разрешение через улучшенную обработку $\gamma W$-бокса / решётки / $K$–$\pi$ **подтверждает предсказанный УГМ канал**. Статус: [Т-структурно] для исключений 4-го поколения / vector-like кварков / лептокварков, [С] для исключения стерильного нейтрино, [О] для величины.
:::

**Эскиз доказательства.** (1) $N_{\text{gen}}=3$ — [Т] ([§1](/docs/physics/particle-physics/fermion-generations#1-число-поколений-из-топологии-gap-вакуума), [поколения фермионов, Теор. 6.1](/docs/physics/particle-physics/fermion-generations#thm-6-1)): ассоциатор-свободный Фано-триплет единствен, $\{1,2,4\}$, и равен единственной подгруппе порядка 3 в $\mathbb{Z}_7^\ast$. Значит ровно три киральных поколения и $3\times3$-матрица смешивания. (2) Юкавские матрицы — $3\times3$; их би-унитарная диагонализация даёт $3\times3$ **унитарную** CKM ([Теор. 1.1](#thm-1-1)); без дальнейших кварковых состояний унитарность первого ряда точна. (3) Киральность ($\gamma_5$ определённа на $\chi_{\text{int}}$) запрещает vector-like партнёров; единственная хиггс-линия запрещает лептокварковые скаляры; seesaw-спектр не содержит лёгких стерильных. (4) Поэтому любой наблюдаемый дефицит унитарности — не свойство фундаментальной $V_{\text{CKM}}$ и обязан возникать при извлечении — из радиационных/решёточных входов SM. $\blacksquare$
