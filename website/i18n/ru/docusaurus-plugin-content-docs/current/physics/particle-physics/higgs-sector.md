...
2. **Из Фано-структуры:** единственная Фано-линия, проходящая через $E$ и $U$, — это $\{A,E,U\}$ (реестрово-каноническая Хиггсова линия, T.1.3; её третья точка $A$ лежит в $3$-секторе — **ни одна** Фано-линия не лежит целиком внутри $\bar 3=\{L,E,U\}$, поскольку $\{L,E,U\}=\{4,5,6\}$ не является линией). Оператор хиральности $\Gamma_{AEU}$ определяется этой линией. $\text{Gap}(E,U)$ — конкретная когерентность, нарушаемая Хиггсом, а остальные линейные когерентности $\text{Gap}(A,E)$ и $\text{Gap}(A,U)$ фиксируют вложение дублета.
...
:::warning [С] Условная
Параметр $\lambda_4$ определён из спектрального действия Шамседдина–Конна с RG-коррекцией (см. [теорему о хиггсовской квартике](#теорема-хиггсовская-квартика) [С]). Условность: свободный параметр $f_0$ в спектральном действии. Октонионная коррекция из $V_3$ дополнительно модифицирует $M_H$.
:::
...
$$\delta M_H^2 \approx \frac{3 \cdot (73.8)^2 \cdot (0.047)^2}{4 \cdot 16.6} \approx 0.54 \; (\text{в Gap-единицах})$$

(Указанные входные данные дают $0.54$; $\approx 5.5$ потребовало бы $\bar A\approx0.15$ — когерентность конфайнмент-сектора, а не среднее $0.047$.)
...
**Шаг 1 (Конечность по полевому пространству $\to$ конечный функциональный интеграл).** Gap-статсумма на компактной цели $(S^1)^{21}$ конечна — конечность по полевому пространству **[Т]**; полная попорядковая UV-конечность — структурная [С] ([T-66](/docs/physics/gravity/quantum-gravity#теорема-уф-конечность)). Следовательно, функциональный интеграл $Z = \int [D\theta] \exp(-S_{\mathrm{Gap}}[\theta])$ **конечен и определён** без регуляризационной неоднозначности. Квантовое эффективное действие $\Gamma_{\mathrm{eff}} = -\ln Z$ — конечная, конкретная величина.
...
Это стандартный результат Шамседдина–Конна–Марколли (2007, Thm 11.2) для NCG Стандартной модели. Применимость к УГМ-тройке верифицирована:

**Доказательство.**

**Шаг 1 (Проверка применимости).** Конечная спектральная тройка $(A_{\text{int}}, H_{\text{int}}, D_{\text{int}})$ УГМ ([теорема T-53](/docs/core/foundations/spacetime#теорема-спектральная-тройка) [Т]) удовлетворяет предпосылкам теоремы Шамседдина–Конна–Марколли:
...
### 6.0 Запрет второго хиггсовского дублета [Т] {#запрет-второго-дублета}

:::tip [Т] Структурный запрет (T-296)
УГМ запрещает второй хиггсовский дублет. Категориальная единственность, которая *выделяет* пару $(E,U)$, одновременно *исключает* всякого другого скалярного кандидата.
:::

**Теорема (нет-2HDM).** В УГМ существует ровно один конденсирующийся скалярный канал — $\gamma_{EU}$. Второго хиггсовского дублета (а значит и спектра $H^\pm, A^0, H^0$ типа 2HDM/MSSM) не существует.

**Доказательство.** (i) Конденсация требует $\kappa_0$-канала: теорема о вакууме T-64 даёт $\langle\gamma_{ij}\rangle \neq 0$ только для пары, выделенной $\kappa_0 = \omega_0|\gamma_{OE}||\gamma_{OU}|/\gamma_{OO}$, чьё морфизменное содержание — ровно $\mathrm{Hom}(O,E)\cdot\mathrm{Hom}(O,U)$ (T-42a). (ii) Другие пары сектора $\bar 3$ — лишь $(L,E)$ и $(L,U)$; ни одна не входит в $\kappa_0$ ($\mathrm{Hom}(O,L)$ в нём отсутствует), потому ни одна не получает вакуумного среднего. (iii) По инцидентности ($\lambda=1$) пара $(L,U)$ лежит на единственной прямой $\{D,L,U\}$, уже занятой как юкавский канал Color-U второго поколения ([правила отбора](/docs/physics/gauge-symmetry/fano-selection-rules)) — это канал масс, не скалярный сектор. $\blacksquare$

**Фальсификация.** Открытие заряженного Хиггса $H^\pm$ или второго нейтрального скаляра дублетного типа на LHC/HL-LHC опровергло бы категориальную единственность $(E,U)$ — то есть ударило бы по самой $\kappa_0$, а не по периферийной подгонке. УГМ ставит на это весь класс хиггс-секторов 2HDM/MSSM.
...
### 6.4 Динамическая тёмная энергия [Т на O-канале; П для не-O-остатка]

Дрейф уравнения состояния тёмной энергии теперь **выведен** на уровне состояния: $1 + w_{\text{eff}} = -\tfrac{2}{3}\,d\ln\mathcal{G}_O/d\ln a$ с положительным полом и трёхветвевой классификацией форм — [закон дрейфа Λ, T-254/T-255](/docs/physics/gravity/cosmological-constant#теорема-лямбда-дрейф). Рассматриваемые здесь хиггсовские каналы ($\bar{3}$-to-$\bar{3}$ — не-O) вносят вклад в этот дрейф лишь через секторно-подавленную поправку $O(\mathcal{G}_{\text{non-O}}/\mathcal{G}_O) \sim 10^{-3}$ ([секторная Gap-граница](/docs/physics/cosmology-phys/berry-phase#теорема-секторная-gap-граница) [Т]) — субдоминантный канал. Прежний анзац сохранён для протокола:
...
Открытым **[П]** остаётся мост RG-масштаб ↔ $H(t)$ для *не-O* каналов; O-канальному дрейфу такой мост не нужен — он идёт напрямую через M3-отождествление $a = 1/\mathrm{Gap}_s$. Старый численный анзац $w_a \sim -10^{-2}$ вытеснен ограничениями форм T-255 (DESI-квадрант требует осцилляторной ветви с настоящим пересечением $-1$).

---

### 6.5 Скорость хирального туннелирования [Т] {#скорость-хирального-туннелирования}

:::tip Теорема T-185b [Т]: Предсказание стабильности хиральности
Хиральный вакуум стабилен относительно туннелирования с временем жизни, многократно превышающим возраст Вселенной:

$$\tau_{\text{chiral}} \sim \frac{1}{\mu} \exp\!\left(\frac{B}{\hbar}\right) \gg \tau_{\text{universe}} \approx 4{,}4 \times 10^{17}\;\text{с}$$

где $B \geq \pi\sqrt{12}\,\mu \approx 10{,}88\,\mu$ — баунс-действие WKB через барьер $\Delta V \geq 6\mu^2$ (T-69 [Т]).
:::

**Вывод.** WKB-скорость туннелирования между хиральными вакуумами $\theta = 0$ и $\theta = \pi$:
...
В физических единицах при $\mu \sim M_{\text{Planck}}$: экспонента $e^{10{,}88 \cdot M_{\text{Planck}} / T_{\text{eff}}}$ астрономически велика для любого $T_{\text{eff}} \ll M_{\text{Planck}}$.

**Фальсифицируемое предсказание.** Наблюдение спонтанного переключения хиральности (правый нейтрино из левого без массовой вставки) при энергии ниже планковской фальсифицирует теорему топологической защиты T-69 [Т] и кубический потенциал $V_3$ (T-99 [Т]).

**Статус.** [Т] — следует из T-69 [Т] (топологический барьер), T-64 [Т] (единственный вакуум), T-99 [Т] ($V_3$ — единственный $PT$-нечётный член).
...
Экспериментальное значение: $M_H^{\text{exp}} = 125.20 \pm 0.11$ ГэВ (PDG 2024). В Стандартной модели $M_H$ — свободный параметр. В некоммутативной геометрии (NCG) Шамседдина–Конна масса Хиггса **вычисляется** из спектральной тройки. Вопрос: может ли УГМ сделать то же самое?
...
Неподвижная точка Вильсона–Фишера Gap-теории $g_4^* = 4\pi^2/63 \approx 0.627$ **не** является хиггсовской квартикой $\lambda_H$ Стандартной модели. Наивное отождествление даёт $M_H = \sqrt{2 g_4^*} \cdot v \approx 275$ ГэВ — **неверный** результат (напечатанные в прежних черновиках «$0.063$»/«$87$ ГэВ» ошибочно вычисляли $4\pi^2/63$ с множителем 10; корректное значение $0.627$ всё равно не воспроизводит $M_H=125$ ГэВ, так что вывод «$g_4^*\neq\lambda_H$» остаётся в силе).
:::
...
| ИК-значение | $4\pi^2/63 \approx 0.627$ | $\approx 0.13$ (из $M_H = 125$ ГэВ) |
...
### 7.4 Сравнение с NCG Шамседдина–Конна {#сравнение-ncg}

В подходе Шамседдина–Конна–Марколли (CCM) история предсказания $M_H$ прошла три этапа:
...
Однако без RG-коррекции точная формула Шамседдина–Конна (2012) давала $\sim 170$ ГэВ — **неверный** результат.
...
**(c) Со скалярным полем $\sigma$ (Шамседдин–Конн-ван Сёйлеком 2013)**: введение $\sigma$-поля из внутренних флуктуаций изменяет граничное условие при $\Lambda$, приводя к $M_H \approx 126$ ГэВ — первое корректное предсказание из NCG.
...
- **Наивное $g_4^* \to M_H$**: неверно ($g_4^* \neq \lambda_H$), даёт $\sim 275$ ГэВ.
- **Статус**: **[С]** — условно на вычислении $\varepsilon_i$.
- **Сравнение с NCG**: УГМ воспроизводит структуру CCM, но добавляет октонионную $V_3$-коррекцию, аналогичную $\sigma$-полю Шамседдина–Конна-ван Сёйлека.
