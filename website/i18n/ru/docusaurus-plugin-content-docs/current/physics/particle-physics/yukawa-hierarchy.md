...
:::tip [Т] $y_t$ — единственная $O(1)$ Юкава; [С] её точное значение
Правило отбора Фано делает $y_t$ **единственной** $O(1)$ Юкавской связью — это и есть подлинное [Т]-содержание (пункт (a)). Наблюдаемое $m_t\approx173$ ГэВ отвечает $y_t(m_t)\approx0.94$: $O(1)$, но **не** закреплено в инфракрасной квазификсированной точке Пендлтона–Росса (которая даёт перелёт — см. (b)). Статус: «ровно одна $O(1)$ Юкава» **[Т]**; численное $m_t\approx173$ ГэВ **[С]** (совместимо, но не предсказано фиксированной точкой).
:::
...
**(b) Инфракрасная квазификсированная точка — честно.** Инфракрасная квазификсированная точка Пендлтона–Росса / Хилла для одной $O(1)$ топ-Юкавы вычисляется (при $c_1=9/2$, $c_3=8$, $c_4=9/4$, $g_s^2(\mu_\text{EW})\approx1.48$, $g_W^2\approx0.42$) как

$$y_t^{(\text{FP})} = \sqrt{\frac{c_3 g_s^2(\mu_\text{EW}) + c_4 g_W^2}{c_1}} \approx \sqrt{\frac{8\cdot1.48 + 2.25\cdot0.42}{4.5}} \approx 1.7,$$

т.е. $m_t^{\text{FP}} = y_t^{\text{FP}}\,v/\sqrt2 \approx 210\text{–}230$ ГэВ — известный **перелёт** квазификсированной точки СМ. Физический топ, $y_t(m_t)\approx0.94$ ($m_t\approx173$ ГэВ), лежит **ниже** этого аттрактора: он $O(1)$, но не в самой фиксированной точке. Поэтому УГМ предсказывает *качественный* факт «ровно одна $O(1)$ Юкава» **[Т]**; точное $m_t$ — граничное условие ниже FP, статус **[С]**.
...
**Статус.** Результаты, зависящие от петлевых поправок с $\lambda_3$, **повышены** с [Г] до **[С при числовых коэффициентах]**: качественная иерархия масс и структура смешивания — **[Т]** (Фано-комбинаторика); точные числовые значения — **[С]** (зависят от порядка теории возмущений и конкретного значения $\varepsilon_{\text{eff}}$).
...
Замкнутая аналитическая форма (испр. 10.08.2026 по прибору E26)
$$\varepsilon_\mathrm{eff}=\frac{4\,|\bar\gamma|_\mathrm{сект}}{9\,(1+\Sigma_0/4)}$$
выведена из символической минимизации $V_\mathrm{Gap}$ плюс леммы Шура в [T-216](/docs/proofs/categorical/fundamental-closures#t-216); формула (c) ниже — **уравнение самосогласования** для $\varepsilon_{33}$ (её $|\bar\gamma|$ приходит из константы $\lambda_3 = 2\mu^2/(3|\bar\gamma|)$ и само зависит от $\varepsilon_{33}$), а не замкнутое значение — чтение её как значения и породило прежний «разрыв двух порядков». Решение самосогласования (E26, без подгоночных параметров) даёт $\varepsilon_\mathrm{eff} = 0{,}0569$ против петлевого $0{,}0587$ — $3\%$; значение остаётся [С] лишь по оговорке секторного анзаца.
:::
...
:::warning Символическая форма пока не воспроизводит феноменологическое число
Подстановка $N_{33}^{(\mathrm{Fano})} = 2$ и $|\bar{\gamma}| \approx 0.15$ в числитель и главный член знаменателя даёт $8/(9 \cdot 0.15) = 5{,}93$ — то есть $O(1)$, примерно **на два порядка выше** феноменологического $\varepsilon_{\mathrm{eff}} \approx 0.059$. Прежние редакции этой страницы печатали цепочку так, будто она вычисляется в $0.059$; она не вычисляется, и расхождение составляет множитель $100$.

Число $0.059$ приходит **другим** маршрутом — петлевой оценкой $\varepsilon_{\mathrm{eff}} = \lambda_3\varepsilon/(4\pi) \approx 74 \times 0.01 / 12.6 = 0{,}0587$ из §[Эффективный параметр](#функциональная-зависимость-ε) — и является феноменологическим.

Установленное поэтому распадается надвое. **Структурное** выражение выше имеет статус [Т]: оно следует из символической минимизации $V_{\mathrm{Gap}}$ и леммы Шура, а $N_{33}^{(\mathrm{Fano})} = 2$ — комбинаторный факт о плоскости Фано. **Численное** значение имеет статус [С при T-64]: закрытие разрыва в два порядка требует либо полной минимизации на $(S^1)^{21}/G_2$ с истинным $\Sigma_0/(2\mathcal{G}^{(0)}_{\mathrm{total}})$ (которое должно было бы дать подавление в $\sim 100$ раз), либо поправки к канонической подстановке. Вопрос открыт.

**Разрешено 10.08.2026 (прибор E26).** «Разрыв двух порядков» был артефактом чтения (c) — *уравнения самосогласования*, в котором $|\bar\gamma|$ само есть функция $\varepsilon_{33}$, — как замкнутого значения, и двойного счёта $N_{33}$ (он уже сидит внутри самосогласования; повторное умножение завышает вдвое). Решение самосогласованной минимизации с нуля с константами теоремы 13.5, амплитуды свободны в границах Коши–Шварца: $\varepsilon^*_{33} = 0{,}1314$, амплитудная сумма $\Sigma_0 = 0{,}1035$, $r_4 = 1/2$ (тождество, см. [T-216](/docs/proofs/categorical/fundamental-closures#t-216)), что даёт $\varepsilon_\mathrm{eff} = \frac{4}{9} \cdot 0{,}1314 / 1{,}0259 = 0{,}0569$ против петлевого $0{,}0587$ — согласие $3\%$; конфайнмент и электрослабое подавление ($\varepsilon_{3\bar 3}, \varepsilon_{\bar 3\bar 3} \to 0$) воспроизведены минимизатором, а не навязаны. Прогон по 21 амплитуде вне $SU(3)$-анзаца (волна 2) её подтвердил: секторная селекция точна, $\varepsilon_\mathrm{eff} = 0{,}0549$ ($\times 0{,}93$) — оговорка снята.
:::
...
$$\boxed{\varepsilon_{\mathrm{eff}} = \frac{4\,|\bar{\gamma}|_{\mathrm{сект}}}{9\,(1 + \Sigma_0/4)}}$$

*(Испр. 10.08.2026 по E26 — см. [§9](#9-аналитическая-формула-ε): $|\bar\gamma|$ в числителе, $N_{33}$ один раз внутри самосогласования, $\Sigma_0$ — амплитудная сумма, $r_4 \equiv 1/2$; самосогласованное вычисление $0{,}0569$/$0{,}0549$ против петлевых $0{,}0587$.)*
