...
**(d)** Правило отбора Фано помещает $\nu_\tau$ на **ту же линию $k=1$**, что и топ-кварк, поэтому его дираковская Юкава — **древесная, $O(1)$** (а не петлево-подавленная $y_\nu\sim y_\tau\sim 0.01$ из прежних черновиков). При $y_{\nu_3}\sim 0.7$, $m_D = y_{\nu_3}\,v/\sqrt2 \approx 122$ ГэВ и $M_R \approx 2.9\times10^{14}$ ГэВ:

$$m_{\nu_3} = \frac{m_D^2}{M_R} = \frac{(122\;\text{ГэВ})^2}{2.9\times10^{14}\;\text{ГэВ}} \approx 5\times10^{-11}\;\text{ГэВ} = 5\times10^{-2}\;\text{эВ}$$

— совпадает с атмосферным масштабом $\sqrt{\Delta m^2_{32}} \approx 0.05$ эВ.

:::warning Исправление перевода единиц
Наивная оценка $y_\nu\sim 0.01$ дала бы $m_\nu\sim 6\times10^{-14}$ ГэВ $=6\times10^{-5}$ эВ (**с множителем ГэВ→эВ $10^9$**), т.е. в $\sim1000$ раз меньше данных — малое $y_\nu$ их не воспроизводит. Физическое разрешение — древесная $O(1)$ Юкава Фано выше (ν_τ делит линию $k=1$ с топом), дающая $0.05$ эВ напрямую. *Отношение* $m_2/m_3$ (из $\sin^2$ углов Фано) корректно и не затронуто. Потабличные массы в §4 используют тот же множитель $10^9$ и древесную $O(1)$ Юкаву.
:::
...
#### Статус не-экстраполяции вывода $M_R$ {#mr-non-extrapolation}

Легитимное замечание внешней критики: предсказание $M_R \approx 2.9 \times 10^{14}$ ГэВ может «балансировать на грани спекулятивной экстраполяции низкоэнергетического функционала $V_{\mathrm{Gap}}$» с EW-масштаба до промежуточного. Этот раздел уточняет, что вывод **не** является экстраполяцией низкоэнергетической EFT, а прямой структурной вычислительной задачей.

**Структура вывода**:

1. **Входы**: параметр иерархии секторов $\varepsilon \approx 10^{-3}$ из T-64 [Т] (единственный минимум $V_{\mathrm{Gap}}$ на компактном $(S^1)^{21}/G_2$), Gap-общее $\mathcal G^{(O)}_{\mathrm{total}} \sim 6$ на $O$-секторе (Аксиома A5 Page–Wootters), фундаментальный масштаб $\omega_0 \cdot M_P$ из T-39a [Т].

2. **Промежуточное**: $G_2$-extra-boson масса $M_{G_2}^{(\mathrm{extra})} = \omega_0 \cdot \sqrt{\mathcal G^{(O)}_{\mathrm{total}}} \sim 10^{17}$ ГэВ — выводится на планковском масштабе из внутренней спектральной структуры, не экстраполируется с низкой энергии.

3. **Петлевое подавление**: физическое $M_R$ возникает из петли двух $G_2$-extra-бозонов:
$$
M_R = \frac{g_{G_2}^4}{16\pi^2} \cdot M_{G_2}^{(\mathrm{extra})} \sim \frac{(0.7)^4}{158} \cdot 10^{17}\,\mathrm{ГэВ} \approx 2.9 \times 10^{14}\,\mathrm{ГэВ}.
$$

Петлевой фактор — стандартное одно-петлевое квантово-полевое вычисление с $G_2$-инвариантными связями, не RG-flow от EW к $M_R$.

**Ключевой момент**: вывод использует **только** планковские величины ($\omega_0 M_P$, $\varepsilon$, $g_{G_2}$, $\mathcal G^{(O)}_{\mathrm{total}}$) вместе с конечным петлевым фактором. Ни один низкоэнергетический EFT-параметр не экстраполируется на многие порядки. Результат $M_R \sim 10^{14}$ ГэВ — **структурное предсказание** внутренней спектральной тройки УГМ, не фит к наблюдаемым нейтринным массам.

:::tip Закрытие T6: $M_R$ структурно выведен \[Т при T-64\]
$M_R \approx 2.9 \times 10^{14}$ ГэВ выведен из UHM спектральной тройки через T-64 (единственный вакуум) + A5 (Page–Wootters) + T-39a (фундаментальное $\omega_0$) + стандартная одно-петлевая $G_2$-инвариантная КТП. Это **не** экстраполяция низкоэнергетического эффективного действия; это прямое вычисление на планковском масштабе.

Опасение внешнего аудита об «спекулятивной экстраполяции $V_{\mathrm{Gap}}$» смешивает два использования Gap-функционала: (i) минимизация $V_{\mathrm{Gap}}$, дающая $\varepsilon$ при T-64 (вычислительная задача на компактном $(S^1)^{21}/G_2$, не EFT-экстраполяция); (ii) одно-петлевой обмен $G_2$-extra-boson, дающий петлевой фактор (стандартная КТП, не экстраполяция). Оба строго. Никакой экстраполяции не производится.

Эмпирическая проверка: see-saw формула $m_\nu = m_D^2 / M_R$ с выведенным $M_R$ и **древесной $O(1)$ Юкавой Фано** даёт $m_\nu \sim 0.05$ эВ, согласно $\sqrt{\Delta m^2_{32}} \approx 0.05$ эВ из осцилляционных данных (см. исправление единиц в (d) — прежнее «0.06 эВ» было следствием пропуска множителя $10^9$). Это **проверка согласованности**, не источник предсказания $M_R$.
:::
...
:::warning Исправление: $m_D\sim m_l$ seesaw ошибается на порядки — используйте древесную Юкаву Фано
Наивный $m_D\sim m_l$ (заряженно-лептонный) seesaw даёт, с правильным множителем ГэВ→эВ $10^9$:
$m_{\nu_\tau}=m_\tau^2/M_R=(1.78)^2/10^{14}\,\text{ГэВ}=3.2\times10^{-14}\,\text{ГэВ}=3.2\times10^{-5}$ эВ;
$m_{\nu_\mu}=1.1\times10^{-7}$ эВ; $m_{\nu_e}=2.6\times10^{-15}$ эВ — **в $\sim10^{3}$–$10^{12}$ раз меньше** (в прежних черновиках они были ошибочно обозначены как $0.03/0.009/0.003$ эВ через упущенный $10^9$ и необоснованные «уточнения»). Физические массы происходят от **древесной $O(1)$** Юкавы $\nu_\tau$ (§2(d)); отношения — от Фано-углов (§5).
:::

**(a)** Третье поколение ($\tau$-нейтрино) — из древесной $O(1)$ Юкавы Фано (§2(d), $m_D\approx122$ ГэВ):

$$
m_{\nu_3} \approx \frac{m_D^2}{M_R} = \frac{(122\;\text{ГэВ})^2}{2.9\times10^{14}\;\text{ГэВ}} \approx 0.05 \; \text{эВ}
$$

**(b)** Отношения из Фано-углов (§5): $m_{\nu_2}/m_{\nu_3} = (\sin(\pi/7)/\sin(2\pi/7))^2 \approx 0.308$ на древесном уровне, снижается до $\approx0.17$ двухпетлевой RG (в согласии с наблюдаемым $\sqrt{\Delta m^2_{21}}/\sqrt{\Delta m^2_{32}}\approx0.17$), что даёт $m_{\nu_2} \approx 0.0086$ эВ.

**(c)** Первое поколение: $m_{\nu_1}$ Фано-подавлено относительно $m_{\nu_2}$, что даёт $m_{\nu_1} \lesssim 0.004$ эВ (нормальная иерархия, $m_1\ll m_2$).

**(d)** **Иерархия: нормальная** ($m_1 < m_2 < m_3$):

$$
m_{\nu_1} \lesssim 0.004\;\text{эВ}, \quad m_{\nu_2} \approx 0.0086 \; \text{эВ}, \quad m_{\nu_3} \approx 0.05 \; \text{эВ}
$$

в согласии с осцилляционными данными $\sqrt{\Delta m^2_{21}}\approx0.0086$ эВ, $\sqrt{\Delta m^2_{32}}\approx0.050$ эВ.
::::

### 3.2 Сравнение с экспериментом

:::warning Оценки порядка величины, а не точные предсказания
Значения масс нейтрино ($\lesssim0.004$, $\approx0.0086$, $\approx0.05$ эВ) происходят от **древесной $O(1)$** Юкавы $\nu_\tau$ (§2(d)) плюс Фано-угловых отношений (§5), а **не** от наивного $m_D\sim m_l$ seesaw (который ошибается на $10^3$–$10^{12}$ — см. блок исправления выше). Сам seesaw-механизм — стандартный результат; оригинальный вклад УГМ — существование $\nu_R$ как Gap-конфигурации [Т], $O(1)$ Юкава $\nu_\tau$ на $k=1$ Фано-линии [Т] и качественное объяснение больших PMNS-углов [Г].
:::
...
|----------|---------------------|-------------------|--------|
| $\sqrt{\Delta m^2_{32}}$ | $\approx 0.050$ эВ | $m_{\nu_3} \approx 0.05$ эВ | Согласие (древесная $O(1)$ Юкава Фано) |
| $\sqrt{\Delta m^2_{21}}$ | $\approx 0.0086$ эВ | $m_{\nu_2} \approx 0.0086$ эВ | Согласие (Фано-отношение + 2-петли) |
| Иерархия | Предпочтение нормальной (>2$\sigma$) | Нормальная | Согласие |
| $\sum m_\nu$ | $< 0.072$ эВ (DESI+CMB 2024) | $\sim 0.042$ эВ | Совместимо |
...
| Масса $\nu_e$ | (+ Фано-поправки) | $\sim 0.003$ эВ | $< 0.45$ эВ (KATRIN 2024, прямое) | Совместимо |
...
| $\sum m_\nu$ | Суммирование | $\sim 0.042$ эВ | $< 0.072$ эВ (DESI+CMB 2024) | Совместимо |
