...
Поскольку $G_2 = \mathrm{Aut}(\mathbb{O})$ сохраняет октонионное умножение, она сохраняет структурные константы $f_{ijk}$ как **тензор** (ассоциативную 3-форму $\varphi$). Она **не** переставляет семь координатных линий Фано — это делает лишь конечная реперная подгруппа $\Gamma_{\!\text{oct}}\cong PSL(2,7)\subset G_2$; общий $g\in G_2$ вращает координатные оси (неприводимость $\mathbf 7$):

$$g \in G_2 \ \Rightarrow\ g^\ast\varphi = \varphi; \qquad g \in \Gamma_{\!\text{oct}} \ \Rightarrow\ g \text{ переставляет Фано-линии}.$$
...
$G_2$-симметрия — это **непрерывная симметрия кинематики** УГМ, которая **спонтанно нарушается** при динамической фиксации вакуума. До минимизации $V_{\text{Gap}}$: преобразования из $G_2$ переименовывают базис $\{A, S, D, L, E, U, O\}$, сохраняя октонионную структуру и все $G_2$-инварианты ($P$, $R$, $\Phi$, спектр $\Gamma$). **После** минимизации (T-64 [Т]): конкретный вакуум $\Gamma_{\text{vac}}$ фиксируется, нарушая $G_2 \to H$ (стабилизатор вакуума). Булев фрагмент $\mathrm{Dec}(\Omega) \cong 2^7$ кристаллизуется как pointer basis, выбранный спонтанным нарушением симметрии — аналогично хиггсовскому механизму $SU(2) \times U(1) \to U(1)_{\text{em}}$. [Мо́ды Голдстоуна](/docs/applied/coherence-cybernetics/goldstone-modes) — безмассовые возбуждения вдоль нарушенных направлений $G_2/H$.
...
### Теорема 11.2 (Группа ковариантности Фано-диссипатора)

:::tip[Статус: Теорема \[Т\]]
По Фано–атомной пропорциональности $\mathcal{D}_{\mathrm{Fano}} = \tfrac23\mathcal{D}_{\mathrm{atom}}$ Фано-диссипатор ковариантен относительно конечной октонионной реперной группы $\Gamma_{\!\text{oct}}=\mathrm{Aut}(PG(2,2))\cong PSL(2,7)\subset G_2$, и **не** относительно полной непрерывной $G_2$. Канонический вполне $G_2$-ковариантный диссипатор — это $\mathcal{D}_{G_2}$ (построение на структурных константах, $(A_a)_{bc}=\varphi_{abc}/\sqrt6$). Полное изложение: [Фано-канал §5](/docs/proofs/gap/fano-channel#g2-ковариантность).
:::
...
**(a)** Для $g\in\Gamma_{\!\text{oct}}$ элемент $g$ переставляет семь координатных линий Фано: существует перестановка $\sigma_g$ на $\{1,\ldots,7\}$ такая, что

$$g\, \Pi_p\, g^\dagger = \Pi_{\sigma_g(p)}.$$

Общий $g\in G_2$ этого **не** делает: $\mathbb{C}^7$ — неприводимый $G_2$-модуль (Шур), поэтому никакое координатное 3-подпространство $\mathrm{span}(\text{линия }p)$ не $G_2$-инвариантно. Значит, ковариантность ниже точна лишь для конечной подгруппы $\Gamma_{\!\text{oct}}$.
...
- Фано-диссипатор $\mathcal{D}_{\text{Fano}} = \tfrac23\mathcal{D}_{\text{atom}}$: ковариантен относительно реперной группы $\Gamma_{\!\text{oct}}$, **не** полной $G_2$ **[Т]** (T-11.2)
- Атомарный диссипатор $\mathcal{D}_{\text{atom}}$: **НЕ** $G_2$-ковариантен **[Т]** (T-11.1)
- Канонический $G_2$-ковариантный диссипатор $\mathcal{D}_{G_2}$ (структурные константы $\varphi_{abc}$) **[Т]**
