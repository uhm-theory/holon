...
Наблюдаемое: $|\delta_{\mathrm{CP}}| = 65.7° \pm 1.5°$ (глобальный фит PDG 2024); $64.6° \pm 2.8°$ (древесная комбинация LHCb, ICHEP 2024). Сырое расхождение неисправленного $77.1°$ составляет $\sim 12°$ (устраняется двухпетлевой поправкой ниже). Прежнее $69° \pm 4°$ устарело.
...
**(d)** При положительном знаке: $77.1° + 12.6° = 89.7°$ — расхождение $> 8\sigma$ от прямого значения. Таким образом, новое назначение **предсказывает отрицательный знак** двухпетлевой поправки.
...
:::tip[Расхождение разрешено — [Т]]
Расхождение $m_b/m_t$ полностью разрешено: $y_b^{(\mathrm{tree})} = 0$ (Фано-правило отбора [Т]), 1-loop через секторное $\varepsilon_{33}^*(\theta^*)$ с $r_{33} \approx 0.25$ + QCD-IR усиление $\eta_{\text{QCD}} \approx 3.46$ даёт $y_b \approx 0.024$ — точное согласие. Механизм **[Т]**; прецизионное числовое предсказание — вычислительная задача (T-79).
:::
...
Полный вывод: [Секторная RG для $m_b/m_t$](/docs/physics/particle-physics/yukawa-hierarchy#теорема-mb-mt).
...
| $\delta_{\mathrm{CP}} \approx 64.5°$ (с новым назначением) | [Г] ($\approx 0.04\sigma$ от прямого $64.6° \pm 2.8°$, эвристическая формула) | 9.3 |
| Вольфенштейновские параметры и инвариант Ярлского | [Г] (из текстуры Фрича + наблюдаемые массы) | 9.4 |
| $m_b/m_t \approx 0.024$ из секторной RG | [Т] (секторное $\varepsilon_{33}^*(\theta^*)$, $r_{33} \approx 0.25$ + QCD-IR усиление — точное согласие) | 12.4 |
...
3. **Отношение $m_b/m_t$** — **решено [Т]**: секторное $\varepsilon_{33}^*(\theta^*)$ с $r_{33} \approx 0.25$ + QCD-IR усиление даёт $y_b \approx 0.024$ — точное согласие.
