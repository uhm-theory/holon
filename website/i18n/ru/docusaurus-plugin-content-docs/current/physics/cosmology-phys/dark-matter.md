...
где $c_{ij}$ — коэффициенты, определяемые из условия аномалии $\partial_\mu j^\mu_A = \frac{g^2}{32\pi^2} G\tilde{G}$. В Gap-вакууме $\theta_{\mathrm{QCD}} = 0$ **точно** ([T-99 \[Т\]](/docs/physics/gauge-symmetry/confinement#теорема-структурное-theta-qcd)); аксион $a$ описывает флуктуации $a \propto \delta\theta$.
...
:::info Повышение статуса порядковой оценки [С при T-50, CKR]
Порядок величины $\Omega_X h^2 \sim 0.1$–$0.4$ имеет статус **[С при T-50, CKR стандартная космология]**:
...
Точный численный коэффициент имеет неопределённость $\times 2$–$3$ (из CKR). Для стабильности требуется $O$-чётность (см. [§5.3](#53-o-чётность)).
:::
...
**Шаг 1 (Стабилизатор).** $\mathrm{Stab}_{G_2}(e_O) = SU(3)$ [Т] ([T-42e](/docs/proofs/categorical/uniqueness-theorem)). Следовательно, $O$-сектор обладает выделенной $SU(3)$-инвариантной структурой.

**Шаг 2 ($\mathbb{Z}_2$-симметрия из вещественности).** Комплексное сопряжение $\sigma: \gamma_{Oi} \mapsto \bar{\gamma}_{Oi}$ является $\mathbb{Z}_2$-симметрией потенциала $V_{\mathrm{Gap}}$, поскольку структурные константы $f_{ijk} \in \mathbb{R}$ ([T-99 \[Т\]](/docs/physics/gauge-symmetry/confinement#теорема-структурное-theta-qcd), шаг 1).
...
**Шаг 5 (Топологический барьер).** [T-69 \[Т\]](/docs/core/dynamics/composite-systems#теорема-тополог-защита): $\Delta V \geq 6\mu^2 > 0$ предотвращает $O$-чётность-нарушающее туннелирование. $\blacksquare$
...
:::tip Полная декомпозиция $\Omega_{\mathrm{DM}}$ [С при T-50, CKR]
...
| $\Omega_X h^2$ | $\sim 0.1$--$0.4$ [С при T-50, CKR] | [§5.4 выше](#54-детали-ckr-производства-o-реликта) |
...
- [Космологический генезис](/docs/physics/cosmology-phys/origin)
- [Фазовая диаграмма](/docs/applied/coherence-cybernetics/phase-diagram-cc)
