...
| 5 | **Топологическая** | $\pi_2(G_2/T^2) \cong \mathbb{Z}^2$: невырожденный Gap нестягиваем |
...
## 5. Секторная Gap-граница {#секторная-gap-граница}
...
:::tip Теорема 7.2 [Т]
Среднеполевые показатели **точны** в УГМ по трём независимым механизмам (см. [Механизм точности](/docs/consciousness/hierarchy/swallowtail-transitions#механизм-точности) для полного аргумента): (i) топологическая жёсткость Тома–Арнольда катастрофы $A_2$ обеспечивает критические показатели как топологические инварианты класса катастрофы; (ii) УГМ-динамика детерминирована (Линдблад + регенерация), а не термодинамически стохастична, так что Ландау-потенциал — это *эффективный потенциал детерминированного потока*, и его седловые точки — фактические аттракторы — никакая Гинзбург-флуктуационная поправка неприменима; (iii) размерность параметра порядка $d_{\mathrm{eff}} = \binom{7}{2} = 21$ даёт количественную $1/N$ перекрёстную проверку с флуктуационными поправками $\approx 5\%$, в пределах экспериментального разрешения PCI.
:::
...
### 9.2 Топологический член из Im($S_{\text{Keldysh}}$) [Т] {#теорема-l-top-кельдыш}
...
**Шаг 1 (Келдышевское действие — комплексная структура).** Из [вывода лагранжиана из линдбладиана](/docs/core/dynamics/gap-thermodynamics#полный-лагранжиан) [Т]:
...
**Статус:** **[Т]**. Топологический лагранжиан выводится из мнимой части келдышевского действия ([теорема L_top из Келдыша](#теорема-l-top-кельдыш) [Т]), а единственность формы следует из $S_3$-аргумента ниже + единственности $\mathrm{Im}(S_K)$.
:::
...
- [Топологическая защита когерентности](/docs/applied/coherence-cybernetics/topological-protection)
