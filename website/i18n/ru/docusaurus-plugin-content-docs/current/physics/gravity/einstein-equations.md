...
Вывод уравнений Эйнштейна из Gap-действия через спектральное действие Шамседдина–Конна. Читатель узнает, почему гравитация эмерджентна в УГМ.
:::


## Обзор

Центральный результат гравитационного сектора УГМ: **уравнения Эйнштейна выводятся** из Gap-действия через спектральное действие Шамседдина–Конна [Т]. Полная спектральная тройка из [T-53 [Т]](/docs/core/foundations/spacetime#теорема-спектральная-тройка) воспроизводит действие Эйнштейна–Гильберта + Стандартную модель. Дополнительный аргумент — теорема Лавлока. Гравитация **не является** фундаментальным взаимодействием — она эмерджирует из Gap-кривизны.
...
- Линеаризация $h_{\mu\nu} \sim |\gamma_{ij}|^2 \sin^2(\theta_{ij})$: **[С при слабом поле]** (справедливо в слабополевом приближении, полная нелинейная связь — открытая задача)
:::

:::warning Происхождение $\eta_{\mu\nu}$: решено [Т]
Формула $h_{\mu\nu} = \sum |\gamma_{ij}|^2 \cdot \mathrm{Gap}^2$ даёт только неотрицательные компоненты. **Выделенное направление времени** ($g_{00}>0$) **выведено** [Т] из механизма Пейджа–Вуттерса. **Полная лоренцева сигнатура** $(+1,-1,-1,-1)$ — **[Т при рефлективной положительности]**: лоренцев относительный знак ($g_{aa}<0$) следует из требования ограниченности снизу PW-генератора (рефлективная положительность Остервальдера–Шрадера), а не из произвольного анзаца — KO-dim 6 конечной спектральной тройки [T-53](/docs/core/foundations/spacetime#лоренцева-сигнатура) фиксирует *чётную/нечётную градуировку*, но не сигнатуру метрики (строгая реализация — через крин-спектральные тройки). Соглашение $(+1,-1,-1,-1)$ (east-coast) используется последовательно.
:::
...
**(b)** Гравитационная постоянная (согласована с вывода Фирца–Паули §(b) ниже, $G\propto\mu^2$):

$$
G_{\mathrm{Gap}} = \frac{4\pi\mu^2}{\langle|\gamma_{\mathrm{ST}}|^2\rangle}
$$

где $\langle|\gamma_{\mathrm{ST}}|^2\rangle = \frac{1}{6}\sum_{\substack{i,j \in \mathrm{ST} \\ i < j}} |\gamma_{ij}|^2 \sim \varepsilon_0^2$ — средний квадрат модуля когерентности в пространственно-временном секторе (6 пар из 4 направлений).

:::warning Единая ньютоновская постоянная: $G\propto\mu^2$, и $\mu$ планковского масштаба
Линеаризованный (Фирца–Паули) вывод ниже даёт $G_N^{(ST)}=4\pi\mu^2/\varepsilon_0^2$ ($G\propto\mu^2$), согласуясь с формой выше. **Первичная** ньютоновская постоянная — результат спектрального действия $G_N = 3\pi/(7 f_2\Lambda^2)$ с обрезанием $\Lambda\sim M_P$ (§ниже, T-65 [Т]); формула эмерджентной гравитации через $\mu$ — эффективное соотношение, в котором $\mu$ есть **планковский** Gap-параметр ($\mu_{\text{phys}}=\mu\,\omega_0\sim M_P$), а **не** масштаб $\sim$мэВ, используемый в $\Lambda$-секторе. Обе тогда согласны, что $G_N^{-1/2}\sim M_P$.
:::
...
**Статус [Т]:** Полная спектральная тройка $(A, H, D)$ из [T-53 [Т]](/docs/core/foundations/spacetime#теорема-спектральная-тройка) удовлетворяет аксиомам Конна. Спектральное действие Шамседдина–Конна $S = \mathrm{Tr}(f(D_A/\Lambda))$ воспроизводит действие Эйнштейна–Гильберта с $G_N = 3\pi/(7 f_2\Lambda^2)$ [Т]. Дополнительный аргумент: теорема Лавлока (применимость к эмерджентной метрике — [С при T-120], см. [анализ ограничений](#34-ограничения-аргумента-лавлока) ниже). T-120 [Т] выводит $M^4$ как гладкое 4-многообразие; теорема Лавлока требует гладкое 4D многообразие + диффеоинвариантность + метрический тензор — все условия выполнены эмерджентным $M^4$.
...
| 10 | UV-конечность | NCG-замкнуто | $G_2$-Ward + $\mathcal N=1$ SUSY + APS | **Сильнее** (по полевому пространству [Т], попорядково [С] — T-66) |
...
| 13 | Сознание | Не рассматривается спектральным действием | E-секторная феноменология: $\mathrm{Coh}_E > 1/7$ (No-Zombie, T-8.1 [Т]), иерархия интериорности L0–L4, гедонистическая валентность $V_{\mathrm{hed}} = dP/d\tau$ (T-103 [Т]+[И]), 23 фальсифицируемых предсказания | **Расширение только в УГМ** |
...
Связь $G \propto \mu^2/|\gamma_{\mathrm{space}}|^2$ означает, что гравитация тем **сильнее**, чем больше масштаб Gap-моды $\mu$ и чем **ниже** когерентность пространственного сектора. В пределе полной декогеренции ($|\gamma_{\mathrm{space}}| \to 0$) гравитационная постоянная формально расходится — эффективное «усиление гравитации» вблизи сингулярностей.

### 3.2a Информационно-гравитационная взаимность: $G_N$ как обратная обучаемость (T-264) {#взаимность-обучаемость-гравитация}

Gap-фазы $\theta_{\mu\nu}$ несут обе нагрузки теории сразу: их кинетика — эмерджентный гравитон (§2.1, мостовая лемма Фирца–Паули), а их *оцениваемость* — квантовая информация Фишера, та самая величина, что лимитирует обучение (Браунштейн–Кейвс; [T-263](/docs/core/dynamics/evolution#теорема-наилучший-обучающий-поток)). Два прочтения — один функционал от $\Gamma$.

#### Теорема T-264 (информационно-гравитационная взаимность) [Т при FP-лемме, ведущий порядок] {#теорема-информационно-гравитационная-взаимность}

:::tip Теорема
**(a) Точная парная лемма [Т].** Для состояния с развязанным $(i,j)$-блоком (произвольные населённости $\Gamma_{ii}, \Gamma_{jj}$; когерентность $\gamma_{ij}$ вплоть до границы PSD) квантовая информация Фишера фазового направления $X_{ij} = \partial\Gamma/\partial\theta_{ij} = i[\Pi_i, \Gamma]$ (касательная паспортного заряда, T-260) **точно** равна
...
независимо от дисбаланса населённостей ($R$-сокращение). В полном $7\times 7$ вакууме поправка от перекрёстных когерентностей есть $O(\bar\varepsilon)$ (машинно: $\leq 0{,}8\%$ при $\bar\varepsilon = 0{,}01$, $\leq 4{,}4\%$ при $0{,}02$).

**(a′) Безусловный сэндвич [Т].** Для *любой* полноранговой $\Gamma$ (без предположения развязки): $2|\gamma_{ij}|^2/\lambda_{\max}(\Gamma) \leq \mathrm{QFI}(\partial_{\theta_{ij}}) \leq 2|\gamma_{ij}|^2/\lambda_{\min}(\Gamma)$ — немедленно из SLD-формулы по касательной с $2\lambda_{\min} \leq \lambda_m + \lambda_n \leq 2\lambda_{\max}$ и $\|X_{ij}\|_{\mathrm{HS}}^2 = 2|\gamma_{ij}|^2$. С гершгориновским контролем вакуумного спектра, $|\lambda_k - 1/7| \leq \rho_{\max} := \max_k \sum_{l \neq k}|\gamma_{kl}|$, относительное отклонение от $14|\gamma_{ij}|^2$ ограничено **явной константой** $7\rho_{\max}/(1 - 7\rho_{\max})$; измеренные $0{,}23\%$–$4{,}4\%$ лежат глубоко внутри границы. Свидетель: $0/400$ нарушений в смешанных режимах, включая сильную когерентность.

**(b) Взаимность [Т при FP-лемме, ведущий порядок].** При вакуумных населённостях $\Gamma_{ii} = 1/7 + O(\varepsilon^2)$ среднее ST-сектора удовлетворяет $\langle|\gamma_{\mathrm{ST}}|^2\rangle = \varepsilon_0^2 = \tfrac{1}{14}\langle\mathrm{QFI}\rangle_{\mathrm{ST}}$, и константа Фирца–Паули $G_N^{(ST)} = 4\pi\mu^2/\varepsilon_0^2$ принимает вид

$$
\boxed{\; G_N^{(ST)} \cdot \big\langle \mathrm{QFI}(\theta_{\mu\nu}) \big\rangle_{\mathrm{ST}} \;=\; 56\pi\mu^2 \;=\; 8\pi N \mu^2 \big|_{N=7} \;}
$$

— произведение гравитационной связи на среднюю фишеровскую обучаемость пространственно-временных фаз есть **архитектурная константа**.

**(c) $\Lambda$-сторона [Т, T-254].** Физически наблюдаемый вакуумный член квадратичен по непрозрачности часового канала: $\Lambda_{\mathrm{phys}} = \frac{f_4\omega_0^4}{32\pi G_N}\,\mathcal{G}_O^2$ ([закон Λ-дрейфа](/docs/physics/gravity/cosmological-constant#теорема-лямбда-дрейф)). Чтение $\mathcal{G}_O$ как меры *потерянной фазовой информации* O-канала (Gap = декогерированная фаза) — **[И]** — делает $\Lambda$ квадратом **необучаемости собственных часов вселенной**.
:::

**Доказательство.** *(a)* Для развязанного блока SLD-формула по касательной $\mathrm{QFI} = 2\sum_{m,n}|X_{mn}|^2/(\lambda_m + \lambda_n)$ сводится к блоку. Диагонализуем блок с населённостями $p = \Gamma_{ii}$, $q = \Gamma_{jj}$ и когерентностью $g$: собственные значения $\lambda_\pm = \tfrac{p+q}{2} \pm R$, где $R = \sqrt{(p-q)^2/4 + |g|^2}$; касательная $X = i[\Pi_i, \Gamma]$ чисто внедиагональна в собственном базисе с $|\langle +|\Pi_i|-\rangle|^2 = |g|^2/(4R^2)$, а $(\lambda_+ - \lambda_-)^2 = 4R^2$. Отсюда $\mathrm{QFI} = 2 \cdot 2 \cdot \frac{4R^2}{p+q} \cdot \frac{|g|^2}{4R^2} = \frac{4|g|^2}{p+q}$ — $R$ сокращается, так что результат верен при любом дисбалансе и любом допустимом $|g|$. *(b)* Подстановка парной леммы при $\Gamma_{ii} = 1/7$ в константу мостовой леммы $G_N^{(ST)} = 4\pi\mu^2/\varepsilon_0^2$ (§2.1): $\langle\mathrm{QFI}\rangle_{\mathrm{ST}} = 14\varepsilon_0^2(1 + O(\bar\varepsilon))$. *(c)* — это T-254 [Т] плюс [И]-чтение. $\blacksquare$

**Машинная верификация**: тождество сбалансированного блока до $6{,}7 \cdot 10^{-16}$; тождество развязанной пары в $\mathbb{C}^7$ — отношение $1{,}000000$ на случайных населённостях/когерентностях; измерено масштабирование перекрёстной поправки ($0{,}23\%/0{,}8\%/4{,}4\%$ при $\bar\varepsilon = 0{,}005/0{,}01/0{,}02$); подстановочная цепочка (b) точна.

**Следствия.**

1. **Гравитация расходится ровно там, где фазы перестают быть обучаемыми [Т при (b)].** $|\gamma_{\mathrm{ST}}| \to 0 \Rightarrow \langle\mathrm{QFI}\rangle \to 0 \Rightarrow G \to \infty$: предсказание §3.1–3.2 об усилении гравитации в областях высокой декогеренции получает информационно-теоретический механизм — **сила гравитации есть обратная информация Фишера, доступная в пространственно-временных фазах**.
2. **Слоган SLU — со знаком и разрешением по секторам [И].** Ванчуринское *«гравитация — это эффективность обучения»* отображается в два тождества с определёнными знаками и секторами: $G^{-1} \propto$ обучаемость **пространственных** фаз (b); $\Lambda \propto$ (необучаемость)$^2$ **часовой** фазы (c). Эффективное обучение $\Leftrightarrow$ слабая гравитация: гравитационное поле — тень того, что *не* выучено (или невыучиваемо). Там, где SLU постулирует, что метрика *есть* статистика обучения, УГМ выводит обе грани из одной $\Gamma$ и фиксирует константу ($8\pi N\mu^2$).
3. **Дополнение к T-263.** Те же фазы, вдоль которых движется оптимальный обучающий поток (T-261/T-263, BKM-спуск; Bures-оценивание), — это фазы, чьи вторые моменты гравитируют. Обучение и гравитация — две Петцево-геометрические грани одного когерентностного поля: динамика вдоль фаз и кривизна из фаз.
...
**Ковариантность проекции.** 4D-диффеоморфная ковариантность наследуется от $G_2$-ковариантности через секторную декомпозицию (T-53 [Т]) и спектральный формализм Шамседдина–Конна [Т] (T-121).
...
:::note Почему наблюдаемое $w \approx -1$ [С] (T-266)
То, что Вселенная в режиме $V \gg$ кинетическая (космологическая постоянная), не случайно: [T-266](/docs/physics/gravity/cosmological-constant#теорема-стадия-вселенной) показывает, что вакуум **релаксировал на свой терминальный аттрактор** ($P = 3/7$) с дробной точностью $\sim 10^{-60}$, так что остаточный кинетический член — задаваемый скоростью приближения к стадии — подавлен в $\kappa/H_0 \sim 10^{58}$ раз относительно $V$; отсюда $w \approx -1$. Малое отклонение DESI $|1+w_0| \sim 0.09$ есть усиленный в $\kappa/H_0$ раз остаток. (Это касается *стадии* и *формы* $w$, не *величины* Λ — дефицит $\gtrsim 27$ порядков ниже остаётся открытым.)
:::
...
**Шаг 3 (Лоренцева сигнатура — [Т при рефлективной положительности]).** KO-размерность 6 внутренней спектральной тройки фиксирует реальную структуру $J$ с $J^2 = +1$, $J\chi = -\chi J$ (правильные значения mod 8 для KO-dim 6; прежнее «$J^2=-1$» было ошибкой таблицы mod 8 — Стандартная модель Шамседдина–Конна использует $J^2=+1$ при KO-6, евклидова). KO-dim 6 — это **внутренние данные удвоения фермионов, и сама по себе не фиксирует сигнатуру пространства-времени.** Лоренцев знак фиксируется физическим требованием ограниченности снизу PW-генератора (рефлективная положительность Остервальдера–Шрадера); его строгая реализация — **крейновская фундаментальная симметрия** $\beta$ (Франко–Экштейн; ван ден Дунген) с PW-ограничением, задающим временноподобное направление (см. [Пространство-время §Лоренцева сигнатура](/docs/core/foundations/spacetime#лоренцева-сигнатура)). При этой конструкции Вик-ротация $\mathcal{W}: D_{\mathrm{Lor}} \mapsto iD_{\mathrm{Eucl}}$ преобразует спектральное действие:
