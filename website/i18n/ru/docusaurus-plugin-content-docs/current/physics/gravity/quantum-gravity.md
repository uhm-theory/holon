...
Gap-функциональный интеграл как альтернативная формулировка квантовой гравитации: определённость на компактном целевом пространстве $(S^1)^{21}$, **конечность по полевому пространству [Т]** (компактная цель), полная попорядковая УФ-конечность **[С]** (структурная), полное спектральное действие [Т], Gap-разрешение информационного парадокса чёрных дыр.

:::info Статус
Спектральное действие [Т]: полная спектральная тройка из T-53 воспроизводит действие Эйнштейна–Гильберта + Стандартную модель. **Конечность по полевому пространству [Т]:** решёточная статсумма $Z_N$ конечна при любом $N$, поскольку цель $(S^1)^{21}/G_2$ **компактна** (конечный объём, ограниченный интегранд) — строгий результат (§3), с остающимся [П] для континуального предела. **Полная попорядковая УФ-конечность [С]** (структурная): компактность + $G_2$-тождества Уорда + $\mathcal{N}=1$ голоморфность (Зайберг) + секторное подавление $\varepsilon^{12}$ (T-219) — **не** отозванный точный след «$7-7=0$». Информационный парадокс — [С] (унитарность [Т], Gap-описание горизонта — анзац). Энтропия $S_{\text{BH}}$ — [С при T-65, T-73, Wald]: ведущий член $A/(4G_N)$ [Т] из формулы Вальда + спектральное действие; коэффициент Gap-поправки $c_{\mathrm{Gap}}$ явно вычислен [С при T-65, T-73, T-74] (§6.3). Решёточная верификация — [П].
:::
...
**Шаг 2 (Разложение спектрального действия).** По формуле Шамседдина–Конна (1996):
...
:::note Соглашение об индексах моментов (по всему корпусу)
Индекс $f_{2k}$ совпадает с индексом коэффициента теплового ядра $a_{2k}$, который он умножает: $f_0 \leftrightarrow \Lambda^4 a_0$, $f_2 \leftrightarrow \Lambda^2 a_2$, $f_4 = f(0) \leftrightarrow a_4$ — как в [спектральной формуле для $\Lambda_{\text{CC}}$](/docs/physics/gravity/cosmological-constant#теорема-спектральная-лямбда), T-70 и T-254. Осторожно при сравнении с литературой: оригиналы Chamseddine–Connes нумеруют моменты по степени $\Lambda$ ($f_4 \Lambda^4 a_0 + f_2 \Lambda^2 a_2 + f(0)\, a_4$), т.е. их $f_4$ — это наш $f_0$, а их $f_0 = f(0)$ — наш $f_4$.
:::

Три момента $f_0, f_2, f_4$ входят в физический лагранжиан:
- $f_0 \, a_0$ → **космологическая постоянная** ($\Lambda_\mathrm{cc}$).
- $f_2 \, a_2$ → **действие Эйнштейна–Гильберта** (Ньютонова $G_N$).
- $f_4 \, a_4$ → **Ян–Миллс кинетика** + Вейль-квадрат + Хиггс-потенциал.
...
Выбор $f(u) = e^{-u}$ **принят** (зафиксирован теорией как определение), а не **выведен** из независимого принципа УГМ. Перечисленные ниже физические мотивировки (регуляризация тепловым ядром, целочисленные моменты, совместимость с $\Lambda = M_P$) — это обоснования выбора, не вывод. В программе спектрального действия Конна–Шамседдина (Chamseddine–Connes 1996, *Comm. Math. Phys.* 186, 731–750; Connes–Chamseddine 2010) срез $f$ аналогично является определительным входом — обычно функция-горбик или усечённая гауссиана — с физическими наблюдаемыми, зависящими от небольшого числа моментов $f_0, f_2, f_4$. Предсказания УГМ, зависящие от отдельных моментов (размерные константы: $G_N$, $\Lambda_{\mathrm{cc}}$), таким образом являются условными относительно канонического выбора; **структурные** предсказания, перечисленные во [врезке $f$-независимости](#f-independence) ниже, выполняются для любого разумного $f$.
:::

С таким выбором:
- $f_0 = \int_0^\infty u \, e^{-u} \, du = \Gamma(2) = 1$ — сырой регуляторный момент $\Lambda^4$-члена; **ренормированный** коэффициент при $a_0$, входящий в $\Lambda$-бюджет, фиксируется самосогласованно вакуумом ([T-70](/docs/physics/particle-physics/higgs-sector#теорема-f0-канонический)).
- $f_2 = \int_0^\infty e^{-u} \, du = 1$.
- $f_4 = f(0) = 1$ — UV-конечный ($\Lambda^0$) коэффициент есть значение $f$ в нуле; никакая регуляризация не требуется.

Ниже последовательно используется соглашение о моментах спектрального действия $(f_0, f_2, f_4) = (1, 1, 1)$.
...
| Выбор | $f(u)$ | $f_0$ | $f_2$ | $f_4$ | $f_0/f_2^2$ |
|---|---|---|---|---|---|
| Экспоненциальный (канонический УГМ) | $e^{-u}$ | $1$ | $1$ | $1$ | $1$ |
| Гауссовский | $e^{-u^2}$ | $1/2$ | $\sqrt{\pi}/2$ | $1$ | $2/\pi$ |
| Резкий cut-off | $\Theta(1-u)$ | $1/2$ | $1$ | $1$ | $1/2$ |
| Усечённый гауссовский (Connes–Chamseddine) | $e^{-u^2/2}$, $u \leq 1$ | численный | численный | $1$ | численный |

Хотя абсолютные численные значения $G_N$ и $\Lambda_\mathrm{cc}$ зависят от $f$ (через $f_0$ и $f_2$ индивидуально), **соотношения**, релевантные для физики УГМ, более жёстко связаны:
...
2. **Значения моментов**: у экспоненциального все три момента равны единице, $(f_0, f_2, f_4) = (1, 1, 1)$; у гауссовского — $(1/2, \sqrt{\pi}/2, 1)$. Чистые целочисленные значения экспоненциального предпочтительны для строгих выводов.
...
**Шаг 5 (Проекция на $M^{3+1}$).** Лоренцева сигнатура $(+1,-1,-1,-1)$ из [T-53 — расщепление $(1,3)$ [Т] + знак [Т при рефлективной положительности]](/docs/core/foundations/spacetime#лоренцева-сигнатура): расщепление $(1,3)$ выведено (PW-время + $S^3$), а лоренцев относительный знак фиксируется требованием ограниченности снизу PW-генератора (рефлективная положительность Остервальдера–Шрадера), а не ad-hoc анзацем (одна KO-dim 6 её не определяет; строгий маршрут — крин-спектральные тройки). Wick-поворот определяется относительно этой конструкции.
...
Таким образом, Gap-функциональный интеграл воспроизводит стандартную квантовую гравитацию в низкоэнергетическом пределе, но, в отличие от неё, имеет **строго конечную меру по полевому пространству** **[Т]** благодаря компактности $(S^1)^{21}/G_2$ (конечный объём цели) — единственная безусловная часть УФ-конечности; полная попорядковая конечность — структурный аргумент **[С]** выше. Реконструкция спектрального действия полностью строга **[Т]** (T-53 → спектральная тройка → Шамседдин–Коннс); аргумент Лавлока — дополнительный.
...
Gap-функциональный интеграл перенормируем в скалярном секторе — расходимости попорядково поглощаются конечным набором констант связи $(\mu^2, \lambda_3, \lambda_4)$:
...
**Итог:** Gap-теория **перенормируема** (не конечна) в своём скалярном секторе. Гравитационные расходимости **экранированы** эмерджентностью метрики. Полная УФ-конечность аргументирована в §4 (часть по полевому пространству [Т]; попорядковая — структурная [С]).

---

## 4. УФ-конечность Gap-теории [Т по полевому пространству, С попорядково] {#уф-конечность}

#### Теорема (UV-конечность Gap-теории) [Т по полевому пространству, С попорядково] {#теорема-уф-конечность}

:::tip Теорема 4.1 (УФ-конечность Gap-теории на $(S^1)^{21}$) [Т по полевому пространству, С попорядково]
Gap-теория на $(S^1)^{21}$ с $G_2$-симметрией и $\mathcal{N}=1$ SUSY перенормируема; её **конечность по полевому пространству** строга **[Т]** (компактная цель), а **полная попорядковая УФ-конечность** — структурный аргумент **[С]**.
...
**Шаг 3 ($\mathcal{N}=1$ SUSY-подавление).** По теоремам о неперенормировке Зайберга (1993): $\mathcal{N}=1$ SUSY запрещает перенормировку суперпотенциала (теорема голоморфности), а D-члены получают лишь конечные поправки. Остаточный вакуумный вклад подавлен **секторным произведением** $\Lambda_{\text{residual}}\sim\varepsilon^{12}M_P^4$ (T-219 [Т при T-64]). *(Прежний точный «$7-7=0$ бозе-ферми след» **отозван [✗]** — $\mathbb{Z}_2$-градуировка на нечётном внутреннем $\mathbb{C}^7$ не может иметь след $0$, а $\mathbf{14}\to\mathbf7\oplus\mathbf7$ не существует, т.к. $G_2$-присоединённое неприводимо; см. [Λ-бюджет Тм 4.4](/docs/proofs/gap/lambda-budget#теорема-susy-компенсация). Конечность несут голоморфность + компактность, а не точный след.)*
...
**Шаг 5 (Область строгости).** Для скалярно-фермионного сектора ($\theta_{ij}$, $\tilde{\theta}_{ij}$) конечность по полевому пространству (шаги 1, 4) строга [Т]; композиция шагов 1–4 в полную попорядковую конечность — структурная [С]. Гравитационная УФ-конечность следует автоматически из эмерджентности метрики: $h_{\mu\nu} \sim \sum \theta^2$ — составной оператор, не фундаментальное поле. Расходимости составных операторов подавлены формфакторами при $p > \Lambda_{\text{Gap}}$. $\blacksquare$
:::

### Тройная защита от расходимостей

Аргумент УФ-конечности (Теорема 4.1) опирается на три взаимодополняющих механизма:
...
| $\mathcal{N}=1$ СУСИ | Секторное подавление $\varepsilon^{12}$ (T-219); голоморфность (шаг 3) | $E > m_{3/2} \sim 10^{13}$ ГэВ |

Эти три фактора — **компактность + $G_2$ + СУСИ** — совместно несут структурный аргумент конечности **[С]** (часть по полевому пространству строга **[Т]**). Ни один из них по отдельности не является достаточным:
...
:::tip Статус: по полевому пространству [Т], попорядково [С]
УФ-конечность скалярно-фермионного сектора — **структурный** аргумент [С]: компактность $(S^1)^{21}$ + $G_2$-тождества Уорда ($21 \to 7$ расходимостей) + $\mathcal{N}=1$ голоморфность (Зайберг) + секторное подавление $\varepsilon^{12}$ (T-219). (Шаг с точным «$7-7=0$ следом» отозван — см. блок-уточнение ниже.) Гравитационная УФ-конечность автоматична из эмерджентности метрики.
:::

:::warning Теоретическое уточнение: точный след «7 - 7 = 0» отозван; конечность держится на других опорах
Буквальный след «7 бозонных - 7 фермионных = 0 расходимостей» **отозван [✗]** ($\mathbb{Z}_2$-градуировка на нечётном внутреннем $\mathbb{C}^7$ имеет след $\in\{\pm1,\dots,\pm7\}$, никогда $0$; а $\mathbf{14}\to\mathbf7\oplus\mathbf7$ не существует — $G_2$-присоединённое неприводимо; [Λ-бюджет Тм 4.4](/docs/proofs/gap/lambda-budget#теорема-susy-компенсация)). Что **выполняется** — **структурный** аргумент конечности [С], а не строгое пертурбативное доказательство в каждом порядке: $\hat{A}$-род тора $(S^1)^{21}$ равен нулю (APS-индекс → нет аномалий), неперенормировка суперпотенциала голоморфна (Зайберг), а остаточная вакуумная энергия несёт секторное подавление $\varepsilon^{12}M_P^4$ (T-219). Полная УФ-конечность за пределами ведущего порядка держится на компактности целевого пространства, а не на бозе-ферми следе.
:::
