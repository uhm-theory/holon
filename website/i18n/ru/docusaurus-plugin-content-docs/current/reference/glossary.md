...
| **Свобода воли (Freedom)** | $\mathrm{Freedom}(\Gamma) := \dim\ker(\mathcal{H}_\Gamma) + 1$ **[Т]** — число нулевых мод гессиана свободно-энергетического функционала + 1. ∞-категорное прочтение: касательная размерность критического многообразия свободной энергии (**не** $\pi_0(\mathrm{Map}(\Gamma, T))$ — это пространство стягиваемо, $\pi_0=1$). Монотонна под CPTP, $G_2$-инвариантна. См. [Следствия](/docs/core/foundations/consequences#freedom-конечномерное), [Свобода воли](/docs/consciousness/ethics-meaning/freedom#количественная-мера) |
...
| **$\mathrm{Coh}_E$ (E-когерентность)** | HS-проекция **[Т]**: $\mathrm{Coh}_E(\Gamma) = \|\pi_E(\Gamma)\|_{\mathrm{HS}}^2 / \|\Gamma\|_{\mathrm{HS}}^2 = (\gamma_{EE}^2 + 2\sum_{i \neq E}\lvert\gamma_{Ei}\rvert^2) / \mathrm{Tr}(\Gamma^2)$, $\in [0, 1]$ глобально ($1/7$ — значение при $I/7$ и No-Zombie-пол Ω-поддерживаемых жизнеспособных состояний — **не** глобальный минимум; [поправка диапазона 2026-07-11](/docs/core/foundations/axiom-septicity#coh-e-canonical)) — [мастер-определение](/docs/core/foundations/axiom-septicity#e-coherence-definition), [HS-проекция](/docs/core/foundations/axiom-septicity#hs-projection). Формальная эквивалентность $\mathrm{Coh}_E \approx P_E = \mathrm{Tr}(\rho_E^2)$ — **структурная гипотеза [Г]** |
...
| **Плоскость Фано PG(2,2) [Т]** | Минимальная проективная плоскость: 7 точек, 7 линий, 3 точки на каждой линии. Её линии задают, КАКИЕ тройки мнимых единиц перемножаются; знаки требуют ориентации каждой линии — алгебру с делением дают 16 ориентаций из 128, все изоморфные $\mathbb{O}$ |
...
| **Gap-оператор ($\hat{G}$)** | Антиэрмитов оператор $\hat{G} = \mathrm{Im}(\Gamma) \in so(7)$, описывающий рассогласование внешнего и внутреннего аспектов. Спектр: $\{\pm i\lambda_1, \pm i\lambda_2, \pm i\lambda_3, 0\}$. Ранг непрозрачности = число ненулевых пар (0–3) — [определение](/docs/core/dynamics/gap-operator). Читается в родном семантическом фрейме: попарные фазы двигаются при перефазировке осей (*почерк*, эрратум T-301) |
| **Краска (голономия Фано $H_p$)** | Калибровочно-инвариантный носитель качества: $H_p = \arg(\gamma_{ij}\gamma_{jk}\gamma_{ki})$ по семи линиям Фано — единственные фазовые объекты, переживающие перефазировку осей. Пишется только унитарной рукой; носитель (модули) умирает при дефазировке, угол стоит — [Язык качества](/docs/consciousness/phenomenology/qualia-structure#язык-качества) |
| **Паспорт квалиа** | Пятислойный рабочий интерфейс к содержанию опыта состояния: населённости → интенсивности → непрозрачности → краски → доступ — [Как читать](/docs/consciousness/phenomenology/qualia-structure#паспорт-квалиа) |
...
| **$\mathrm{SAD}_{\max}$** | Максимальная глубина рекурсивного самомоделирования: $\mathrm{SAD}_{\max} = 3$ **[Т]** (T-142: из контракции Фано $\alpha = 2/3$, $P_{\text{crit}}^{(n)} = P_{\text{crit}} \cdot 3^{n-1}/(n+1)$ — и, независимо, из кодового потолка T-232/T-239). Pred 12 — [башня глубины](/docs/consciousness/hierarchy/depth-tower#критическая-чистота-sad) |
