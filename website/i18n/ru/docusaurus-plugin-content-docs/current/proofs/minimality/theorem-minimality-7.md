...
Это создаёт каузально замкнутую структуру без внешних причин.
...
**(Б) Категориальный аргумент из κ₀.** Формула κ₀ ([Т при кинетике первого порядка], [вывод](/docs/core/foundations/axiom-septicity#вывод-kappa0-cycle-flux); Тм. 15.3.1):
...
**Каскадное следствие:** P1, P2 — [Т]. Трек B (октонионный вывод) полностью строг.
...
### 9.3 Трек Σ: диагностический вывод {#трек-сигма}

**Теорема Σ (T-224) [Т]** добавляет четвёртый независимый селектор семёрки, вовсе не использующий алгебру — только упаковку сфер и счёт дизайнов: совершенная локализуемость одиночного сбоя (D1–D2) вынуждает $n = 2^r - 1$; нетривиальная грамматика состояний (D3) — $n \geq 7$; а жёсткость грамматики (D4) выполняется **только** при $n = 7$ (нелинейные совершенные коды Васильева ломают единственность начиная с $n = 15$). Отождествление осей УГМ с внутренне диагностируемыми статус-битами — интерпретационный шаг [И]. См. [Σ-исчисление](/docs/applied/research/syndrome-calculus#t-224).

| Аспект | Трек A | Трек B | Трек Σ |
|--------|--------|--------|--------|
| Вход | (AP)+(PH)+(QG) | P1+P2 (алгебра с делением) | (D1)–(D4) диагностируемость |
| Аппарат | аксиоматическое замыкание | теорема Гурвица | совершенные коды, $S(2,3,7)$ |
| Выход | $N = 7$ | $N = 7$ через $\mathrm{Im}\,\mathbb{O}$ | $N = 7$ + грамматика Фано, единственно |
