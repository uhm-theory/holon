...
**(C27-M)** (Непрерывный предел Gap): существует предел $a \to 0$ решётки Gap-полей $\theta_{ij}(x)$, в котором $\sigma$-модель на $(S^1)^{21}/G_2$ определяет гладкое 7-мерное целевое пространство $\mathcal{M}_7$; *(помечено **C27-M** для отличия от C27 окна сознания «аттрактор в окне»; «-M» маркирует M-теоретический/ToE блок C27-M–C30)*

**(C28-M)** (Суперсимметрическое расширение): SUSY-расширение Gap-интеграла ([SUSY из $G_2$](/docs/physics/particle-physics/susy)) является корректным квантовым суперсимметричным функциональным интегралом;
...
- **T-170 (Полное непертурбативное соответствие) [С при C27-M, C28-M]**: требует непертурбативного определения M-теории на $G_2$-многообразиях (активная исследовательская задача в матфизике).
...
- **(C27-M/C28-M переформулированы)**: существование **непертурбативного** определения $Z_{\text{M}}$ для M-теории на $G_2$-многообразиях — **открытая задача M-теории**, не УГМ.
...
**Статус:** [Т] в пределах области $\mathbf{SpinNet}_{SU(2)}^{\text{bd}}$.
...
**Статус:** [Т]. Кластерная конструкция явно реализует **любой** спин через цепочку голономов. Ценой является увеличение числа голономов: $M_{\text{total}} = O(|E| \cdot j_{\max})$ вместо $M = |V|$.
...
**Статус:** [Т]. Доказательство использует:
...
|---------|-------------|--------|---------|
| **T-170** | Восстановление M-теоретического предела | [Т] на уровнях определённости M-теории | T-170' [Т] (пертурб.) + T-170'' [Т] (непертурб. УГМ); C27-M/C28-M переформулированы как внешние открытые задачи M-теории |
...
| **C27-M** | Непрерывный предел Gap | [П] | — |
| **C28-M** | Суперсимметрическое расширение | [П] | — |
...
- **Статус-реестр:** T-170 — T-174, C27-M — C30 ([Реестр](/docs/reference/status-registry))
