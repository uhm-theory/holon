...
| (v) Двойственность Пуанкаре | См. ниже | Явная верификация [Т] |
...
**Шаг 8 (Лоренцева сигнатура).** Реальная структура $J$ с KO-размерностью 6 (T-53 [Т]: $J^2 = +1$, $J\chi = -\chi J$) в сочетании с ограничением Пейдж–Вуттерса $E_O = -E_{\text{rest}}$ вынуждает собственные значения противоположных знаков: $\mathrm{spec}(D_O) = \{+\omega_0\}$ (временна́я компонента, положительная) против $\mathrm{spec}(D_{\{A,S,D\}}) \subset \{-\lambda_a\}$ (пространственная компонента, отрицательная). Формула расстояния Конна $d(x,y) = \sup\{|f(x) - f(y)| : \|[D,f]\| \leq 1\}$ транслирует спектральные знаки в метрическую сигнатуру: $g_{00} = 1/|D_O|^2 > 0$, $g_{aa} = -1/|D_{3,a}|^2 < 0$, что даёт сигнатуру $(+1,-1,-1,-1)$. $\blacksquare$
