...
:::note Область применимости: верификация условия первого порядка
Условие первого порядка (order-one) — седьмая аксиома реконструкции Конна и шаг, чаще всего отмечаемый во внешних аудитах NCG-выводов (ср. Chamseddine–Connes 2008 о "first-order / one-form" ослаблении). В данном доказательстве оно **налагается** как структурное ограничение на $D$ (допустимы те операторы Дирака, для которых условие выполнено), а не выводится из алгебры $A_{\text{int}}$ самой по себе. Проверка для конкретного $D_{\text{int}}$ из T-53 сводится к вычислению на ограничении произведения троек на Хиггсову линию $\{A,E,U\}$; это вычисление намечено, но не полностью расписано здесь (тот же пробел, что и в T-119, указан как framework-conditional в [Таблице стратификации строгости](/docs/reference/status-registry#стратификация-строгости)).
:::
...
:::info Явная редукция $M_3(\mathbb C)\to\mathbb H$
Стрелка $A_\mathrm{int}\to A_F$ **не** является изоморфизмом алгебр — это редукция, индуцированная вещественной структурой $J$ плюс нарушением ФЭ. $A_\mathrm{int}=\mathbb C\oplus M_3(\mathbb C)_{\mathbf 3}\oplus M_3(\mathbb C)_{\bar{\mathbf 3}}$ имеет $\dim_\mathbb R=1+18+18=37$; $A_F=\mathbb C\oplus\mathbb H\oplus M_3(\mathbb C)$ имеет $\dim_\mathbb R=1+4+18=23$. Редукция:
1. Первый множитель $M_3(\mathbb C)_{\mathbf 3}$ несёт $SU(3)_C$ цветовое действие — удерживается в $A_F$ как $M_3(\mathbb C)$.
2. Второй множитель $M_3(\mathbb C)_{\bar{\mathbf 3}}$ редуцируется к $\mathbb H\subset M_2(\mathbb C)\subset M_3(\mathbb C)$ через $J$-совместимость + ограничение Хиггсовой линии $\{A,E,U\}$ (T-1a): 2-мерное подпространство слабого изоспина $\mathrm{span}\{|E\rangle,|U\rangle\}$ поддерживает антикоммутирующую вещественную структуру $J^2=+1,\ [J,\gamma]=0$, коммутант которой — $\mathbb H$ (Barrett 2007, §3.2; Chamseddine–Connes 2007). Оставшееся содержание $M_3(\mathbb C)_{\bar{\mathbf 3}}\setminus\mathbb H$ проецируется прочь, поскольку не удовлетворяет условию первого порядка с оператором Дирака, ограниченным на Хиггсову линию.
3. Чистый эффект — **Morita-совместимая** редукция: $A_F$ и $A_\mathrm{int}$ имеют **одну и ту же** категорию бимодульных представлений, реализующих SM-фермионы (Alvarez–Gracia-Bondía–Martín 1995), т. е. бимодульная структура сохраняется.
4. Проверка: размерный учёт — число SM-фермионов из бимодулей $A_F$ = 16 на поколение (SM), соответствует $\dim H_F = 7 \cdot 2 = 14 + 2$ правых нейтрино.

Это разрешает вопрос о том, что $A_\mathrm{int}$ и $A_F$ различаются как алгебры: эквивалентность на уровне категорий бимодулей (Morita), а не объектов.
:::
