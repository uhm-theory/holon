...
### 5.2. Статусы закрытых уязвимостей

| Уязвимость | Статус | Механизм закрытия |
|---|---|---|
| $F = \&$: феноменальный функтор | [Т] из $\iota^* \dashv \mathrm{Inf}$ | Башня Постникова $\&(\Gamma)$ |
| Время Пейджа–Вуттерса | [Т] точно | Коединица $(\Pi \dashv \flat)$ |
| $\Delta F > 0$, $\Lambda > 0$ | [Т] безусловно | Черн–Вейль + Gap > 0 (T-55) |
...
**(Char-IV) Идентификация ковариации максимальной энтропии (Vanchurin 2026).** Принцип максимальной энтропии на $\mathcal{D}(\mathbb{C}^7)$ единственным образом отождествляет обратный метрический тензор с физической ковариацией — с точностью до нормировочной константы Char-III:

$$g^{ij}_B(\Gamma) = 4\,C_{\mathrm{est}}^{ij}(\Gamma),\qquad C_{\mathrm{est}} := \mathcal F_{\mathrm{SLD}}^{-1}$$

где $C_{\mathrm{est}}$ — минимальная ковариация оценщика, насыщающая квантовую границу Крамера–Рао. Множитель $4$ не факультативен: это ровно константа Браунштейна–Кейвса из Char-III, и его отбрасывание делает тождество внутренне противоречивым (см. примечание об исправлении в шаге 4).
...
**Шаг 3 (Метрически-независимая физическая величина — Лемма).**
:::info Лемма (SLD-билинейная форма Petz-независима)
SLD-билинейная форма $C^{\mathrm{SLD}}_{ij}(\Gamma):=\tfrac12\operatorname{Tr}\!\bigl(\Gamma\{L_i,L_j\}\bigr)$, где $L_i$ — SLD, определяемый $\partial_i\Gamma=\tfrac12(L_i\Gamma+\Gamma L_i)$, включает только $\Gamma$ и её производную; **никакая метрика на $\mathcal D(\mathbb C^7)$ не входит в её определение**. Поэтому $C^{\mathrm{SLD}}$ — физическая величина, приписывающая каждому $\Gamma$ тензор типа $(0,2)$ независимо от выбора метрики.

**Расположение индексов, точно.** $C^{\mathrm{SLD}}_{ij}$ несёт *нижние* индексы: это ковариация SLD-*операторов*, а она по определению и есть квантовая информация Фишера в форме SLD, $C^{\mathrm{SLD}} = \mathcal F_{\mathrm{SLD}}$. Объект с *верхними* индексами — ковариация *параметров*, то есть минимальная ковариация оценщика, насыщающая квантовую границу Крамера–Рао, — есть её обратная: $C_{\mathrm{est}}^{ij} = (\mathcal F_{\mathrm{SLD}}^{-1})^{ij}$. Эти два объекта не взаимозаменяемы, и их смешение как раз и породило исправляемую ниже ошибку.
:::
...
и спросим: **какая** Petz-метрика удовлетворяет $(\star)$? По Braunstein–Caves 1994 $\mathcal F_{\mathrm{SLD}}=4g_B$, а по Лемме $C^{\mathrm{SLD}}=\mathcal F_{\mathrm{SLD}}$; поэтому $(\star)$ читается как $g_{ij}=(g_B)_{ij}$ и выполняется для Бюреса. Равносильно, в форме через ковариацию оценщика, которая и несёт MaxEnt-прочтение: $(g_B)^{-1} = 4\,C_{\mathrm{est}}$. Для любой другой Petz-метрики $g_f$ ($f\neq\tfrac{1+t}{2}$) условие $(\star)$ нарушается, поскольку соответствующая информация Фишера $\mathcal F_f\neq\mathcal F_{\mathrm{SLD}}$ (Petz 1996; различные монотонные средние дают различные тензоры Фишера). Следовательно, $(\star)$ удовлетворяется **единственным образом** метрикой Бюреса. $\square_{\mathrm{IV}}$

:::danger Исправление (06.08.2026): селекторное уравнение имело неверный вид
Прежняя версия записывала селектор как $g^{ij}=C^{ij}_{\mathrm{SLD}}$, то есть $(g_B)^{-1}=C_{\mathrm{SLD}}$, обосновывая это словами «обратная к $\mathcal F_{\mathrm{SLD}}$ есть SLD-ковариация». Это утверждение неверно как записано, причём сразу по двум причинам. Объект, определённый в шаге 3, есть $\tfrac12\operatorname{Tr}(\Gamma\{L_i,L_j\})$, а это **и есть** сама $\mathcal F_{\mathrm{SLD}}$, а не её обратная; и даже при подразумеваемом прочтении был утрачен множитель $4$ из Char-III. Сцепление двух утверждений даёт $(g_B)^{-1}=\mathcal F_{\mathrm{SLD}}=4g_B$, то есть $g_B^2=\tfrac14 I$, что ложно для любой невырожденной метрики.

Проверено машиной при $N=4$: $\max|C^{\mathrm{SLD}}-\mathcal F_{\mathrm{SLD}}| = 0$ точно, $\max|C^{\mathrm{SLD}}-4g_B| = 1{,}5\times10^{-12}$, тогда как заявленное тождество $(g_B)^{-1}=C^{\mathrm{SLD}}$ имеет относительную невязку $0{,}994$ — оно неверно и приближённо. Исправленные тождества $g_B = \tfrac14 C^{\mathrm{SLD}}$ и $g_B^{-1}=4(C^{\mathrm{SLD}})^{-1}$ выполняются до $4\times10^{-13}$.

**Что уцелело.** *Вывод* Char-IV — что селектор однозначно выделяет Бюреса — не затронут, поскольку единственность опирается на то, что различные монотонные средние дают различные тензоры Фишера, а это утверждение о *форме* тензора, не о его нормировке. Ничто другое в T-187 от этого шага не зависит; Char-IV и ранее была зафиксирована как интерпретативная, а не логически независимая (см. уточнение области ниже). Неверным было только выписанное уравнение.
:::
