...
#### Теорема (Локализация трудной проблемы) [Т] {#теорема-локализация-hard-problem}
...
**Шаг 1.** По T-187 [Т]: A2 однозначно определяет метрику Бюреса через три независимые характеризации (экстремальность Петца, универсальность Ульмана, насыщение SLD-Фишера).
...
**Шаг 1 (MaxEnt-распределение).** Рассмотрим распределение максимальной энтропии $\tilde{\rho}$ на $\mathcal{D}(\mathbb{C}^N)$, сконцентрированное вблизи состояния $\Gamma$, с ограничениями на среднее $\langle\Gamma'\rangle = \Gamma$ и ожидаемый квадрат Бюрес-расстояния $\langle d_B^2(\Gamma', \Gamma)\rangle = \sigma^2$. В римановых нормальных координатах (где $g_B \to \delta_{ij}$ локально) MaxEnt-распределение гауссово (Jaynes 1957):

$$\tilde{\rho}(\tilde{\Gamma}) = \frac{1}{Z}\exp\!\left(-\frac{\lambda}{2}\delta_{ij}(\tilde{\Gamma}^i - \bar{\Gamma}^i)(\tilde{\Gamma}^j - \bar{\Gamma}^j)\right)$$

**Шаг 2 (Ковариация в нормальных координатах).** Ковариационная матрица MaxEnt-распределения в нормальных координатах пропорциональна единичной: $c^{ij} = (\sigma^2/\dim) \cdot \delta^{ij}$. При соответствующем перемасштабировании $c^{ij} = \delta^{ij}$.

**Шаг 3 (Преобразование к общим координатам).** В произвольных координатах на $\mathcal{D}(\mathbb{C}^N)$ ковариация преобразуется как (2,0)-тензор: $C^{ij} = (\partial x^i/\partial\tilde{x}^r)(\partial x^j/\partial\tilde{x}^s) \cdot c^{rs}$. Обратный метрический тензор преобразуется тождественно: $G^{ij} = (\partial x^i/\partial\tilde{x}^r)(\partial x^j/\partial\tilde{x}^s) \cdot \delta^{rs}$. Поскольку $c^{rs} = \delta^{rs}$ в нормальных координатах:

$$G^{ij} = C^{ij}$$

**Шаг 4 (Выделение Бюреса).** Отождествление $G^{-1} = C$ выполняется для **любой** Petz-метрики в её собственных нормальных координатах. Но ковариация $C^{ij}$ — **физическая наблюдаемая**: она не зависит от выбора метрики. Следовательно, метрика, обратная которой равна физической ковариации, **единственно выделена**. Для квантовых состояний SLD-квантовая информация Фишера $\mathcal{F}_{\mathrm{SLD}} = 4g_B$ — единственная квантовая информация Фишера, равная обратной квантовой ковариации в смысле Cramér–Rao (Braunstein–Caves 1994). Это замыкает круг: MaxEnt → $G^{-1} = C$ → $G = \mathcal{F}_{\mathrm{SLD}}^{-1}/4 = g_B^{-1}$ → Бюрес. $\square_{\mathrm{IV}}$
...
**Совместная область.** Char-I + II + III + IV вместе фиксируют Бюреса **четырьмя** независимыми аргументами: минимум-информационное-расстояние + очищение-когерентность + классическое-оценочное-насыщение + **ковариация максимальной энтропии**. Четвёртая характеризация — сильнейшая: она выводит метрику из статистической структуры самого состояния, без какого-либо выбора интерпретационного кластера.
