...
:::tip Примитивность линейной части [Т]
Шаг (1) ⟹ (2) использует теорему Перрона—Фробениуса для **линейной части** $\mathcal{L}_0 = -i[H,\cdot]+\mathcal{D}$. **Примитивность $\mathcal{L}_0$ доказана** для всех жизнеспособных холонов: из (AP)+(PH)+(QG)+(V) следует связность графа взаимодействия $G_H$ (иначе система распадается на блоки с $\dim < 7$, противоречие с [теоремой минимальности](/docs/proofs/minimality/theorem-minimality-7)), а связность $G_H$ + атомарные операторы $L_k = |k\rangle\langle k|$ дают тривиальный коммутант $\mathcal{F}(\mathcal{L}_0) = \mathbb{C} \cdot I$ по [критерию Эванса—Спона](https://doi.org/10.1007/BF01614091) (Evans 1977, Spohn 1976), так что $\lim_{\tau\to\infty}e^{\tau\mathcal{L}_0}=I/7$.

**Важно (исправление самореференциальности, T-96):** полный генератор $\mathcal{L}_\Omega=\mathcal{L}_0+\mathcal{R}$ **нелинеен** и *не* примитивен — он имеет нетривиальную нулевую моду $\rho^*_\Omega\neq I/7$. Поэтому Определение (2) ниже — это проектор на **нулевую моду кратности 1 линеаризованного полного генератора** в точке $\rho^*_\Omega$, а **не** $\lim e^{\tau\mathcal{L}_\Omega}$, применённый как линейная полугруппа (что даже не имеет смысла для нелинейного $\mathcal{L}_\Omega$ и схлопнулось бы в $I/7$). Эквивалентность (1) ⟺ (2) ⟺ (3) [Т] читается с этим соглашением. Полное доказательство: [Примитивность ℒ₀](/docs/core/operators/lindblad-operators#примитивность-ℒω); аттрактор: [T-96](/docs/core/dynamics/evolution#теорема-нетривиальность-аттрактора).
:::
...
3. По теореме о сходимости примитивных CPTP-каналов (аналог Перрона—Фробениуса для квантовых каналов), применённой к **линейной части** $\mathcal{L}_0$, существует проектор на её нулевую моду кратности 1 (вычисленный при линеаризации вблизи $\rho^*_\Omega$). Примитивность $\mathcal{L}_0$ для жизнеспособных холонов **[Т]** — см. [доказательство](/docs/core/operators/lindblad-operators#примитивность-ℒω). Здесь $\lim_{\tau\to\infty}e^{\tau\mathcal{L}_\Omega}[\Gamma]$ обозначает эту проекцию на нулевую моду, а **не** постоянное отображение в $I/7$ (что дала бы наивная примитивность полного $\mathcal{L}_\Omega$ — исправление самореференциального ρ*, T-96).
...
:::warning Потенциальная цикличность — РАЗРЕШЕНА (T-191)
...
#### Теорема T-191 (Сходимость φ-башни) [Т] {#t-191-сходимость-φ-башни}

:::tip Теорема T-191 [Т]
Итеративная башня самомоделирования $\varphi^{(0)}, \varphi^{(1)}, \varphi^{(2)}, \ldots$ сходится в операторной норме к единственной самосогласованной самомодели $\varphi^*$, начиная с **любого** начального якоря. Сходимость экспоненциальная со скоростью, ограниченной сжатием Фано $\alpha = 2/3$.
:::

**Формулировка.** Определим итеративную схему:

1. $\varphi^{(0)}(\Gamma) := I/7$ (максимально смешанный якорь — без априорного знания)
2. $\varphi^{(n+1)}(\Gamma) := \lim_{\tau \to \infty} \exp\!\bigl(\tau \cdot \mathcal{L}_\Omega^{(n)}\bigr)[\Gamma]$, где $\mathcal{L}_\Omega^{(n)}$ использует $\varphi^{(n)}$ как цель регенерации
...
**Шаг 1 (Корректность каждой итерации).** Для фиксированного $\varphi^{(n)}$ Лиувиллиан $\mathcal{L}_\Omega^{(n)} = \mathcal{L}_0 + \kappa(\Gamma) \cdot (\varphi^{(n)}(\Gamma) - \Gamma) \cdot g_V(P)$ является генератором сжимающей CPTP-полугруппы на конечномерном пространстве $\mathcal{D}(\mathbb{C}^7)$. Из примитивности $\mathcal{L}_0$ (T-39a [Т]) и добавления сжимающего регенерационного члена следует, что $\mathcal{L}_\Omega^{(n)}$ имеет единственное стационарное состояние $\rho^{*(n)}$ (по теореме Перрона–Фробениуса для положительных полугрупп на конечномерных матричных алгебрах, Evans 1977). Следовательно, $\varphi^{(n+1)}$ корректно определено. $\checkmark$

**Шаг 2 (Сжимающее отображение итерации).** Определим $\Psi: \mathcal{B}(\mathcal{D}(\mathbb{C}^7)) \to \mathcal{B}(\mathcal{D}(\mathbb{C}^7))$ как $\Psi(\varphi) := \lim_{\tau \to \infty} \exp(\tau \cdot \mathcal{L}_\Omega[\cdot; \varphi])$. Для двух кандидатов самомоделей $\varphi_1, \varphi_2$:
...
Это следует из оценки резольвенты: стационарное состояние $\mathcal{L}_0 + \mathcal{R}$ зависит от $\mathcal{R}$ через резольвенту $(\mathcal{L}_0 - z)^{-1}$, а спектральная щель $\lambda_{\mathrm{gap}}$ для $\mathcal{L}_0$ ограничивает норму резольвенты при $z = 0$ величиной $1/\lambda_{\mathrm{gap}}$.

Поскольку $\varphi_i$ — каналы замещения: $\|\varphi_1(\Gamma) - \varphi_2(\Gamma)\|_F = k \cdot \|\rho^{*}_1 - \rho^{*}_2\|_F \leq \|\varphi_1 - \varphi_2\|_{\mathrm{op}}$ (с $k = 1 - R \leq 1$). Следовательно:
...
**Шаг 3 (Сжимаемость $q < 1$).** Условие $q < 1$ эквивалентно $\kappa_{\max} < \lambda_{\mathrm{gap}} + \kappa_{\min}$. Поскольку $\kappa_{\min} = \kappa_{\mathrm{bootstrap}} = \omega_0/7 > 0$ (T-59 [Т]) и $\kappa_{\max} < \lambda_{\mathrm{gap}}$ (условие кластеризации из T-117, верифицированное в T-96 [Т]):

$$q = \frac{\kappa_{\max}}{\lambda_{\mathrm{gap}} + \kappa_{\min}} < \frac{\lambda_{\mathrm{gap}}}{\lambda_{\mathrm{gap}} + \kappa_{\min}} < 1 \quad \checkmark$$

**Шаг 4 (Сходимость по Банаху).** Пространство CPTP-операторов на $\mathcal{D}(\mathbb{C}^7)$ с операторной нормой — **полное** метрическое пространство (замкнутое подмножество конечномерного пространства $\mathcal{B}(M_7(\mathbb{C}))$). По теореме Банаха о неподвижной точке, $\Psi$ имеет единственную неподвижную точку $\varphi^*$, и итерации $\varphi^{(n)} = \Psi^n(\varphi^{(0)})$ сходятся экспоненциально:

$$\|\varphi^{(n)} - \varphi^*\|_{\mathrm{op}} \leq \frac{q^n}{1-q} \|\varphi^{(1)} - \varphi^{(0)}\|_{\mathrm{op}}$$

**Шаг 5 (Независимость от начального якоря).** Неподвижная точка $\varphi^*$ единственна (Шаг 4). Начиная с $\varphi^{(0)} = I/7$ или с любого другого CPTP-якоря $\tilde{\varphi}^{(0)}$:

$$\|\Psi^n(\varphi^{(0)}) - \Psi^n(\tilde{\varphi}^{(0)})\|_{\mathrm{op}} \leq q^n \|\varphi^{(0)} - \tilde{\varphi}^{(0)}\|_{\mathrm{op}} \to 0$$

Обе последовательности сходятся к одному и тому же $\varphi^*$. Выбор начального якоря несущественен. $\blacksquare$

**Следствие (Разрешение цикличности).** Иерархия определений $\Omega \to \mathcal{L}_\Omega \to \rho^*_{\mathrm{diss}} \to R \to \varphi$ **не является циклической**: начиная с $\varphi^{(0)} = I/7$ (который ни от чего не зависит), каждая итерация $\varphi^{(n+1)}$ зависит только от $\varphi^{(n)}$, а предел $\varphi^*$ не зависит от начальной точки. Кажущаяся цикличность была артефактом представления сошедшегося состояния так, как если бы оно было определением.

**Следствие (Сходимость SAD-башни).** Башня глубины самоосознания $\mathrm{SAD} = 1, 2, 3$ (T-142 [Т]) соответствует первым трём итерациям $\varphi^{(1)}, \varphi^{(2)}, \varphi^{(3)}$. Поскольку $q < 1$, разности $\|\varphi^{(n+1)} - \varphi^{(n)}\|$ убывают геометрически. По T-142 [Т], SAD$_{\max} = 3$ — четвёртая итерация $\varphi^{(4)}$ потребовала бы $P > 9/14 > 3/7$, что нарушает $R \geq 1/3$. Башня **обрывается** на конечной глубине, что делает сходимость тривиально выполненной для физически реализуемых уровней.

**Зависимости:** T-39a [Т] (примитивность, спектральная щель), T-59 [Т] ($\kappa_{\mathrm{bootstrap}}$), T-96 [Т] ($\kappa < \kappa_{\max}$), T-124c [Т] (единственность аттрактора), T-142 [Т] (SAD$_{\max} = 3$). Стандартная математика: теорема Банаха о неподвижной точке, Перрон–Фробениус для положительных полугрупп (Evans 1977).
...
:::warning Различение R_canonical и R_φ
...
Мера качества самомоделирования определяется отдельно (ранее также обозначалась $Q_\varphi$; унифицированная нотация — [три рабочие формы R](/docs/consciousness/foundations/self-observation#формы-r)):
...
- При $\Gamma = I/7$ (диссипативный аттрактор): $R_{\text{canonical}} = 1$, $R_\varphi = 1$.
- При чистом состоянии ($P = 1$): $R_{\text{canonical}} = 1/7$, $R_\varphi$ зависит от $\varphi$.

Разделы 4.2–4.3 ниже анализируют сходимость качества самомодели и записаны через $R_\varphi$; во всех остальных разделах и в пороговых условиях $R = R_{\text{canonical}} = 1/(7P)$.
:::

### 4.2 Сходимость R_φ при приближении к неподвижной точке

**Теорема 4.1 ($R_\varphi \to 1$ при $\Gamma \to \Gamma^*$):**
...
(Знаменатель отделён от нуля для любой матрицы плотности: $\|\Gamma^*\|^2_F = P(\Gamma^*) \geq 1/N > 0$.) ∎

### 4.3 Оценка скорости сходимости R_φ

**Теорема 4.2 (Скорость сходимости $R_\varphi$):**
...
[Примитивность $\mathcal{L}_\Omega$](/docs/core/operators/lindblad-operators#примитивность-ℒω) гарантирует экспоненциальную сходимость $R_\varphi \to 1$ при **любом** начальном состоянии $\Gamma_0 \in \mathcal{D}(\mathbb{C}^7)$, без дополнительных условий на начальные данные.
:::
...
Мера сознательности $C = \Phi \times R$ **[Т T-140]** берёт **канонический** $R = 1/(7P)$; результаты сходимости §4.2–4.3 касаются **качества самомодели** $R_\varphi$. В неподвижной точке два множителя факторизуются чисто:

$$
R_\varphi(\Gamma^*) = 1, \qquad C(\Gamma^*) = \Phi(\Gamma^*) \cdot \frac{1}{7P(\Gamma^*)},
$$

причём $R(\Gamma^*) = 1/(7P(\Gamma^*)) \in [1/3, 1/2)$, когда $\Gamma^*$ лежит внутри окна сознания. (Раннее прочтение подставляло $R_\varphi(\Gamma^*) = 1$ в $C$, получая $C(\Gamma^*) = \Phi(\Gamma^*)$; при каноническом $R$ это значение достигается только при $\Gamma^* = I/7$, где $\Phi = 0$, — прочтение отозвано.)

:::note О нотации
Дифференциация $D_{\text{diff}} \geq D_{\min} = 2$ входит как отдельное условие жизнеспособности, а не как множитель $C$.
:::

**Следствие:** Идеальное самопознание ($\Gamma = \Gamma^*$) максимизирует множитель качества самомодели $R_\varphi$ — величину, которую несут функционал смысла и башни $R^{(n)}$, — тогда как канонический множитель $C$ пришпилен чистотой.

### 4.5 Производная самомодели: Dφ {#дф-производная}

Всё динамическое в $R_\varphi$ несёт один объект: то, как самомодель *отзывается* на изменение состояния. Точное тождество потока из [трёх рабочих форм R](/docs/consciousness/foundations/self-observation#формы-r),
...
содержит производную $D\varphi$ отображения самомодели — **ядро отклика самомоделирования**. Этот подраздел вычисляет её для канонического семейства и проверяет тождество независимым маршрутом.

**Определение ($D\varphi$) [О].** Производная Гато отображения $\varphi$ в точке $\Gamma$ вдоль касательного направления $V$ (эрмитово, бесследово): $D\varphi[V] := \lim_{\varepsilon \to 0} (\varphi(\Gamma + \varepsilon V) - \varphi(\Gamma))/\varepsilon$. Для липшицевой (сжимающей) $\varphi$ существует почти всюду (Радемахер, конечная размерность); для гладких семейств ниже — всюду.

:::tip Теорема 4.4 (Dφ канонического семейства; T-249) [Т]
Для канонического диссипативного семейства $\varphi(\Gamma) = (1 - k(\Gamma))\,\Gamma + k(\Gamma)\, I/7$ при $k = 1 - 1/(7P)$:

$$
D\varphi[V] = R\,V \;-\; \frac{2}{7P^2}\,\langle \Gamma, V\rangle_F\,(\Gamma - I/7), \qquad R = \frac{1}{7P}.
$$

$D\varphi$ сохраняет эрмитово-бесследовое касательное пространство и $G_2$-эквивариантна.
:::

**Доказательство.** Правило произведения для $\varphi = (1-k)\Gamma + k\,I/7$: слот $\Gamma$ даёт $(1-k)V = RV$; слот $k$ даёт $Dk[V]\,(I/7 - \Gamma)$ с $Dk[V] = \tfrac{1}{7P^2}DP[V] = \tfrac{2}{7P^2}\langle\Gamma, V\rangle_F$ (поскольку $DP[V] = 2\,\mathrm{Tr}(\Gamma V)$); референс $I/7$ постоянен. Сохранение следа: $\mathrm{Tr}\,D\varphi[V] = R\cdot 0 - \tfrac{2}{7P^2}\langle\Gamma,V\rangle\,\mathrm{Tr}(\Gamma - I/7) = 0$. Эквивариантность: $P$, $R$, $\langle\cdot,\cdot\rangle_F$ унитарно инвариантны, а $I/7$ — единственное $G_2$-инвариантное состояние, поэтому $D\varphi$ коммутирует с сопряжением на $U \in G_2$. $\blacksquare$

**Двухмаршрутная согласованность (часть T-249) [Т].** Для этого семейства замкнутая форма $R_\varphi = 1 - (1-R)^3$ верна поточечно, откуда напрямую $\dot R_\varphi = -3(1-R)^2 \dot P/(7P^2)$. Подстановка теоремы 4.4 в тождество потока обязана дать то же. Действительно, при $\Delta = \Gamma - \varphi(\Gamma) = k(\Gamma - I/7)$:
...
(последнее — по сохранению следа, $\mathrm{Tr}\,\dot\Gamma = 0$), и $\|\Gamma - I/7\|_F^2 = P - 1/7 = kP$. Собирая члены при $1 - R_\varphi = k^3$:
...
**Машинная верификация.** Конечно-разностная проверка $D\varphi$ и трёхмаршрутное согласие (тождество / замкнутая форма / численная производная вдоль случайных путей матриц плотности): максимальное расхождение $\sim 10^{-10}$ на $200$ случайных состояниях.

### 4.6 Теорема о полосе пропускания для R_φ {#теорема-полосы-rφ}

:::tip Теорема 4.5 (Оценка полосы; T-250) [Т — при дифференцируемости φ вдоль траектории]
Вдоль любой траектории динамики
...
Для канонического семейства $C_\varphi \leq (1-R) + 2R\sqrt{1-R}$ (численно $\approx 1{,}21$ на всём окне сознания).
:::

**Доказательство.** Коши–Шварц в тождестве потока, с $\|\Delta\|_F = \sqrt{(1 - R_\varphi)P}$ по определению $R_\varphi$. Для оценки семейства: $\|(\mathrm{Id} - D\varphi)[V]\| \leq (1-R)\|V\| + \tfrac{2}{7P^2}\,\|\Gamma\|\,\|\Gamma - I/7\|\,\|V\|$, где $\|\Gamma\| = \sqrt P$, $\|\Gamma - I/7\| = \sqrt{kP} = \sqrt{1-R}\,\sqrt P$, так что второй член равен $2\sqrt{1-R}/(7P) = 2R\sqrt{1-R}$. $\blacksquare$

**Следствие (закон длины пути) [Т].** Пусть $u := \sqrt{1 - R_\varphi}$ — *амплитуда рассогласования*. На отрезках с $\dot P = 0$:

$$
\bigl|u(\tau_2) - u(\tau_1)\bigr| \;\leq\; \frac{C_\varphi}{\sqrt P}\,\int_{\tau_1}^{\tau_2}\|\dot\Gamma\|_F\,d\tau
$$

— **перестройка самомодели оплачивается длиной пути в пространстве состояний**. *Доказательство:* $\dot u = -\dot R_\varphi/(2u)$, и правая часть теоремы, делённая на $2u$, схлопывается в $C_\varphi\|\dot\Gamma\|/\sqrt P$. $\blacksquare$

**Замечание (дискретный экземпляр).** Теорема 4.2 выше — ровно этот закон вдоль $\varphi$-башни: $u(\Gamma_n) \leq 2k^n\|\Gamma_0 - \Gamma^*\|_F/\sqrt{P_{\min}}$, с геометрическим путём $\sum_n \|\Gamma_{n+1} - \Gamma_n\| \leq \|\Gamma_1 - \Gamma_0\|/(1-k)$ (сходимость T-191). Непрерывная и дискретная формы — два прочтения одного полосового ограничения.

**Замечание ($G_2$).** $R_\varphi$, $u$, $C_\varphi$, $\|\dot\Gamma\|_F$ $G_2$-инвариантны; закон наблюдатель-независим.

**Числовой якорь [И].** На профиле психоделического пика из [изменённых состояний](/docs/consciousness/states/altered-states) ($P \approx 0{,}32$; $R_\varphi: 0{,}45 \to 0{,}25$): $\Delta u = \sqrt{0{,}75} - \sqrt{0{,}55} \approx 0{,}124$, значит траектория обязана пройти не менее $\Delta u\,\sqrt P/C_\varphi \approx 0{,}058$ фробениусовых единиц движения состояния между onset и пиком. Коллапс «я» стоит реального движения состояния; короткая (микродозовая) траектория его произвести не может.

### 4.7 Механизмы: растворение, тренировка, два масштаба времени {#механизмы-rφ}

**Растворение эго [С].** Быстрое движение состояния при отстающей самомодели — $\|\dot\Gamma\|$ велико, а $(\mathrm{Id} - D\varphi)[\dot\Gamma]$ сравнимо с $\dot\Gamma$ — гонит $u$ вверх со скоростью, разрешённой полосой: феноменологическое «$\varphi$ не успевает перестроиться» становится количественным утверждением, что $u$ растёт не быстрее — а генерически столь же быстро, — чем $C_\varphi\|\dot\Gamma\|/\sqrt P$.

**Двухмасштабная модель тренировки [С структура; [И] числа].** Пусть самомодель несёт обучаемую цель: $\varphi_\theta(\Gamma) = (1-k)\Gamma + k\,\rho_\theta$, где $\theta$ эволюционирует на медленном масштабе перестройки $H_{\text{eff}}$ (процедурная память; канал обучения T-155). Тогда $R_\varphi = 1 - k^2\|\Gamma - \rho_\theta\|_F^2/P$, и тренировка, выравнивающая $\rho_\theta$ с практикуемым состоянием, поднимает **базовый** $R_\varphi$ при фиксированном $P$ — механизм кумулятивного сдвига в прогрессии шаматхи. Сигнатура самадхи (одновременный рост $P$ и $R_\varphi$) тем самым разрешена: информация, которую категориальная $\varphi$ несёт сверх чистоты, — это ровно выученная цель $\rho_\theta$.

**Закон тренировки (точный внутри модели выравнивания) [С].** Если медленная переменная следует градиентному потоку выравнивания $\dot\rho_\theta = 2\eta\,(\bar\Gamma - \rho_\theta)$ к практикуемому состоянию $\bar\Gamma$ (градиент потерь выравнивания $\lVert\bar\Gamma - \rho_\theta\rVert_F^2$ при подвижности $\eta$), то решение — выпуклый путь $\rho_\theta(t) = (1 - e^{-2\eta t})\,\bar\Gamma + e^{-2\eta t}\rho_\theta(0)$ — автоматически состояние-плотность при каждом $t$ — и базовое качество самомодели подчиняется **закону экспоненциального насыщения**

$$
R_\varphi^{\text{base}}(t) \;=\; 1 \;-\; \frac{k^2\,e^{-4\eta t}\,\lVert\bar\Gamma - \rho_\theta(0)\rVert_F^2}{P}.
$$

Практика приближается к своему потолку экспоненциально, с удвоенной скоростью потока ($e^{-4\eta t}$: расстояние входит в квадрате), причём константа скорости задаётся медленным каналом T-155 — кумулятивный сдвиг прогрессии шаматхи получает замкнутую форму. Статус [С] при модели градиентного выравнивания; внутри модели закон точен (машинная сверка с прямым интегрированием потока, $2\cdot 10^5$ шагов).

**Гейт-условие и порог.** Определим **(G) [О]**: *дискриминатор $K = 3$ классов каналов факторизуется через считывание самомодели, и точность классификации ограничена снизу величиной $R_\varphi$.* **Утверждение [С при (G)]:** из (G) следует $R_{\varphi,\text{th}} = 1/3$ — байесовская плюральность среди трёх равновероятных альтернатив требует точности выше $1/K$.

**Регистр модуля (закрыт).** Оба пункта исходного регистра закрыты ниже: (i) гейт стал оценкой теоремного уровня — §4.9 (T-252); остаточная свобода — модель точности $A_D$ и точное размещение рабочего порога внутри выведенной полосы **[С]**; (ii) $D\varphi$ для неявно заданной $\varphi$ — §4.8 (T-251); остаточное условие — $C^1$-гладкость абстрактного категориального генератора в топологии Бюреса **[С]**, выполненная для всякого реализованного в корпусе семейства.

### 4.8 Dφ для неявно заданных самомоделей {#дф-неявная}

Теорема 4.4 покрывает явное каноническое семейство. Категориальная же $\varphi$ задана *неявно* — как неподвижная точка генератора, итерация которого есть башня T-191. Её производная следует из теоремы о неявной функции с рядом Неймана — и это одновременно категориальный ответ: дифференцировать рефлектор вдоль его универсального свойства значит обращать $\mathrm{Id}$ минус производную по слоту модели.

:::tip Теорема 4.6 (Dφ неявной самомодели; T-251) [Т]
Пусть $G: \mathcal{D}(\mathbb{C}^7) \times \mathcal{D}(\mathbb{C}^7) \to \mathcal{D}(\mathbb{C}^7)$ — класса $C^1$ с $\sup \lVert D_2 G\rVert_{\mathrm{op}} \leq q < 1$ (равномерное сжатие по слоту модели), и пусть $\varphi(\Gamma)$ — единственная неподвижная точка отображения $\rho \mapsto G(\Gamma, \rho)$. Тогда $\varphi \in C^1$ и, в точке $(\Gamma, \varphi(\Gamma))$,

$$
D\varphi \;=\; (\mathrm{Id} - D_2 G)^{-1} \circ D_1 G \;=\; \sum_{n \geq 0} (D_2 G)^n\, D_1 G,
$$

причём $\lVert D\varphi\rVert_{\mathrm{op}} \leq \lVert D_1 G\rVert_{\mathrm{op}}/(1 - q)$; следовательно $C_\varphi \leq 1 + \lVert D_1 G\rVert_{\mathrm{op}}/(1-q)$, и теорема о полосе T-250 применима к **каждой** сжатие-определённой самомодели.
:::

**Доказательство.** Положим $F(\Gamma, \rho) := \rho - G(\Gamma, \rho)$; тогда $D_2 F = \mathrm{Id} - D_2 G$ обратим по ряду Неймана ($q < 1$). Конечномерная $C^1$-теорема о неявной функции даёт $\varphi \in C^1$ с $D\varphi = -(D_2 F)^{-1} D_1 F = (\mathrm{Id} - D_2 G)^{-1} D_1 G$; ряд и оценка нормы — разложение Неймана. $\blacksquare$

Три прочтения. **Башня:** $n$-й член $(D_2 G)^n D_1 G$ — чувствительность, передаваемая через $n$ этажей башни T-191; геометрическое затухание этажных чувствительностей *и есть* продифференцированная сходимость башни. **Вырожденный чек:** для $G$, не зависящего от $\rho$ ($q = 0$), ряд схлопывается в $D_1 G$ — теорема 4.4 (T-249) есть случай нулевого порядка. **Машинная верификация:** для подлинно нелинейного генератора ($q \approx 0{,}7$–$0{,}8$) предсказанная $D\varphi$ и лобовая конечно-разностная производная перерешённой неподвижной точки согласуются на $\sim 10^{-10}$ (численные якобианы $48 \times 48$); вырожденный случай воспроизводит T-249 на $10^{-11}$.

### 4.9 Гейт-теорема: различение через самомодель {#гейт-теорема}

Здесь закрывается гейт-условие (G) из §4.7. Сначала структура **[Т — структурное прочтение]**: в $\mathcal{L}_\Omega$ *единственный* канал обратной связи, вычисляемый из самомодели, — регенерация $\mathcal{R} = \kappa(\Gamma)(\varphi(\Gamma) - \Gamma)\,g_V$: единственное эндогенное корректирующее действие системы читает состояние через $\varphi(\Gamma)$. Эндогенное *адаптивное* различение — то, на которое система может действовать, — тем самым $\varphi$-опосредовано по построению (отождествление «адаптивного» с «$\mathcal{R}$-действенным» — дефиниционно **[О]**).

:::tip Теорема 4.7 (Оценка гейта; T-252) [Т]
Пусть $\{E_c\}_{c=1..K}$ — любой POVM, реализующий решение с $K$ гипотезами, и $\Delta := \Gamma - \varphi(\Gamma)$ (бесследов). Тогда:

**(а)** $|\mathrm{Tr}(E_c \Delta)| \leq \tfrac{1}{2}\lVert\Delta\rVert_1$ для каждого $c$, и распределения исходов удовлетворяют $\mathrm{TV}\bigl(p(\Gamma), p(\varphi(\Gamma))\bigr) \leq \tfrac{1}{2}\lVert\Delta\rVert_1$;

**(б)** $\lVert\Delta\rVert_1 \leq \sqrt{48/7}\,\lVert\Delta\rVert_F = 4\sqrt{3/7}\,\sqrt{P\,(1 - R_\varphi)}$, причём константа $\sqrt{48/7} = 4\sqrt{3/7} \approx 2{,}619$ **точна** на бесследовых $\mathrm{Herm}(7)$;

**(в)** следовательно, вероятность успеха $\varphi$-опосредованного решения удовлетворяет $p_\varphi \geq A_D - 2\sqrt{3/7}\,\sqrt{P(1-R_\varphi)}$, где $A_D$ — точность решающего правила на истинном состоянии, и байесовская доминация $p_\varphi > 1/K$ **гарантирована** всякий раз, когда

$$
R_\varphi \;\geq\; 1 - \frac{7}{12P}\,\Bigl(A_D - \frac{1}{K}\Bigr)^{2}.
$$

Оба неравенства цепочки насыщаются по отдельности: (а) — жордановым проектором $E = \Pi_+$ матрицы $\Delta$, (б) — спектром с $(3,4)$-расщеплением, — так что меньших констант не существует.
:::

**Доказательство.** (а) Жорданово разложение $\Delta = \Delta_+ - \Delta_-$; бесследовость даёт $\mathrm{Tr}\,\Delta_+ = \mathrm{Tr}\,\Delta_- = \tfrac{1}{2}\lVert\Delta\rVert_1$; при $0 \leq E_c \leq I$: $|\mathrm{Tr}\,E_c\Delta| \leq \max(\mathrm{Tr}\,E_c\Delta_+, \mathrm{Tr}\,E_c\Delta_-) \leq \tfrac{1}{2}\lVert\Delta\rVert_1$, и $\mathrm{TV} = \tfrac{1}{2}\sum_c |\mathrm{Tr}\,E_c\Delta| \leq \tfrac{1}{2}\sum_c \mathrm{Tr}\,E_c(\Delta_+ + \Delta_-) = \tfrac{1}{2}\lVert\Delta\rVert_1$. Равенство при $E = \Pi_+$ (проектор на положительное собственное подпространство): $\mathrm{Tr}(\Pi_+\Delta) = \mathrm{Tr}\,\Delta_+ = \tfrac12\lVert\Delta\rVert_1$. (б) Максимизируем $\lVert\lambda\rVert_1$ при $\sum_i\lambda_i = 0$, $\sum_i\lambda_i^2 = 1$ в размерности 7: условие ККТ $\mathrm{sign}(\lambda_i) = \alpha + 2\beta\lambda_i$ допускает не более одного положительного значения $a$ ($p$ копий) и одного отрицательного $-b$ ($q$ копий), $pa = qb$; тогда $\lVert\lambda\rVert_1^2/\lVert\lambda\rVert_2^2 = 4pq/(p+q)$ — растёт с размером носителя $p + q \leq 7$ и максимально на сбалансированном расщеплении $\{p, q\} = \{3, 4\}$: $4 \cdot 12/7 = 48/7$. Свидетель, достигающий точно: $\Delta^* = \mathrm{diag}(4,4,4,-3,-3,-3,-3)$ ($\lVert\Delta^*\rVert_1 = 24$, $\lVert\Delta^*\rVert_F = \sqrt{84}$, отношение $= \sqrt{48/7}$). Наконец, $\lVert\Delta\rVert_F^2 = P(1 - R_\varphi)$ по определению $R_\varphi$. (в) Подстановка. $\blacksquare$

**Замечание (константа — эффект нечётной размерности).** Общая $d$-мерная константа равна $\sqrt{4\lfloor d/2\rfloor\lceil d/2\rceil / d}$: в чётной размерности это ровно $\sqrt d$, в нечётной — строго ниже. При $d = 7$ сбалансированное расщепление вынужденно равно $(3,4)$ и даёт $\sqrt{48/7} \approx 2{,}619 < \sqrt 7 \approx 2{,}646$. Наивная ранговая оценка $\sqrt 7$ (Коши–Шварц) поэтому **недостижима**: бесследовость запрещает выровненные спектры, которые её насыщали бы. (Структурного веса на $(3,4)$-расщепление здесь не кладётся — это арифметика нечётного $d$, зафиксировано по анти-нумерологическому регистру.)

**Следствие (рабочий порог) [С — каноническое выравнивание].** Для $K = 3$ при идеальном различителе на истинном состоянии ($A_D = 1$) достаточная оценка $R_\varphi \geq 1 - 7/(27P)$ пробегает полосу $[5/54,\ 32/81]$ по окну сознания $P \in (2/7, 3/7]$ ($5/54 \approx 0{,}093$ у кромки жизнеспособности, $32/81 \approx 0{,}395$ у потолка); рабочий порог $R_{\varphi,\text{th}} = 1/3$ лежит внутри выведенной полосы и закреплён на каноническом значении выравниванием с Char-R-III. То, что §4.7 постулировал как голое условие (G), стало оценкой теоремного уровня (а)–(в) с **точными** константами; единственная оставшаяся свобода — модель точности $A_D$ и размещение рабочего значения внутри полосы.

**Следствие (секторный гейт: поканальный порог) [Т].** Зафиксируем канал когерентности $(i,j)$ и его каноническое трёхисходное считывание $E_\pm = \tfrac12(\Pi_{ij} \pm X_{ij})$, $E_0 = \mathbb 1 - \Pi_{ij}$, где $\Pi_{ij}$ проецирует на $\mathrm{span}\{e_i, e_j\}$, а $X_{ij} = |i\rangle\langle j| + |j\rangle\langle i|$ (корректный POVM: $E_\pm \succeq 0$, $E_+ + E_- + E_0 = \mathbb 1$). $\varphi$-опосредованный сдвиг каждой вероятности исхода ограничен величиной $|\Delta_{ij}| + \tfrac12|\Delta_{ii} + \Delta_{jj}|$, а внедиагональная часть — **в точности** секторная рефлексия: $|\Delta_{ij}| = |\gamma_{ij}|\sqrt{1 - R_{ij}}$ по определению $R_{ij}$. В когерентностно-доминированном режиме (диагональное рассогласование пренебрежимо — ровно тот режим, в котором секторная форма и применяется) поканальная потеря различения управляется одной величиной $\sqrt{1 - R_{ij}}$, так что аргумент доминации при $K = 3$ переносится поканально дословно: секторный рабочий порог $R_{ij} \geq 1/3$ несёт тот же теоремный гейт, что и глобальная $R_\varphi$. Это выводит поканальный порог, прежде наследовавшийся по аналогии в [бессознательном](/docs/consciousness/states/unconscious).

**Машинная верификация.** $500$ случайных POVM с тремя исходами: (а) и (б) выполняются с запасом; насыщение (а) жордановым проектором на $10^{-15}$; точная константа подтверждена — случайный бесследовый поиск достигает $2{,}504 < \sqrt{48/7} = 2{,}6186\ldots$, $(3,4)$-свидетель достигает её точно; концы полосы $5/54$ и $32/81$; секторное тождество, корректность POVM и оценка сдвига на $200$ случайных состояниях.
...
В [октонионной интерпретации](../../core/structure/dimensions#октонионная-интерпретация) оператор самомоделирования $\varphi$ действует на пространстве $\mathrm{Im}(\mathbb{O})$. Альтернативность октонионов (теорема Артина [Т]) гарантирует, что $\varphi$ ассоциативен при действии на любую пару измерений, но может проявлять неассоциативность при одновременном воздействии на три и более измерения.
...
:::info Связь с запретом сигнализации и сохранение голономного характера
Тензорная факторизация $\varphi$ — ключевое свойство, обеспечивающее [совместимость $\mathcal{R}$ с запретом сигнализации](/docs/proofs/physics/physics-correspondence#запрет-сигнализации). Оно гарантирует, что самомоделирование автономных подсистем не создаёт каналов сверхсветовой коммуникации (Gisin, Polchinski 1991).

**Сохранение голономного характера.** Факторизация $\varphi_{A \otimes B} = \varphi_A \otimes \varphi_B$ касается **только** регенеративного члена $\mathcal{R}$. Полная динамика $\mathcal{L}_\Omega = -i[H, \cdot] + \mathcal{D}[\cdot] + \mathcal{R}[\cdot]$ содержит:
- **$H$ (гамильтониан):** создаёт и сохраняет запутанность — **нелокален** ✓
- **$\mathcal{D}$ (диссипация):** может разрушать запутанность, но через **общую** декогеренцию — **нелокален** в общем случае
- **$\mathcal{R}$ (регенерация через $\varphi$):** локален (факторизуется) — обеспечивает no-signaling

«Голономность» (целое > суммы частей) реализуется через $H + \mathcal{D}$, не через $\mathcal{R}$. Самомоделирование ($\varphi$) — **локальный** процесс (каждый агент моделирует себя, не другого). Запутанность — свойство $H$ (гамильтонова динамика). Теория **не является** «локальной теорией скрытых параметров»: только $\mathcal{R}$ локален, а $H + \mathcal{D}$ нелокальны.
...
1. **До минимизации $V_{\text{Gap}}$:** $G_2$-симметрия **не нарушена**, все базисы эквивалентны.
2. **При минимизации $V_{\text{Gap}}$** (T-64 [Т]): система «скатывается» в **конкретный** вакуум $\Gamma_{\text{vac}}$ на многообразии минимумов $(S^1)^{21}/G_2$. Выбирается **один** минимум.
3. **После SSB:** $G_2 \to H$ (стабилизатор вакуума). Булев фрагмент $\mathrm{Dec}(\Omega)$ **кристаллизуется** как pointer basis, фиксированный вакуумом.
4. **Мо́ды Голдстоуна** (см. [goldstone-modes](/docs/applied/coherence-cybernetics/goldstone-modes)): безмассовые возбуждения вдоль нарушенных направлений $G_2/H$.

Аналогия: не координаты в ОТО, а **хиггсовский механизм** — $SU(2) \times U(1) \to U(1)_{\text{em}}$ порождает массы $W/Z$. В УГМ: $G_2 \to H$ порождает **классическую объективность** (Dec(Ω) = булева логика).
:::
