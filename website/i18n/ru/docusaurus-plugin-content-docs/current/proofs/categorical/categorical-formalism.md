...
15. [L-унификация](#l-унификация)
    - [15.3 Сопряжение $\mathcal{D}_Ω$ ⊣ ℛ](#сопряжение-adjunction)
...
:::note Зависимость от внешнего каркаса (см. [Стратификация строгости §T-76](/docs/reference/status-registry#стратификация-строгости))
...
#### Теорема T-192 (Exp^(2) — строгая 2-категория) [Т] {#t-192-exp2-2-категория}
...
:::warning Область применимости: расширение на Exp слабее сайт-уровневого результата
Сайт-уровневая часть T-76 — что $(\mathbf{DensityMat}, J_{Bures})$ является сайтом Гротендика и $\mathbf{Sh}_\infty(\mathbf{DensityMat}, J_{Bures})$ — ∞-топос — имеет статус [T] благодаря прямой проверке аксиом в [§6.3.1](#631-bures-топология-на-densitymat) плюс HTT 6.2.2.7. Расширение на $\mathbf{Sh}_\infty(\mathbf{Exp})$ здесь — **Утверждение 10.2 (требует проверки)**: полная Жиро-проверка (малые копределы, эффективные объединения, спуск) для сайта $\mathbf{Exp}$ находится в ожидании. См. [Стратификация строгости §T-76](/docs/reference/status-registry#стратификация-строгости).
:::
...
Для состояния Γ ∈ $\mathrm{Ob}(\mathcal{C})$ ∞-категорное определение:
...
### 15.3 Сопряжение $\mathcal{D}_Ω$ ⊣ ℛ и вывод κ₀ {#сопряжение-adjunction}
...
**Тождество 1:** $(\varepsilon_{\mathcal{R}(S)}) \circ (\mathcal{R}(\eta_S)) = \mathrm{id}_{\mathcal{R}(S)}$ для всех $S \in \mathbf{Set}$.

**Доказательство тождества 1.** Пусть $S \in \mathbf{Set}$. Тогда $\mathcal{R}(S) = \bigoplus_{s \in S} \Omega_s$ — свободный $\Omega$-пучок на $S$. Единица $\eta_{\mathcal{R}(S)}: \mathcal{R}(S) \to \mathcal{R}(\mathcal{D}_\Omega(\mathcal{R}(S)))$ вкладывает $\bigoplus_s \Omega_s$ в $\bigoplus_{\chi \in \mathrm{Hom}(\mathcal{R}(S), \Omega)} \Omega_\chi$. Каждый генератор $\Omega_s$ свободного пучка отображается в компоненту $\Omega_{\pi_s}$, где $\pi_s: \mathcal{R}(S) \to \Omega$ — проекция на $s$-ю слагаемую. Коединица $\varepsilon_S: \mathcal{D}_\Omega(\mathcal{R}(S)) \to S$ отображает каждый предикат $\chi$ в элемент $s \in S$, индексирующий слагаемое, на которое $\chi$ проецирует. Композиция $\varepsilon_{\mathcal{R}(S)} \circ \mathcal{R}(\eta_S)$ отправляет $\Omega_s \xrightarrow{\eta} \Omega_{\pi_s} \xrightarrow{\varepsilon} \Omega_s$, что есть тождество на каждом генераторе. По универсальности копроизведения, композиция равна $\mathrm{id}_{\mathcal{R}(S)}$. $\checkmark$

**Тождество 2:** $(\mathcal{D}_\Omega(\varepsilon_\Gamma)) \circ (\eta_{\mathcal{D}_\Omega(\Gamma)}) = \mathrm{id}_{\mathcal{D}_\Omega(\Gamma)}$ для всех $\Gamma \in \mathbf{Sh}_\infty(\mathcal{C})$.

**Доказательство тождества 2.** Пусть $\Gamma \in \mathbf{Sh}_\infty(\mathcal{C})$ и $T = \mathcal{D}_\Omega(\Gamma) = \mathrm{Hom}(\Gamma, \Omega)$. Единица $\eta_T: T \to \mathcal{D}_\Omega(\mathcal{R}(T)) = \mathrm{Hom}(\bigoplus_{\chi \in T} \Omega_\chi, \Omega)$ отображает каждый предикат $\chi \in T$ в морфизм вычисления $\mathrm{ev}_\chi: \bigoplus_{\chi'} \Omega_{\chi'} \to \Omega$, который проецирует на $\chi$-ю компоненту. Коединица $\varepsilon_\Gamma: \Gamma \to \mathcal{R}(T)$ — каноническое вложение (из шага (b)). Функтор $\mathcal{D}_\Omega$, применённый к $\varepsilon_\Gamma$, даёт $\mathcal{D}_\Omega(\varepsilon_\Gamma): \mathrm{Hom}(\mathcal{R}(T), \Omega) \to \mathrm{Hom}(\Gamma, \Omega)$ через предкомпозицию: $f \mapsto f \circ \varepsilon_\Gamma$. Композиция $\mathcal{D}_\Omega(\varepsilon_\Gamma) \circ \eta_T$ отправляет $\chi \mapsto \mathrm{ev}_\chi \circ \varepsilon_\Gamma = \chi$ (поскольку $\varepsilon_\Gamma$ вкладывает $\Gamma$ в $\bigoplus \Omega_\chi$ и $\mathrm{ev}_\chi$ извлекает $\chi$-компоненту, восстанавливая исходный предикат). Следовательно, композиция равна $\mathrm{id}_T$. $\checkmark$

Оба треугольных тождества выполнены, что завершает доказательство того, что $\mathcal{D}_\Omega \dashv \mathcal{R}$ является корректным сопряжением. $\blacksquare$
...
| Вывод κ₀ | Единица сопряжения $\mathcal{D}_Ω$ ⊣ ℛ | §15.3 |
...
15. **L-унификация** — L ≅ Ω ≅ source(L_k), вывод κ₀ из сопряжения $\mathcal{D}_Ω$ ⊣ ℛ (раздел 15)
...
| Вывод κ₀ | Единица сопряжения $\mathcal{D}_Ω$ ⊣ ℛ (раздел 15) |
