...
- **При $\iota_\mathrm{max}$**: $\mathcal T$ имеет единое глобальное состояние $\Gamma_\mathrm{tot}$; `spawn_child` — унитарное вложение $\mathcal{D}(\mathbb C^{7k}) \hookrightarrow \mathcal{D}(\mathbb C^{7(k+1)})$, сохраняющее $\Gamma_\mathrm{tot}$. Фильтрованный копредел по башне существует в $\mathbf{Sh}_\infty(\mathcal C)$ (по кополноте представимых $\infty$-категорий, HTT 5.5.1). Ординальная глубина не ограничена — $\omega^\omega$ достижим для башен длины $\omega^\omega$, **при условии**:
  ...
  - T-204 [Т]: ограниченная рациональность даёт плавную деградацию при лимите $d_\mathrm{eff}$.
...
**Часть (iv)** следует из T-211 [Т]: $\mathfrak{T}$ — полная $(\infty,1)$-подкатегория лурьевского $\mathbf{Topoi}_\infty$, поэтому наследует все аксиомы Giraud, а следовательно descent.
