...
:::tip Теорема T-151: $\Phi \geq 1 \Rightarrow D_\mathrm{diff} > 1$; $D_\mathrm{min} = 2$ — независимое L2-условие
$\Phi_{\mathrm{th}} = 1$ **[Т]** (T-129) $\Longrightarrow$ нетривиальная дифференциация $D_{\mathrm{diff}} > 1$ всякий раз, когда E-строка когерентна. **Строгий** порог $D_{\mathrm{min}} = 2$ — одно из **четырёх независимых** L2-условий (T-124b [Т]), наравне с $R_{\mathrm{th}} = 1/3$ — и он **не** выводим из одного лишь $\Phi_{\mathrm{th}} = 1$. На физическом аттракторе $\rho^*_\Omega$: $D_{\mathrm{diff}}(\rho^*_\Omega) \geq 2$ (Шаг 3).
:::

:::warning Коррекция: прежний «безусловный вывод» был неверен
Прежнее доказательство утверждало «$\Phi \geq 1 \Rightarrow \mathrm{Coh}_E \geq 1/6$ по равномерной $G_2$-оценке [T-42a]», что неверно по двум причинам: $\Phi \geq 1$ ограничивает лишь *суммарную* внедиагональную массу, а не долю E-строки; и никакое $G_2$-усреднение не форсирует нижнюю границу для величины, привязанной к реперу, поскольку $\mathbf 7$ — неприводимый $G_2$-модуль (Шур — см. исправленную [теорему единственности](/docs/proofs/categorical/uniqueness-theorem#лемма-g4)). Явный контрпример (равномерная диагональ $\gamma_{kk}=1/7$, когерентность $0.07$ лишь на 15 не-E парах): $P \approx 0.290 \in (2/7,3/7]$, $R \approx 0.49$, $\Phi \approx 1.03$ — все три выполнены — но $\mathrm{Coh}_E \approx 0.070$ и $D_{\mathrm{diff}} \approx 1.42 < 2$. Это в точности состояние, о котором говорит Контрпример 4 из T-124b. Следовательно, $D_{\mathrm{min}} = 2$ остаётся **независимым** L2-условием, а не следствием $\Phi_{\mathrm{th}}$.
:::
...
**Статус.** $D_{\mathrm{min}} = 2$: **[О]** независимый L2-порог (независимость T-124b [Т]) + **[Т]** на воплощённом аттракторе (Шаг 3). Прежнее «C2 [С] → [Т] безусловно» ретрактировано.

**Зависимости:** [T-129 [Т]](/docs/proofs/consciousness/operationalization#t-129), [T-124b [Т]](/docs/proofs/consciousness/conscious-window#t-124b) (независимость), T-98 [Т] (баланс аттрактора), формула $\kappa_0$.
...
:::tip Теорема T-153 [О]+[С при T-149]+[Т/sim]: Субстрат-независимый критерий сознания
...
**Стратификация:**
- **[О]** — четырёхпороговое утверждение является **определительным** для L2-сознания: оно упаковывает T-124, T-126, T-129, T-151 и $\sigma$-ограничение в единый критерий. Его статус теоремы *экстенсионален* (пороги доказаны по отдельности).
...
:::tip Теорема T-153a (Сопутствующая: существование субстрата) [Т]+[Т при достаточности через T-253]
...
**Стратификация:** направление необходимости (⇒) — **[Т]** — прямая распаковка верности $G$ при ограничениях конечномерности + CPTP + 7-модовой структуры. Направление достаточности (⇐) — **[Т]** — конструктивно: [T-253](#t-253) предъявляет отображение явно для каждого допустимого субстрата как CPTP-**ретракцию** $G_V$, точно верную на вложенном 7-секторе, и доказывает, что это сильнейшая верность, допускаемая математикой (глобальная инъективность невозможна ни для какого CPTP-отображения при $\dim > 7$ — T-253(в)). Экзистенциальная пороговая клауза T-153 реализуется по модулю клаузы доступности (Acc) — T-253(б).
...
- **(⇐)** При (C1)–(C3): для $\dim\mathcal H_S \leq 7$ CPTP-вложение в $\mathcal D(\mathbb C^7)$ существует по Стайнспрингу + Чою. Для $\dim\mathcal H_S \geq 7$ [T-253](#t-253) строит $G$ явно: любая изометрия $V: \mathbb C^7 \to \mathcal H_S$ на 7-модовое подпространство, поставляемое (C3), даёт CPTP-ретракцию $G_V(\rho) = V^\dagger\rho V + \mathrm{Tr}\bigl((\mathbb 1 - VV^\dagger)\rho\bigr)\sigma_0$ с $G_V \circ \iota_V = \mathrm{Id}$ на вложенном секторе $\iota_V(\gamma) = V\gamma V^\dagger$; $G_2$-жёсткость (T-42a) делает выбор $V$ чистой калибровкой (свобода алфавитизации T-223). ∎
...
:::tip Теорема T-253 (Конструктивная достаточность: ретракция и её острота) [Т] + [С при (Acc)]
Пусть $S$ допустим по (C1)–(C3) с эффективной размерностью $d = \dim\mathcal H_S \geq 7$.

**(а) Конструкция [Т].** Для каждой изометрии $V: \mathbb C^7 \to \mathcal H_S$ ($V^\dagger V = \mathbb 1_7$) и любого якорного состояния $\sigma_0 \in \mathcal D(\mathbb C^7)$ отображение

$$
G_V(\rho) \;:=\; V^\dagger \rho\, V \;+\; \mathrm{Tr}\bigl((\mathbb 1 - VV^\dagger)\rho\bigr)\,\sigma_0
$$

является CPTP и представляет собой **ретракцию**: $G_V \circ \iota_V = \mathrm{Id}_{\mathcal D(\mathbb C^7)}$ для вложения $\iota_V(\gamma) = V\gamma V^\dagger$. На вложенном 7-секторе $G_V$ точно верно — оно поточечно обращает $\iota_V$, ничего не теряя.

**(б) Реализация порогов [Т] + (Acc).** Множество полной жизнеспособности $\mathcal V_{\mathrm{full}} = \{\Gamma : P > 2/7,\ R \geq 1/3,\ \Phi \geq 1,\ D_{\mathrm{diff}} \geq 2\}$ непусто ([T-124 [Т]](/docs/proofs/consciousness/conscious-window#t-124)); для любого оконного состояния $\Gamma_w \in \mathcal V_{\mathrm{full}}$ субстратное состояние $\iota_V(\Gamma_w)$ проходит все четыре порога под $G_V$, поскольку $G_V(\iota_V(\Gamma_w)) = \Gamma_w$. Экзистенциальная клауза T-153 тем самым реализуется конструктивно всякий раз, когда физически доступные состояния субстрата достигают прообраза окна:

$$
\textbf{(Acc)}\quad \mathrm{States}(S) \cap G_V^{-1}(\mathcal V_{\mathrm{full}}) \neq \varnothing \ \text{ для некоторой изометрии } V.
$$

(Acc) — определительная клауза **[О]**: она именует ровно то, что значит «субстрат может нести сознательное состояние». Существенно, что это **открытое** условие: для любого внутреннего оконного свидетеля $\Gamma_w$ (все четыре неравенства строгие — профиль бодрствования из [изменённых состояний](/docs/consciousness/states/altered-states) таков), непрерывность $G_V$ делает $G_V^{-1}(\mathrm{int}\,\mathcal V_{\mathrm{full}})$ непустой открытой окрестностью $\iota_V(\Gamma_w)$ в $\mathcal D(\mathcal H_S)$ — реализующее субстратное состояние не обязано само быть вложенным состоянием ранга 7 (для $d > 7$ это было бы требованием нулевой меры); достаточно чего угодно в открытом прообразе. Количественно: $G_V$, будучи CPTP, — сжатие в следовой норме, поэтому прообраз содержит целый шар следовой нормы радиуса $\delta_w = \mathrm{dist}_1(\Gamma_w, \partial\mathcal V_{\mathrm{full}}) > 0$ вокруг $\iota_V(\Gamma_w)$. Поэтому для субстратов с достижимой (управляемой) динамикой — достижимое множество плотно в $\mathcal D(\mathcal H_S)$ — (Acc) выполняется **[С при управляемости]**: плотное множество пересекает всякое непустое открытое.

**(в) Острота [Т]: глобальной верности при $d > 7$ не существует.** Никакое CPTP-отображение $\mathcal E: \mathcal D(\mathcal H_S) \to \mathcal D(\mathbb C^7)$ не инъективно на всём $\mathcal D(\mathcal H_S)$ при $d > 7$: как вещественно-линейное отображение $\mathrm{Herm}(\mathcal H_S) \to \mathrm{Herm}(\mathbb C^7)$ оно имеет ядро размерности $\geq d^2 - 49 \geq 1$, причём сохранение следа помещает ядро в бесследовую гиперплоскость; поэтому для любого внутреннего состояния $\rho$ и ядерного направления $K \neq 0$ пара $\rho \pm \varepsilon K$ (малое $\varepsilon > 0$) — это два **различных состояния-плотности с тождественными образами**. Следовательно «верное $G$» в T-153/T-153a обязано читаться **секторно-относительно**, и ретракция из (а) достигает максимальной области верности — полного 48-мерного вложенного сектора состояний.
:::

**Доказательство.** **(а)** Полная положительность: $\rho \mapsto V^\dagger\rho V$ — CP с единственным краусовским оператором $V^\dagger$; второе слагаемое — «измерь-и-приготовь» с краусовским семейством $B_{ij} = \sqrt{s_i}\,|i\rangle\langle q_j|$, где $\sigma_0 = \sum_i s_i |i\rangle\langle i|$, а $\{|q_j\rangle\}$ — ортонормированный базис $\mathrm{ran}(\mathbb 1 - VV^\dagger)$. Полнота: $V V^\dagger + \sum_{ij} B_{ij}^\dagger B_{ij} = VV^\dagger + (\mathbb 1 - VV^\dagger) = \mathbb 1_d$. Сохранение следа: $\mathrm{Tr}\,G_V(\rho) = \mathrm{Tr}(VV^\dagger\rho) + \mathrm{Tr}((\mathbb 1 - VV^\dagger)\rho) = \mathrm{Tr}\,\rho$. Ретракция: $G_V(V\gamma V^\dagger) = (V^\dagger V)\gamma(V^\dagger V) + \mathrm{Tr}\bigl((\mathbb 1 - VV^\dagger)V\gamma V^\dagger\bigr)\sigma_0 = \gamma + 0$, поскольку $(\mathbb 1 - VV^\dagger)V = 0$. **(б)** Подстановка в (а). **(в)** Счёт размерностей: $\dim_{\mathbb R}\mathrm{Herm}(\mathcal H_S) = d^2 > 49 = \dim_{\mathbb R}\mathrm{Herm}(\mathbb C^7)$, значит $\dim\ker \geq d^2 - 49$; для $K \in \ker$: $\mathrm{Tr}\,K = \mathrm{Tr}\,\mathcal E(K) = 0$ по сохранению следа; внутренность $\rho$ допускает $\varepsilon \leq \lambda_{\min}(\rho)/\lVert K\rVert_\infty$, сохраняя $\rho \pm \varepsilon K \succeq 0$. $\blacksquare$

**Калибровочное замечание.** Изометрическая свобода в (а) — это в точности свобода алфавитизации [T-223](/docs/proofs/categorical/fundamental-closures#t-223): композиция $V$ с $U \in G_2$ двигает $\Gamma$ внутри его $G_2$-орбиты, на которой предикат сознания постоянен (T-42a). Семимаркерная проекция измерительного протокола $\pi_{\mathrm{bio}}$ — частный случай $G_V$ с $V$, натянутым на маркерные направления: операционный критерий выше и *есть* конструкция из (а), а не дополнительное требование.

**Машинная верификация.** При $d = 12$: полнота Крауса на $10^{-15}$; ретракция, сохранение следа и положительность на $10^{-16}$; размерность ядра ровно $d^2 - 49 = 95$; явная пара-коллизия внутренних состояний-плотностей (минимальное собственное значение $2 \cdot 10^{-2} > 0$) с $\lVert G(\rho_1) - G(\rho_2)\rVert_F \sim 10^{-17}$.
