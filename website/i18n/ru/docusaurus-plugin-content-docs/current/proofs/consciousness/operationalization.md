...
:::tip Теорема T-129 [Т]: Порог интеграции Φ_th = 1
...
**Шаг 6 (Единственность).** Любой $\Phi_{\text{th}} \neq 1$ не является наименьшим порогом, совместимым с $P_{\text{crit}} = 2/7$:
- $\Phi_{\text{th}} < 1$: **слишком слабый порог** — допускает нежизнеспособные состояния (uniform diagonal с $\Phi \in (\Phi_{\text{th}}, 1)$ даёт $P = (1+\Phi)/7 < 2/7$, что нарушает жизнеспособность, несмотря на $\Phi \geq \Phi_{\text{th}}$).
- $\Phi_{\text{th}} > 1$: **слишком строгий порог** — исключает состояния, которые на самом деле жизнеспособны (равномерно-диагональное состояние с $\Phi = 1 < \Phi_{\text{th}}$ даёт $P = 2/7 = P_{\text{crit}}$ — граничное жизнеспособное состояние, однако $\Phi < \Phi_{\text{th}}$ ошибочно сигнализирует «L2 не достигнута»). Это нарушает необходимость: $\Phi_{\text{th}}$ должен быть **наименьшим** значением, гарантирующим $P \geq P_{\text{crit}}$.

$\blacksquare$

#### Два порога как вложенные мажоритарные критерии {#вложенные-мажоритарные-критерии}

:::tip Теорема (вложенная структура доминирования $P_{\text{crit}}$ и $\Phi_{\text{th}}$) [Т]
Разложим $\Gamma$ на **три взаимно HS-ортогональные компоненты**:

$$
\Gamma \;=\; \underbrace{\tfrac{I}{7}}_{\text{изотропная}} \;+\; \underbrace{\big(\mathrm{diag}(\Gamma)-\tfrac{I}{7}\big)}_{\text{структура населённостей}} \;+\; \underbrace{\Gamma_{\text{hollow}}}_{\text{когерентности}},
\qquad P = \underbrace{\tfrac17}_{c_1} + \underbrace{\|\mathrm{diag}(\Gamma)-\tfrac I7\|_F^2}_{c_2} + \underbrace{P_{\text{coh}}}_{c_3}.
$$

Тогда оба L2-порога — **критерии доминирования на одном этом разложении**:

- $P > P_{\text{crit}} = 2/7 \iff c_2 + c_3 > c_1$ — **структурное большинство** (структура перевешивает изотропную часть);
- $\Phi \geq 1 \iff c_3 \geq c_1 + c_2$ — **когерентностное большинство** (когерентности перевешивают весь диагональный вес).

Когерентностное большинство **влечёт** структурное: $c_3 \geq c_1 + c_2 \Rightarrow c_2 + c_3 \geq c_1 + 2c_2 \geq c_1$, с равенством ровно при $c_2 = 0$ и $c_3 = c_1$ — единственное равномерно-диагональное граничное состояние из T-129a. Таким образом, T-129a — в точности **согласованность вложения** двух мажоритарных критериев, и оба порога — случаи того же принципа плюрального доминирования, что даёт $R_{\text{th}} = 1/3$ ($K=3$ гипотезы $\to$ плюральность $1/3$; двухкомпонентные разбиения $\to$ большинство $1/2$ фробениусова веса). Проверено численно: нет нарушений вложения на $2\times10^4$ случайных состояниях; граница достигается PSD-состоянием $\tfrac I7 + c(J - I)$, $c = 1/\sqrt{294}$.
:::

**Замечание о репере.** $P$ и разбиение изотропная/бесследовая базис-инвариантны; разбиение диагональ/hollow (а значит и $\Phi$) берётся в **каноническом репере измерений** $(A,S,D,L,E,O,U)$ — те же реперные данные, что $\mathrm{Coh}_E$ и $\kappa_0$ (см. [Следствие 5 теоремы единственности](/docs/proofs/categorical/uniqueness-theorem)).
...
Порог $\Phi_{\text{th}} = 1$ является **универсальным** на всём пространстве $\mathcal{D}(\mathbb{C}^7)$: для любого состояния $\Gamma$ с $\Phi(\Gamma) \geq 1$ выполняется $P(\Gamma) \geq P_{\text{crit}}$. Строгое неравенство $P > P_{\text{crit}}$ выполнено для всех состояний, кроме единственного граничного случая: $\Phi = 1$ и $P_{\text{diag}} = 1/7$ (uniform-diagonal).
:::
...
**(c)** Если $\Phi \geq 1$, то $P = P_{\text{diag}}(1 + \Phi) \geq P_{\text{diag}} \cdot 2 \geq \frac{2}{7} = P_{\text{crit}}$. Таким образом, $P \geq P_{\text{crit}}$.

**(d)** Равенство $P = P_{\text{crit}}$ достигается только при $P_{\text{diag}} = 1/7$ (uniform-diagonal, равенство в Коши–Шварце) **и** $\Phi = 1$ — это единственный граничный случай. На границе жизнеспособности система существует, но с нулевым запасом.

**(e)** Для всех остальных состояний (либо $P_{\text{diag}} > 1/7$, либо $\Phi > 1$) условие $\Phi \geq 1$ даёт $P > P_{\text{crit}}$ **строго**.

**(f)** Порог $\Phi_{\text{th}} = 1$ является **наименьшим** универсальным порогом: при $\Phi_{\text{th}} < 1$ существуют экстремальные состояния с $P_{\text{diag}} = 1/7$ и $\Phi \in (\Phi_{\text{th}}, 1)$, для которых $P < P_{\text{crit}}$. $\blacksquare$

**Интерпретация:** T-129 устанавливал $\Phi_{\text{th}} = 1$ на экстремальном семействе. T-129a показывает, что этот порог — **binding constraint** на всём $\mathcal{D}(\mathbb{C}^7)$: экстремальный случай определяет универсальный порог ($P \geq P_{\text{crit}}$), а все прочие состояния удовлетворяют ему с запасом ($P > P_{\text{crit}}$). Единственная точка равенства — граничная (uniform-diagonal с $\Phi = 1$), практически неустойчивая.
...
### SAD–L эквивалентность (T-136) {#г-89-повышение}
...
### Коммутативность φ-башни (T-150) {#г-90-повышение}
...
| SAD: наблюдаемая vs конструктивная | [T-136 [Т]](#t-136) (через [T-150](/docs/proofs/consciousness/substrate-closure#t-150)) | ЗАКРЫТО |
