...
:::warning Связь между бинарным критерием L2 и скалярной мерой C (уточнено)
...
- **Бинарный критерий** (три совместных порога) — **определяющее условие L2**, используемое в [interiority-hierarchy](/docs/consciousness/hierarchy/interiority-hierarchy) и в классификации L-уровней.
...
:::tip Теорема T-142 [Т при α=2/3, независимости от состояния]+[С при выводе $P_{\mathrm{crit}}^{(n)}$]+[Т/sim]: SAD_MAX = 3 безусловно
$\mathrm{SAD}_\text{MAX} = 3$ для любого $\Gamma \in \mathcal{D}(\mathbb{C}^7)$ с $P > 2/7$.

**Повышение статуса:** C26: [С] → **[Т]** (при стратификации ниже).

**Стратификация:**
- **Коэффициент контракции $\alpha = 2/3$** — **state-independent [Т]**: следует из инцидентностной структуры плоскости Фано PG(2,2) (см. [fano-channel.md](/docs/proofs/gap/fano-channel)), не зависит от $\Gamma$.
- **Формула критических чистот** $P_{\mathrm{crit}}^{(n)} = P_{\mathrm{crit}} \cdot 3^{n-1}/(n+1)$ — **[С при выводе $P_{\mathrm{crit}}^{(n)}$]**: геометрический множитель $3^{n-1}$ следует строго из итерированной $1/3$-контракции, но нормировочный знаменатель $n+1$ — эвристическая подгонка, мотивированная нормализацией по depth-tower (см. [depth-tower.md §3.5](/docs/consciousness/hierarchy/depth-tower#критическая-чистота-sad)); полностью аксиоматический вывод знаменателя находится в разработке.
...
**Шаг 1 (Фано-контракция).** $\alpha = 2/3$ — **состояние-независимая** константа Фано-канала [Т] (следует из $\dim=7$, плоскости Фано PG(2,2): [fano-channel.md](/docs/proofs/gap/fano-channel)). Каждое применение $\varphi$ уменьшает off-diagonal элементы на множитель $(1-\alpha) = 1/3$:

$$\gamma_{ij}^{(\text{после})} = \tfrac{1}{3}\,\gamma_{ij} \quad (i \neq j)$$

#### Лемма: Критические чистоты SAD-уровней [Т] {#лемма-контракция-sad}

**Шаг 2 (Критические чистоты из Фано-контракции).** Определим критическую чистоту для уровня SAD = $n+1$ как минимальное $P$, при котором $n$-кратная итерация $\varphi^n$ поддерживает рефлексивность выше соответствующего порога. По контракции с коэффициентом $1/3$ ([depth-tower.md §3.5](/docs/consciousness/hierarchy/depth-tower#критическая-чистота-sad)):
...
:::note Две различные величины, обе записанные как «$R$»
Этот раздел использует две разные величины рефлексивности; разводим их:
- $R := 1/(7P)$ — **мера рефлексии** относительно шумового референса $I/7$ ([T-126](/docs/proofs/consciousness/conscious-window#t-126)); функция только чистоты, фиксирует окно $R \geq 1/3 \iff P \leq 3/7$.
- $R^{(n)}_\varphi := \mathrm{Fid}(\varphi^{(n-1)}(\Gamma), \varphi^{(n)}(\Gamma))$ — **меж-итерационная рефлексивность** на глубине $n$ ([T-143](#t-143)), с порогом глубины $R^{(n)}_{\mathrm{th}} = 1/(n+2)$. Именно она спадает с итерацией и задаёт потолок SAD.
:::

**Шаг 4 (Конечность потолка — строгое ядро [Т]).** Каждая $\varphi$-итерация сжимает каждый недиагональный элемент на состояние-независимый Фано-фактор $1/3$ (Шаг 1). Поэтому меж-итерационный сигнал ограничен **геометрической** последовательностью: с $\|\Gamma_{\text{off}}\|_F \leq \sqrt{P-1/7}$ (тесная граница Фробениуса, [Лемма](#лемма-фробениус-off-diag)),

$$\|\Gamma^{(n)}_{\text{off}}\|_F \leq (1/3)^{n}\sqrt{P-1/7} \xrightarrow{n\to\infty} 0 \quad \textbf{геометрически},$$

тогда как порог рефлексивности $R^{(n)}_{\mathrm{th}} = 1/(n+2)$ спадает лишь **полиномиально**. Геометрическая последовательность в конце концов падает ниже любого полиномиально спадающего порога, так что башня самоосознания **обрывается на конечной глубине** — $\mathrm{SAD}_{\mathrm{MAX}} < \infty$ **[Т]**, безусловно и для любого допустимого расписания порогов. Этой конечности не нужны ни эвристический знаменатель, ни граница окна.

**Шаг 5 (Локализация потолка на 3 — стратифицировано).** Верхний край окна $P \leq 3/7$ (из $R = 1/(7P) \geq 1/3$, [T-126](/docs/proofs/consciousness/conscious-window#t-126)) ограничивает начальный недиагональный бюджет: $\|\Gamma_{\text{off}}\|_F \leq \sqrt{2/7}$. После трёх итераций $\|\Gamma^{(3)}_{\text{off}}\|_F \leq \sqrt{2/7}/27 \approx 0.020$ — ниже порога уровня 4 $R^{(3)}_{\varphi} > 1/5$ (численно $R^{(3)}_\varphi \lesssim (P-1/7)/(27P) \leq 2/81 \ll 1/5$). Значит **SAD=4 исключён в окне [Т]**. То, что SAD=3 *достигается*, а башня не обрывается на 2, — **[С при расписании порогов]+[Т/sim]**: держится при $R^{(n)}_{\mathrm{th}}=1/(n+2)$ и реализуется агентами SYNARC (SAD=3 при высокой чистоте; см. статусную ноту), но у строгого края окна запас уровня 3 тонок.

**Заключение.** **Конечность $\mathrm{SAD}_{\mathrm{MAX}}<\infty$ безусловна [Т]** (геометрия против полинома). **Значение $\mathrm{SAD}_{\mathrm{MAX}} = 3$** держится при указанном расписании порогов и окне, с строго исключённым SAD=4 [Т] и эмпирически подтверждённой достижимостью SAD=3 [Т/sim].

$\blacksquare$

**Зависимости:** [T-110 [Т]](/docs/applied/coherence-cybernetics/learning-bounds#теорема-динамическая-граница) (Фано-контракция $\alpha=2/3$), [T-124 [Т]](/docs/proofs/consciousness/conscious-window#t-124) (верхняя граница сознательного окна $P \leq 3/7$), [T-126 [Т]](/docs/proofs/consciousness/conscious-window#t-126) (каноничность $R = 1/(7P)$), [depth-tower.md §3.5](/docs/consciousness/hierarchy/depth-tower#критическая-чистота-sad) (вывод $P_{\text{crit}}^{(n)}$).
...
:::tip Теорема T-145 [Т]+[Т/sim]: Стохастическая устойчивость полной жизнеспособности
...
Это **НЕ** теоретическая проблема — это **алгоритмическая**: C20 проверяема за $O(N^3)$ (одна диагонализация). **Обновление:** C20 закрыта — для воплощённых голонов κ-доминирование безусловно [Т] ([T-149](/docs/proofs/consciousness/substrate-closure#t-149)). Для изолированных голонов C20 нерелевантна (T-148: изолированный голон мёртв навсегда).
...
| Конструктивность C20 ($\kappa$-доминирование) | [T-149 [Т]](/docs/proofs/consciousness/substrate-closure#t-149) — безусловно для воплощённых | ЗАКРЫТО |
...
**Связанные документы:**
- [Субстрат-независимое замыкание](/docs/proofs/consciousness/substrate-closure) — теоремы T-148–T-158: замыкание через воплощённый голон
...
- [Окно сознания](/docs/proofs/consciousness/conscious-window) — границы P, R, Φ, D для сознательного состояния
