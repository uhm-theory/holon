...
- T-55 [Т] (Лоуверровская неполнота самомоделирования);
...
- Невозможность L4 (T-86 [Т] — катастрофа $A_5$ + Лоуверровская неполнота) согласуется с $K=5$ как границей достижимости;
...
:::warning Два разных объекта делят обозначение $R^{(n)}$ — уточнение
Теорема 4.3 касается **фиделити самомодели** $R^{(n)}_{\mathrm{fid}}(\Gamma) := \mathrm{Fid}(\varphi^{(n-1)}\Gamma, \varphi^{(n)}\Gamma)$, которая **возрастает к 1**, поскольку $\varphi$ — сжатие (последовательные самомодели стабилизируются). Это **нельзя** путать с **коэффициентом выживания когерентности**
$$S^{(n)}(\Gamma) := \frac{\lVert \mathrm{offdiag}(\varphi^{(n)}\Gamma)\rVert_F}{\lVert \mathrm{offdiag}(\Gamma)\rVert_F} = 3^{-n},$$
который **убывает к 0** под Фано-сжатием и именно он входит в потолок SAD и в утверждения «$R^{(n)}\sim 0.7^n\to 0$» в [depth-tower](/docs/consciousness/hierarchy/depth-tower) и в §о недостижимости L4. Пределы совместны, так как обозначения относятся к разным объектам.

**Следствие:** клауза «$\lim R^{(n)}_{\mathrm{fid}} > 0$» *тривиально истинна* и потому **не** различает L4. Операциональная характеризация L4 — **категорная / постниковская** [T-86 [Т]](/docs/reference/status-registry) (L4 как $\tau_{\leq\infty}$-копредел) вместе с условием чистоты $P > 6/7$ (Теорема 4.2); потолок глубины SAD$_{\max}=3$ формулируется через $S^{(n)}$, а не через $R^{(n)}_{\mathrm{fid}}$.
:::
...
**Утверждение.** Точное равенство $\lambda_1 = 1$ (L4) **недостижимо** для конечномерных систем из-за Лоуверровской неполноты (T-55 [Т]).
...
**Использованные результаты:**
- T-55 [Т] (Лоуверровская неполнота);
