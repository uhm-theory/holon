...
description: "Мастер-определение операторов Линдблада L_k в УГМ: атомарные и Фано-структурированные операторы, CPTP-верификация, Фано–атомная пропорциональность и канонический G₂-ковариантный диссипатор"
---
...
- Фано-плоскость несёт таблицу умножения октонионов — точнее: её линии задают, КАКИЕ тройки перемножаются, а знаки задаёт **ориентация** каждой линии. Из $2^7 = 128$ ориентаций настоящую алгебру с делением дают ровно **16** (машина: согласуется с классическим счётом $480 = 30 \times 16$ по всем размеченным плоскостям Фано). Все 16 дают изоморфные копии $\mathbb{O}$, поэтому выбор есть калибровка разметки, а не добавочная структура, — но одна плоскость знаков не определяет
...
| 10 | PG(2,2) ≅ таблица умножения $\mathrm{Im}(\mathbb{O})$ | **[Т]** стандартная алгебра |
...
**(c) Тип 3: Регенерация из A1+A4.** Функтор регенерации $\mathcal{R}$ — восстанавливающий член дуальности $(\mathcal{D}_\Omega, \mathcal{R})$ (категорное прочтение [И], [§15.3](/docs/proofs/categorical/categorical-formalism#сопряжение-adjunction)) — порождает $\mathcal{R}[\Gamma, E] = \kappa(\Gamma) \cdot (\rho_* - \Gamma) \cdot g_V(P)$ с $\kappa_0 = \omega_0 \cdot |\gamma_{OE}| \cdot |\gamma_{OU}| / \gamma_{OO}$ **[Т при кинетике первого порядка]** ([вывод](/docs/core/foundations/axiom-septicity#вывод-kappa0-cycle-flux)), где $\omega_0$ — из A4. Стационарное состояние: $\rho_*$ (единственное, [примитивность](#примитивность-ℒω) [Т]).

**Шаг 2. Структурная различимость.**

| Свойство | Aut (Гамильтониан) | $\mathcal{D}$ (Диссипация) | ℛ (Регенерация) |
|----------|:---:|:---:|:---:|
...
**Доказательство:** LGKS-теорема (1976) даёт единственное разложение на гамильтонову и диссипативную части. Диссипативная часть единственным образом разделяется на $\mathcal{D}$ (Фано-контракция, $dP/d\tau \leq 0$) и $\mathcal{R}$ (замещающий канал, $dP/d\tau \geq 0$) при ограничениях A5 (PW-привязка $\mathcal{R}$ к O-сектору), Фано-структурированности $\mathcal{D}$ и ковариантности относительно октонионной реперной группы $\Gamma_{\text{oct}} = \mathrm{Aut}(\mathrm{PG}(2,2)) \cong PSL(2,7)$ (группа ковариантности Фано-диссипатора — см. [Фано-канал, Тм. 5.1b](/docs/proofs/gap/fano-channel#g2-ковариантность)).
:::

### Следствие: K = 3 для порога рефлексии {#следствие-k3}

Триадная декомпозиция определяет **ровно три** режима поведения системы: автономный (ℛ доминирует, аттрактор $\rho_*$), хаотический $(\mathcal{D}$ доминирует, аттрактор $I/N$), внешний (Aut доминирует, аттрактор $\sigma_{\text{env}}$). Число конкурирующих гипотез $K = 3$ — структурное следствие аксиом, не постулат.
...
## Ковариантность диссипаторов и калибровочная группа {#g2-ковариантность}

Группа $G_2 = \mathrm{Aut}(\mathbb{O})$ сохраняет октонионное умножение и, следовательно, [Фано-3-форму](/docs/physics/gauge-symmetry/g2-structure) $\varphi$. Её связь с диссипаторами точна и сильнее прежнего лозунга «Фано $G_2$-ковариантен».

:::tip Теорема: Фано–атомная пропорциональность [Т]
$\mathcal{D}_{\text{Fano}} = \tfrac23\,\mathcal{D}_{\text{atom}}$ как супероператоры (каждая точка на $r=3$ линиях, каждая пара на $\lambda=1$ линии ⟹ $\mathcal{P}_{\text{Fano}}=\tfrac13\mathrm{Id}+\tfrac23\Delta$; и $\sum_p (L_p^{\text{Fano}})^\dagger L_p^{\text{Fano}}=I$). [Полное доказательство →](/docs/proofs/gap/fano-channel#g2-ковариантность). Это *выводит* $\alpha=2/3$ из геометрии инцидентности.
:::

:::warning Теорема: пиннинг-диссипаторы нарушают $G_2$ до конечной реперной группы [Т]
Поскольку $\mathcal{D}_{\text{Fano}} = \tfrac23\mathcal{D}_{\text{atom}}$, оба диссипатора имеют **одинаковую** группу симметрии: конечную октонионную реперную группу $\Gamma_{\!\text{oct}} = \mathrm{Aut}(PG(2,2)) \cong PSL(2,7) \subset G_2$ (плюс $S_7$-эквивариантность для атомарной формы). **Ни один** из них не ковариантен относительно полной непрерывной $G_2$: так как $\mathbb{C}^7$ — неприводимый $G_2$-модуль (Шур), общий $g\in G_2$ переводит координатный линейный проектор $\Pi_p$ в проектор ранга 3 на *повёрнутое* подпространство, откуда $\mathrm{diag}(g\Gamma g^\dagger)\neq g\,\mathrm{diag}(\Gamma)\,g^\dagger$.

[Доказательство →](/docs/proofs/gap/fano-channel#g2-ковариантность) | Статус: **[Т]**
:::

#### Теорема T-260: гранд-каноническое происхождение диагонального калибровочного тора [Т] {#теорема-происхождение-тора}

Теорема о слома́нном репере выше говорит, что́ пинчинг-динамика *разрушает* из непрерывной $G_2$. Дополнительный вопрос — какую непрерывную симметрию она *сохраняет* и почему эта симметрия компактна. Ответ оказывается гранд-каноническим механизмом T-258: уцелевший тор порождён сохраняемым вещественным реестром холона, а его компактность есть целочисленность этого реестра.

:::tip Теорема (сохранённые заряды ⇒ компактный тор) [Т]
Для Фано-диссипатора с произвольными положительными скоростями линий $\{\gamma_p\}$:

1. **Алгебра сохранённых зарядов = диагональная алгебра.** В картине Гейзенберга $\mathcal{D}^\dagger[Q] = 0$ тогда и только тогда, когда $Q$ диагональна: $\mathcal{D}$ действует как множитель Шура $Q_{ij} \mapsto -r_{ij}Q_{ij}$ с $r_{ij} > 0$ при всех $i \neq j$ (инцидентность BIBD) и $r_{ii} = 0$, так что $\ker\mathcal{D} = \mathrm{span}\{\Pi_k\}$, $\dim = 7$. Семь сохранённых зарядов — в точности паспортные населённости $\gamma_{kk} = \mathrm{Tr}(\Pi_k\Gamma)$.
2. **Компактность ⟺ целочисленность.** Экспонента алгебры зарядов — диагональный тор $\{\exp(i\sum_k \varphi_k\Pi_k)\} = U(1)^7 \subset U(7)$. Каждый фактор — окружность, а не прямая, ровно потому, что спектр заряда целочислен: $\mathrm{spec}(\Pi_k) = \{0,1\} \subset \mathbb{Z}$ даёт $e^{2\pi i\Pi_k} = \mathbb{1}$ точно; каскадные счётчики заполнения $\hat{N}_k$ (целые числа суб-холонов ⊕-примитива) сохраняют целочисленность и потому тор; генератор с иррациональным отношением в спектре наматывался бы плотно — $\mathbb{R}$-орбита, которая никогда не замыкается.
3. **Полная группа ковариантности.** Всякая диагональная унитарная — точная симметрия $\mathcal{D}$, а вращения, переставляющие линейные проекторы, образуют конечную реперную группу $\Gamma_{\!\text{oct}} = \mathrm{Aut}(PG(2,2)) \cong PSL(2,7)$ (теорема о реперной поломке выше); значит полная унитарная группа ковариантности пинчинг-динамики есть $U(1)^7 \rtimes \Gamma_{\!\text{oct}}$ — компактный тор гранд-канонических фаз, расширенный реперными перестановками.
:::

**Доказательство.** *(1)* Множитель Шура самосопряжён в скалярном произведении Гильберта–Шмидта, так что действие в картине Гейзенберга — тот же множитель; $Q_{ij}r_{ij} = 0$ при $r_{ij} > 0$ вынуждает $Q_{ij} = 0$ вне диагонали. Положительность $r_{ij}$: всякая пара $(i,j)$, $i \neq j$, лежит ровно на одной линии ($\lambda = 1$), так что при всех положительных скоростях линии, встречающие $\{i,j\}$ в одной точке, дают строго положительный распад. *(2)* $e^{i\varphi\Pi_k}$ имеет собственные значения $\{e^{i\varphi}, 1\}$ и замыкается тогда и только тогда, когда $\varphi \in 2\pi\mathbb{Z}$; при иррациональном отношении в $\mathrm{spec}(G)$ уравнение $e^{i\varphi G} = \mathbb{1}$ не имеет решений $\varphi \neq 0$ (обмотка Вейля). *(3)* Диагональная $U$ коммутирует с каждой диагональной $\Pi_p$, так что $\mathcal{D}[U\Gamma U^\dagger] = U\mathcal{D}[\Gamma]U^\dagger$ почленно; недиагональное утверждение — теорема о реперной поломке. $\blacksquare$

**Машинная верификация.** Анизотропные случайные скорости: $\dim\ker\mathcal{D} = 7$ точно; $\|\mathcal{D}^\dagger[Q_{\text{diag}}]\| = 2\cdot10^{-16}$ против $\|\mathcal{D}^\dagger[Q_{\text{off}}]\| = 4{,}9$; ковариантность диагональных унитарных $1{,}4\cdot10^{-16}$, недиагональный свидетель $0{,}22$; $\|e^{2\pi i\Pi_k} - \mathbb{1}\| = 2\cdot10^{-16}$, целый каскадный счётчик $9\cdot10^{-16}$; иррациональный генератор $\mathrm{diag}(1,\sqrt{2})$: минимальный возврат $0{,}044 > 0$ на $\varphi \in (0; 200]$ — плотная обмотка, никогда не замыкается.

**Гранд-каноническое прочтение (T-258).** Фазы тора $\varphi_k$ сопряжены целочисленным паспортным заполнениям ровно так, как $U(1)$-фаза волновой функции сопряжена числу частиц в гранд-каноническом ансамбле — и как в «Самообучающейся Вселенной» Ванчурина, где $U(1)$ возникает из термодинамической эквивалентности $S \to S + h\Delta N$, $\Delta N \in \mathbb{Z}$. Компактность калибровочного тора, таким образом, не постановление: это **целочисленность вещественного реестра холона**. Проверка 4 [гранд-канонического словаря](/docs/applied/coherence-cybernetics/sensorimotor#гранд-канонический-словарь) тем самым *выведена* на стороне УГМ; соответствие с собственной $U(1)$ SLU остаётся интерпретивным [И].

:::tip Теорема: канонический $G_2$-ковариантный диссипатор $\mathcal{D}_{G_2}$ [Т]
Существует истинно $G_2$-ковариантный диссипатор Линдблада, построенный из структурных констант $\varphi_{abc}$: $(A_a)_{bc} = \varphi_{abc}/\sqrt6$, $\mathcal{D}_{G_2}[\Gamma]=\sum_a(A_a\Gamma A_a^\dagger-\tfrac12\{A_a^\dagger A_a,\Gamma\})$. Тогда $\sum_a A_a^\dagger A_a=I$ (CPTP) и $\mathcal{D}_{G_2}[g\Gamma g^\dagger]=g\,\mathcal{D}_{G_2}[\Gamma]\,g^\dagger\ \forall g\in G_2$, поскольку $\varphi$ $G_2$-инвариантна. [Доказательство →](/docs/proofs/gap/fano-channel#g2-ковариантность).
:::

### Степень $G_2$-нарушения при смешанном наблюдении

Для канонического [когерентно-сохраняющего самомоделирования](/docs/proofs/categorical/formalization-phi) с параметром $\alpha$ (баланс атомарного и Фано-наблюдения), $\mathcal{P}_\alpha = \alpha \, \mathcal{P}_{\text{base}} + (1 - \alpha) \, \mathcal{P}_{\text{Fano}}$, диссипатор равен $\mathcal{D}_\alpha = \tfrac{2+\alpha}{3}\,\mathcal{D}_{\text{atom}}$, а его $G_2$-нековариантность
...
**строго положительна при каждом** $\alpha\in[0,1]$:

| $\alpha$ | Режим | $\Delta_{G_2}(\alpha)$ (динамика) |
|----------|-------|---------------------|
| $0$ | Чисто Фано | $\tfrac23\Delta_{\max} > 0$ (нарушает $G_2$ до $\Gamma_{\!\text{oct}}$) |
| $\alpha^* \in (0,1)$ | Смешанный (оптимальный) | $\tfrac{2+\alpha^*}{3}\Delta_{\max}$ |
| $1$ | Чисто атомарный | $\Delta_{\max}$ |

:::info Замечание: кинематическая $G_2$ vs. динамический репер [И]
Следует различать два факта. **Кинематически** калибровочная группа голономного *представления* — это $G_2=\mathrm{Stab}(\varphi)$; физически инвариантное содержание — спектр (6) плюс $\varphi$-относительные углы (28), что даёт $48\to34$ параметра ([теорема единственности](/docs/proofs/categorical/uniqueness-theorem#g2-ригидность) [Т]). Этот счёт — свойство представления и **не** зависит от $\alpha$. **Динамически** пиннинг (Фано)-диссипатор выделяет функциональный репер $\{A,S,D,L,E,O,U\}$ и потому нарушает кинематическую $G_2$ до конечной $\Gamma_{\!\text{oct}}$ — этот выбор репера и есть подлинная «цена самонаблюдения», а несломанный $\mathcal{D}_{G_2}$ — симметричная референсная динамика. Реперно-зависимые наблюдаемые ($\mathrm{Coh}_E$, $\Phi$, $\kappa_0$) определены в этом фиксированном физическом репере, а не среди 34 $G_2$-инвариантов.
:::
