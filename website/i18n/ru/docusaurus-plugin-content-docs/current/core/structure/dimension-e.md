...
2. **Категориальный (κ₀):** Формула $\kappa_0 = \omega_0 \cdot |\gamma_{OE}| \cdot |\gamma_{OU}| / \gamma_{OO}$ ([Т при кинетике первого порядка], [вывод](/docs/core/foundations/axiom-septicity#вывод-kappa0-cycle-flux); Тм. 15.3.1) явно использует E как отдельный объект категории через $\mathrm{Hom}(O, E)$. При удалении E: κ₀ не определён, скорость регенерации $\kappa(\Gamma) = \kappa_{\text{bootstrap}} + \kappa_0 \cdot \mathrm{Coh}_E$ теряет оба E-зависимых фактора.
...
#### Теорема (7D-достаточность для всех мер сознания) [Т] {#теорема-7d-достаточность}

**Формулировка.** Минимальный 7D-формализм $\Gamma \in \mathcal{D}(\mathbb{C}^7)$ **достаточен** для вычисления всех наблюдаемых, связанных с сознанием. Ни одна наблюдаемая УГМ не зависит от выбора между 7D- и 42D-формализмами.

**Доказательство.**

**Шаг 1 (Категориальная эквивалентность).** По T-58 [Т] (Морита-эквивалентность): ∞-топосы $\mathbf{Sh}_\infty(\mathcal{C}_7, J_B)$ и $\mathbf{Sh}_\infty(\mathcal{C}_{42}, J_B)$ **эквивалентны** как $(\infty,1)$-категории. Это означает, что каждый объект, морфизм и высший морфизм в одном топосе имеет единственный аналог в другом.

**Шаг 2 (Эквивалентность наблюдаемых).** Наблюдаемая в УГМ — это морфизм $O: \Gamma \to \mathbb{R}$ в $\mathbf{Sh}_\infty(\mathcal{C})$. По категориальной эквивалентности (Шаг 1): $O^{7D}(\Gamma) = O^{42D}(\iota_{PW}(\Gamma))$ для каждой наблюдаемой $O$ и каждого состояния $\Gamma$. Ошибка реконструкции **нулевая** — не малая, не контролируемая, а **точно нулевая** — поскольку эквивалентность категорий сохраняет все морфизмы точно.

**Шаг 3 (Явные 7D-формулы для величин с частичным следом).** Величины, формально требующие 42D частичного следа, имеют **точные 7D-представления** через HS-проекции:

| Величина | 42D-определение | 7D-формула | Ошибка |
|----------|----------------|------------|--------|
| $\mathrm{Coh}_E$ | $\|\rho_E - P_{\bar{E}}(\rho_E)\|_F^2 / \|\Gamma\|_F^2$ | $\|\pi_E(\Gamma)\|_{HS}^2 / \|\Gamma\|_{HS}^2$ | **0** (T-154 [Т]) |
| $D_{\text{diff}}$ | $\exp(S_{vN}(\rho_E))$ | $1 + 6 \cdot \mathrm{Coh}_E / \mathrm{Coh}_E^{\max}$ | **0** в экстремумах (T-128 [Т]) |
| $C$ | $\Phi \cdot R$ | $\Phi \cdot R$ | **0** (обе определены в 7D) |
| $P$, $R$, $\Phi$ | То же, что 7D | Диагональ/внедиагональ $\Gamma$ | **0** (тождество) |

**Шаг 4 (Что представляет фактор $\mathbb{C}^6$).** В 42D-расширении $\mathcal{H}_{42} = \mathbb{C}^7 \otimes \mathbb{C}^6$ фактор $\mathbb{C}^6$ — это **временной регистр** часов Пейджа—Вуттерса: 6 условных состояний $\Gamma(\tau_k)$ для $k = 1, \ldots, 6$ (на одно меньше 7, поскольку 7-е фиксировано условием нормировки $\mathrm{Tr}(\Gamma_{\text{total}}) = 1$). Этот фактор **не вводит** новых физических степеней свободы — он является математическим устройством учёта для кодирования временной эволюции в рамках безвременного формализма (Уилер—ДеВитт, T-87 [Т]).

**Заключение:** 7D-формализм — **не приближение** 42D-формализма. Оба являются точными описаниями одной и той же физики, связанными категориальной эквивалентностью. 42D-расширение — **вычислительное удобство** для частичных следов, а не онтологическая необходимость. $\blacksquare$

**Зависимости:** T-58 [Т] (Морита), T-87 [Т] (PW), T-95 [Т] (каноническая реконструкция), T-128 [Т], T-154 [Т].
...
**Эквивалентный 7D вычислительный маршрут [T-128].**
...
*Заметка об охвате (T-301):* FV — это **E-срез** содержания: интенсивность и реляционная позиция качества. Полное содержание состояния — его 28 параметров (7 населённостей + 21 когерентность), а калибровочно-инвариантная *краска* живёт в голономиях Фано; полный пятислойный паспорт — в [Структуре квалиа](/docs/consciousness/phenomenology/qualia-structure).
...
**7D формула [T-128]** (вычислительный маршрут):
...
Измерению соответствует $e_5 \in \mathrm{Im}(\mathbb{O})$. Данное отождествление является **теоремой** [Т]: [цепочка мостов T15](/docs/core/foundations/axiom-septicity#мост-p1p2) (все шаги [Т]) выводит октонионную структуру из (AP)+(PH)+(QG)+(V); [T-177 [Т]](/docs/reference/status-registry) и [T-183 [Т]](/docs/reference/status-registry) доказывают комбинаторную и функциональную единственность каждой роли. Конкретное присвоение $E = e_5$ фиксировано с точностью до $G_2$-калибровочной эквивалентности ([T-42a [Т]](/docs/proofs/categorical/uniqueness-theorem)). Детали и $G_2$-оговорка: [Октонионная интерпретация](./dimensions#октонионная-интерпретация), [структурный вывод](../../proofs/minimality/theorem-octonionic-derivation).
:::
