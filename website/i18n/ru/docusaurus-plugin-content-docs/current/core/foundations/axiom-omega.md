...
| **Аксиома 4 (Масштаб)** | $\omega_0 = \lambda_{\min}(H_{\text{eff}}) > 0$ — минимальное ненулевое собственное значение эффективного гамильтониана | Выведенное спектральное свойство: $\omega_0 > 0$ для любой жизнеспособной системы ($\omega_0 = 0 \Rightarrow$ нет динамики $\Rightarrow P < P_{\text{crit}}$). Разные голономы имеют разные $\omega_0$, как разные атомы имеют разные массы. См. [T-186, Когезивное замыкание §5.4](/docs/proofs/categorical/cohesive-closure) |
| **Аксиома 5 (Пейдж–Вуттерс)** | Тензорная факторизация $\mathcal{H} = \mathcal{H}_O \otimes \mathcal{H}_{\text{rest}}$ с часовым сектором и связью $\hat{C}\Gamma = 0$ | **Исторически аксиома; ныне выводима** из A1–A4 через спектральную тройку ([T-87 [Т]](#pw-constraint)) — сохранена в списке для полноты изложения. *Независимых* аксиом, таким образом, **четыре** |

:::warning Две взаимовыводимые аксиоматические базы (T-190 Аксиоматическое замыкание)
...
| **B** | [Структурный вывод](../../proofs/minimality/theorem-octonionic-derivation): P1+P2 → $\mathbb{O}$ → $\dim(\mathrm{Im}(\mathbb{O})) = 7$ | [Т] Математически строго |
...
### Топология Гротендика на $\mathcal{C}$ {#топология-гротендика}
...
**Ключевой момент:** **CPTP-сжимаемость** любой Petz-допустимой метрики (свойство, разделяемое всем Petz-семейством, с Бюресом как его выделенным минимумом — см. [T-187](/docs/proofs/categorical/cohesive-closure#53-closing-the-last-open-question-why-bures-t-187)) обеспечивает **стабильность** покрытий. Пара (тождество + стабильность) образует Гротендик-**покрытие** (Johnstone, *Elephant* C2.1.1); полная топология Гротендика $J_{Bures}$ — это топология, **порождённая** этим покрытием (Elephant C2.1.10), и **транзитивность** $J_{Bures}$ автоматична из порождения — не требует прямого ε-δ аргумента.
...
:::warning Уточнение: фазы Берри и локальные системы
...
:::note Корректность типизации: $\mathcal{C}$ — полная конечномерная CPTP-категория
...
:::tip Теорема T-87 (A5 из спектральной тройки) [Т] (**расширенное доказательство**)
...
**Цепочка**: [T-53](/docs/core/foundations/spacetime#теорема-спектральная-тройка) → Ведденберн + киральная градуировка → тензорная факторизация → PW-ограничение → A5.
:::
...
**[Т] P1:** Пространство состояний ≅ $\mathrm{Im}(\mathcal{A})$, где $\mathcal{A}$ — нормированная алгебра с делением.
...
3. **Стрела времени:** Эволюция направлена к T; стягиваемость ⟹ стрела **единственна с точностью до гомотопии** (2-й закон)
4. **Свобода воли:** находится **не** здесь (все пути к $T$ гомотопны), а в плоских направлениях $\dim\ker(\mathcal H_\Gamma)$ — см. [Свобода воли](#свобода-воли)
...
graph TD
    ...
    subgraph "Уровень 1: Структура примитива"
        ...
        Atoms["T_Ω = {S₀,...,S₆}<br/>S_i = |i⟩⟨i| — базисные предикаты"]
        Z7["ℤ₇-действие на T_Ω"]
    end
...
:::info Определение (Свобода воли в УГМ) — исправлено 2026-07
...
— число независимых направлений, в которых агент может двигаться без энергетического штрафа (плюс тривиальная мода «на месте»). См. [Следствия §Свобода воли [Т]](/docs/core/foundations/consequences#freedom-конечномерное).
:::

:::warning Почему не $\pi_0(\mathrm{Map}(\Gamma,T))$
...
#### Теорема T-59 (Спектральный зазор Фано-диссипатора) [Т]+[Т/sim] {#теорема-kappa-bootstrap-bound}

:::info Стратификация
Аналитический вывод $\kappa_{\text{bootstrap}} = \omega_0/N$ (в единицах $\omega_0$: $1/7$) из единицы сопряжения $\eta$ и спектральной структуры Фано-диссипатора — **[Т]** (шаги ниже). Конкретное численное значение $1/7$ дополнительно сверено с точностью $10^{-10}$ в интеграционном тесте SYNARC `mvp_int_2` G5 — это эмпирическое подтверждение — **[Т/sim]**. Жёсткого аналитически-эмпирического разделения не утверждается; оба слоя независимо корректны и взаимно согласованы.
:::
...
|-----------|------------------|---------------|
| Максимально смешанное | $1/7 \approx 0.14$ | Значение при $I/7$ — **не** минимум (глобальный минимум $0$ при пустом E-секторе; см. [поправку диапазона 2026-07-11](/docs/core/foundations/axiom-septicity#coh-e-canonical)) |
...
```mermaid
graph TD
    Prim["T = (Sh_∞(C), J_Bures, ω₀)<br/><b>Единственный примитив</b>"] --> T["∞-топос Sh_∞(C)"]
...
**Связанные документы:**
- [Структурный вывод N=7 через октонионы](../../proofs/minimality/theorem-octonionic-derivation) — P1+P2 → $\mathbb{O}$ → N=7 (Трек B)
- [Аксиома (AP+PH+QG+V)](./axiom-septicity) — автопоэзис, феноменология, квантовое основание, жизнеспособность (расширение до **(MaxEnt)** для аксиоматического замыкания T-190)
