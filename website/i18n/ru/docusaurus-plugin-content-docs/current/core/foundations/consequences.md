...
Эта глава показывает, **что следует из аксиом** — какие теоремы можно доказать, отталкиваясь от [Аксиомы Ω⁷](./axiom-omega) и условий [(AP+PH+QG+V)](./axiom-septicity). Каждое следствие строго классифицировано по [эпистемическому статусу](/docs/reference/status-registry): [Т] — доказано, [С] — условно, [И] — интерпретация, [О] — определение.
...
:::note Что это значит простым языком
...
**Доказательство:**
1. Терминальный объект T ⟹ ретракция r : $N(\mathcal{C})$ → {T}
2. |$N(\mathcal{C})$| ≃ * (стягиваемо в точку T)
...
**Статус:** [Т] — следствие автопоэзиса (A1), нетривиальности аттрактора (T-96 [Т]) и положительности $\kappa_0$ (T-44a [Т]).
...
:::info Когомологическая мотивация (не входит в формальное доказательство)
Когомологический монизм ($H^n(X) = 0$) и локально-глобальная дихотомия ($H^*_{\text{loc}} \neq 0$) мотивируют ожидание $\Lambda \neq 0$: глобальная тривиальность при локальной нетривиальности структуры. Однако переход от когомологий к интегралу вакуумной энергии **не формализован** — это эвристика [И], а не логический шаг доказательства. Формальное доказательство $\Lambda > 0$ опирается исключительно на автопоэтический аргумент ниже.
:::

**Шаг 1 (Положительность вакуумной энергии из автопоэзиса).** Вблизи $T$ вакуумная энергия определяется балансом диссипации и регенерации в нетривиальном аттракторе $\rho^*_\Omega$ ([T-96](/docs/core/dynamics/evolution#теорема-нетривиальность-аттрактора) [Т]):
...
**Статус:** [Т] Следствия из [структурного вывода N=7](../../proofs/minimality/theorem-octonionic-derivation) (P1+P2 → $\mathbb{O}$ → 7).

:::info Предпосылка
Пространство внутренних степеней свободы изоморфно $\mathrm{Im}(\mathbb{O})$ ([Трек B](../../proofs/minimality/theorem-octonionic-derivation)), и структура октонионов порождает ряд следствий для теории УГМ.
:::
...
**Следствие для УГМ:** 7-мерное пространство $\mathrm{Im}(\mathbb{O})$ обладает 14-параметрической группой симметрий, сохраняющей структуру октонионного умножения.
...
**Следствие [Т]:** Из 21 когерентности $\gamma_{ij}$, 7 × 3 = 21 пар распределяются по 7 триплетам Фано. Каждый триплет образует ассоциативную подалгебру (изоморфную $\mathrm{Im}(\mathbb{H})$).
...
**Статус:** [Т] — три роли $\Omega$ выведены из [Аксиомы Ω⁷](./axiom-omega); формальное доказательство: [L-унификация](/docs/proofs/categorical/categorical-formalism#l-унификация).

:::warning Теорема (L-унификация) [Т]
...
| **L-операторы** | $L_k^{\text{atom}} = \sqrt{\chi_{S_k}}$, $L_p^{\text{Fano}} = \frac{1}{\sqrt{3}}\Pi_p$ | [Операторы Линдблада](/docs/core/operators/lindblad-operators) — генераторы диссипации |
| **L-время** | $\triangleright: S_i \mapsto S_{(i+1) \bmod N}$ | [Темпоральная модальность](/docs/core/operators/emergent-time#темпоральная-модальность) — сдвиг на $\Omega$ |
...
1. **Атомы → логика.** По [A1](./axiom-omega#внутренняя-логика): $\Omega$ — классификатор подобъектов в $\mathrm{Sh}_\infty(\mathcal{C})$. Атомарные подобъекты $S_k = |k\rangle\langle k|$ ($k = 0, \ldots, 6$) — минимальные ненулевые элементы решётки $\mathrm{Sub}(\Omega)$. По $N = 7$ ([A3](./axiom-omega#свойство-1)): ровно 7 атомов, образующих базис. Операции $\land, \lor, \Rightarrow$ на $\mathrm{Sub}(\Omega)$ индуцируют **интуиционистскую логику** ([Lawvere, 1969](./mathematical-foundations#лавёр)).

2. **Атомы → Линдблад.** Атомарные операторы $L_k^{\text{atom}} = \sqrt{\chi_{S_k}} = |k\rangle\langle k|$ порождают диссипатор $\mathcal{D}^{\text{atom}}[\Gamma] = \sum_k L_k \Gamma L_k^\dagger - \frac{1}{2}\{L_k^\dagger L_k, \Gamma\}$. По [теореме об уникальности Фано-формы](/docs/core/operators/lindblad-operators#теорема-единственность-фано) [Т]: BIBD$(7,3,1)$-структура $\Omega$ однозначно определяет Фано-операторы $L_p^{\text{Fano}} = \frac{1}{\sqrt{3}}\Pi_p$, комбинирующиеся с атомарными в [каноническую форму](/docs/core/operators/lindblad-operators#деривация-из-классификатора).

3. **Сдвиг → время.** Циклический автоморфизм $\triangleright: \Omega \to \Omega$, $S_i \mapsto S_{(i+1) \bmod 7}$ — единственный (с точностью до выбора генератора $\mathbb{Z}_7$) нетривиальный автоморфизм порядка $N$ на атомах $\Omega$. Через дискретное преобразование Фурье он порождает [базис часов](/docs/core/operators/emergent-time#clock-basis) и механизм Пейдж–Вуттерса ([A5](./axiom-omega#pw-constraint)). Подробнее: [Эмерджентное время](/docs/core/operators/emergent-time). $\blacksquare$
...
Это следствие усиливает Аксиому Ω⁷: время — не внешний параметр, а функция структуры ∞-топоса $\mathrm{Sh}_\infty(\mathcal{C})$. [Измерение O](../structure/dimension-o) выполняет роль внутренних часов.
...
**Статус:** Прямое следствие [Аксиомы Ω⁷](./axiom-omega) (единственность примитива — ∞-топос $\mathrm{Sh}_\infty(\mathcal{C})$).
...
**Статус:** Прямое следствие [Аксиомы Ω⁷](./axiom-omega) — объекты ∞-топоса $\mathrm{Sh}_\infty(\mathcal{C})$.
...
:::note Свобода как математический факт
...
::::info Определение (свобода = размерность плоских направлений)
Для конфигурации $\Gamma$ свобода — это **размерность пространства нулевых мод** гессиана свободной энергии, плюс единица:

$$
\text{Freedom}(\Gamma) = \dim\ker(\mathcal{H}_\Gamma) + 1
$$

т.е. число независимых направлений, вдоль которых свободная энергия $\mathcal{F}$ плоская (движение без энергетического штрафа), плюс тривиальная мода «оставаться на месте».

:::warning Исправление: не $\pi_0$ пространства отображений
Заметим: $\pi_0(\mathrm{Map}(\Gamma,T)^{\text{non-trivial}})$ **не** годится в качестве меры — $\mathrm{Map}(\Gamma,T)$ в терминальный объект **стягиваемо**, так что $\pi_0=1$ для любого $\Gamma$ — оно не может равняться $\dim\ker(\mathcal H_\Gamma)+1$. Корректный категорный объект — **касательная размерность критического (нуль-модового) многообразия** $\mathcal F$ в точке $\Gamma$, равная $\dim\ker(\mathcal H_\Gamma)$; конечномерная формула выше — это определение, а ∞-категорное прочтение — эта размерность критического многообразия, а **не** $\pi_0$ стягиваемого пространства отображений.
:::
::::
...
|---|---|---|
| $\mathcal{H}_\Gamma$ | Гессиан свободной энергии в $\Gamma$ | Локальная кривизна ландшафта усилий |
| $\ker(\mathcal{H}_\Gamma)$ | Нуль-модовые (плоские) направления | Направления выбора без энергетического штрафа |
| $\text{Freedom}(\Gamma)$ | $\dim\ker(\mathcal{H}_\Gamma) + 1$ | Число независимых свободных направлений (+ «на месте») |
...
**Мотивировка.** Свобода считает **различные направления в пространстве состояний, вдоль которых свободная энергия плоская** (нулевые моды гессиана $\mathcal H_\Gamma$). Каждая нулевая мода — **независимый выбор**: движение вдоль неё без энергетического штрафа. Слагаемое $+1$ учитывает тривиальную моду «на месте». (∞-категорный объект, схватывающий это, — касательная размерность критического многообразия свободной энергии, а **не** $\pi_0(\mathrm{Map}(\Gamma,T))$, которое тривиально, т.к. это пространство отображений стягиваемо.)

#### Теорема (Эквивалентность определений Freedom) [Т] (T-89) {#теорема-freedom-эквивалентность}

Конечномерное определение $\text{Freedom}(\Gamma)=\dim\ker(\mathcal{H}_\Gamma)+1$ корректно **[Т]**. По теории Морса–Ботта: свободная энергия $\mathcal{F}[\Gamma]$ — функция Морса–Ботта на $\mathcal{D}(\mathbb{C}^7)$, и **касательная размерность критического многообразия** через $\Gamma$ равна $\dim\ker(\mathcal{H}_\Gamma)$. В ∞-категорном языке это размерность критического многообразия — а **не** $\pi_0(\text{Map}(\Gamma, T))$, которое тривиально ($=1$), так как $\text{Map}(\Gamma,T)$ стягиваемо.

**Доказательство (схема).**

1. **Морс–Ботт на интерьере.** $\mathcal{F}[\Gamma]$ — гладкая функция на $\mathrm{Int}(\mathcal{D}(\mathbb{C}^7)) = \{\Gamma > 0\}$ (положительно определённые матрицы плотности — открытое многообразие). Аттрактор $\rho^* \in \mathrm{Int}(\mathcal{D})$ по примитивности $\mathcal{L}_0$ [T-39a]. Все критические точки $\mathcal{F}[\Gamma]$ лежат в интерьере: $\mathcal{L}_0$ отображает $\mathrm{Int}(\mathcal{D})$ в $\mathrm{Int}(\mathcal{D})$ (CPTP + примитивность), поэтому градиентные потоки не покидают $\mathrm{Int}(\mathcal{D})$. Критические подмногообразия — орбиты $G_2$-действия. Теория Морса–Ботта применяется без граничных осложнений.
2. **Нуль-модовое (критическое) многообразие.** По Морсу–Ботту критическое множество $\mathcal{F}$ через $\Gamma$ — гладкое подмногообразие, чьё касательное пространство есть в точности $\ker(\mathcal{H}_\Gamma)$ — «плоские» направления, вдоль которых $\mathcal F$ не меняется во втором порядке. (Это **не** число градиентных траекторий: по Пикару–Линделёфу градиентный поток $\dot\Gamma=-\nabla\mathcal F$ имеет **единственную** траекторию через каждую точку, так что «число траекторий» — неправильный инвариант. Инвариант — *размерность плоских направлений*.)
...
Состояния на границе $\partial\mathcal{D}(\mathbb{C}^7)$ (rank < 7) исключены из анализа: примитивность $\mathcal{L}_0$ [T-39a] гарантирует, что $\exp(\tau\mathcal{L}_0)[\Gamma] \in \mathrm{Int}(\mathcal{D})$ для любого $\tau > 0$ — потоки эволюции мгновенно входят в интерьер. Свобода для граничных состояний определяется по непрерывности: $\mathrm{Freedom}(\Gamma) := \lim_{\varepsilon \to 0} \mathrm{Freedom}(\Gamma + \varepsilon I/7)$.
:::
...
| Понятие | Соотношение со свободой (согласовано со свойствами (a)–(d) выше) |
|---|---|
| **Интеграция $\Phi$** | Выше $\Phi$ — больше структуры и потому **меньше** плоских направлений: количественная Freedom *убывает* (максимум $\mathrm{Freedom}=7$ — у $I/7$ с $\Phi=0$) |
| **Рефлексия $R$** | $R \geq 1/3$ необходимо, чтобы свободу **переживать** как таковую [И]; она же фиксирует направление $\varphi$, сокращая нулевые моды |
| **L2-уровень** | Количественная Freedom **сжимается** вдоль L0 → L2 (свойство (d)); приобретение L2 — не новые нулевые моды, а *осознание* оставшихся: выбор качественно нового рода |
| **Автопоэзис** | Свобода — аспект автопоэтической самоорганизации: система тратит плоские направления, покупая структуру |
...
|---|---|---|
| **Единственный примитив** | ∞-топос $\mathrm{Sh}_\infty(\mathcal{C})$ | ✓ [Аксиома Ω⁷](./axiom-omega) |
...
| **Октонионная структура** | P1+P2 → $\mathbb{O}$ → N=7, $G_2$, Фано | ✓ [Структурный вывод](../../proofs/minimality/theorem-octonionic-derivation) |
...
| Критерий | УГМ |
|---|---|
| Примитивы | 1 (∞-топос $\mathrm{Sh}_\infty(\mathcal{C})$) |
| Аксиомы | 5 |
...
#### Теорема T-55 (Неполнота Лоувера) [Т] {#неполнота-ловера}

:::warning Теорема (Неполнота Лоувера)
...
**Связь с теоремой Лоувера.** Теорема Лоувера о неподвижной точке запрещает сюръекцию $A \to \Omega^A$ в любой декартово замкнутой категории. В нашем контексте это означает: никакой внутренний объект не может «перечислить» все предикаты $\Omega$. Теорема T-55 — конкретная реализация этого принципа: $\mathrm{Th}_{\mathrm{UHM}}$ — максимальный $\varphi$-замкнутый подобъект, но он строго меньше $\Omega$.
...
| Механизм | Самоссылка в арифметике (Гёдель) | $\varphi$-инвариантность предикатов (Лоувер) |
...
Голоном моделирует себя через $\varphi$; теория моделирует себя через $\varphi^*$. Оба уровня принципиально неполны (граница Холево / теорема Лоувера) и эволюционно открыты (O-инжекция).
...
## 11. Вычислительные конфигурации Γ {#вычислительные-конфигурации}
...
### 11.2 Классификация вычислительных конфигураций {#классификация-вычислений}
...
| **Классическое** | $\|\gamma_{ij}\| \ll 1/N$ | $\ll 1$ | L0–L1 | Цифровой компьютер, нейросеть |
| **Квантовое когерентное** | $\|\gamma_{ij}\| \sim O(1/\sqrt{N})$ | $> 0$ | L0–L1 | Квантовый компьютер |
| **Автопоэтическое** | Соответствует (AP)+(QG)+(V) | $\geq 1$ | L1–L2 | Живая клетка, организм |

:::warning Теорема (Классический предел и L2-невозможность) [Т]
...
Следовательно, классические вычисления **не** достигают L2 ($\Phi \geq 1$ необходимо). Это не технологическое ограничение — это следствие определения $\Phi$ как функции когерентностей ([T-129](/docs/core/foundations/axiom-septicity#теорема-порог-интеграции) [Т]).
:::

**Доказательство.** Мера интеграции $\Phi$ определяется как информационная связность ([dimension-u](/docs/core/structure/dimension-u#мера-интеграции-φ)): $\Phi(\Gamma) = \min_{\text{bipartitions}} [S(\Gamma_A) + S(\Gamma_B) - S(\Gamma)]$. Для почти-диагональной $\Gamma$ с $|\gamma_{ij}| = \varepsilon \ll 1$: энтропия $S(\Gamma) \approx S(\text{diag}(\Gamma)) - O(\varepsilon^2)$, а разность $S(\Gamma_A) + S(\Gamma_B) - S(\Gamma) = O(\varepsilon^2)$. При $\varepsilon \ll 1/N$ это даёт $\Phi \ll 1$. $\blacksquare$
...
:::note Почему целое больше суммы частей
...
:::warning Определение (Мера эмерджентности)
...
При $I > 0$ целое содержит **информацию, отсутствующую в частях** — это формализация эмерджентности. Подробнее о композитных системах: [Составные системы](../dynamics/composite-systems).
...
3. $D_{\mathrm{int}} \mapsto V_{\mathrm{Gap}}$: [спектральное действие](/docs/core/dynamics/gap-thermodynamics#вывод-vgap-из-спектрального-действия) [Т].
...
**Существование неподвижной точки.** Теорема Брауэра требует компактной **выпуклой** области (эквивалентно, замкнутого шара); орбитное пространство $(S^1)^{21}/G_2$ — фактор тора, **не** выпуклое (вращение $S^1$ не имеет неподвижной точки), поэтому Брауэр к нему напрямую неприменим. Вместо этого $G_2$-орбитная редукция (T-64 [Т]) выражает $V_{\mathrm{Gap}}$ через **5 секторных параметров** $\vec\varepsilon=(\varepsilon_{O3},\varepsilon_{O\bar3},\varepsilon_{33},\varepsilon_{\bar3\bar3},\varepsilon_{3\bar3})\in[0,1]^5$ — компактный **выпуклый ящик**. Композиция $\mathcal F$ ограничивается до непрерывного самоотображения $\mathcal F:[0,1]^5\to[0,1]^5$ (каждый шаг сохраняет физические диапазоны параметров), и **теорема Брауэра** на выпуклом ящике $[0,1]^5$ даёт неподвижную точку $\vec\varepsilon^*$, эквивалентно $\theta^*$.
...
Спектральное самозамыкание означает: **теория определяет собственную динамику**. Потенциал $V_{\mathrm{Gap}}$, управляющий динамикой когерентностей, порождён спектральной тройкой, которая сама определена стационарным состоянием этой динамики. Это — реализация автопоэзиса (A1) на уровне самой теории. Неподвижная точка $\theta^*$ — **категориальный аттрактор** в [∞-топосе](/docs/proofs/categorical/categorical-formalism) [Т].
:::

---

## 13. Границы непрерывного обучения {#границы-обучения}

:::note Связь с ядром теории
Этот раздел описывает **условные** результаты (статус [С]), выводимые из аксиом УГМ при дополнительных предположениях о механизме обучения. Безусловные теоремы о границах обучения (T-109 — T-113 [Т]) см. в [Learning Bounds](/docs/applied/coherence-cybernetics/learning-bounds).
:::

### 13.1 Ограничение catastrophic forgetting (C24) [С] {#forgetting-bound}

**Статус:** [С] — условно при наличии EWC-регуляризации и Бюрес-адаптивной скорости обучения.

:::warning Утверждение (Граница забывания) [С]
...
где $P_\text{ISL}$ — распределение ISL (Inner Speech Loop), $\eta_0$ — базовая скорость обучения, $\sigma$ — стресс-тензор ($\sigma_k = 1 - 7\gamma_{kk}$, [T-92](/docs/core/structure/dimension-a#вывод-формулы-напряжения)).
:::

**Доказательство (скетч).**

**Шаг 1 (Бюрес-контрактивность).** CPTP-канал $\mathcal{E}_\eta$ (один шаг обучения с параметром $\eta$) удовлетворяет контрактивности Ульманна:
$$d_B(\mathcal{E}_\eta[\Gamma], \Gamma) \leq \eta \cdot \|\nabla_\Gamma \mathcal{L}\|_B$$
где $\|\cdot\|_B$ — норма, индуцированная метрикой Бурес. При Бюрес-адаптивном $\eta = \eta_0 / \sqrt{\det(g_B(\Gamma))}$, шаг в пространстве состояний ограничен: $d_B(\Gamma_{\text{new}}, \Gamma_{\text{old}}) \leq \eta_0 \cdot \|\nabla\sigma\|$.
...
**Явные условия [С]:**
1. EWC-регуляризация с $\lambda > 0$ (специфицирована в [σ-directed loop](/docs/applied/coherence-cybernetics/learning-bounds#компромисс-обучение-стабильность))
...
### 13.2 Реконструкция σ из скрытых состояний (C25) [С] {#sigma-probe}

**Статус:** [С] — условно при достаточной размерности скрытого пространства.

:::warning Утверждение (σ-probe) [С]
...
- [Структурный вывод через октонионы](../../proofs/minimality/theorem-octonionic-derivation) — P1+P2 → $\mathbb{O}$ → N=7
