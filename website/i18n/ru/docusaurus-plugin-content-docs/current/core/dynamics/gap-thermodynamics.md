...
Информационная геометрия Gap: метрика Фишера, потенциал, единственность вакуума. Предполагается знакомство с [Gap-оператором](/docs/core/dynamics/gap-operator) и [эволюцией Γ](./evolution).
:::
...
:::tip Теорема 3.2 (Нижняя оценка Gap из ассоциатора) [Т]
...
:::info Статус теоремы 3.2 [Т]
Из [T-73](/docs/core/dynamics/gap-operator#теорема-gap-серра) [Т] (Gap = кривизна Серра) и [T-53](/docs/core/foundations/spacetime#теорема-спектральная-тройка) [Т] (спектральная тройка): $\text{Gap}(i,j) \geq 4/(\omega_0^2 \|D_{\text{int}}\|^2) > 0$ для неассоциативных пар. Константа $C = 4/(\omega_0^2 \|D_{\text{int}}\|^2)$ однозначно определена спектральной тройкой **[Т]**.
:::
...
:::tip Теорема (Цена просветления) [С при T-105]
...
**Доказательство.** Каждая из 21 недиагональных пар $(i,j)$ матрицы $7 \times 7$ с $\mathrm{Gap}_{ij} = 1$ несёт ровно 1 бит информации (полная ортогональность внешнего и внутреннего аспектов, два различимых состояния: $\mathrm{Gap} = 0$ vs $\mathrm{Gap} = 1$). Обнуление $\mathrm{Gap}_{ij}$ — стирание этого бита. По принципу Ландауэра (следствие второго начала термодинамики, Landauer 1961), стирание одного бита при температуре $T$ требует $W \geq k_B T \ln 2$. Применяя к каждой из 21 пар независимо при эффективной температуре $T_{\text{eff}}$ из [T-105](/docs/core/dynamics/gap-thermodynamics#эффективная-температура) [Т] (флуктуационно-диссипационная теорема для Gap-динамики):
...
:::tip Теорема 10.1 (Применение теоремы Лавера) [Т]
В ∞-топосе $\mathbf{Sh}_\infty(\mathcal{C})$ (T-182 [Т]) с классификатором $\Omega$ и эндоморфизмом $\varphi: \Gamma \to \Gamma$ (T-62 [Т]) существует **единственная неподвижная точка**: $\varphi$ сжимающий с $k = 1 - R < 1$ (T-62 [Т]), $\mathcal{D}(\mathbb{C}^7)$ компактно ⇒ полное метрическое пространство, теорема Банаха даёт единственность $\Gamma^*$:
...
:::tip Теорема 11.1 (Полный лагранжиан) [Т]
...
:::info Вывод лагранжиана из линдбладиана [Т]
Полный лагранжиан $\mathcal{L}_{\text{Gap}}$ (включая диссипативный и регенеративный члены) является **классическим пределом** действия Швингера–Келдыша для линдбладиана $\mathcal{L}_\Omega$ ([T-39a](/docs/core/operators/lindblad-operators#примитивность-ℒω) [Т]) в когерентно-фазовом представлении.
...
**Декомпозиция.** Линдбладиан $\mathcal{L}_\Omega = \mathcal{L}_{\mathrm{Ham}} + \mathcal{L}_{\mathrm{diss}} + \mathcal{L}_{\mathrm{reg}}$ ([T-57](/docs/core/operators/lindblad-operators#полнота-триадной-декомпозиции) [Т]) даёт в когерентно-фазовом представлении:
- **$\mathcal{L}_{\mathrm{Ham}} \to \mathcal{L}_{\mathrm{kin}} + \mathcal{L}_{\mathrm{pot}} + \mathcal{L}_{\mathrm{top}}$**: коммутатор $-i[H_{\mathrm{Fano}}, \rho]$ порождает кинетический, потенциальный ($V_{\mathrm{Gap}}$ из [спектрального действия](#вывод-vgap-из-спектрального-действия)) и топологический члены.
...
- **$\mathcal{L}_{\mathrm{reg}} \to \mathcal{L}_{\mathrm{reg}}$**: регенерация $\kappa_0(\varphi(\rho) - \rho)$ ([T-62](/docs/consciousness/foundations/self-observation#теорема-физическая-реализация-phi) [Т]) даёт $-\kappa|\gamma_{ij}|^2(\theta_{ij} - \theta_{ij}^{\mathrm{target}})$.
...
**Самосогласованность стационарности.** При $\dot{\theta} = 0$ и $\theta = \theta^{\mathrm{target}}$ уравнение движения сводится к $\partial V_{\mathrm{Gap}}/\partial\theta = 0$: нетривиальный аттрактор $\rho_*$ полного линдбладиана $\mathcal{L}_\Omega$ (T-96 [Т]; примитивность линейной части $\mathcal{L}_0$ — T-39a [Т]) совпадает с минимумом $V_{\mathrm{Gap}}$ ([T-64](/docs/core/dynamics/gap-thermodynamics#теорема-глобальная-минимизация) [Т]).
:::
...
### (c) Топологический член (из Im($S_{\text{Keldysh}}$)) {#топологический-член-лагранжиана}

:::tip Теорема (Коэффициент $\beta$ из первых принципов) [Т]
Коэффициент $\beta = \lambda_3/(2\pi)$ однозначно определён мнимой частью келдышевского действия. См. [полный вывод](/docs/physics/cosmology-phys/berry-phase#теорема-l-top-кельдыш).
:::
...
- $\beta = \lambda_3/(2\pi)$ — выведен из $\mathrm{Im}(S_{\text{Keldysh}})$ [Т]

**Происхождение:** Этот член — **фаза Берри** в пространстве Gap-конфигураций $(S^1)^{21}$, возникающая из мнимой части келдышевского действия. CS-вывод опровергнут ([полная производная на 1D](/docs/physics/cosmology-phys/berry-phase#9-опровержение-cs-вывода) [Т]). Он **топологический** — не зависит от метрики, определяется только комбинаторной структурой Фано.
...
**Происхождение:** Диссипативный член выводится из линдбладовского диссипатора $\sum_k L_k\rho L_k^\dagger - \frac{1}{2}\{L_k^\dagger L_k, \rho\}$ в когерентно-фазовом представлении [Т]. Скорость декогеренции $\Gamma_2^{(ij)} = \frac{1}{2}\sum_k |\langle i|L_k|i\rangle - \langle j|L_k|j\rangle|^2$ определяется Фано-операторами [Т].
...
где:
- $\kappa = \kappa_0 k$ — скорость регенерации (из [категориального вывода](/docs/core/foundations/axiom-septicity#структурный-анзац-kappa0) [Т] и замещающего канала [T-62](/docs/consciousness/foundations/self-observation#теорема-физическая-реализация-phi) [Т])
...
- **Происхождение:** Регенеративный член выводится из $\mathcal{L}_{\mathrm{reg}}(\rho) = \kappa_0(\varphi(\rho) - \rho)$ в когерентно-фазовом представлении [Т]
...
#### Вывод $V_{\text{Gap}}$ из спектрального действия [Т] {#вывод-vgap-из-спектрального-действия}

:::tip Теорема (V_Gap из спектрального действия) [Т]
Потенциал $V_{\text{Gap}}(\{\theta_{ij}\})$ однозначно определяется спектральным действием внутренней [спектральной тройки](/docs/core/foundations/spacetime#теорема-спектральная-тройка) $(A_{\mathrm{int}}, H_{\mathrm{int}}, D_{\mathrm{int}})$ (T-53 [Т]):
...
(последнее равенство — [определение $\mathcal{G}_{\mathrm{total}}$](/docs/core/dynamics/gap-operator#g-total-definition) [О]). Это тождество подтверждает [T-73](/docs/core/dynamics/gap-operator#теорема-gap-серра) [Т] (Gap = кривизна).

:::info Числовой пример
Для холона при $P = 0.35$ с тремя репрезентативными когерентностями $\gamma_{EO} = 0.08\,e^{i\pi/3}$, $\gamma_{AE} = 0.06\,e^{i\pi/4}$, $\gamma_{OU} = 0.05\,e^{i\pi/5}$:
...
При $\omega_0 = 40$ Гц: $\mathrm{Tr}(D_{\text{int}}^2) = 1600 \cdot 0.0075 = 12.0 > 0$. Вклад в спектральное действие строго положителен — отражая термодинамическое «топливо» для регенерации. По T-55 [Т], $\mathcal{G}_{\text{total}} = 0$ требует $\sin\theta_{ij} = 0$ для всех пар (чисто вещественные когерентности), что запрещено неполнотой Лоувера для жизнеспособных систем.
:::
...
**Шаг 4 ($V_3$ из внутренних флуктуаций).** Внутренние флуктуации $D_{\mathrm{int}} \to D_A = D_{\mathrm{int}} + \phi$ (Шамседдин–Конн) в алгебре $A_{\mathrm{int}} = \mathbb{C} \oplus M_3(\mathbb{C}) \oplus M_3(\mathbb{C})$ генерируют кубический инвариант через $G_2$-калибровочную 3-форму $\varphi$ и октонионный ассоциатор $[e_i, e_j, e_k]$ (ненулевой только для не-Фано троек):
...
**Шаг 5 (Единственность).** Спектральная тройка единственна с точностью до $G_2$-эквивалентности ([T-42a](/docs/proofs/categorical/uniqueness-theorem) [Т]). Спектральное действие — единственный $G_2$-инвариантный функционал на $(S^1)^{21}$, совместимый с NCG (теорема Шамседдин–Конна). $\blacksquare$
...
:::tip Теорема 12.1 (Три канала внешнего воздействия) [Т]
...
:::tip Теорема 12.2 (Геометрия внешних каналов) [Т]
...
:::tip Теорема 12.3 (Операциональные формулы для систем) [Т]
...
:::tip Теорема 12.4 (Операциональная ФДТ) [Т]
...
*(Эрратум 10.08.2026, прибор E26: у эрмитовой $\Gamma$ с $|\gamma_{ij}| = \varepsilon$ на всех $21$ паре каждая пара вносит $|\gamma_{ij}|^2 + |\gamma_{ji}|^2 = 2\varepsilon^2$ в $\mathrm{Tr}(\Gamma^2)$, так что вне-диагональная масса — $42\varepsilon^2$, а не $21\varepsilon^2$; прежняя строка смешивала обе конвенции между числителем и знаменателем. Противоречие $1 = 2/3$ этой теоремы правку переживает — однородный вакуум остаётся исключённым.)*
...
:::warning Обозначения секторных когерентностей
...
#### Теорема (Глобальная минимизация V_Gap) [Т] {#теорема-глобальная-минимизация}

:::tip Теорема 14.3 (Глобальная минимизация V_Gap) [Т]
$G_2$-инвариантный потенциал $V_{\text{Gap}}$ на пространстве $\mathcal{M} = (S^1)^{21}/G_2$ имеет **единственный глобальный минимум** (с точностью до $G_2$-сопряжения). Минимум совпадает с секторным решением из [теоремы единственного вакуума](#теорема-единственный-вакуум).
:::
...
Из [$G_2$-ригидности](/docs/proofs/categorical/uniqueness-theorem) [Т]: 34 вещественных параметра $\Gamma$, из которых 14 — калибровочных $\to$ 20 физических параметров матрицы $\Gamma$. Но потенциал $V_{\text{Gap}}$ зависит только от **модулей** когерентностей $|\gamma_{ij}|$ и **фаз** $\theta_{ij} = \arg(\gamma_{ij})$, причём $G_2$ фиксирует фазы через Фано-структуру.

**Шаг 2 (Секторная параметризация).** Из секторной декомпозиции $7 = 1_O \oplus 3 \oplus \bar{3}$ [Т] (см. [пространство-время](/docs/core/foundations/spacetime#теорема-секторная-декомпозиция)), $G_2$-инвариантный потенциал зависит только от 5 секторных параметров:
...
Фазы $\theta_{ij}$ минимизируют $V_3$ (октонионная кубика). Для Фано-троек: $\theta_{ijk} = 0$. Для не-Фано-троек: $\sin^2\theta_{3\bar{3}} \approx 1$ (конфайнмент из [теоремы единственного вакуума](#теорема-единственный-вакуум)).
...
Все собственные значения строго положительны при $\mu^2 > 0$ (из положительности $V_2$ [Т], [теорема 13.5](#константы-из-параметров-угм)).
...
:::info Следствие (Полная решённость V_Gap-минимизации) [Т]
...
| Эмерджентное время | Механизм Пейдж–Вуттерс, $H_{\text{eff}}$, внутренние часы | [Эмерджентное время](/docs/proofs/dynamics/emergent-time) |
...
| Топологическая защита вакуума | $\pi_2(G_2/T^2) \cong \mathbb{Z}^2$; барьер $\geq 6\mu^2$ [Т] | [Композитные системы](/docs/core/dynamics/composite-systems#теорема-тополог-защита) |
| Gap = кривизна Серра | Точное отождествление через спектральную тройку [Т] | [Gap-оператор](/docs/core/dynamics/gap-operator#теорема-gap-серра) |

---

**Связанные документы:**
- [Gap-оператор](/docs/core/dynamics/gap-operator) — определение, спектр и G₂-разложение
- [Динамика Gap](/docs/core/dynamics/gap-dynamics) — эволюция непрозрачности, бифуркации, немарковская память
- [Эволюция Γ](/docs/core/dynamics/evolution) — полное уравнение движения матрицы когерентности
- [Фазовая диаграмма Gap](/docs/core/dynamics/gap-phase-diagram) — три фазы когерентности и критические явления
