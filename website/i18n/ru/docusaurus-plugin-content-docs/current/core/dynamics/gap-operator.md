...
#### Конвенция: исчезающая когерентность {#конвенция-нулевой-когерентности}

**[О]** При $|\gamma_{ij}| < \varepsilon_{\min}$ фаза $\arg(\gamma_{ij})$ — а с ней и $\sin(\arg\gamma_{ij})$ — не определена, а операционально канал не несёт информационного потока. Конвенция: $\mathrm{Gap}(i,j) := 1$ (максимальная непрозрачность) при исчезающей когерентности. Именно этот предел инстанцирует феноменология состояний («$\gamma_{AE} \to 0 \Rightarrow \mathrm{Gap}(A,E) \to 1$» во сне и анестезии). Заметьте различие, которое кодирует конвенция: $\mathrm{Gap}$ измеряет *фазовую* прозрачность, поэтому слабый, но синфазный канал выше $\varepsilon_{\min}$ имеет малый $\mathrm{Gap}$ (прозрачен) при низкой пропускной способности — прозрачность и сила суть независимые оси, и конвенция замыкает вырожденный угол, где фазовая ось теряет смысл. Со всем этим едет заметка о фрейме (эрратум T-301): попарные фазы читаются в родном семантическом фрейме и двигаются при перефазировке осей — *почерк*; калибровочно-инвариантное фазовое содержание тройки — её голономия Фано (см. [Структуру квалиа](/docs/consciousness/phenomenology/qualia-structure#язык-качества)).
...
:::info Необходимость комплексной Γ [T-132]
...
| Ранг | Спектр $\hat{\mathcal{G}}$ | $H$ | $\dim(H)$ | $G_2/H$ | $\pi_2(G_2/H)$ |
|------|---------------------------|-----|-----------|---------|-----------------|
| 0 | $(0,0,0)$ | $G_2$ | 14 | $\{pt\}$ | 0 |
...
**Следствие.** Только при ранге 3 с общим спектром вторая гомотопическая группа $\pi_2(G_2/T^2) \cong \mathbb{Z}^2 \neq 0$, что обеспечивает **топологическую защиту**: невырожденные Gap-конфигурации не могут быть непрерывно стянуты к тривиальным ($G_2$ односвязна, поэтому $\pi_1(G_2/T^2) = 1$; нетривиальный инвариант живёт в $\pi_2$, равной $\pi_1(T^2) = \mathbb{Z}^2$). Это — один из [пяти типов защиты Gap](/docs/core/dynamics/gap-phase-diagram#защита-gap).
