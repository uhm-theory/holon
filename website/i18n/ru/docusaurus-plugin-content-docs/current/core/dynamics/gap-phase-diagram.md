...
Среднеполевые критические показатели **точны** для Gap-системы. $N = 7$ строго установлено (Теорема S [Т], T-183 [Т]). Эффективная размерность $d_{\mathrm{eff}} = \binom{7}{2} = 21 > d_c = 4$; даже при $G_2$-редукции ($21 - 14 = 7$) имеем $7 > d_c = 4$:
