...
:::tip Теорема 7.2 (Точность среднеполевых показателей) [Т]
Среднеполевые критические показатели **точны** для Gap-системы по трём независимым строгим механизмам — топологическая жёсткость Тома–Арнольда катастрофы $A_4$ (swallowtail), детерминированная (нестохастическая) УГМ-динамика, и перекрёстная проверка large-$N$ с $d_{\mathrm{eff}} = \binom{7}{2} = 21$ моды параметра порядка — объединено в [Механизм точности](/docs/consciousness/hierarchy/swallowtail-transitions#механизм-точности) [Т].

**(a)** Размерность параметра порядка $d_{\text{eff}} = 21$ = число независимых off-diagonal когерентностей $\Gamma \in \mathcal D(\mathbb C^7)$. Это подлинный счёт флуктуационных мод в любой стохастической переинтерпретации динамики.

**(b)** Пространственно-размерная форма критерия Гинзбурга **неприменима**, потому что УГМ — $(0{+}1)$-мерная (нет пространственного интегрирования). Среднеполевая точность вместо этого устанавливается топологически (Том–Арнольд) и динамически (детерминированный поток).

**(c)** Вблизи трикритической точки эффективная теория — $\varphi^6$, $A_4$ катастрофа коразмерности 3, соответствующая трём физическим управляющим параметрам УГМ $(\kappa, \gamma_{\text{Линдблад}}, \Delta F)$. Показатели $\{\alpha,\beta,\gamma,\nu,\delta\} = \{1/2, 1/4, 1, 1/2, 5\}$ — топологические инварианты класса $A_4$.
:::

:::note О независимости 21 моды когерентности
Счёт $d_{\text{eff}} = \binom{7}{2} = 21$ относится к 21 независимой off-diagonal паре $(i,j)$ с $1 \le i < j \le 7$, соответствуя $42 = 21\cdot 2$ вещественным off-diagonal компонентам $\mathfrak{su}(7)$. $G_2 \subset SO(7)$ редуцирует это до $21 - 14 = 7$ $G_2$-инвариантных мод в глубоко нарушенной фазе, но полные $21$ входят в флуктуационный счёт вблизи порога (до $G_2$-фиксации) — согласовано с катастрофически-теоретическим счётом коразмерности-3 деформаций $A_4$. Это не изменяет топологическую защиту показателей в (I).
:::
