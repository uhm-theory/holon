...
:::tip Утверждение С.1 (Субъективная дилатация времени) [Т]
Отношение субъективного и физического приращений времени пропорционально субъективному темпу:

$$
\frac{\delta\tau_{\text{субъ}}}{\delta\tau_{\text{физ}}} \propto \mathcal{T}(\tau) = \frac{|\gamma_{OE}|}{\gamma_{OO}}
$$

**Вывод [Т].** Связь $\gamma_{OE}$ с переживанием времени — **не** семантический постулат, а **теорема**, следующая из трёх [Т]-результатов:
1. **T-87 [Т]:** O — измерение часов. Время $\tau$ возникает из корреляций между O и остальными измерениями через механизм Пейджа–Вуттерса.
2. **T-186(a) [Т]:** E — измерение опыта. Феноменальный функтор $F \cong \&|_{\mathcal{D}}$ извлекает E-сектор как носитель субъективного содержания.
3. **T-88 [Т]:** Связь $\gamma_{OE}$ входит в скорость регенерации $\kappa_0 = \omega_0 \cdot |\gamma_{OE}| \cdot |\gamma_{OU}| / \gamma_{OO}$, управляющую скоростью обновления переживательного состояния системы.

Отношение $|\gamma_{OE}|/\gamma_{OO}$ есть **скорость производства переживательного содержания на один такт часов** — выводимая величина, а не конвенция. $\square$
...
Повышение понимания ($\gamma_{LE}$) и качества самомодели ($R_\varphi$) при углублении связи с основанием. Субъективно: время одновременно «насыщено» и «прозрачно».

**Числовой пример.** Опытный практикующий випашьяну: $\gamma_{LE} = 0.28$, $R_\varphi = 0.65$, $\gamma_{EO} = 0.20$, $\mathcal{T} = 0.60$. Субъективный темп умеренно повышен, но ключевое отличие от потока — высокая $\gamma_{LE}$ (осознание присутствует) и высокое качество самомодели $R_\varphi$ ([три рабочие формы R](/docs/consciousness/foundations/self-observation#формы-r)). Медитатор одновременно «в потоке» и «наблюдает себя» — состояние, невозможное без $R_\varphi \geq R_{\varphi,\text{th}} = 1/3$.
