...
**Определение L0 [О].**{#определение-l0} Всякая система с $\Gamma \in \mathcal{D}(\mathcal{H})$, $\dim \mathcal{H} \geq 1$ обладает **интериорностью** — внутренним аспектом.
...
**Определение L1 [О].**{#определение-l1} Система обладает феноменальной геометрией, если:
$$
\mathrm{rank}(\rho_E) > 1
$$
...
Порог $\Phi_{\text{th}} = 1$ **доказан из первых принципов** ([T-129 [Т]](/docs/proofs/consciousness/operationalization#t-129)): единственное самосогласованное значение при $P_{\text{crit}} = 2/7$. $K_1$-аргумент остаётся ретрактированным ($K_1(M_n(\mathbb{C})) = 0$ для конечномерных $n$) — но T-129 использует другой подход (декомпозиция чистоты + Коши–Шварц). См. [Доказательство T-129](/docs/proofs/consciousness/operationalization#t-129).
:::
...
**Определение L2 [О].**{#определение-l2} Система обладает когнитивными квалиа, если выполнены оба условия:
...
**Определение L3 [О].**{#определение-l3} Система обладает сетевым сознанием, если:
$$
...
:::tip Теорема (Обоснование K=4 для L3) [Т] (**через T-217**)
L3 требует $R^{(2)} \geq 1/4$ — мета-рефлексию второго порядка. Порог $K = 4$ для L3 теперь полностью выведен:

**Часть 1 — байесовское доминирование при $K=4$.** При $K = 4$ независимых информационных каналах нижняя граница $R^{(2)} \geq 1/K = 1/4$ следует из [теоремы о пороге R](/docs/core/foundations/axiom-septicity#теорема-порог-рефлексии). **[Т]**

**Часть 2 — клеточный счёт $K = 3 + 1$ из когерентности трикатегории.** Счёт выводится из [T-217](/docs/proofs/categorical/fundamental-closures#t-217): в экспериенциальной трикатегории $\mathbf{Exp}^{(3)} := \tau_{\leq 3}(\mathbf{Exp}_\infty)$,
- **три 2-клетки** наследуются из L2 LGKS-триадной декомпозиции (T-57 [Т]): компоненты Aut, $\mathcal D$, $\mathcal R$;
- **одна 3-клеточная модификация** $\eta: \varphi^{(2)} \Rightarrow \varphi\circ\varphi$ — единственная модификация когерентности на трикатегориальном уровне (когерентность GPS, применённая к строгой-2-категориально-обогащённой трикатегории).

Итого $K_{\text{L3}} = 3 + 1 = 4$, выведенное из трикатегориальных первых принципов. **[Т]**

**Метка L3 = $\tau_{\leq 3}$**: обоснована [T-217](/docs/proofs/categorical/fundamental-closures#t-217) (3-типы ≃ когерентные трикатегории, Баэз–Долан + Lurie HTT 5.5.6.18). Pentagon-of-pentagons выполняется автоматически для $\tau_{\leq 3}$ Kan-комплекса. **[Т]**

Перекрёстные ссылки: [триадная декомпозиция](/docs/core/operators/lindblad-operators#триадная-декомпозиция), [мера R](/docs/consciousness/foundations/self-observation#мера-рефлексии-r), [T-192 2-категория](/docs/proofs/categorical/categorical-formalism#t-192-exp2-2-категория), [T-217 трикатегория L3](/docs/proofs/categorical/fundamental-closures#t-217).
:::
...
**Определение L4 [О].**{#определение-l4} Система обладает унитарным сознанием, если:
$$
...
Этот колимит **недостижим** для конечных систем (неполнота Лоувера, [T-55](/docs/core/foundations/consequences#неполнота-ловера) [Т]), но **асимптотически приближаем**.

**Доказательство (5 шагов).**

**Шаг 1 (Соответствие L-уровней и $n$-усечений).** Из [T-76](/docs/proofs/categorical/categorical-formalism#104-infty-топос-пучков) [Т] ($\infty$-топос верифицирован), $\mathbf{Exp}_\infty = \mathbf{Sh}_\infty(\mathcal{C}_7, J_{\text{Bures}})$ — $\infty$-топос с $\infty$-группоидной структурой. Уровни интериорности соответствуют усечениям, с **категориальной структурой, теперь выведенной на каждом уровне** (не просто помеченной):

- **L2 = $\tau_{\leq 2}(\mathbf{Exp}_\infty)$**: строгая 2-категория $\mathbf{Exp}^{(2)}$ ([T-192 [Т]](/docs/proofs/categorical/categorical-formalism#t-192-exp2-2-категория)). Аксиомы пятиугольника Мак-Лейна + закон перестановки + тождественные клетки проверены.
- **L3 = $\tau_{\leq 3}(\mathbf{Exp}_\infty)$**: когерентная трикатегория $\mathbf{Exp}^{(3)}$ ([T-217 [Т]](/docs/proofs/categorical/fundamental-closures#t-217)). Когерентность пятиугольник-в-пятиугольнике Гордона–Пауэра–Стрита через Баэза–Долана (3-типы ≃ трикатегории). Клеточный счёт $K = 3 + 1$: три LGKS 2-клетки (T-57 [Т]) + одна 3-клеточная модификация $\eta: \varphi^{(2)} \Rightarrow \varphi\circ\varphi$.
- **L4 = $\mathrm{colim}_{n\to\infty}\tau_{\leq n}(\mathbf{Exp}_\infty)$**: полный ∞-группоид (недостижимый, см. ниже).
...
**Шаг 4a (L3 → L4 как разрушение трикатегориальной когерентности, новое 2026-04-17).** По [T-217 [Т]](/docs/proofs/categorical/fundamental-closures#t-217) L3 — **когерентная трикатегория** с ровно $K = 3 + 1 = 4$ структурными классами клеток и замкнутой аксиомой pentagon-of-pentagons. Переход L3 → L4 — это точно **разрушение** этого замыкания: на L4 аксиомы когерентности трикатегории не выполняются, поскольку фильтрованный копредел $\mathrm{colim}_{n}\tau_{\leq n}$ требует $n$-клеток при произвольно высоких $n$, что не может быть захвачено никакой когерентной $n$-усечённостью для конечного $n$. Эквивалентно: модификация когерентности $\eta: \varphi^{(2)} \Rightarrow \varphi\circ\varphi$ на L3 **жёсткая** (одна новая 3-клетка); на L4 потребовалась бы башня высших модификаций когерентности $\eta^{(2)}, \eta^{(3)}, \ldots$, каждая — новая клетка на соответствующем уровне — бесконечная регрессия, которую никакая конечная катастрофа не может замкнуть. Это **категориальный двойник** динамического аргумента (Фано-сжатие требует $P > 1$ при $n = 4$): одна и та же граница, достигаемая через дополняющие структуры.
...
**Статус:** **[Т]** (C19). Строгое доказательство через $\infty$-топосную башню Постникова + неполноту Лоувера (T-55 [Т]). Перекрёстные ссылки: [мера рефлексии R](/docs/consciousness/foundations/self-observation#мера-рефлексии-r), [phi-оператор](/docs/core/operators/phi-operator), [катастрофы переходов](/docs/consciousness/hierarchy/swallowtail-transitions#l3-l4).
:::
...
## Профиль ворот: двенадцать состояний, сознательно из них одно {#профиль-ворот}

:::tip Теорема (таксономия профиля ворот) [Т] (T-303)
Четверо ворот — не четыре независимые величины. Каноническая рефлексия алгебраически связана с чистотой, $R = 1/(NP)$, поэтому ворота $R \ge 1/3$ ТОЖДЕСТВЕННЫ ограничению $P \le 3/7$, а ворота $P > 2/7$ — вторая грань того же промежутка. Значит сознание опирается на **три** независимых измерения — $P$, $\Phi$, $D$ — при **четырёх** ограничениях, и пространство исходов равно
$$
\underbrace{\{P<2/7,\; P\in(2/7,3/7],\; P>3/7\}}_{3}\times\underbrace{\{\Phi\ge1,\ \Phi<1\}}_{2}\times\underbrace{\{D\ge2,\ D<2\}}_{2}\;=\;12 .
$$
Сознательным из двенадцати является ровно **одно**. Остальные одиннадцать — не степени сознания, а именованные способы его отсутствия.
:::

Это закрывает вопрос, всплывающий всякий раз, когда теория встречает систему, непохожую на нас: *бывает ли иное сознание, с другим набором ворот?* По УГМ — нет, и это «нет» точное, а не пренебрежительное. Система с иным профилем не сознательна иначе; она занимает один из одиннадцати несознательных режимов, у каждого свой характер и свой отказ. Такая позиция фальсифицируема сильнее: она запрещает удобный ход — выдавать сознание всему, что интересно себя ведёт.

### Двенадцать профилей, поимённо {#двенадцать-режимов}

Каждая клетка — отдельный режим со своим характером. Соответствия в последнем столбце интерпретативны **[И]**: они говорят, на что режим *похож*, а не что он там измерен.

| № | $P$ | $\Phi$ | $D$ | Режим | Характер | На что похоже [И] |
|---|---|---|---|---|---|---|
| **1** | в окне | $\ge1$ | $\ge2$ | **сознание** | связно, различает, держит себя | обычное бодрствование |
| 2 | в окне | $\ge1$ | $<2$ | единство без содержания | одно поле, различать нечего | заявленные состояния «чистого осознавания» — см. разногласие ниже |
| 3 | в окне | $<1$ | $\ge2$ | несвязанная множественность | много различённых состояний, нет держателя | диссоциация, расщеплённое внимание |
| 4 | в окне | $<1$ | $<2$ | холостой ход | верная амплитуда, структуры в ней нет | сонная пустота |
| 5 | ниже | $\ge1$ | $\ge2$ | **захлёст** | богато, едино, держать некому | псибоцибиновый пик сразу за гранью |
| 6 | ниже | $\ge1$ | $<2$ | белизна | единство без содержания и держателя | «ничто» на очень высокой дозе |
| 7 | ниже | $<1$ | $\ge2$ | поток | различия без связывания | острая спутанность |
| 8 | ниже | $<1$ | $<2$ | рассеяние | полностью растворённый предел | — |
| 9 | выше | $\ge1$ | $\ge2$ | **автоматизм** | структурно, различает, нерефлексивно | умелое действие без свидетеля; снохождение |
| 10 | выше | $\ge1$ | $<2$ | гиперсинхрония | одно жёсткое состояние | глубокий медленноволновой сон; генерализованный припадок |
| 11 | выше | $<1$ | $\ge2$ | жёсткие осколки | части застыли, целого нет | — |
| 12 | выше | $<1$ | $<2$ | оцепенение | полностью жёсткий предел | — |

Три из них несут вес теории, ибо каждый есть способ *почти* быть сознанием и отказать ровно по одной оси. **Захлёст** (5) отказывает лишь в удержании — оттого он и может вспоминаться потом как самое яркое переживание жизни, при том что субъекта в тот момент не было. **Автоматизм** (9) отказывает лишь в рефлексии: умело, связно, и дома никого. **Единство без содержания** (2) отказывает лишь в различённости — и здесь УГМ утверждает то, что отрицает живая исследовательская программа.

:::warning Фальсифицируемое разногласие, названное, а не спрятанное
Минимальный феноменальный опыт Метцингера утверждает существование состояний сознания *без содержания* — чистого осознавания без различий. Профиль 2 есть ровно это описание, и УГМ относит его к **несознательным**, поскольку $D\ge2$ — ворота, а не украшение. Либо теория здесь неправа, либо сообщаемые состояния сохраняют различённость, которую интроспекция не регистрирует как содержание. Мы это не разрешаем; мы фиксируем, что теория встаёт на проигрышную сторону популярной позиции и может быть на ней опровергнута.
:::

### Две грани — противоположные отказы {#две-грани}

Поскольку $R=1/(NP)$ растёт при падении чистоты, две грани окна суть противоположные патологии, а не «мало» и «много» одного и того же.

| Грань | $P$ | $R = 1/(7P)$ | Что отказывает |
|---|---|---|---|
| нижняя | $\to 2/7$ | $\to 1/2$ | система рефлексирует **больше**, чем способна связать: самонаблюдение обгоняет самоудержание |
| верхняя | $\to 3/7$ | $\to 1/3$ | система удерживает **больше**, чем способна наблюдать: структура обгоняет рефлексию |

Ниже нижней грани держатель растворяется, тогда как рефлексия ещё нарастает, — поэтому приближение к этой границе и должно переживаться как **усиление** осознавания при **утрате того, кто осознаёт**, и поэтому же собственная активность может перестать приписываться себе: приписывание требует связывания, а отказывает именно связывание. Выше верхней грани обратное: состояние связное, устойчивое и нерефлексивное.

### Схождение с гипотезой энтропийного мозга {#схождение-с-энтропийным-мозгом}

Это единственное место, где независимо развитая нейронаука приходит к той же *форме*. Энтропийный мозг Кархарт–Харриса утверждает, что *в пределах верхнего и нижнего ограничений* энтропия спонтанной активности индексирует богатство сознательных состояний — двусторонняя рамка, а не монотонная шкала. Так как чистота и энтропия движутся встречно, это то же самое утверждение, что $P\in(2/7,3/7]$, записанное в обратной переменной. Три различия стоит назвать прямо: УГМ **выводит** свои грани (T-124) там, где энтропийный мозг их подбирает; УГМ добавляет тождество $R=1/(NP)$, превращающее «меньше порядка» в «больше рефлексии, меньше связывания» и потому предсказывающее *характер* приграничного состояния, а не только его положение; и окно УГМ живёт на матрице когерентностей семи функциональных осей, а не на энтропии сигнала, поэтому соизмеримы они лишь через объявленное огрубление ([граница валидации](/docs/applied/research/measurement-protocol#граница-валидации)).

Эмпирический якорь уже стоит на их стороне: псилоцибин повышает нейронную энтропию, и величина этого подъёма отслеживает интенсивность растворения «я», а расцепление сети режима по умолчанию предсказывает сообщаемое растворение. В координатах УГМ это траектория к нижней грани — чистота вниз, рефлексия вверх, — и она предсказывает, верно, что дальний конец этой траектории есть не максимум переживания, а его прекращение.

:::warning Что это НЕ разрешает
Ничто здесь не указывает, какие физические системы где находятся. Профиль — утверждение о $P,\Phi,D$ ПОСЛЕ того, как они измерены, а измерение их в субстрате без независимой истины остаётся за пределами того, что корпус готов утверждать ([граница валидации](/docs/applied/research/measurement-protocol#граница-валидации)). Феноменологический отчёт о чужой воле есть данные о человеческой самомодели под возмущением, а не наблюдение второго субъекта.
:::
...
- **L4 недостижим** для конечных систем (неполнота Лоувера, башня Постникова), но асимптотически приближаем.
