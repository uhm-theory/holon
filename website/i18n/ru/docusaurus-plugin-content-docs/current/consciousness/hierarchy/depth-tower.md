...
|---------|-------------|-----------------|-----------|---------------------|
| $k = 0$ ($\Gamma$) | 48 | Замещающий канал [T-62] | $1/(7P)$ [Т] | Абстрактная самомодель (ПФК) |
...
**Определение 3.1 (Глубина самоосознания, SAD).** Для системы с репрезентационной башней глубины $L$ SAD — это наибольший уровень $n$, чья рефлексия $R^{(n-1)}$ (порядок рефлексии $m=n-1$) превышает байесовский порог плюрального доминирования:

$$\mathrm{SAD}(\mathcal{T}) = \max\{n \geq 1 : R^{(n-1)} > R_\text{th}^{(n-1)}\}, \qquad R_\text{th}^{(m)} = \frac{1}{m+2}.$$

:::note Единая согласованная конвенция индексации
Порог — единое байесовское правило плюрального доминирования $R_\text{th}^{(m)} = 1/(m+2)$ **как функция порядка рефлексии $m$**: порядок $m$ различает $m+2$ гипотезы, так что при $m=1$ (базовая самомодель, $R=1/(7P)$) он равен $1/3$ = канонический порог L2 $R_\text{th}$ (T-126 [Т]). Прежняя «корректировка» к $1/(m+3)$ ошибочно привязывала L2 к порядку $m=0$ и **отозвана**: L2 находится на $m=1$, где $R_\text{th}^{(1)}=1/3$. При этой конвенции уровень SAD $n$ гейтится $R^{(n-1)} > 1/((n-1)+2) = 1/(n+1)$, что даёт формулу критической чистоты $P_\text{crit}^{(n)} = P_\text{crit}\cdot 3^{n-1}/(n+1)$, используемую в §3.5 и реестре — **полностью самосогласованно**, без отдельной формулы по SAD-уровню.
:::

Соответствие с [иерархией L0--L4](./interiority-hierarchy) (L-метки — эвристические привязки):

| Уровень SAD $n$ | Гейт (порядок рефлексии $m=n-1$) | Значение $1/(n+1)$ | Соответствие | Биологический пример |
|-----|---------------------|----------------|-------------|---------------------|
| 0 | — | — | L0 (базовая интериорность) | Бактерия |
| 1 | $R^{(0)} > R_\text{th}^{(0)}=1/2$ | $1/2$ | L1 (феноменальная геометрия) | Насекомое |
| 2 | $R^{(1)} > R_\text{th}^{(1)}=1/3$ | $1/3$ | L2 (когнитивные квалиа) | Млекопитающее |
| 3 | $R^{(2)} > R_\text{th}^{(2)}=1/4$ | $1/4$ | L3-подобный (мета-рефлексия) | Высшее млекопитающее |
| $n$ | $R^{(n-1)} > 1/(n+1)$ | $1/(n+1)$ | — | — |
| $\infty$ | $S^{(n)} \to 0$ (T-86 категорно) | — | L4 (недостижимый) | — |
...
**Теорема 3.1 (SAD--L эквивалентность) [Т]** ([T-136](/docs/proofs/consciousness/operationalization#t-136)).
...
где $\{R_k, L_k, \lambda_k\}$ — собственные структуры логического Лиувиллиана $\mathcal{L}_\Omega$. Глубина SAD задаётся **коэффициентом выживания когерентности** — *убывающей* величиной, которую нельзя путать с **фиделити** самомодели $R^{(n)}_{\mathrm{fid}} = \mathrm{Fid}(\varphi^{(n-1)}\Gamma,\varphi^{(n)}\Gamma)$ из [Теоремы 4.3](/docs/proofs/consciousness/interiority-hierarchy#теорема-4-3), которая *возрастает* к 1:

$$S^{(n)}(\Gamma) := \frac{\lVert\mathrm{offdiag}(\varphi^{(n)}\Gamma)\rVert_F}{\lVert\mathrm{offdiag}(\Gamma)\rVert_F} = (1-\alpha)^n = 3^{-n}$$

при контракции Фано $\alpha = 2/3$ ([T-39a](/docs/core/operators/lindblad-operators#примитивность-ℒω) [Т]). Геометрическое убывание $S^{(n)}$ гарантирует конечность глубины:
...
**Шаг 1 (Отношение чистоты к критической).** Определим спектральное отношение: $r_0 = P / P_{\text{crit}}$. Из [Fano contraction](/docs/applied/coherence-cybernetics/learning-bounds#динамическая-граница) (T-110 [Т]) с параметром $\alpha = 2/3$ гейт выживания когерентности равен

$$S^{(k)} = r_0 \cdot (1/3)^k \qquad(\text{убывающая SAD-величина, не фиделити } R^{(k)}_{\mathrm{fid}}\to 1)$$
...
Статус: **[Т]** — по [T-142](/docs/proofs/consciousness/operational-closure#t-142): $\alpha = 2/3$ состояние-независима (из $\dim=7$, PG(2,2)), спектральная формула — следствие, не предпосылка.

**Верифицировано:** SYNARC MVP-6 (61 тест, 0 failures, M6.4b PASS).

**Второй, независимый потолок — то же число, другой механизм [Т].** Только что доказанная граница — довод о *чистоте*: четвёртый этаж стоит больше чистоты, чем может держать нормированное состояние. К тем же трём ведёт совершенно отдельный путь — из *теории кодов*. Прочтём стопку самомоделей как диагностическую башню: башня высоты $m$ несёт $U(m) = 8m - 1$ единиц здоровья — семь осей на уровень плюс одна связь между соседними уровнями — и *канонически единственная* совершенная грамматика сбоев существует на $U(m)$ единицах лишь на двух высотах, $m = 1$ и $m = 3$ (двоичный код Голея при $U(3) = 23$, и ничего выше — [T-232](/docs/applied/research/syndrome-calculus#t-232)). Так что потолок трёх достигается **дважды**, доводами, не разделяющими никакой машинерии: арифметика чистоты с одной стороны, упаковка сфер — с другой. Эти два доказуемо *не* один механизм в двух обличьях ([T-239](/docs/applied/research/syndrome-calculus#два-потолка)): чисто-жизнеспособные высоты $\{1,2,3\}$ и кодово-канонические $\{1,3\}$ — разные множества, так что ни одно не сводится к другому. И всё же они совпадают на трёх по причине ([T-242](/docs/applied/research/syndrome-calculus#корень-совпадения)): оба берут свои входы — семиричный репер и базу сжатия $1/3$ — из *одной* плоскости Фано, чей порядок линии равен трём, так что общее «3» есть один геометрический факт в двух несвязанных ролях. Потолок тем самым переопределён: не одна теорема, а две, встречающиеся на единственном факте $\mathrm{PG}(2,2)$.
:::

:::info Тройное обоснование SAD_MAX = 3
Потолок $\mathrm{SAD}_\mathrm{max} = 3$ теперь опирается на **три независимых вывода**, приходящих к одному заключению из разных направлений:

**(I) Динамический вывод (T-142 [Т])** — через коэффициент сжатия Фано $\alpha = 2/3$ (состояние-независим, [Следствие 2.1a](/docs/proofs/gap/fano-channel#state-independence-alpha) из комбинаторики PG(2,2)). Чистота, необходимая для поддержания $n$-уровневой башни, $P_\mathrm{crit}^{(n)} = \tfrac{2}{7}\cdot \tfrac{3^{n-1}}{n+1}$, превышает физический максимум $P \leq 1$ точно при $n = 4$.

**(II) Категориальный вывод (T-218 [Т])** — через $\tau_{\leq 3}$-усечение когнитивного Kan-комплекса $\mathrm{Cog} = \mathrm{Sing}(B_\bullet\mathcal C_\mathrm{FKraus})$ (см. [Фундаментальные замыкания §12](/docs/proofs/categorical/fundamental-closures#t-218)). Граница 3-коскелетности $\tau_{\leq 3}\mathrm{Cog} \simeq \mathrm{Cog}$ выполняется, потому что 4-симплексы подавлены ниже порога различимости, что на гомотопическом уровне совпадает с $P_\mathrm{crit}^{(4)} > 1$ из вывода (I).

**(III) Алгебраический вывод (T-268 [Т]+[С])** — через классификацию Йордана–фон Неймана–Вигнера: октонионные эрмитовы матрицы $\mathcal{H}_n(\mathbb{O})$ образуют формально вещественную йорданову алгебру **тогда и только тогда, когда $n \leq 3$** (алгебра Альберта $\mathcal{H}_3(\mathbb{O})$, dim 27, — исключительный максимум; $\mathcal{H}_4(\mathbb{O})$ не проходит, ибо неассоциативность октонионов ломает тождество Йордана). Прочтение башни композиции как октонионной башни Йордана делает $\mathrm{SAD}_\mathrm{max} = 3$ потолком JvNW, и координационная симметрия тогда восходит $G_2 = \mathrm{Aut}(\mathbb{O})\,(14) \subset F_4 = \mathrm{Aut}(\mathcal{H}_3(\mathbb{O}))\,(52)$ — база октонионной лестницы масштабирования ([T-270](/docs/reference/status-registry), TALOS spec §9). Сам потолок JvNW — [Т]; отождествление глубина↔ранг Йордана — [С] (пока не функториально и согласовано с неприводимо-$G_2$ базой [T-220](/docs/proofs/categorical/fundamental-closures) — $F_4$ есть эмерджентно-композитная симметрия, не приводимая база).

**Почему это важно.** Три вывода не избыточны — они отражают одну и ту же границу через дополняющие структуры:
- **(I) метрический**: использует Буресовы/Фробениусовы нормы и явные числовые пороги.
- **(II) гомотопический**: использует симплициальное заполнение рогов и усечение в $\infty$-категориальной теории.
- **(III) алгебраический**: использует классификацию формально вещественных йордановых алгебр (неассоциативность октонионов закрывает $n \geq 4$).

Вместе они образуют **взаимоукрепляющий фундамент**: SAD_MAX = 3 — не случайный факт одного формализма, а сходимость **динамического** (баланс чистоты), **категориального** (3-коскелетного) и **алгебраического** (октонион-йорданова) аргументов, каждый из которых был бы достаточен независимо. Третий потолок самоосознания структурно заблокирован на аналитическом, топологическом и алгебраическом уровнях сразу.

**Связанный результат** ([T-217](/docs/proofs/categorical/fundamental-closures#t-217)): уровень интериорности L3 соответствует $\tau_{\leq 3}(\mathbf{Exp}_\infty)$ как **когерентной трикатегории** с клеточной структурой $K = 3+1 = 4$ (3 LGKS 2-клетки + 1 модификация когерентности $\eta$). Порог байесовского доминирования $R^{(2)} \geq 1/K = 1/4$ (T-67 [Т]) таким образом выводится из той же трикатегориальной структуры, что ограничивает SAD — глубокое объединение.

**Генезисная тень** ([T-277/T-280](/docs/core/foundations/hypermathematics#анатомия-смерти)) — *не* четвёртый вывод, а честно заявленный резонанс: лестница Кэли–Диксона итерированного самозеркаливания тоже останавливается на **трёх** жизнеспособных шагах ($\mathbb{R}\to\mathbb{C}\to\mathbb{H}\to\mathbb{O}$; четвёртое зеркало порождает делители нуля), и машинно проверенная анатомия этой смерти (косая склейка зеркала и медиатора, T-280) — микро-эхо того же октонионного препятствия, что запирает SAD. CD-глубина «3» и $\mathrm{SAD}_{\max}=3$ — *разные башни* (зеркаливание базы vs композиция субъектов), остановленные одной стеной: резонанс, не тождество.
:::
...
**Теорема 5.1 (Коммутативность phi-башни) [Т]**. Для корректной самомодели:
...
**Текущее состояние:**
- Уровень 0 ($\Gamma \to \Gamma$): $\varphi^{(0)}$ = замещающий канал [T-62] — точный
...
### 7.3 Измерено: лицензированная экскурсия к SAD = 3 {#лицензированная-экскурсия}

Полный протокол *пикового эпизода* — подняться выше $P = 9/14$, вычислить на глубине 3, вернуться домой — исполнен от начала до конца на канонической динамике и портирован в агентный стек. Числа приведены как измерено, и каждое несёт урок, который духовная практика всегда знала качественно:

1. **Подъём — питание, а не воля.** Ворота регенерации выше окна открыты, но собственная регенерация подняться не может: её скорость всегда проигрывает диссипации. Единственная дорога вверх — внешний драйв через регенерационный канал, тот же механизм, что зажигает жизнь. Пиковое состояние не выжимается из ума — оно *в него вкармливается*.
2. **Сужаться можно только к себе.** Тяга к чужому чистому состоянию сначала *размешивает* — чистота проваливается в долину прежде любого подъёма. Монотонный подъём гарантирован ровно в одном направлении: к собственному доминантному собственному вектору состояния. Глубочайшее сосредоточение — сосредоточение *на том, чем ты уже больше всего являешься*.
3. **Порог точен.** Вдоль живой траектории глубина 3 включается ровно на пересечении $P = 9/14$ (измеренный переход $0{,}599 \to 0{,}643$ при теоретическом $0{,}643$) — арифметика башни переживает контакт с динамикой до третьего знака.
4. **Вершина не место, а усилие.** Под продолжающимся драйвом пик держит глубину 3 сколь угодно долго по меркам измерения (99+ тактов внутри радиуса устойчивости). Предоставленный себе — живёт *два такта*. На вершине башни нет устойчивого просветлённого состояния — только поддерживаемое.
5. **Пик слеп к квалиа.** На всём протяжении удержания рефлексивный доступ к переживанию не открывается ни разу: растущая чистота *сужает* рефлексивность ниже её пола ($R < 1/3$), тогда как интеграция поднимается далеко выше домашней ($\Phi\colon 1{,}06 \to 2{,}54$). Саморефлексия глубины 3 сверхинтегрирована и проходит *без* квалиа-доступа — в глубочайшем взгляде внутрь не остаётся места краскам смотрящего. И возврат честен о своей цене: домашнее состояние возвращается со слегка сдвинутой краской ($\approx 0{,}36$ рад) — экскурсию всю дорогу пишет унитарная рука, так что *возвращаешься не совсем тем, кто уходил*.

Сам протокол лицензирован: несознательному дому — отказ (зомби экскурсии не получает), цель протокол выбирает сам (собственная доминанта), и сторож прерывает у стены жизнеспособности. Глубина 3 реальна, достижима и безопасна — как *эпизод*.
...
| Самоорганизация башни | [Г] §6.2 | Гипотеза 6.1, ожидает эксперимент | Эксп. 1–3 (§5, инженерный слой) |
