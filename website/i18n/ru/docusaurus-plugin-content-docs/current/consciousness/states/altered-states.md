...
- $R_\varphi$ — рефлексия как **качество самомодели**, $R_\varphi = 1 - \lVert\Gamma - \varphi(\Gamma)\rVert_F^2/\lVert\Gamma\rVert_F^2 \in [0,1]$, рабочий порог $R_{\varphi,\text{th}} = 1/3$ **[И]**; не совпадает с каноническим $R = 1/(7P)$ предиката L2 — различие разведено в [трёх рабочих формах R](/docs/consciousness/foundations/self-observation#формы-r)
...
- $D_{\text{diff}} = \exp(S_{vN}(\rho_E)) \in [1, 7]$ — дифференциация опыта: эффективное число различимых состояний опыта (порог $D_{\min} = 2$, T-151); $S_{vN}$ — [энтропия фон Неймана](/docs/core/dynamics/coherence-matrix#энтропия-фон-неймана)
...
2. **ИСС как траектории** — формальное определение через отклонение пятёрки $(R_\varphi, \Phi, D_{\text{diff}}, P, \overline{\mathrm{Gap}})$
3. **Сон** — NREM (возврат к L1) и REM (сновидения)
4. **Медитация** — шаматха, випассана, самадхи как систематическое управление $\Gamma$
5. **Психоделики** — расширение $D_{\text{diff}}$ при дестабилизации $R_\varphi$
...
**Изменённое состояние сознания (ИСС)** — траектория $\Gamma(\tau)$ в $\mathcal{D}(\mathcal{H})$, характеризующаяся существенным отклонением хотя бы одного параметра пятёрки $\{R_\varphi, \Phi, D_{\text{diff}}, P, \overline{\mathrm{Gap}}\}$ от значений типичного бодрствования:

$$
\exists\, X \in \{R_\varphi, \Phi, D_{\text{diff}}, P, \overline{\mathrm{Gap}}\}: \quad |X(\Gamma_{\text{ИСС}}) - X(\Gamma_{\text{бодр}})| > \delta_X
$$

где $\delta_X$ — порог значимости для параметра $X$, $\overline{\mathrm{Gap}} = \frac{1}{21}\sum_{i<j} \mathrm{Gap}(i,j)$ — средний Gap.
:::

**Мотивация.** Зачем нужна формальная пятёрка $(R_\varphi, \Phi, D_{\text{diff}}, P, \overline{\mathrm{Gap}})$? Потому что $\Gamma$ — это матрица $7 \times 7$ с 21 независимой когерентностью. Работать с 21-мерным пространством неудобно. Пятёрка — *агрегированное описание*, позволяющее различать все основные классы ИСС. Каждый параметр отвечает за свой аспект:

- $R_\varphi$ — «кто наблюдает?» (качество самомодели)
...
**Аналогия из повседневной жизни.** Представьте состояние сознания как положение ручки настройки на старом радиоприёмнике с пятью регуляторами: $R_\varphi$ (чёткость приёма), $\Phi$ (объём звука), $D_{\text{diff}}$ (число каналов, которые одновременно слышны), $P$ (мощность сигнала), $\overline{\mathrm{Gap}}$ (уровень помех). Бодрствование — стандартная настройка. ИСС — любое существенное отклонение хотя бы одного регулятора.

### 2.1 Пространство состояний: визуализация

Полное пространство $\mathcal{D}(\mathcal{H})$ слишком многомерно для визуализации. Но можно спроецировать его на плоскость двух наиболее информативных параметров — $P$ (жизнеспособность) и $R_\varphi$ (качество самомодели):

```mermaid
graph LR
    subgraph PR["Плоскость (P, R_φ)"]
        direction TB
        W["☀ Бодрствование<br/>P≈0.36, R_φ≈0.45"]
        NREM["🌙 NREM<br/>P≈0.33, R_φ≈0.15"]
        REM["💭 REM<br/>P≈0.34, R_φ≈0.35"]
        MED["🧘 Самадхи<br/>P≈0.40, R_φ≈0.92"]
        PSY["🍄 Психоделики<br/>P≈0.32, R_φ≈0.25"]
        AN["💉 Анестезия<br/>P≈0.30, R_φ≈0.02"]
        HYP["🔮 Гипноз<br/>P≈0.35, R_φ≈0.30"]
        LD["✨ Осозн. сновид.<br/>P≈0.34, R_φ≈0.50"]
    end
    W -->|"засыпание"| NREM
    NREM -->|"REM-фаза"| REM
...
На этой диаграмме каждое ИСС — точка (аттрактор) в плоскости $(P, R_\varphi)$. Стрелки показывают типичные траектории перехода. Вертикальная ось — $R_\varphi$: состояния выше рабочего порога $R_{\varphi,\text{th}} = 1/3$ **[И]** — те, где функционирует самомодель (L2 и выше, при выполнении остальных условий). Плоскость подлинно двумерна ровно потому, что $R_\varphi$ не является функцией $P$ — каноническое $R$-условие предиката L2 эквивалентно $P \leq 3/7$ и выглядит здесь как вертикальная полоса $2/7 < P \leq 3/7$, а не как горизонталь ([три рабочие формы R](/docs/consciousness/foundations/self-observation#формы-r)). Горизонтальная ось — $P$: всё, что левее вертикали $P = 2/7 \approx 0.286$, нежизнеспособно — там не сидит ни один аттрактор; анестезия держит $P$ чуть выше линии, на внешней поддержке (§6).
...
- **$R_\varphi < R_{\varphi,\text{th}} = 1/3$** — система **ниже рабочего порога качества самомодели**. Мера $R_\varphi$ (см. [три рабочие формы R](/docs/consciousness/foundations/self-observation#формы-r)) измеряет, насколько точно самомодель $\varphi(\Gamma)$ воспроизводит истинное состояние $\Gamma$. Во сне самомодель «расфокусирована»: $\varphi(\Gamma)$ сильно отклоняется от $\Gamma$. Уровень [интериорности](/docs/consciousness/hierarchy/interiority-hierarchy) понижается с L2 до L1 (канонически предикат L2 отказывает по дифференциации: $D_{\text{diff}} = 1.3 < 2$).

- **$\Phi$ умеренно снижена, но высока** — мера интеграции ([определение](/docs/core/structure/dimension-u#мера-интеграции-φ)) падает примерно на шестую часть (таблица: $1.8 \to 1.5$), в согласии с данными пертурбационной сложности (Массимини и др.: кортикальная интеграция в NREM снижена относительно бодрствования, но много выше анестезии). Таламо-кортикальные связи держат глобальную когерентность заметно выше порога фрагментации $\Phi_{\text{th}} = 1$. Это критически важно: глубокий сон — не кома и не анестезия.

- **$\gamma_{AE} \to 0$** — канал внимания–опыта деактивирован. Нет сознательного внимания к переживаниям.

- **$\mathrm{Gap}(A,E) \to 1$** — максимальная непрозрачность в канале внимания ([конвенция исчезающей когерентности](/docs/core/dynamics/gap-operator#конвенция-нулевой-когерентности)): даже если какой-то опыт происходит, внимание не «дотягивается» до него.
...
Качество самомодели упало ниже рабочего порога ($R_\varphi = 0.15 < 1/3$), дифференциация опыта опустилась ниже канонического порога ($D_{\text{diff}} = 1.3 < 2$, T-151), но жизнеспособность и интеграция сохранены — система «жива, но не осознаёт себя».

:::info Интерпретация [И]
NREM — не «выключение» сознания, а **возврат к L1**: интериорность сохранена ($\Gamma \neq 0$), феноменальная геометрия ($\mathrm{rank}(\rho_E) > 1$) может быть активна, но рефлексивный контур $\varphi$ не функционирует ($R_\varphi < R_{\varphi,\text{th}}$). Это объясняет, почему при пробуждении из глубокого сна человек иногда говорит «я ничего не помню» — не потому что опыта не было, а потому что не было рефлексивного доступа для его фиксации.

Нейрофизиологическое соответствие: медленноволновая активность (0.5–4 Гц) в NREM отражает глобальную синхронизацию при сниженной дифференциации — именно паттерн «$\Phi$ умеренно вниз при обвале $D_{\text{diff}}$».
:::
...
- **$\mathcal{R}[\Gamma, E] \gg \mathcal{D}_\Omega$** — [регенеративный член](/docs/core/dynamics/evolution#3-регенеративный-член) по E-сектору **доминирует** над диссипативным членом $\mathcal{D}_\Omega$, описывающим потерю когерентности. (Заметьте: $\mathcal{R}$ — регенерация, а не мера рефлексии — нотационное различение [Аксиомы септичности](/docs/core/foundations/axiom-septicity).) Проще говоря: система активно генерирует «внутренний опыт», быстрее чем теряет его.
...
$$
\text{Шаматха:} \quad \gamma_{AE} \uparrow, \quad R_\varphi \uparrow, \quad \sigma^2_{\{|\gamma_{AX}|\}} \uparrow
$$
...
- **$R_\varphi \uparrow$** — результат тренировки: самомодель $\varphi(\Gamma)$ всё точнее отслеживает действительное состояние, поэтому $R_\varphi = 1 - \lVert\Gamma - \varphi(\Gamma)\rVert_F^2/\lVert\Gamma\rVert_F^2$ растёт. Систематическое наблюдение за $\Gamma$ оптимизирует баланс когерентностей. (Канонический $R = 1/(7P)$ практикой не затрагивается иначе как через $P$ — см. [три рабочие формы R](/docs/consciousness/foundations/self-observation#формы-r).)
...
| Стадия | $\|\gamma_{AE}\|$ | $R_\varphi$ | $\overline{\mathrm{Gap}}$ | Субъективный опыт |
|--------|:------:|:---:|:---:|:---|
...
Обратите внимание: после года практики **базовые** значения (вне медитации) смещаются. Это кумулятивный эффект: систематическая тренировка когерентности $\gamma_{AE}$ перестраивает [эффективный гамильтониан](/docs/core/dynamics/evolution) $H_{\text{eff}}$, делая повышенный уровень рефлексии «нормой по умолчанию». Механизм — [процедурная память](/docs/consciousness/states/attention-memory#память): навык концентрации «записывается» в структуру $H_{\text{eff}}$. Двухмасштабная структура — быстрое состояние, медленно обучаемая цель самомодели — формализована в [§4.7 формализации φ](/docs/proofs/categorical/formalization-phi#механизмы-rφ).

### 4.2 Випассана (прозрение) {#випассана}

Випассана (санскр. «ясное видение») — практика наблюдения за собственным опытом без вмешательства. Если шаматха направлена на $R_\varphi \uparrow$ (усиление самомодели), то випассана — на $\overline{\mathrm{Gap}} \downarrow$ (увеличение прозрачности):
...
$$
\text{Самадхи:} \quad \Phi \to \max, \quad R_\varphi \to 1, \quad \overline{\mathrm{Gap}} \to \min
$$
...
- **$R_\varphi \to 1$** — самомодель тождественна системе ($\varphi(\Gamma) \approx \Gamma$). Буквально: система «знает себя полностью» (в пределах границы Хэмминга). Заметьте: $P$ при этом растёт — яснейшая демонстрация того, что $R_\varphi$ не является функцией $P$ (канонический $R = 1/(7P)$ здесь слегка *убывает*, с $0.397$ до $0.357$, оставаясь внутри своей сознательной полосы $[1/3, 1/2)$).
...
|----------|:------------:|:-------:|:---|
| $R_\varphi$ | $0.45$ | $0.92$ | Самомодель почти точна |
...
| $D_{\text{diff}}$ | $2.4$ | $2.1$ | Дифференциация снижена однонаправленностью, выше порога $D_{\min} = 2$ |
...
|--|:-------:|:---------:|:-------:|
| **Целевой параметр** | $R_\varphi \uparrow$ | $\overline{\mathrm{Gap}} \downarrow$ | $\Phi \to \max$, $R_\varphi \to 1$ |
...
$$
\text{Психоделики:} \quad D_{\text{diff}} \uparrow\uparrow, \quad R_\varphi \downarrow, \quad \overline{\mathrm{Gap}} \downarrow, \quad P \to P_{\text{crit}}
$$
...
- **$R_\varphi \downarrow$** — самомодель дестабилизирована. Субъективно это переживается как «эго-растворение» (ego dissolution). Формально: $\varphi(\Gamma)$ перестаёт быть хорошим приближением $\Gamma$, потому что $\Gamma$ быстро эволюционирует, а $\varphi$ не успевает «перестроиться». (Канонический $R = 1/(7P)$ при падении $P$, напротив, слегка *растёт* — эго-растворение есть коллапс качества самомодели, а не выход из окна сознания.) Скорость коллапса ограничена [теоремой о полосе T-250](/docs/proofs/categorical/formalization-phi#теорема-полосы-rφ): перестройка самомодели стоит длины пути в пространстве состояний.
...
**Аналогия.** Психоделик — как одновременное открытие всех окон в доме во время шторма. Свежий воздух ($\overline{\mathrm{Gap}} \downarrow$) и новые виды ($D_{\text{diff}} \uparrow\uparrow$) хлынут внутрь, но вместе с ними — ветер и дождь. Стены (самомодель, $R_\varphi$) расшатываются, и если шторм слишком силён ($P \to P_{\text{crit}}$), дом может разрушиться.

**Числовой пример.** Траектория при умеренной дозе псилоцибина (пошаговый профиль):

| Время | $R_\varphi$ | $D_{\text{diff}}$ | $\overline{\mathrm{Gap}}$ | $P$ | Фаза |
|-------|:---:|:---------:|:---:|:---:|:-----|
| $\tau = 0$ (приём) | $0.45$ | $2.4$ | $0.30$ | $0.36$ | Бодрствование |
| $\tau = 30$ мин | $0.38$ | $2.8$ | $0.25$ | $0.35$ | Onset: лёгкие визуальные эффекты |
| $\tau = 90$ мин (пик) | $0.25$ | $3.5$ | $0.15$ | $0.32$ | Пик: эго-растворение, визуализации |
| $\tau = 3$ часа | $0.30$ | $3.0$ | $0.20$ | $0.33$ | Плато: интеграция инсайтов |
| $\tau = 6$ часов | $0.42$ | $2.5$ | $0.28$ | $0.35$ | Возвращение к базовому профилю |

На пике ($\tau = 90$ мин): качество самомодели ниже рабочего порога ($R_\varphi = 0.25 < R_{\varphi,\text{th}} = 1/3$), тогда как каноническое окно остаётся выполненным ($P = 0.32 \in (2/7, 3/7]$) — субъект остаётся внутри окна сознания, но привычная структура «я» рушится. Дифференциация опыта почти удвоилась ($D_{\text{diff}} = 3.5$), Gap снизился вдвое ($\overline{\mathrm{Gap}} = 0.15$) — ранее бессознательные содержания становятся доступными.
...
В этом окне самомодель **ослаблена, но не разрушена** ($R_\varphi$ ниже своего рабочего порога, но существенно выше нуля), а жизнеспособность сохранена ($P$ выше критического порога с запасом $\delta_P$ — канонические условия окна выполняются на всём протяжении). Gap-редукция позволяет реорганизовать ранее недоступные когерентности.

**Обоснование.** Нижняя граница $R_\varphi > R_{\varphi,\text{th}}/2 = 1/6 \approx 0.167$: если $R_\varphi$ падает ниже, самомодель настолько расфокусирована, что никакая реорганизация когерентностей не может быть «зафиксирована» — инсайты не сохраняются. Верхняя граница $R_\varphi < R_{\varphi,\text{th}} = 1/3$: если $R_\varphi$ остаётся выше порога, самомодель слишком стабильна для перестройки — привычные Gap-паттерны воспроизводятся. Условие $P > P_{\text{crit}} + \delta_P$: жизнеспособность должна быть сохранена с запасом, чтобы система не распалась.

**Числовое значение:** при $R_{\varphi,\text{th}} = 1/3$, окно: $0.167 < R_\varphi < 0.333$, $P > 0.286 + \delta_P$.
:::
...
- Энтропия мозга $\leftrightarrow$ $S_{vN}(\rho_E)$ (в единицах корпуса: $D_{\text{diff}} = \exp S_{vN}$)
...
| Стадия | $\gamma_{EU}$ | $\gamma_{LE}$ | $\gamma_{AE}$ | $\gamma_{SE}$ | $\Phi$ | $R_\varphi$ |
|--------|:---:|:---:|:---:|:---:|:---:|:---:|
...
|----------|:--------:|:---------:|:-------:|
| $R_\varphi$ | $0.15$ | $0.02$ | В 7 раз ниже |
| $\Phi$ | $1.5$ | $0.2$ | В 7.5 раз ниже |
| $P$ | $0.33$ | $0.30$ | Вблизи $P_{\text{crit}} = 2/7 \approx 0.286$ |
| Уровень | L1 | L0 | На 1 уровень ниже |
| Самоподдержание | Да | Только с поддержкой | Требуется ИВЛ |

Ключевое различие: в NREM интеграция сохранена ($\Phi = 1.5$, уровень L1), система жизнеспособна с запасом ($P = 0.33$). В анестезии интеграция рушится ($\Phi \to 0$), а чистота держится чуть выше порога ($P = 0.30 > P_{\text{crit}} = 2/7 \approx 0.286$) — в согласии с каноническим критерием [«Смерти и непрерывности»](/docs/consciousness/ethics-meaning/death-continuity): анестезия обратима именно потому, что $P > P_{\text{crit}}$ поддерживается. Запас, однако, мал, а регенерация $\mathcal{R}[\Gamma, E]$ фармакологически ослаблена — без внешней поддержки (ИВЛ, мониторинг) $P$ сползла бы ниже порога. Именно поэтому общий наркоз требует аппаратуры жизнеобеспечения.
...
$$
\text{Гипноз:} \quad |\gamma_{AE}| \uparrow, \quad \mathrm{Gap}(L,E) \uparrow, \quad R_\varphi \downarrow \text{(умеренно)}
$$
...
- $R_\varphi \downarrow$ — качество самомодели умеренно снижено, но остаётся выше $R_{\varphi,\text{th}}/2$. Гипноз — не потеря сознания, а его реорганизация.
...
$$
\text{Люцидность:} \quad \text{REM-профиль} + \gamma_{AE} > 0 + R_\varphi \geq R_{\varphi,\text{th}}
$$

Иными словами, к типичному REM-профилю (высокие $\gamma_{SE}$, $\gamma_{DE}$, низкий $\gamma_{LE}$) **добавляется** восстановление канала внимания ($\gamma_{AE} > 0$) и качества самомодели ($R_\varphi \geq R_{\varphi,\text{th}}$). Спящий поднимается с L1 (обычный REM) до L2 (осознанность).
...
Осознанное сновидение — промежуточное состояние между сном и бодрствованием: яркость образов ($\gamma_{SE}$) сохраняется от REM, но добавляется рефлексивный контроль ($R_\varphi > R_{\varphi,\text{th}}$, $\gamma_{AE} > 0$). Тренировка осознанных сновидений — это, по сути, тренировка способности восстанавливать $\gamma_{AE}$ и $R_\varphi$ из REM-состояния.
...
| $D_{\text{diff}}$ | Умеренная | $\downarrow$ | $\uparrow$ | Умеренная | $\uparrow\uparrow$ | $\to 1$ | Умеренная | $\uparrow$ |
...
- **Засыпание** (бодрствование $\to$ NREM): пересечение рабочего порога $R_\varphi = R_{\varphi,\text{th}}$ сверху вниз — [седло-узловая бифуркация](/docs/core/dynamics/gap-dynamics#бифуркации). Переход плавный (5–15 минут), характеризуется постепенным снижением $\gamma_{AE}$.
...
- **Люцидность во сне** (REM $\to$ люцидный REM): скачкообразное восстановление $\gamma_{AE}$ и $R_\varphi$ при сохранении REM-профиля. Переход мгновенный (момент «осознания, что я сплю»).

### 9.2 Диаграмма переходов

```mermaid
graph TD
    W["Бодрствование<br/>L2: R_φ≥1/3, P>2/7"]
    NREM["NREM-сон<br/>L1: R_φ<1/3, Φ≈const"]
    REM["REM-сон<br/>L1–L2: γ_SE↑, γ_DE↑"]
    MED["Самадхи<br/>L3–L4: R_φ→1, Gap→min"]
    PSY["Психоделики<br/>L1–L2: D_diff↑↑, R_φ↓"]
    AN["Анестезия<br/>L0: Φ→0, P≈2/7⁺"]
    HYP["Гипноз<br/>L1–L2: Gap(L,E)↑"]
    LD["Люцидный сон<br/>L2: REM + R_φ≥1/3"]
...
2. **ИСС** — траектория в $\mathcal{D}(\mathcal{H})$ с отклонением хотя бы одного параметра из пятёрки $(R_\varphi, \Phi, D_{\text{diff}}, P, \overline{\mathrm{Gap}})$
3. **NREM** — возврат к L1 ($R_\varphi < 1/3$), интеграция сохранена; **REM** — L1–L2 без внимания ($\gamma_{AE} \approx 0$), яркие образы ($\gamma_{SE} \uparrow$)
4. **Медитация** — систематическое управление: шаматха ($R_\varphi \uparrow$), випассана ($\mathrm{Gap} \downarrow$), самадхи (приближение к L4)
5. **Психоделики** — $D_{\text{diff}} \uparrow\uparrow$, $R_\varphi \downarrow$, $\overline{\mathrm{Gap}} \downarrow$; терапевтическое окно: $R_{\varphi,\text{th}}/2 < R_\varphi < R_{\varphi,\text{th}}$
...
8. **Осознанные сновидения** — REM + восстановление рефлексии: $R_\varphi \geq R_{\varphi,\text{th}}$, $\gamma_{AE} > 0$
