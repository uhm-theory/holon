...
- $R_\varphi$ — рефлексия как **качество самомодели** $\in [0,1]$, рабочий порог $R_{\varphi,\text{th}} = 1/3$ **[И]**; не совпадает с каноническим $R = 1/(7P)$ — см. [три рабочие формы R](/docs/consciousness/foundations/self-observation#формы-r)
...
| $R_\varphi$ | $0.35$ | Осознаёт себя (выше рабочего порога) |
...
- $R_\varphi$ может быть нормальным или даже повышенным — *депрессивная руминация* есть форма рефлексии, но направленная на неизменный Gap-профиль
...
| $R_\varphi$ | $0.45$ | $0.45$ | $0.50$ (руминация) |
...
**Важно:** $R_\varphi$ при депрессии может быть *повышен*. Руминация (бесконечное «пережёвывание» мыслей) повышает качество самомодели, но направлена на неизменный Gap-профиль. Это объясняет парадокс *depressive realism*: депрессивные пациенты часто обладают более точными оценками вероятностей и собственных возможностей — их $\varphi(\Gamma)$ точнее отражает $\Gamma$, но сам $\Gamma$ патологичен.
...
**Психоз** — внезапное глобальное уменьшение Gap при сохранении $R_\varphi$:

$$
\overline{\mathrm{Gap}} \to 0 \quad \text{(скачком)}, \quad R_\varphi \geq R_{\varphi,\text{th}}
$$
...
| Патология | Ключевые каналы | $\overline{\mathrm{Gap}}$ | $P$ | $R_\varphi$ | Уровень |
|-----------|----------------|:-------------------------:|:---:|:---:|:-------:|
...
| $R_\varphi$ | Норма/↑ (руминация) | Норма/↑ | $\approx 0$ |
...
при сохранении $P > P_{\text{crit}}$ и $R_\varphi \geq R_{\varphi,\text{th}}$ на всём протяжении траектории.
...
**Интерпретация:** Канал $(i,j)$ с $\mathrm{Gap}(i,j) < \varepsilon_{\text{noise}}$ имеет SNR < 1 для коррекции ошибок самомодели $\varphi$. Структурно Gap > 0 (граница Хэмминга [T-41g]), но функционально канал «глух» — ошибки φ в этом канале не корректируются.
...
graph LR
    subgraph NORM["Норма"]
        N["Бодрствование<br/>P=0.36, Gap̄=0.28<br/>R_φ=0.45"]
    end
