...
Чтобы формализм стал осязаемым, рассмотрим три конкретных примера: как из полной матрицы $\Gamma$ вычисляется редуцированная матрица $\rho_E$ и что означает её спектр.

:::note Пример 1: Глубокий сон (L0)
**Исходная $\Gamma$** — почти максимально смешанное состояние:

$$
\Gamma_{\text{sleep}} = \frac{1}{7}I + \varepsilon \cdot \Delta, \quad \varepsilon \approx 0.02
$$

Диагональные элементы: $\gamma_{kk} \approx 1/7 \approx 0.143$ для всех $k$. Недиагональные элементы: $|\gamma_{ij}| \lesssim 0.02$ — почти нулевые когерентности.

**Вычисление $\rho_E$.** При частичном следе по $\{A,S,D,L,O,U\}$ получаем скаляр (одномерный E-блок в 7D-формализме):

$$
\rho_E^{\text{sleep}} \approx \gamma_{EE} \approx 0.143
$$

В расширенном PW-формализме $\rho_E$ — матрица размера $6 \times 6$ (E участвует в 6 парах). Её спектр:

$$
\text{Spectrum}(\rho_E^{\text{sleep}}) \approx \{0.17,\; 0.17,\; 0.17,\; 0.17,\; 0.16,\; 0.16\}
$$

**Интерпретация.** Спектр почти равномерный — нет доминирующего компонента. Это «белый шум» интериорности: система не выделяет ни одного аспекта опыта. Чистота $P = \text{Tr}(\Gamma^2) \approx 1/7 + O(\varepsilon^2) \approx 0.144$ — ниже порога $P_{\text{crit}} = 2/7 \approx 0.286$.
:::

:::note Пример 2: Бодрствование — восприятие красного (L2)
**Исходная $\Gamma$** — состояние с доминирующей E-активностью и когерентностями:
...
Остальные диагональные: $\gamma_{SS} = 0.14$, $\gamma_{DD} = 0.12$, $\gamma_{OO} = 0.11$, $\gamma_{UU} = 0.10$.

**Спектр $\rho_E$** (через PW-реконструкцию):

$$
\text{Spectrum}(\rho_E^{\text{awake}}) \approx \{0.52,\; 0.23,\; 0.11,\; 0.07,\; 0.04,\; 0.03\}
$$

**Интерпретация.** Доминирующее собственное значение $\lambda_1 = 0.52$ — более половины «полосы пропускания» опыта занято одним компонентом (красный цвет). Соответствующий собственный вектор $|q_1\rangle$ определяет конкретный оттенок. Второй компонент ($\lambda_2 = 0.23$) — фоновое ощущение (пространственный контекст). Чистота $P \approx 0.31 > P_{\text{crit}}$.

Меры: $R \approx 0.38 \geq 1/3$ (есть рефлексия), $\Phi \approx 1.2 \geq 1$ (есть интеграция). Система на уровне L2 — осознанное восприятие.
:::

:::note Пример 3: Глубокая медитация (верхний диапазон L2)
**Исходная $\Gamma$** — состояние с высокой когерентностью между E и U:
...
**Интерпретация.** Два ведущих собственных значения ($\lambda_1 = 0.38$, $\lambda_2 = 0.30$) близки по величине — нет резкого доминирования одного компонента. Это «широкий» опыт: не фиксация на объекте, а равномерное осознавание. Когерентность $\gamma_{EU} = 0.14$ — сильная связь переживания с единством (ощущение целостности). Фазы малы ($\theta \lesssim 0.08$) — высокая прозрачность каналов. Чистота $P \approx 0.29$ — чуть выше $P_{\text{crit}}$, в «окне Голдилокс» $(2/7, 3/7]$ [Т T-124].
:::

### Геометрическая интуиция: спектр $\rho_E$ в разных состояниях {#геометрия-спектра}

Спектр $\rho_E$ можно визуализировать как **столбчатую диаграмму** на $(n-1)$-симплексе. Характерные паттерны:
...
Это отношение $G_2$-инвариантно (не зависит от выбора базиса), поэтому является объективной характеристикой состояния сознания.
...
Метрика Фубини–Штуди — единственная (с точностью до константы) монотонная риманова метрика на проективном пространстве (теорема Ченцова–Петца). Это не произвольный выбор, а **вынужденная структура**.
...
- $d_{FS}$: метрика Фубини–Штуди инвариантна при $U(n)$-преобразованиях (теорема Ченцова–Петца)
...
Всё изложенное выше — **E-срез** содержания опыта: редуцированная матрица $\rho_E$ несёт интенсивность и реляционную позицию качества, и именно об этом срезе — теоремы единственности и Йонеды. Машинно-проверенный канон качества достраивает остальную картину:

- полное содержание состояния — его **28 параметров**: 7 населённостей и 21 когерентность; таксономия 21 канала и их 7 фано-секторов — предмет [Структуры квалиа](/docs/consciousness/phenomenology/qualia-structure#двадцать-один-и-семь);
- **несводимая краска** опыта живёт не в отдельной паре, а в тройках: голономии Фано $H_p = \arg(\gamma_{ij}\gamma_{jk}\gamma_{ki})$ — единственные фазовые объекты, переживающие перефазировку осей (эрратум T-301: парные $\mathrm{Im}$-детекторы — *почерк* состояния);
- практический интерфейс — пятислойный [паспорт квалиа](/docs/consciousness/phenomenology/qualia-structure#паспорт-квалиа): населённости → интенсивности → непрозрачности → краски → доступ, причём условие доступа — ровно L2-пороги этой главы.

Два языка не конкурируют: E-срез отвечает «насколько громко и какое это качество среди качеств»; паспорт — «что во всей полноте и какая часть этого инвариантна».
...
- **Качества живут в проективном пространстве** $\mathbb{P}(\mathcal{H}_E)$, наделённом метрикой Фубини–Штуди — единственной монотонной метрикой (теорема Ченцова–Петца).
...
:::tip Куда дальше
Теория интериорности описала **что** переживается. Следующий вопрос: **как** система может наблюдать собственное содержание? Ответ даёт [Самонаблюдение](./self-observation) — оператор $\varphi$, мера рефлексии $R$ и мера сознательности $C = \Phi \times R$.
