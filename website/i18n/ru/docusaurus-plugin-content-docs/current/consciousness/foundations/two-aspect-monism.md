...
**Ответ на объективистское no-go ([T-221](/docs/proofs/categorical/fundamental-closures#t-221)).** Двухаспектный монизм — УГМ-реализация **категориально-монистического маршрута** — четвёртого не-объективистского варианта, помимо трёх (реляционистский / фрагменталистский / многосубъектно-мирный), идентифицированных List (2025) и DeBrota–List (2026). Именно $\&$-модальность T-186 делает перволичный реализм (FPR) *теоремой* в УГМ, а не опциональным постулатом. См. [Теории сознания §Мета-уровень](/docs/consciousness/comparative/consciousness-theories#no-go-objectivism) для полного обсуждения.
:::
...
### Калибровка есть $G_2$-репер {#калибровка-как-g2-репер}

Остаток «калибровки» можно **назвать точно**, опираясь на теорему, уже установленную в этой главе, а не оставлять голой аналогией с массой электрона.

:::tip Заострение остатка-калибровки
Феноменальный функтор $F$ **верен на $G_2$-орбитах** [Т] — $F(\Gamma_1) \cong F(\Gamma_2)$ тогда и только тогда, когда $\Gamma_2 = U\Gamma_1 U^\dagger$ для некоторого $U \in G_2 = \mathrm{Aut}(\mathbb{O})$ (см. блок [Верность функтора на $G_2$-орбитах](#faithful-g2-box) в конце главы). Следовательно, $F$ разрешает лишь $G_2$-**инвариантную** часть $\Gamma$ и **слеп** к выбору представителя внутри орбиты — $G_2$-**реперу** (точке $14$-мерной калибровочной группы $G_2 = \mathrm{Aut}(\mathbb{O})$ против $48 = 7^2 - 1$ конфигурационных параметров бесследовой эрмитовой $7\times 7$).

Это чисто разделяет два необъяснённых пункта:

- **Реляционная структура** опыта (Ёнеда) — это $G_2$-инвариантная часть: универсальная, тождественная для всякой системы на одной орбите. Это то, что $F$ видит и что теория объясняет.
- **Калибровка** — «какой конкретный $[|q\rangle]$ есть красное», то единственное, что $F$ не может закрепить изнутри, — **есть в точности репер**. Расплывчатый остаток становится теоретико-групповым объектом: выбором представителя в пространстве модулей $\mathcal{D}(\mathbb{C}^7)/G_2$, то есть сечением $G_2$-расслоения. Это отождествление **условно** — оно опирается на теорему верности, которая имеет место, — а не новый постулат.

**Открытое направление (гипотеза).** Если репер не внутренен для $\Gamma$, естественный кандидат его закрепить — **внешние граничные данные** системы, физическая среда, в которую погружена конфигурация. Двухаспектный монизм помещает физику именно на *внешнюю* сторону, где такие граничные условия и живут; поэтому предложение «внешнее погружение закрепляет феноменальный репер» **согласуется** с онтологией, а не расширяет её. Установлено лишь отождествление (калибровка $=$ репер); что граничные данные *закрепляют* репер — гипотеза, и помечена как таковая. Однако есть структурная причина, по которой репер не может быть *внутренним*: действие $G_2$ на $\mathcal{D}(\mathbb{C}^7)$ имеет **переменный тип орбит** — прямое вычисление (построение $G_2$ как стабилизатора октонионной формы Фано и измерение размерности орбиты) даёт $0$ в максимально смешанном центре, $11$ в чистом состоянии, $14$ в общем. Переменная изотропия означает, что пространство модулей $\mathcal{D}(\mathbb{C}^7)/G_2$ **стратифицировано**, а орбитное отображение не допускает **глобального сечения** — значит, *глобальной* внутренней калибровки не существует, и репер можно закрепить лишь локально. Тогда то, что эту роль исполняет внешний, локальный источник, естественно, а не факультативно. Развёрнутое изложение — включая то, почему дискретный скелет $G_2$-репера (его двенадцать корней и семимерное представление) координатизирован так же, как традиционные системы небесной калибровки, и вытекающее монистическое предсказание для умов в иных планетарных средах — дано в [Части XXI исследования HomoHoloGraph](../../applied/research/homoholograph.md).
:::

Это укрепление, а не украшение: открытый вопрос с *названной структурой* ($G_2$-репер) и *кандидатом-механизмом* (внешние граничные данные) — более острый вопрос, чем необъяснённая константа. Он также заостряет [шкалу ниже](#шкала-сознательности): две системы на одной $G_2$-орбите имеют тождественные $C = \Phi \times R$ *и* тождественную феноменальную структуру, различаясь лишь калибровкой, недостижимой ни для какого внутреннего измерения.
