...
:::note Конвенция гигиены артикуляций
Термины **самонаблюдение**, **самомоделирование**, **самореференция** в этом документе — *поднятые технические термины*, а не риторические сокращения. Каждый несёт явную операторную факторизацию по протоколу гигиены артикуляций (NO-19 в Noesis / [`reference/articulation-hygiene`](/docs/reference/articulation-hygiene)):

- **самомоделирование** ⟼ CPTP-функтор $\varphi: \mathrm{D}(\mathbb{C}^7) \to \mathrm{D}(\mathbb{C}^7)$ с уравнением неподвижной точки $\rho^* = \varphi(\Gamma)$ (T-96).
- **самонаблюдение** ⟼ terminal-coalgebra-структура на $\Gamma$-динамике, характеризуемая мерой рефлексии $R = 1/(7P)$ (T-126).
- **самореференция** ⟼ Lawvere-fixed-point-морфизм, ограниченный T-2f\*-depth-стратификацией (105.T в Diakrisis).

Пара (оператор $\Phi$, фиксированный объект $t$) явна при каждом вхождении; описующая и описываемая позиции структурно разведены даже когда визуально совпадают в термине.
:::
...
Сознание — не эпифеномен и не отдельная субстанция. **Сознание — это способ, которым Γ переживает собственную конфигурацию** [И].

:::info Онтологический статус [П]
Каждая конфигурация $\Gamma$ имеет «внешнюю» (объективную) и «внутреннюю» (субъективную) стороны. Они неразделимы — это не дуализм, а **двухаспектный монизм** [П]. Математическая структура (функториальный изоморфизм $F: \mathbf{Phys} \to \mathbf{Phen}$) — [Т] (T-186). Онтологическое отождествление внутренней стороны с *переживанием* — единственный постулат [П] УГМ, выходящий за рамки самого $\infty$-топоса.
:::

## Оператор самомоделирования φ {#оператор-самомоделирования-φ}

> **Алгебраический генезис самонаблюдения (T-277).** У оператора φ есть предок на глубочайшем слое теории: *алгебраическое самозеркаливание* — шаг Кэли–Диксона $A \mapsto A \oplus A$, «система и её зеркальная копия, взаимодействующие через сопряжение». Итерация этого акта от голого различения порождает всю семимерную арену, в которой φ затем живёт (три жизнеспособных зеркала ⟹ $\Omega^3$, $7 = 2^3 - 1$), а его гарантированная неподвижная точка — уроборос $\rho^* = \varphi(\Gamma)$ — есть то, что вынуждает скаляры быть континуумом ([T-286](/docs/core/foundations/hypermathematics#уровень-поля)). Самонаблюдение — не свойство, добавленное к структуре; это операция, из которой структура *сделана*: см. [Гипер-математика §2](/docs/core/foundations/hypermathematics#зеркальный-функтор) [Т для алгебры; И для прочтения].
...
Определение φ **не содержит порочного круга**: диссипативное стационарное состояние $\rho^*_{\mathrm{diss}} = I/7$ выводится из примитивности линейной части $\mathcal{L}_0$ [T-39a] — это свойство **динамики**, не зависящее от $\varphi$. Мера рефлексии $R(\Gamma) = 1 - \|\Gamma - \rho^*_{\mathrm{diss}}\|_F^2/\|\Gamma\|_F^2$ определяется **только** состоянием $\Gamma$ и референсом $\rho^*_{\mathrm{diss}} = I/7$ (константой), а параметр $k = 1 - R$ выводится из $R$ (см. [теорему ниже](#теорема-k-из-r)). Таким образом, $\varphi_k$ определён через независимые объекты ($\rho^*$ из динамики, $R$ из состояния системы), а не через себя.
:::
...
### Алгебраическая эквивалентность: $R = 1/(7P)$ {#алгебраическая-эквивалентность-r}

:::info Логический статус
Равенство $R = 1/(7P)$ — это **алгебраическое тождество** определения Фробениуса с $\rho^*_{\mathrm{diss}} = I/7$ на компактном $\mathcal D(\mathbb C^7)$ — не вывод из независимых аксиом. Содержательное утверждение — что это определение канонично, установлено тремя независимыми характеризациями (HS-угловая, $G_2$-инвариантность, $K=3$ байесов порог) в [T-126 [Т]](/docs/proofs/consciousness/conscious-window#t-126).
:::

Начиная с Фробениус-мастер-определения:

**Шаг 1.** $R = 1 - \|\Gamma - I/7\|_F^2 / \|\Gamma\|_F^2$.

**Шаг 2.** Знаменатель: $\|\Gamma\|_F^2 = \mathrm{Tr}(\Gamma^2) = P$ (чистота).

**Шаг 3.** Числитель — Pythagoras в HS. Запишем $\Gamma = I/7 + \Delta$ с $\Delta$ бесследовым. Тогда $\langle\Delta, I/7\rangle_F = \mathrm{Tr}(\Delta)/7 = 0$, так что
$$\|\Delta\|_F^2 = \|\Gamma\|_F^2 - \|I/7\|_F^2 = P - 1/7.$$
Эквивалентное прямое разложение: $\mathrm{Tr}((\Gamma - I/7)^2) = P - 2/7 + 7/49 = P - 1/7$.

**Шаг 4.** Подстановка:
$$R = 1 - \frac{P - 1/7}{P} = \frac{1/7}{P} = \frac{1}{7P}.$$

**Геометрическая интерпретация.** Эквивалентная замкнутая форма: $R(\Gamma) = \cos^2\theta_{\mathrm{HS}}(\Gamma, I/7)$, квадрат косинуса угла Гильберта–Шмидта между $\Gamma$ и $I/7$. Это идентифицирует $R$ как нормированную проекцию $\Gamma$ на единственную $G_2$-фиксированную референцию.

Результат: **$R = 1/(7P)$** — элегантная формула, связывающая рефлексию с чистотой.

:::note Независимо ли $R$ измеряется из данных?
При первом порядке $n=1$ канонический $R = 1/(7P)$ — по построению строго убывающая репараметризация чистоты $P$ — **не несёт информации сверх $P$**. Подлинная независимость появляется только в **высших порядках** $R^{(n)}$ для $n\ge 2$ (фиделити последовательных самомодельных итераций $\varphi^{(n-1)}\Gamma, \varphi^{(n)}\Gamma$), которые зависят от категорной самомодели $\varphi$ и не являются функциями только $P$. Полное обсуждение: [T-126 пункт о независимой наблюдаемости](/docs/proofs/consciousness/conscious-window#независимая-наблюдаемость-r).
:::
...
### Три рабочие формы R {#формы-r}

<!-- DRY: единая точка разграничения трёх величин рефлексии. Всякий документ, использующий R_φ или R^(n) рядом с каноническим R, должен ссылаться сюда. -->

Корпус работает с **тремя родственными, но различными** величинами рефлексии. Их смешение порождает числовые парадоксы — «самадхи с $R = 0{,}92$» или «анестезия с $R = 0{,}02$», невозможные для канонической формы. Этот подраздел — единственная точка, где три величины разведены.

| Форма | Определение | Диапазон | Порог | Типичное употребление |
|-------|-------------|----------|-------|------------------------|
| канонический $R$ | $1/(7P)$ — HS-близость к $I/7$ | $[1/7,\ 1]$ | $R_{\text{th}} = 1/3$ **[Т]** (T-126) | предикат L2, $C = \Phi \times R$ (T-140), реестр, протоколы измерения |
| качество самомодели $R_\varphi$ | $1 - \lVert\Gamma - \varphi(\Gamma)\rVert_F^2 / \lVert\Gamma\rVert_F^2$ — мастер-определение в [Аксиоме септичности](/docs/core/foundations/axiom-septicity). На категориальной $\varphi$-модели (T-62) имеет замкнутую форму $R_\varphi = 1 - (1-R)^4\,\lVert\Gamma - I/7\rVert_F^2 / \lVert\Gamma\rVert_F^2$ — феноменологическая форма есть функция канонической $R$ и расстояния до центра (машинно проверено: мастер $=$ замкнутая на 200 состояниях; $R_\varphi(I/7)=1$) | $[0,\ 1]$ | рабочий $R_{\varphi,\text{th}} = 1/3$ **[И]** | феноменология состояний (сон, медитация, психоделики, патология); функционал смысла |
| fidelity-башня $R^{(n)}$ | $F(\varphi^{(n-1)}(\Gamma), \varphi^{(n)}(\Gamma))$, $n \geq 2$ | $[0,\ 1]$ | $R^{(n)}_{\text{th}} = 1/(n+2)$ | глубина самомодели L3/L4. **Не** критерий SAD — см. предупреждение ниже |
| точность самопрогноза $A_\varphi$ | $1 - \sum(m-e)^2 / \sum(m-\bar m)^2$ по дневнику — **эмпирическая** оценка, центрированная на собственной базе человека | $(-1,\ 1]$ | порога нет: читается как доля | флагманское число прибора (`/pair`, `/state`); меряет *день*, а не среднее |

#### Почему эмпирическую форму обязательно центрировать {#центрирование-a-phi}

Четвёртая строка — не лишнее имя, а исправление дефекта, который прибор нёс,
пока его не измерили. Дневниковая оценка $R_\varphi$ была построена в форме
самого определения, $1 - \sum(m-e)^2/\sum m^2$, и эта форма безопасна на
матрицах и опасна на оценках. На матрицах общая часть знаменателя — компонента
$I/7$, её доля $32.2\,\%$, и чтение почти не смещается ($0.9983$ против
$0.9976$ центрированного). В дневнике же оценки живут около $6$, а не около
$0$, поэтому знаменатель *почти целиком* есть среднее самого человека, и мера
отвечает на вопрос, которого никто не задавал: «знаете ли вы свой обычный
уровень?» вместо «знаете ли вы свой день?».

Измерено: **ленивый прогнозист**, который каждый день
называет своё среднее — и, значит, не знает о себе ничего, — получает $0.994$,
$0.979$, $0.945$ при разбросе оценок $\pm0.8$, $\pm1.5$, $\pm2.5$. Хуже того,
вся лестница навыка сжимается в $0.979\ldots1.000$: нулевой навык и полное
предвидение отстоят на два пункта. Центрированная форма растягивает ту же
лестницу на весь диапазон — $-0.03$, $+0.41$, $+0.74$, $+0.93$, $+0.99$,
$+1.00$ для прогноза, схватывающего $0, 0.25, 0.5, 0.75, 0.9, 1$ дневного
отклонения, — и в точности совпадает с аналитическим ожиданием $1-(1-w)^2$
($0.5 \rightarrow 0.75$ по формуле, $0.738$ измерено) `[С]`.

Три следствия, которые прибор теперь несёт. Ноль означает «вы знали свой
обычный уровень», и отсюда начинают почти все. Отрицательные значения
сохраняются, а не обрезаются: прогноз, легший дальше от дня, чем собственная
база человека, — факт, который стоит назвать: слова, которыми человек себя
описывает, разошлись с жизнью, которой он живёт. И точность не определена
раньше **пяти сверенных дней** — на одном дне объяснять нечего, а
нецентрированная форма и там печатала число.

Одна половина прибора была здорова с самого начала и показывает, почему:
половина по **связкам** делит на $\sum o^2$, где $o$ — знаковая связка,
центрированная на нуле, а нуль уже означает «связи нет», — поэтому
предсказание «ничего» честно получает ноль. Болезнь сидела ровно в половине
уровней.

:::warning Величин с записью $R^{(k)}$ две — и потолок SAD даёт только одна

В таблице выше стоит **fidelity-башня**: она живёт в $[0,1]$ и растёт к
единице по мере того, как самомодель устаканивается. Критерий SAD
$\mathrm{SAD} = \max\{k : R^{(k)} > 1/(k+2)\}$ использует **другую**
величину — убывающее отношение из [Башни глубины §3.5](/docs/consciousness/hierarchy/depth-tower#критическая-чистота-sad),

$$S^{(k)} = r_0 \cdot (1/3)^k, \qquad r_0 = P/P_{\mathrm{crit}},$$

сверху не ограниченное и падающее втрое на этаж (сжатие Фано $\alpha = 2/3$).
Доказательство в башне глубины говорит об этом прямо — «убывающая величина
SAD, не фиделити $R^{(k)}_{\mathrm{fid}} \to 1$», — но общее имя и общий порог
$1/(k+2)$ делают путаницу лёгкой, а цена её немала: **подстановка фиделити
уничтожает потолок**. Величина, растущая к единице, берёт порог, падающий как
$1/(k+2)$, на любом этаже, — значит SAD оказался бы неограничен, и
$\mathrm{SAD_{MAX}} = 3$ испарился бы. Машинная демонстрация `[О]`.
:::


#### Измерение $R_\varphi$ по всему состоянию, а не по его диагонали {#r-phi-по-связкам}

Определение $R_\varphi$ дано по всей матрице, но всякому *прибору* для него
приходится о чём-то человека спрашивать, и то, о чём спросить возможно, задаёт
границы измеряемого. Прямой вопрос — про **уровни**: насколько высоко пойдёт
завтра каждый из семи. Прибор, построенный на одном этом вопросе, меряет
$R_\varphi$ только по диагонали $\Gamma$.

Недостаёт при этом не малости. Масса состояния делится точно: диагональ несёт
$1/(1+\Phi)$, связки — $\Phi/(1+\Phi)$, и на картах боевого пути медиана
второго равна **0.552**. Значит прибор по одним уровням сообщает число, верное
про **45 %** состояния, а читается оно как про всё.

О связках спросить можно, и вопрос обязан обходиться без имён измерений:
назвать их значит подсказать ответ теорией вместо того, чтобы получить его от
человека. Человек *способен* сообщить согласованность: **когда одно
наблюдаемое идёт высоко, второе идёт с ним, против него или само по себе**, —
а это ровно $\operatorname{sign}\gamma_{ij}$. Наблюдаемая сторона — эмпирическая
корреляция пары по записи.

Обе половины складываются затем по своим долям массы, а не по выбранному весу:

$$
R_\varphi^{\text{полное}} \;=\; \frac{1}{1+\Phi}\,R_\varphi^{\text{уровни}}
\;+\; \frac{\Phi}{1+\Phi}\,R_\varphi^{\text{связки}}
$$

Веса суть доли массы того самого состояния, которое меряется, и потому
сложение здесь есть разложение, а не сведение `[О]`. Отсюда две дисциплины, и
называть их стоит потому, что именно их отсутствие и породило разрыв: половину
нельзя подавать как целое, а когда есть лишь одна половина, чтение обязано
назвать, какая это половина и какую долю массы она покрывает. пар двадцать одна по вопросу в день, поэтому первое
полное чтение — через три недели.

Четыре правила разрешают любой смешанный случай:

1. **Тест полосы.** Обращая окно Златовласки: у всякой сознательной системы канонический $R$ лежит в $[1/3,\ 1/2)$, а у любого состояния вообще $R \geq 1/7$. Приведённая «рефлексия» вне этих границ — самадхи $0{,}92$, анестезия $0{,}02$, «натренированный» $0{,}70$ — с необходимостью есть $R_\varphi$.
2. **Зависимость от $P$.** Канонический $R$ — строго убывающая функция чистоты, не несущая информации сверх $P$: его нельзя поднять практикой иначе как через $P$. Всякое утверждение вида «тренировка усиливает рефлексию» касается $R_\varphi$ (с кумулятивной перестройкой $H_{\text{eff}}$) или башни $R^{(n)}$ — никогда канонического $R$.
3. **Принадлежность формул.** Мера сознательности $C = \Phi \times R$ (T-140) берёт **канонический** $R$ — числовой свидетель: эмпирический экземпляр T-153, $C = \Phi/(7P) = 1{,}149/3{,}003 = 0{,}383$. Функционал смысла берёт **$R_\varphi$**: $\text{Meaning}_{\text{peak}} = P \cdot D_{\text{diff}} \cdot \Phi \cdot R_\varphi$ — с каноническим $R$ произведение схлопнулось бы в $D_{\text{diff}} \cdot \Phi / 7$, уничтожив зависимость от $P$.
4. **Когда формы связаны — и когда нет.** Для диссипативной формы замещения (T-62) $\varphi_k(\Gamma) = (1-k)\Gamma + k \cdot I/7$ при $k = 1 - R$ прямое вычисление даёт $\lVert\Gamma - \varphi_k(\Gamma)\rVert_F^2 = k^2 (P - 1/7)$, откуда $R_\varphi = 1 - (1-R)^3$ — монотонная связь. Для **категориальной** самомодели связь рвётся: $R_\varphi$ движется независимо от $P$ (самадхи поднимает $P$ и $R_\varphi$ одновременно). Эта независимость — сигнатура того, что категориальная $\varphi(\Gamma)$ несёт информацию сверх чистоты; количественный перенос между формами ограничен лишь [С]-неравенствами [раздела о высших порядках](#рефлексия-высших-порядков-rn).

**Утверждение (перенос порога при K = 3-прочтении) [С].** Char-R-III выводит $R_{\text{th}} = 1/3$ из байесовской доминации среди $K = 3$ классов линдбладовских каналов. Если различение трёх классов гейтируется точностью самомодели — категориальное прочтение, при котором именно $\varphi(\Gamma)$ должна отличать диссипацию, регенерацию и автоморфизм, — тот же аргумент доминации применяется к $R_\varphi$ дословно: плюральность среди трёх равновероятных альтернатив требует точности выше $1/K = 1/3$. При этом прочтении $R_{\varphi,\text{th}} = 1/3$ — условная теорема **[С]**, а не голая калибровка — и сама гейт-клауза теперь ограничена на теоремном уровне: любое решение, опосредованное самомоделью, теряет не более $2\sqrt{3/7}\,\sqrt{P(1-R_\varphi)}$ точности ([T-252](/docs/proofs/categorical/formalization-phi#гейт-теорема) **[Т]**, точная константа), что пришпиливает достаточный порог к полосе $[5/54,\ 32/81]$ по окну, с $1/3$ как каноническим выравниванием внутри неё **[С]**.

**Динамика $R_\varphi$ — точный первый шаг.** Записывая $R_\varphi = 1 - N/P$ с $N = \lVert\Gamma - \varphi(\Gamma)\rVert_F^2$ и дифференцируя вдоль потока:
...
где $D\varphi$ — производная отображения самомодели вдоль траектории. Кросс-проверка: для диссипативной формы замещения $\varphi_k = (1-k)\Gamma + k \cdot I/7$ при постоянном $k$ имеем $\Gamma - \varphi_k(\Gamma) = k(\Gamma - I/7)$ и $\mathrm{Id} - D\varphi_k = k\,\mathrm{Id}$, откуда $\dot N = k^2 \dot P$ и $dR_\varphi/d\tau = -k^2 \dot P/(7P^2)$ — в согласии с замкнутой формой $R_\varphi = 1 - k^2(1 - R)$ выше. Для **категориальной** $\varphi$ объект $D\varphi$ — ровно то место, где теорию динамики $R_\varphi$ ещё предстоит развить; тождество — её фиксированная стартовая точка. Модуль развёрнут в [Dφ и динамике $R_\varphi$](/docs/proofs/categorical/formalization-phi#дф-производная): явная $D\varphi$ канонического семейства с двухмаршрутной согласованностью (T-249 [Т]) и теорема о полосе с законом длины пути (T-250 [Т]).

**Статус.** $R_\varphi$ — определение **[О]**; её рабочий порог $R_{\varphi,\text{th}} = 1/3$ — калибровка **[И]**, заимствующая байесовское обоснование $K = 3$ для качества самомодели. Канонические пороги — теоремы (T-126, T-124, T-129). Документы, рисующие пространство состояний в плоскости $(P, R_\varphi)$, делают это законно: плоскость двумерна ровно потому, что $R_\varphi$ не является функцией $P$.
...
:::info Нецикличность и каноничность R [T-126]
...
## Ретро-пополнение времени: R как условие категориальной полноты мышления [С] {#ретро-пополнение-времени}

Суть одной строкой: без способности пере-читать собственное прошлое разум доказуемо не может составить почти половину своих мыслей — и рефлексивность $R$ есть в точности та операция, которая эту способность возвращает.

### Дыра, сформулированная комбинаторно

Вспомним нерв траекторий (симплициальную структуру над орбитной категорией $\varphi$, где морфизм — «подожди $k$ шагов канала»). *Композиция мыслей* — это заполнение рога: два ребра треугольника известны, третье обязано существовать. Внутренние роги всегда вынуждаются композицией. Внешние роги просят базовый моноид *факторизовать*: при уже совершённом ожидании длины $a$ и целевом композите длины $b$ решить

$$
x \circ a = b .
$$

В моноиде $(\mathbb{N}_0, +)$ это вычитание, и оно **отказывает всякий раз, когда $b < a$**: обратных элементов нет — нельзя факторизовать сквозь ожидание, которое длиннее цели. Прямая перепись независимых внешних рогов (сетка шагов $0..6$) даёт долю отказов $42{,}9\%$: почти половина формально поставимых композиций мысли не существует над голым временем-вперёд. Нерв — квазикатегория, а не Кан-комплекс.

### Пополнение, сформулированное физически

Категориальное лечение — групповое пополнение $(\mathbb{N}_0,+) \to (\mathbb{Z},+)$: допусти отрицательные нетто-шаги — и всякая факторизация существует. Физически время не течёт назад — но система, у которой след является первоклассным объектом (разум, *хранящий* свою историю), реализует отрицательный шаг законно: нетто-шаг $-r$ не отматывает мир, он **пере-читает $r$ записанных шагов собственного прошлого** и переинтерпретирует их в нынешнем контексте. Мир стоит; движется состояние знания.

Это и есть действие оператора рефлексии: $R$ измеряет, какая часть состояния есть собственная модель системы о себе, а пере-чтение следа — самонаблюдение, применённое вдоль оси времени. Инженерная половина уже существует как орган re-read (ретроспективная переинтерпретация с явной ценой в битах), а пополненный нерв реализован в кремнии с тремя судами: над пополнением заполняются **все** 49 рогов переписи (моноид отказывал в 21), вложение прямого времени уважает композицию, отрицательный нетто-шаг доказуемо не продвигает состояние мира.

### Теорема-кандидат (R-полнота) [С] {#теорема-кандидат-r-полнота}

*Ось рефлексивности — категориальное условие полноты мышления: нерв траекторий является Кан-комплексом тогда и только тогда, когда база ожиданий замкнута относительно ретроспекции (группово-пополненное время). Единственность заполнителей следует из инъективности $F$; существование — из ретро-пополнения.*

Статус [С]: категориальная половина (группа заполняет всякий рог) строга; содержательное отождествление «ретроспекция = действие оси R» несёт эмпирический вес и стоит на кремниевом стенде. Нетривиальность утверждения — в его направлении: рефлексивность обычно *мотивируют* феноменологически («я знаю, что я знаю»); здесь она возникает как *комбинаторная необходимость* — разум без неё измеримо неполон как система композиций, теряя $42{,}9\%$ внешних композиций на сетке переписи.
...
**Шаг 1.** По определению $\sigma$-loss: $L_\sigma = \|\sigma_{\text{sys},\Omega}\|_2 \geq 0$, причём $L_\sigma = 0$ тогда и только тогда, когда $\mathrm{grounding}(w) = 1$ (все символы полностью укоренены). При $\mathrm{grounding}(w) < 1$: $\exists k$ такой, что $\sigma_k > 0$, следовательно $L_\sigma > 0$ и $\nabla_w L_\sigma \neq 0$ (градиент существует и ненулевой в силу гладкой зависимости $\sigma_k$ от $w$ через CPTP-якорь $\pi$).

**Шаг 2.** Обновление весов $w \leftarrow w - \eta_\sigma \nabla_w L_\sigma$ при скорости обучения $\eta_\sigma > 0$ уменьшает $L_\sigma$ на каждом шаге: $L_\sigma(w') \leq L_\sigma(w) - \eta_\sigma \|\nabla_w L_\sigma\|^2 + O(\eta_\sigma^2)$ (стандартная лемма спуска для $L$-гладких функций; гладкость следует из CPTP-структуры якоря $\pi$, полиномиального по $w$).

**Шаг 3.** По определению $\mathrm{grounding}(w) = 1 - L_\sigma(w) / L_\sigma^{\max}$. Уменьшение $L_\sigma$ эквивалентно увеличению укоренения.

**Шаг 4 (Монотонность).** При непрерывном сенсомоторном потоке каждый шаг обновления удовлетворяет $\Delta L_\sigma \leq -\eta_\sigma \|\nabla_w L_\sigma\|^2 < 0$ (строгое убывание при $\mathrm{grounding} < 1$). Последовательность $\{L_\sigma(\tau)\}$ монотонно убывает и ограничена снизу нулём, следовательно, сходится. $\blacksquare$

**Условие [С]:** Непрерывное обучение (метапластичность) + сенсомоторная среда, обеспечивающая разнообразные $\sigma$-градиенты.
