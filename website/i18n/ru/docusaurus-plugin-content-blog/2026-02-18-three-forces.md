...
Чистота **уменьшается**. Когерентности затухают. Связи между измерениями ослабевают. Но здесь стоит быть точным, потому что точность интереснее лозунга.

Канонический диссипатор **не** гонит состояние к максимально смешанному $I/7$. Посчитайте его генератор как матрицу $49\times49$ и посмотрите на спектр: он равен ровно $\{0^{\times7},\,(-5\gamma/21)^{\times42}\}$. Сорок два затухающих направления — все когерентности, все с одной скоростью $5\gamma/21$ — и **семь** направлений, которые не двигаются вовсе. Эти семь — популяции. Подайте диссипатору любое диагональное состояние, и он вернёт его нетронутым: $\mathcal{D}_\Omega[\operatorname{diag}\lambda] = 0$, до машинного нуля.

Значит разрушение, взятое в одиночку, — это не скатывание в серую кашу. Это *уплощение*: оно стирает всякую связь между измерениями и оставляет сами измерения ровно такими же громкими, какими нашло. Оставленное само по себе, оно производит не одно серое состояние, а семипараметрическое их семейство — всякий возможный профиль популяций, и все они одинаково мертвы в том смысле, что когерентности не осталось.

Серая каша получается из разрушения **плюс вращение**. Вращение всё время наклоняет состояние из базиса классификатора, изготавливая свежие когерентности на съедение диссипатору; диссипатор их съедает и возвращает чуть более плоский профиль; повторить. Именно эта петля схлопывает семимерное ядро диссипатора в единственную точку, и лишь тогда $I/7$ становится единственным пунктом назначения. В корпусе это примитивность $\mathcal{L}_0 = \mathrm{Aut} + \mathcal{D}_\Omega$ (T-39a [Т]).

Маленькая поправка с моралью: вторая сила тоже не может доделать свою работу одна. Даже разрушению нужен соучастник.
