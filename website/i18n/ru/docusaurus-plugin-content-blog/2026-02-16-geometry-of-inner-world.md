...
А вот та часть, которая перестаёт выглядеть совпадением, стоит её проверить. Возьмите шестнадцать кодовых слов $H(7,4)$ и оставьте только веса три — слова ровно с тремя единицами. Их семь. Теперь прочтите каждое как множество позиций, где стоит единица, и спросите, что делают эти семь троек: всякая пара из семи позиций накрыта ровно одной тройкой.

Это плоскость Фано. Не «аналогична», а тот же объект. Код, исправляющий ошибки, и геометрия семи измерений — два прочтения одной структуры, и это проверяется перебором в несколько строк.

Совершенность проверяется тем же дыханием: $2^7/(1+7) = 16$ — в точности число кодовых слов, то есть граница Хэмминга достигнута без остатка. Три проверки чётности дают $2^3 = 8$ классов синдрома: один на «ошибки нет» и по одному на каждую из семи позиций, ни одного лишнего.
...
L4 — категориально **недостижимый** предел [Т] (T-86). Формально: $L4 = \mathrm{colim}_{n \to \infty} \tau_{\leq n}(\mathrm{Exp}_\infty)$, и этот колимит недостижим для конечных систем (неполнота Лоувера, T-55 [Т]). К нему можно **асимптотически приближаться**, но достичь — нельзя. Это не практическое ограничение (нехватка чистоты), а категориальная невозможность: финитная система не может полностью содержать собственное описание.
...
| Категориальная недостижимость L4 | [Т] | T-86, неполнота Лоувера |
