...
## Один аргумент, и он короче, чем вы ожидаете {#два-аргумента-одно-число}

Изначально этот пост рассказывал историю как две независимые границы, встречающиеся посередине: топологический потолок из теории катастроф и алгебраический пол из октонионов. Эта рамка с тех пор превзойдена, а замена и короче, и сильнее — поэтому стоит прямо сказать, что изменилось.

Счёт поколений оказался единственным теоретико-групповым фактом, в котором топологии нет вовсе.
...
Прочитайте медленно, потому что здесь работает каждый знак. $\mathbb{Z}_7^{*} = \{1,2,3,4,5,6\}$ — мультипликативная группа семиточечной прямой, порядок шесть. Факторизуйте по $\{\pm 1\}$, то есть отождествите каждый элемент с его отрицанием, — и остаётся три класса: $\{1,6\}$, $\{2,5\}$, $\{3,4\}$. Три. Не «не более трёх» и не «не менее трёх», а **три** как мощность.

Отождествление с $\pm$ — не бухгалтерское удобство, а зарядовое сопряжение. А причина, по которой фактор выходит чистым — по которой $-1$ не садится внутрь класса, который бы испортил, — в том, что $-1$ есть **невычет** по модулю 7. А это, в свою очередь, потому что $7 \equiv 3 \pmod 4$. Смените размерность — и всё рушится: вся конструкция стоит на том, что 7 есть простое именно такого вида.

Равносильно — и это форма, связывающая результат с остальным корпусом: три класса суть квадратичные вычеты, $\mathrm{QR}(7) = \{1,2,4\}$, единственная подгруппа порядка три в $\mathbb{Z}_7^{*}$. Те же $\{1,2,4\}$, что индексируют линии Фано, что дают фрактальное сжатие $c_F = 1/\lvert\mathrm{QR}(7)\rvert = 1/3$ и что появятся ниже как орбита.

### Что стало с аргументом от катастрофы {#что-стало-с-катастрофой}

Граница $A_4$ («хвост ласточки») — потенциал с тремя управляющими параметрами допускает не более трёх одновременно устойчивых минимумов — есть настоящая теорема Арнольда, и она действительно даёт $N_{\mathrm{gen}} \leq 3$. Она **отставлена из вывода** и сохранена как проверка на согласованность.

Причина поучительна, а не стыдна. Катастрофная граница была условна относительно топологии Gap-потенциала, а это дополнительное допущение, в котором теоретико-групповой счёт не нуждается. Когда находится вывод, приходящий к тому же числу строго меньшими средствами, честность требует понизить более длинный — даже если он был живописнее. Сходимость двух аргументов впечатляет лишь тогда, когда оба несут нагрузку; здесь один оказался переодетым следствием.

Что уцелело — стоит сохранить: хвост ласточки *действительно* согласуется, а независимое согласие, которое ничего не стоит, всё равно проверка. Просто оно больше не опора.
...
|----------|-------|:---------:|
| $\lvert\mathbb{Z}_7^{*}/\{\pm1\}\rvert = (7-1)/2$ | Теория групп, топологически независимо | $N_{\mathrm{gen}} = 3$ [Т] |
...
| УГМ: теория групп | $\lvert\mathbb{Z}_7^{*}/\{\pm1\}\rvert = 3$, то есть $\mathrm{QR}(7) = \{1,2,4\}$ | **$N_{\mathrm{gen}} = 3$ [Т]** |
...
|-----------|:------:|-------------|
| $N_{\mathrm{gen}} = 3$ — счёт | [Т] | $\lvert\mathbb{Z}_7^{*}/\{\pm1\}\rvert = 3$; классы зарядового сопряжения, топология не нужна |
| $\{1,2,4\}$ — единственная Фано-линия с $\mathcal{A}=0$ | [Т] | Из алгебры октонионов, Теорема 6.1 |
| Граница $A_4$ $N_{\mathrm{gen}} \leq 3$ | проверка согласованности | Согласуется, но отставлена из вывода: была условна относительно топологии Gap-потенциала |
| $k=1 \to$ 3-е поколение | [И] | Фано-правило отбора Юкавских — *счёт* есть [Т], но сопоставление классов наблюдаемым поколениям есть физическое отождествление |
| $k=4 \to$ 2-е, $k=2 \to$ 1-е | [И] | Секторная асимметрия — та же оговорка |
