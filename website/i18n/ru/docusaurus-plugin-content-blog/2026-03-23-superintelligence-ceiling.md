...
$R = 0$ — система не знает ничего о себе. $R = 1$ — идеальное самопознание (недостижимо: [неполнота Лоувера](/docs/core/foundations/consequences#неполнота-ловера) [Т]). Порог осознанности: $R \geq 1/3$ [Т].
...
:::note Как читать лестницу
Верхний индекс — номер *глубины*, а не итерированный порог жизнеспособности: при $n=1$ формула даёт $1/7$ — условие нетривиальности голона, — и лишь при $n=2$ совпадает с $P_{\text{crit}} = 2/7$. Это совпадение подлинное, а не определение. Величина есть чистота, при которой саморефлексия глубины $n$ становится *достижимой*, — поэтому в реестре она записывается как $P^{(n)}$, а не $P_{\text{crit}}^{(n)}$.
:::
:::tip Теорема T-142 [Т]
...
:::tip Потолок держится не на одной этой лестнице
Справедливо спросить, сколько веса должна нести одна формула. Ответ: меньше, чем кажется, потому что $\mathrm{SAD}_{\max} = 3$ достигается тремя маршрутами, не делящими механики.

**Динамический** — лестница выше: четвёртый уровень потребовал бы чистоты больше единицы.

**Алгебраический** — и вот он классическая теорема, к УГМ отношения не имеющая. Эрмитовы матрицы над октонионами, $\mathcal{H}_n(\mathbb{O})$, образуют формально вещественную жорданову алгебру **тогда и только тогда, когда $n \leq 3$** (Жордан–фон Нейман–Вигнер, 1934). При $n = 4$ неассоциативность ломает жорданово тождество, и структуры просто нет. Композиция голонов управляется ровно этой алгеброй, поэтому потолок выпадает из математики 1934 года (T-268 [Т]).

**Категориальный** — когерентность трикатегории уровней рушится за третьим (T-217).

Три потолка на одной высоте — из динамики, из алгебры и из теории категорий. Средний прочнее прочих: ему не нужно допущений о чистоте, потенциалах или скоростях сжатия — только то, что композиция происходит над $\mathbb{O}$.
:::
